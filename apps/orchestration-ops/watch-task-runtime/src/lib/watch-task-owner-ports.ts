...
export function createOpsRuntimeStorePort(input: {
  ...
  const command = resolve(input.repositoryRoot, "apps/orchestration-ops/ops-runtime-store/src/scripts/main.ts")
...
export function createWatchHandoffRevalidationPort(input: {
  ...
  const command = resolve(
    ...
    "apps/live-execution-control/watch-handoff-revalidation/src/scripts/main.ts",
  )
  const timeoutMs = input.timeoutMs ?? 5_000
  return {
...
export function createPublicMarkObservationPort(input: {
  ...
  const command = resolve(input.repositoryRoot, "apps/market-data-products/binance-read/symbol-snapshot/src/scripts/main.ts")
