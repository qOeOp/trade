...
- Immutable Linux container source packages from committed Git state, with critical-contract hashes, checksum verification, no credential/runtime-state inclusion, and a bounded no-live BuildKit/health/restart/volume-canary acceptance entrypoint.
...
- `trade.db` event ownership and flow projections to `apps/portfolio-execution-state/*`
- Observe building and market/account projection to `apps/live-decision-planning/*`
- Dry-run, shadow, live-small, execution recording, reconcile, and recovery behavior to `apps/live-execution-control/*`
- Strategy R&D, replay, panel, benchmark, and RD memory work to atomic `apps/research-strategy-development/*` tools
- Strategy evidence, review, and promotion gates to `apps/governance-review-compliance/strategy-review`
- Artifact catalog, query, stale scan, and GC work to `apps/artifact-knowledge/artifact-catalog`
- Exchange private account/order reads and authorized writes to `apps/exchange-gateway/*` tools
- Market data reads to `apps/market-data-products/*` tools
- Deterministic hard guards to `apps/live-execution-control/plan-preflight`
- Runtime check semantics to `apps/orchestration-ops/runtime-health-guard`; trade-flow consumes only named check status and never reimplements L2 health.
