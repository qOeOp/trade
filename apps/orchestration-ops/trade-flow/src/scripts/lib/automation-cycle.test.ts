...
test("automation cycle plan isolates trade db work from R&D artifact jobs", () => {
  ...
  try {
    ...
    assert.equal(asRecord(asRecord(health.processor_spec).command_spec).cwd, "apps/orchestration-ops/runtime-health-guard")
    ...
    assert.equal(asRecord(fastToolJob.command_spec).cwd, "apps/live-execution-control/fast-track-guard")
    ...
    assert.equal(asRecord(slowToolJob.command_spec).cwd, "apps/live-decision-planning/slow-track-plan")
    ...
    assert.equal(asRecord(shadow.command_spec).cwd, "apps/research-strategy-development/forward-evidence-plane/paper-tracker")
    ...
    assert.equal(asRecord(review.command_spec).cwd, "apps/governance-review-compliance/closed-flow-review-sweep")
...
test("automation cycle plan can dispatch a learning strategy R&D supervisor", () => {
  ...
  try {
    ...
    assert.equal(asRecord(rdToolJob.command_spec).cwd, "apps/research-strategy-development/research-control-plane/autonomy-cycle")
...
test("automation cycle plan can drive R&D supervisor from durable program state", () => {
  ...
  try {
    ...
    assert.match(String(activeRd.command), /apps\/research-strategy-development\/research-control-plane\/autonomy-cycle\/src\/scripts\/main.ts/)
