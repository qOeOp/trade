  ...
  try {
    ...
    const result = await runProgramShadowSupervisor(
      ...
      async (command, options): Promise<CommandExecutionResult> => {
        ...
        if (command.cwd === "apps/artifact-knowledge/artifact-catalog") return catalogResult(command.argv)
    ...
    const catalogCommands = commands.filter((command) => command.cwd === "apps/artifact-knowledge/artifact-catalog")
...
test("parity replay fails closed when the Agent command identity drifts", async () => {
  ...
  await recorder.record({
    executable: true,
    cwd: "apps/orchestration-ops/runtime-health-guard",
...
test("program shadow supervisor duration bounds a longer cadence sleep", async () => {
  ...
  try {
    ...
    const result = await runProgramShadowSupervisor(
      ...
      async (command): Promise<CommandExecutionResult> => command.cwd === "apps/orchestration-ops/runtime-health-guard"
        ? healthResult()
        : { exit_code: 0, stdout: JSON.stringify({ ok: true }), stderr: "" },
      {
        clock: () => new Date(now),
