  ...
  try {
    ...
    const result = await runProgramShadowWakeup(
      ...
      async (command): Promise<CommandExecutionResult> => {
        ...
        if (command.cwd === "apps/orchestration-ops/runtime-health-guard") {
          return healthResult()
        }
    ...
    assert.deepEqual(executed.map((item) => item.cwd), [
      "apps/orchestration-ops/runtime-health-guard",
      "apps/orchestration-ops/control-effectiveness-review",
      "apps/orchestration-ops/ops-notify-dispatch",
    ])
...
test("demand-driven shadow leaves per-symbol L2 readiness to the market-data demand owner", async () => {
  ...
  try {
    ...
    const result = await runProgramShadowWakeup(
      ...
      async (command): Promise<CommandExecutionResult> => {
        executed.push({ cwd: command.cwd, argv: command.argv })
        return command.cwd === "apps/orchestration-ops/runtime-health-guard"
          ? healthResult()
  ...
  try {
    ...
    const result = await runProgramShadowWakeup(
      ...
      async (command, options): Promise<CommandExecutionResult> => {
        ...
        if (command.cwd === "apps/orchestration-ops/runtime-health-guard") return healthResult()
        if (command.cwd === "apps/artifact-knowledge/artifact-catalog") {
          return {
            exit_code: 0,
    ...
    const catalogCommands = executed.filter((item) => item.cwd === "apps/artifact-knowledge/artifact-catalog")
...
test("program shadow recovers a running cycle after a stale lease", async () => {
  ...
  try {
    ...
    const health = executed.find((item) => item.cwd === "apps/orchestration-ops/runtime-health-guard")
    assert.equal(jsonArg(health?.argv ?? []).health_id, "health-shadow-cycle-recover-recovery-runtime")
    const notify = executed.find((item) => item.cwd === "apps/orchestration-ops/ops-notify-dispatch")
...
test("program and Agent job-graph entries expose the same semantic parity projection", async () => {
  ...
  try {
    const executor = async (command: { cwd: string }): Promise<CommandExecutionResult> => command.cwd === "apps/orchestration-ops/runtime-health-guard"
      ? healthResult()
      : { exit_code: 0, stdout: JSON.stringify({ ok: true }), stderr: "" }
