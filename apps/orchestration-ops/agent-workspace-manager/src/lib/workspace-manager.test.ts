...
test("workspace execution scope binds one request to exact source and bounded check paths", () => {
  ...
  assert.deepEqual(scope.package_paths, ["apps/sample"])
  assert.throws(() => createAgentWorkspaceExecutionScope({
    run_id: "developer-run-scope",
    request_hash: "a".repeat(64),
    source_revision: "0123456789abcdef",
    allowed_write_prefixes: ["apps/sample"],
    package_paths: ["apps/other"],
    issued_at: "2026-07-23T01:00:00.000Z",
...
test("workspace seed patch reconstructs one exact cumulative revision", () => {
  ...
  try {
    writeFileSync(
      join(first.workspace_root, "apps/sample/index.ts"),
  ...
  try {
    ...
    assert.match(
      readFileSync(join(second.workspace_root, "apps/sample/index.ts"), "utf8"),
      /value = 2/,
    )
  } finally {
...
test("Developer workspace freezes source, bounds writes, captures patch, and cleans up", async () => {
  ...
  try {
    writeFileSync(join(workspace.workspace_root, "apps/sample/index.ts"), "export const value = 2\n")
    writeFileSync(join(workspace.workspace_root, "apps/sample/new.test.ts"), "export const testValue = 3\n")
    const patch = captureAgentWorkspacePatch(workspace)
    assert.deepEqual(patch.changed_files, ["apps/sample/index.ts", "apps/sample/new.test.ts"])
...
test("Developer workspace rejects paths outside policy and escaping symlinks", () => {
  ...
  try {
    ...
    symlinkSync(outside, join(workspace.workspace_root, "apps/sample/escape"))
