...
export function run(args: Args): JSONRecord {
  ...
  try {
    ...
    if (args.action === "record_agent_tool_result") {
      return {
        ok: true,
        action: args.action,
        tool_result: recordAgentRunToolResult(db, {
          call_id: stringField(args.json.call_id),
          run_id: stringField(args.json.run_id),
          request_hash: stringField(args.json.request_hash),
          task_profile: stringField(args.json.task_profile) as "planner" | "developer" | "reviewer" | "explanation",
          tool_name: stringField(args.json.tool_name),
          output_schema_version: stringField(args.json.output_schema_version),
          artifact: args.json.artifact as unknown as AgentArtifactRef,
          occurred_at: stringField(args.json.occurred_at),
        }),
      }
    }
    if (args.action === "read_agent_terminal_tool_result") {
      return {
        ok: true,
        action: args.action,
        tool_result: readAgentRunTerminalToolResult(db, {
          run_id: stringField(args.json.run_id),
          request_hash: stringField(args.json.request_hash),
          task_profile: stringField(args.json.task_profile) as "planner" | "developer" | "reviewer" | "explanation",
          tool_name: stringField(args.json.tool_name),
...
function printHelp(): void {
  console.log([
    ...
    "actions: init | record_cycle | record_job | record_message | list_messages | record_incident | list_incidents | update_incident | list_incident_events | acquire_lock | read_lock | renew_lock | release_lock | summary | parity_status | record_agent_tool_call | record_agent_tool_result | read_agent_tool_usage | read_agent_terminal_tool_result | read_agent_run | watch_create | watch_read | watch_arm | watch_apply_evaluation | watch_handoff | watch_complete | watch_cancel",
  ].join("\n"))
}
