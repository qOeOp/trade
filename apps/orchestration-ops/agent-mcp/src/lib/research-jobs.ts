...
export class ResearchJobService {
  ...
  async submit(input: ResearchJobSubmitInput): Promise<JSONRecord> {
    ...
    try {
      ...
      const started = this.start({
        script: "apps/orchestration-ops/agent-mcp/src/scripts/research-job-worker.ts",
  ...
  async preparePlannerProposal(
    ...
    const response = ownerData(await this.execute({
      script: "apps/research-strategy-development/research-control-plane/state-store/src/scripts/main.ts",
      args: [
        "--db",
        this.rdStateDbPath,
        "--action",
  ...
  private async readDeveloperAgentContext(
    ...
    const artifactResponse = await this.execute({
      script: "apps/orchestration-ops/agent-artifact-store/src/scripts/main.ts",
  ...
  private async recordAgentToolCall(input: {
    ...
    const response = ownerData(await this.execute({
      script: "apps/orchestration-ops/ops-runtime-store/src/scripts/main.ts",
      args: [
        "--db",
  ...
  private runDesigner(action: "context" | "render_prompt" | "validate" | "queue_item", input: JSONRecord): Promise<JSONRecord> {
    return this.execute({
      script: "apps/research-strategy-development/agent-roles/planner/strategy-hypothesis-designer/src/scripts/main.ts",
  ...
  private runProgram(programId: string, payload: JSONRecord): Promise<JSONRecord> {
    return this.execute({
      script: "apps/research-strategy-development/research-control-plane/program-control/src/scripts/main.ts",
