...
```bash
bun apps/orchestration-ops/agent-mcp/src/scripts/main.ts
```
...
私有 HTTP 入口为 `bun apps/orchestration-ops/agent-mcp/src/scripts/http.ts --profile explanation`；token 只从 `TRADE_MCP_HTTP_TOKEN` 或显式 `--token-env` 读取，不进入参数、配置输出或日志。
