  ...
  return async (owner, payload) => {
    ...
    if (owner === "rd_program_read") {
      return ownerData(await execute([
        "apps/research-strategy-development/research-control-plane/program-control/src/scripts/main.ts",
        "--db", profile.rd_state_db, "--program-id", programId, "--json", JSON.stringify({ action: "read" }),
      ], 15_000))
    }
    return ownerData(await execute([
      "apps/research-strategy-development/research-control-plane/autonomy-cycle/src/scripts/main.ts",
...
function auditor(profile: Profile): OperatorAuditor {
  return async (event) => {
    ...
    await execute([
      "apps/orchestration-ops/ops-runtime-store/src/scripts/main.ts",
