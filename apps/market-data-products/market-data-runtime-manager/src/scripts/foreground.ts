...
function readSubscriptionPlan(root: string, args: Args, observedAt: string) {
  const script = resolve(root, "apps/market-data-products/market-data-store/src/scripts/main.ts")
...
function syncActiveFlowDemands(root: string, args: Args, observedAt: string): number {
  const projector = Bun.spawnSync({
    cmd: [
      process.execPath,
      resolve(root, "apps/portfolio-execution-state/flow-projector/src/scripts/main.ts"),
      "--active-flows",
      "--db", "data/trade.db",
    ],
    cwd: root,
  ...
  const storeScript = resolve(root, "apps/market-data-products/market-data-store/src/scripts/main.ts")
