...
HELP_TEXT = """Usage:
  python3 src/scripts/main.py --symbol BTCUSDT
  python3 src/scripts/main.py --symbol BTCUSDT --aggtrades-file /tmp/aggtrades.json --snapshot-file /tmp/snapshot.json
...
def unwrap_tool_payload(payload: Any) -> Any:
    if isinstance(payload, dict) and "ok" in payload and "data" in payload:
        if payload.get("ok") is False:
            raise ValueError(f"upstream tool payload is not ok: {payload.get('error') or 'unknown error'}")
        return payload.get("data")
    return payload


def load_aggtrades_payload(path: str) -> dict[str, Any]:
    payload = unwrap_tool_payload(load_json_file(path))
...
def build_warnings(
    ...
    if aggtrades_source == "file" or snapshot_source == "file":
        warnings.append("file-fed mode trusts upstream tool payloads; verify symbol, market, and freshness before acting")
