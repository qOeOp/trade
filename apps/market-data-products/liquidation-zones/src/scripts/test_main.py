...
class ParseArgsTests(unittest.TestCase):
    def test_registered_command_uses_the_owner_parser(self):
        ...
        self.assertEqual(command["cwd"], "apps/market-data-products/liquidation-zones")
...
class HelperTests(unittest.TestCase):
    ...
    def test_unwrap_tool_payload_rejects_failed_wrapper(self):
        with self.assertRaisesRegex(ValueError, "upstream tool payload is not ok"):
            MODULE.unwrap_tool_payload({"ok": False, "error": "boom", "data": None})

    def test_load_aggtrades_payload_accepts_tool_wrapper(self):
        payload = {
            "ok": True,
            "data": {
                "symbol": "BTCUSDT",
