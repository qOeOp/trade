...
`apps/` 是项目源码货架。每个模块内部自带 `src/`、运行入口、测试和本地契约；根目录 [toolset.json](../toolset.json) 是 agent 使用这些模块的注册表。
...
| `apps/<domain>/<module>/` | 模块根：package / go.mod / requirements / reference / 模块说明 |
| `apps/<domain>/<module>/src/` | 模块实现：scripts、schemas、tests、内部 helper |
| `apps/contracts/<contract>/src/` | 新跨模块 contract / client 层；业务模块优先依赖这里或 CLI contract |
| `apps/<domain>/<internal-engine>/src/` | 无 package / 无 agent entry 的领域内计算内核；只承载共享公式、schema-adjacent parser 或纯计算 |
...
| `orchestration-ops/agent-host-openclaw` | provider-neutral Agent Run request、pinned Gateway profile | typed terminal Agent Run result 与 immutable output ref | OpenClaw agent/session 映射、transport 验证、ops lifecycle、bounded CLI execution | Program cadence、领域 owner、OpenClaw state、credential、promotion/deploy/trade |
| `orchestration-ops/agent-artifact-store` | instruction / context / output / patch text | immutable content-addressed Agent artifact ref | 固定 durable/tmp 根、bytes/hash/secret 重验、跨 Host materialization | 领域语义、删除、workspace execution、merge/deploy/trade |
...
| `research-strategy-development/research-control-plane/agent-run-orchestrator` | owner context、Agent Run result 与 immutable artifact refs | role submission admission / blocker | Agent-neutral Planner/Developer/Reviewer 纵切、context/hash/evidence gate | Host loop、Replay execution、strategy state bypass、promotion/live |
...
| `research-strategy-development/replay-execution-plane/compatibility/legacy-portfolio-cycle` | P10/P11/P13 historical plans/reservations/lanes、canonical owner runners、artifact store | fixed cycle-2 reallocation、two-cycle and consolidated accounting Result/Artifact readback | 存量 portfolio-cycle compatibility certification/migration | canonical public entrypoint、P12+ successor、新经济语义或新消费者 |
...
| `research-strategy-development/replay-execution-plane/certification/legacy-portfolio-cycle-certification` | frozen P10/P11 Trial/reservation 与 P13 sequence fixture | reallocation → two-cycle、consolidated accounting Result/Artifact regression | 历史 portfolio-cycle consumer 独立认证 | runtime、CLI、生产语义、P12+ successor authority |
...
| `market-data-products/l2-order-book-core` | caller-supplied depth snapshot/update、capacity、TL2S payload/rotation parameters | deterministic book projection/hash、sequence decision、segment finalize/recovery descriptors | L2 order-book 与 TL2S deterministic Rust core | 网络、soak/process lifecycle、manifest admission、策略或交易 |
| `market-data-products/l2-order-book-service` | explicit public-network consent、Binance USD-M symbol、bounded runtime/storage/read-port config | raw TL2S segments、epoch manifest proposals、loopback gRPC book/watermark/health | 单标的 public L2 runtime candidate 与 freshness/continuity state | private/write API、market-data admission、Replay/策略/执行、未启用 broker |
| `market-data-products/l2-order-book-compactor` | owner-issued complete epoch compaction job、TL2S segments | Zstd Parquet 与 compaction proposal | 确定性 L2 raw→Parquet worker | authority scan、SQLite 写入、raw 删除、跨 epoch 推断 |
...
- agent-facing 模块之间通过 CLI JSON contract 协作；MCP 只作为 Agent 入口适配既有 Owner CLI，不成为新的领域 owner。源码跨模块 import 只允许指向 `apps/contracts/*`、同 domain 的无 package internal engine，或 `orchestration-ops/trade-flow` 编排层调用 `apps/*` 原子能力。
- `apps/orchestration-ops/trade-flow/src/scripts/commands/*` 是 suite façade；新增原子能力优先落独立 `apps/<domain>/<module>`，不要继续塞进 trade-flow。
