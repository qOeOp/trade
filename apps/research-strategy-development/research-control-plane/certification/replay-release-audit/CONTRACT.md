...
- Capability inventory、epoch、certification registries 与 Plane 下全部 `.ts / .json / .py` certification inputs 提供静态 supported-surface 与源码 identity，但不证明执行成功或发布状态。
...
- Repository `bun run check:replay-release` 是该 audit 的 canonical consumer；它不自行签发结论，只有 audit receipt 才是 durable release verdict。
