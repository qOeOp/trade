...
import { runRdSupervisorLoopWithDeps } from "../../../../program-supervisor/src/lib/rd-supervisor-runner"
...
test("rd supervisor runner writes a draft strategy after a validated campaign candidate", () => {
  ...
  try {
    ...
    const result = runRdSupervisorLoopWithDeps({
      ...
      dbPath,
      catalogDbPath: catalogDb,
      input: { now: "2026-07-09T13:00:00Z", max_iterations: 5, strategy_root: strategyRoot, legacy_draft_materialization: true },
    }, {
      runLoop: () => {
...
function readSchema(name: string): JSONRecord {
  if (name === "rd-supervisor-run-result") {
    return JSON.parse(readFileSync(new URL("../../../../program-supervisor/src/schemas/rd-supervisor-run-result.schema.json", import.meta.url), "utf8")) as JSONRecord
  }
