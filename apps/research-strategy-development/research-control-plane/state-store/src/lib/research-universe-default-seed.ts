...
export function seedDefaultResearchControlPlane(db: Database, now: string): {
  ...
  for (const [node, family] of familyCoverage) {
    upsertUniverseCoverage(db, {
      ...
      scope_ref: family, module_ref: "apps/research-strategy-development/agent-roles/developer/strategy-family-engine",
    ...
    upsertUniverseCoverage(db, {
      ...
      scope_ref: family, module_ref: "apps/research-strategy-development/replay-execution-plane/compatibility/legacy-research-kernel",
...
export function buildDefaultDataSurfaceSeed(now: string): DataSurfaceWrite[] {
  ...
  return definitions.map(([slugValue, name, surfaceType, status, temporalContract]) => ({
    ...
    owner_module: slugValue === "ohlcv" || slugValue === "funding" || slugValue === "open-interest"
      ? "apps/market-data-products/market-data-store" : undefined,
    created_at: now, updated_at: now,
  }))
}
...
export function buildDefaultCapabilitySeed(now: string): PipelineRegistryItemWrite[] {
  const definitions = [
    ["feature", "ohlcv-features", "OHLCV Feature Set", "apps/research-strategy-development/agent-roles/developer/strategy-family-engine", ["ohlcv", "4h", "causal"]],
    ["feature", "funding-features", "Funding Feature Set", "apps/research-strategy-development/agent-roles/developer/strategy-family-engine", ["funding", "derivatives", "causal"]],
    ["forecast_model", "rule-score", "Deterministic Rule Score", "apps/research-strategy-development/agent-roles/developer/signal-engine", ["score", "deterministic"]],
    ["portfolio", "single-asset", "Single Asset Portfolio", "apps/research-strategy-development/replay-execution-plane/compatibility/legacy-research-kernel", ["single_asset", "usdm"]],
    ["portfolio", "cross-sectional-rank", "Cross-sectional Rank Portfolio", "apps/research-strategy-development/agent-roles/developer/candidate-batch", ["panel", "ranking"]],
    ["risk_rule", "fixed-risk-geometry", "Fixed Risk Geometry", "apps/research-strategy-development/agent-roles/developer/strategy-family-engine", ["stop", "target", "time_exit"]],
    ["execution_rule", "bar-close-taker", "Bar-close Taker Execution", "apps/research-strategy-development/replay-execution-plane/compatibility/legacy-research-kernel", ["closed_candle", "taker", "4h"]],
  ] as const
