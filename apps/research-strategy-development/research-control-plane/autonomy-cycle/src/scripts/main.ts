...
function fixedCommand(owner: AutonomyOwner, config: Config, supervisorPayload: JSONRecord, payload: JSONRecord): string[] {
  if (owner === "plan" || owner === "queue") return ["apps/research-strategy-development/research-control-plane/program-control/src/scripts/main.ts", "--db", config.db, "--program-id", config.programId, "--json", JSON.stringify(payload)]
  if (owner === "model_task") return ["apps/research-strategy-development/agent-roles/planner/strategy-hypothesis-designer/src/scripts/main.ts", "--action", "model_task", "--json", JSON.stringify(payload)]
  if (owner === "gateway") return ["apps/orchestration-ops/model-gateway/src/scripts/main.ts", "--profile", config.profile, "--json", JSON.stringify(payload)]
  if (owner === "assess") return ["apps/research-strategy-development/agent-roles/planner/strategy-hypothesis-designer/src/scripts/main.ts", "--action", "assess_model_result", "--json", JSON.stringify(payload)]
  return ["apps/research-strategy-development/research-control-plane/program-supervisor/src/scripts/main.ts", "--supervisor-job", "--db", config.db, "--program-id", config.programId, "--catalog-db", config.catalogDb, "--json", JSON.stringify(supervisorPayload)]
}
