...
test("source reducer stops at the terminal market event and keeps only in-position funding", () => {
  ...
  const result = reduceReplaySourceEvents({
    ...
    instrument_status_epochs: [{
      schema_version: "trade.rd-replay-instrument-status-snapshot.v1", snapshot_id: "status-1",
      venue_id: "binance-usdm", symbol: "BTCUSDT", status: "trading", effective_at: "2020-01-01T00:00:00Z",
      valid_until: null, observed_at: "2026-07-13T00:00:00Z", source_ref: "fixture:status-1", source_hash: HASH,
    }],
    ...
    get_active_protection: () => ({
      protection_mode: "bracket",
      protection_generation: 1, remaining_quantity: 1,
      stop_order_id: "run:order:stop", stop_trigger_price: 95, stop_order_status: "active",
      target_order_id: "run:order:target", target_trigger_price: 110, target_order_status: "active",
    }),
