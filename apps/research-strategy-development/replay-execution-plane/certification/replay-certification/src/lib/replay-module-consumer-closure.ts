...
export function classifyReplayProductionConsumer(
  ...
  const scopes: Array<[string, ReplayProductionConsumerClassification]> = [
    ["apps/research-strategy-development/research-control-plane/", "research-control-plane"],
    ["apps/research-strategy-development/forward-evidence-plane/", "forward-evidence-plane"],
    ["apps/research-strategy-development/agent-roles/", "agent-roles"],
  ]
...
export function discoverReplayModuleConsumerClosure(
  ...
  const allPackageRoots = discoverPackageRoots(join(repoRoot, "apps"), repoRoot)
  ...
  for (const file of discoverTypeScriptSources(join(repoRoot, "apps"))) {
