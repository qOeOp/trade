import {
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_EVIDENCE_SCHEMA_VERSION,
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_JOURNAL_POLICY_VERSION,
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_LIMITATIONS,
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_POLICY_VERSION,
  assertReplayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidence,
  replayPortfolioProtectiveTakeProfitCancelCycleCommitHash,
  ...
  replayPortfolioProtectiveTakeProfitCancelCycleSequenceLedgerEntryHash,
  replayPortfolioProtectiveTakeProfitCancelCycleSequenceTrialBalanceHash,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleCommit,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceAuthority,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidence,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceFingerprint,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceJournalEntry,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceLedgerEntry,
  type ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceTrialBalance,
...
const ACCOUNTS: ReplayPortfolioProtectiveTakeProfitCancelTerminalAccountingJournalAccount[] = [
...
const CREDIT_NORMAL = new Set<ReplayPortfolioProtectiveTakeProfitCancelTerminalAccountingJournalAccount>([
  "opening_equity", "realized_pnl_income", "funding_income", "unrealized_pnl_income",
])
export interface ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceAccountingInput {
  plan: ReplayPortfolioCycleSequencePlan
  reservation: ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceAuthority
  cycles: ReplayPortfolioProtectiveTakeProfitCancelCycleSource[]
}

export function createReplayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidence(
  input: ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceAccountingInput,
): ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidence {
  const commits = createCommits(input.cycles)
  const ledger: ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceLedgerEntry[] = []
  const journal: ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceJournalEntry[] = []
  for (const cycle of input.cycles) {
    for (const cycleEntry of cycle.accounting_evidence.ledger) {
      ...
      ledger.push({ ...body,
        sequence_entry_hash: replayPortfolioProtectiveTakeProfitCancelCycleSequenceLedgerEntryHash(body) })
    }
    ...
    for (const cycleEntry of cycle.accounting_evidence.journal) {
      ...
      journal.push({ ...body,
        sequence_entry_hash: replayPortfolioProtectiveTakeProfitCancelCycleSequenceJournalEntryHash(body) })
    }
  }
  ...
  const fingerprintBody: Omit<ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceFingerprint,
    "fingerprint_hash"> = {
      ...
      policy_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_POLICY_VERSION,
      ...
      limitations_hash: canonicalHash(REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_LIMITATIONS),
    }
  const fingerprint = { ...fingerprintBody,
    fingerprint_hash: replayPortfolioProtectiveTakeProfitCancelCycleSequenceFingerprintHash(fingerprintBody) }
  const body: Omit<ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidence, "evidence_hash"> = {
    schema_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_EVIDENCE_SCHEMA_VERSION,
    policy_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_POLICY_VERSION,
    experiment_id: input.reservation.experiment_id, trial_group_id: input.reservation.trial_group_id,
    trial_group_hash: input.reservation.trial_group_hash, portfolio_id: input.reservation.portfolio_id,
    sequence_plan_hash: input.plan.plan_hash, sequence_reservation_hash: input.reservation.reservation_hash,
    ...
    limitations: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_LIMITATIONS, fingerprint,
  }
  const evidence = { ...body,
    evidence_hash: replayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidenceHash(body) }
  assertReplayPortfolioProtectiveTakeProfitCancelCycleSequenceEvidence(evidence, input)
  return evidence
}

function createCommits(cycles: ReplayPortfolioProtectiveTakeProfitCancelCycleSource[]):
ReplayPortfolioProtectiveTakeProfitCancelCycleCommit[] {
  return cycles.map((cycle) => {
    ...
    const body: Omit<ReplayPortfolioProtectiveTakeProfitCancelCycleCommit, "cycle_commit_hash"> = {
    ...
    return { ...body, cycle_commit_hash: replayPortfolioProtectiveTakeProfitCancelCycleCommitHash(body) }
  })
}
function createTrialBalance(reservation: ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceAuthority,
  endingCash: number, journal: ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceJournalEntry[]):
ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceTrialBalance {
  const raw = Object.fromEntries(ACCOUNTS.map((account) => [account, 0])) as
    Record<ReplayPortfolioProtectiveTakeProfitCancelTerminalAccountingJournalAccount, number>
  ...
  const balances = Object.fromEntries(ACCOUNTS.map((account) => [account,
    ...
    Record<ReplayPortfolioProtectiveTakeProfitCancelTerminalAccountingJournalAccount, number>
  const body: Omit<ReplayPortfolioProtectiveTakeProfitCancelCycleSequenceTrialBalance, "trial_balance_hash"> = {
    settlement_asset: reservation.settlement_asset,
    journal_policy_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_CYCLE_SEQUENCE_JOURNAL_POLICY_VERSION,
  ...
  return { ...body, trial_balance_hash: replayPortfolioProtectiveTakeProfitCancelCycleSequenceTrialBalanceHash(body) }
}
