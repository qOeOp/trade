...
import {
  ...
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_REPLACEMENT_TERMINAL_ARTIFACT_ROLES,
  assertReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactManifest,
  replayPortfolioProtectiveTakeProfitReplacementTerminalArtifactManifestHash,
...
const MANIFEST = "portfolio-protective-take-profit-replacement-terminal-artifact-manifest.json"
const NAMES: Record<ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactRole, string> = {
...
export interface ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactPublishInput {
  ...
  evidence: ReplayPortfolioProtectiveTakeProfitReplacementTerminalEvidence
  authority_frozen_at: string
  artifact_store: ReplayArtifactStore
}

export function publishReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifact(
  input: ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactPublishInput,
): { manifest: ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactManifest; idempotent_replay: boolean } {
  const values: Record<ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactRole, unknown> = {
  ...
  const files = REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_REPLACEMENT_TERMINAL_ARTIFACT_ROLES.map((role) => {
    const name = NAMES[role]
    return { role, name, ...namespace.writeImmutable(name, encode(values[role])) }
  })
  const body: Omit<ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactManifest, "manifest_hash"> = {
    schema_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_REPLACEMENT_TERMINAL_ARTIFACT_MANIFEST_SCHEMA_VERSION,
    artifact_id: `replay-portfolio-protective-take-profit-replacement-terminal:${input.evidence.fingerprint.fingerprint_hash}`,
    ...
    completeness: {
      ...
      required_roles: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_REPLACEMENT_TERMINAL_ARTIFACT_ROLES,
  ...
  const manifest = {
    ...body,
    manifest_hash: replayPortfolioProtectiveTakeProfitReplacementTerminalArtifactManifestHash(body),
  }
...
function readCommitted(
  ...
  input: ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactPublishInput,
  values: Record<ReplayPortfolioProtectiveTakeProfitReplacementTerminalArtifactRole, unknown>,
