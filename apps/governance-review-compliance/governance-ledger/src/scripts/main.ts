...
export function run(args: Args): JSONRecord {
  ...
  try {
    ensureDatabaseIdentity(db, buildDatabaseIdentity(args.environmentId, "governance_ledger"), { allowLegacyMigration: args.migrateIdentity })
    ensureGovernanceLedgerSchema(db)
    if (args.action === "init") {
      return { ok: true, action: "init", db: displayPath(args.dbPath), environment_id: args.environmentId, store_id: "governance_ledger" }
    }
