...
the same machine when you need a meaningful delta. Record the hardware and
toolchain with every comparison.
...
**Don't claim a win without a bench.** Performance claims in engineering changes
should reference a benchmark or profile. "Faster" without
...
### 3. Workspace benchmark selection

The benchmark crates used by the aggregate local command are listed in the `CI_BENCH_CRATES` variable of the workspace
`Makefile` (currently `vibe-core`, `vibe-model`, `vibe-common`,
`vibe-execution`, `vibe-backtest`, and `vibe-live`). To opt a new
bench into the aggregate run, register it in its crate's `Cargo.toml` and
...
should run a local Criterion comparison against `main` for any PR that
...
Record benchmark results alongside the engineering change or in an adapter's
`BENCHMARKS.md` when that file is the maintained technical reference.
...
- **Build with the right profile.** Two profiles inherit from `release` and
  ...
  - `bench-lto`: adds `lto = "fat"` and `codegen-units = 1` to match the
    optimized binary. Use this for numbers recorded in a maintained
    `BENCHMARKS.md` or engineering change: `cargo bench --profile bench-lto`.
