...
struct VibeFormatter;

impl<S, N> FormatEvent<S, N> for VibeFormatter
where
...
/// format uses nanosecond timestamps to align with Vibe logging.
///
...
pub fn init_tracing() -> anyhow::Result<()> {
    ...
    let subscriber = tracing_subscriber::registry()
        .with(filter)
        .with(fmt::layer().event_format(VibeFormatter));
