...
//!
//! This module provides configuration for the Vibe logging subsystem via
//! the `LoggerConfig` and `FileWriterConfig` types.
//!
//! # Spec String Format
//!
//! The `VIBE_LOG` environment variable uses a semicolon-separated format:
//!
//! ```text
...
/// Configuration for the Vibe logger.
#[cfg_attr(
    feature = "python",
...
impl LoggerConfig {
    ...
    /// Parses configuration from the `VIBE_LOG` environment variable.
    ///
    /// # Errors
    ///
    /// Returns an error if the variable is unset or contains invalid syntax.
    pub fn from_env() -> anyhow::Result<Self> {
        let spec = env::var("VIBE_LOG")?;
        Self::from_spec(&spec)
    }
}
...
mod tests {
    ...
    fn test_from_spec_single_module_path() {
        let config = LoggerConfig::from_spec("vibe_okx::websocket=Debug").unwrap();
        assert_eq!(
            config.module_level[&Ustr::from("vibe_okx::websocket")],
            LevelFilter::Debug
        );
        assert!(config.component_level.is_empty());
    }
    ...
    fn test_from_spec_multiple_module_paths() {
        let config =
            LoggerConfig::from_spec("vibe_okx::websocket=Debug;vibe_binance::data=Trace").unwrap();
        assert_eq!(
            config.module_level[&Ustr::from("vibe_okx::websocket")],
            LevelFilter::Debug
        );
        assert_eq!(
            config.module_level[&Ustr::from("vibe_binance::data")],
            LevelFilter::Trace
        );
        assert!(config.component_level.is_empty());
    }
    ...
    fn test_from_spec_mixed_module_and_component() {
        let config = LoggerConfig::from_spec(
            "vibe_okx::websocket=Debug;RiskEngine=Error;vibe_network::data=Trace",
        )
        .unwrap();
        ...
        assert_eq!(
            config.module_level[&Ustr::from("vibe_network::data")],
            LevelFilter::Trace
        );
    ...
    fn test_from_spec_deeply_nested_module_path() {
        let config = LoggerConfig::from_spec("vibe_okx::websocket::handler::auth=Trace").unwrap();
        assert_eq!(
            config.module_level[&Ustr::from("vibe_okx::websocket::handler::auth")],
            LevelFilter::Trace
        );
    }
    ...
    fn test_from_spec_module_path_with_underscores() {
        let config =
            LoggerConfig::from_spec("vibe_trader::adapters::interactive_brokers=Debug").unwrap();
        assert_eq!(
            config.module_level[&Ustr::from("vibe_trader::adapters::interactive_brokers")],
            LevelFilter::Debug
        );
    }
    ...
    fn test_from_spec_full_example_with_modules() {
        let config = LoggerConfig::from_spec(
            "stdout=Info;fileout=Debug;RiskEngine=Error;vibe_okx::websocket=Trace;is_colored",
        )
        .unwrap();
