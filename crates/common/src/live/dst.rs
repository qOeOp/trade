...
//! `vibe_core::time::duration_since_unix_epoch`. Collapsing the two would
...
mod tests {
    use rstest::rstest;
    #[cfg(all(feature = "simulation", madsim))]
    use vibe_core::{datetime::NANOSECONDS_IN_SECOND, time::nanos_since_unix_epoch};
    ...
    // feature on `vibe-common` propagates `vibe-core/simulation`, so
    // `vibe_core::time::nanos_since_unix_epoch` routes through
