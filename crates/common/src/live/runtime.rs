//! The centralized Tokio runtime for a running Vibe system.
//!
//! # Design Rationale
//!
...
const VIBE_WORKER_THREADS: &str = "VIBE_WORKER_THREADS";
...
///
/// The number of OS threads is configured using the `VIBE_WORKER_THREADS`
...
fn initialize_runtime() -> tokio::runtime::Runtime {
    ...
    let worker_threads = std::env::var(VIBE_WORKER_THREADS)
        .ok()
...
/// Sets a custom pre-built Tokio runtime as the global Vibe runtime.
///
...
/// Returns a reference to the global Vibe Tokio runtime.
///
