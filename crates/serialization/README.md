...
```bash
cargo build -p vibe-serialization --features capnp
```
...
- `vibe_serialization::identifiers_capnp`
- `vibe_serialization::types_capnp`
- `vibe_serialization::enums_capnp`
- `vibe_serialization::trading_capnp`
- `vibe_serialization::data_capnp`
- `vibe_serialization::order_capnp`
- `vibe_serialization::position_capnp`
- `vibe_serialization::account_capnp`
- `vibe_serialization::market_capnp`

### Usage example

```rust
use vibe_model::types::Price;
use vibe_serialization::capnp::{ToCapnp, FromCapnp};
...
use vibe_serialization::capnp::{ToCapnp, FromCapnp, identifiers_capnp};
...
```rust
use vibe_core::serialization::Serializable;
use vibe_model::data::QuoteTick;
...
```rust
use vibe_core::serialization::{ToMsgPack, FromMsgPack};
use vibe_model::data::QuoteTick;
...
```rust
use vibe_model::data::QuoteTick;
use vibe_serialization::capnp::{ToCapnp, FromCapnp, market_capnp};
...
cargo bench -p vibe-serialization --features capnp --bench serialization_comparison -- QuoteTick

# Compare all formats for TradeTick
cargo bench -p vibe-serialization --features capnp --bench serialization_comparison -- TradeTick

# Compare all formats for Bar
cargo bench -p vibe-serialization --features capnp --bench serialization_comparison -- Bar

# Run all Cap'n Proto benchmarks (including OrderBookDeltas with varying sizes)
cargo bench -p vibe-serialization --features capnp --bench capnp_serialization

# Run all comparison benchmarks
cargo bench -p vibe-serialization --features capnp --bench serialization_comparison
```
...
cargo bench -p vibe-serialization --no-default-features --features sbe --bench sbe_decoding

# Run exhaustive market data wire benchmarks: Cap'n Proto vs SBE
cargo bench -p vibe-serialization --no-default-features --features "capnp sbe" --bench market_data_capnp_vs_sbe
```
