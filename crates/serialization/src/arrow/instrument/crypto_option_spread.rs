//! Arrow serialization for CryptoOptionSpread instruments.
...
use vibe_model::{
    ...
    instruments::crypto_option_spread::CryptoOptionSpread,
...
impl ArrowSchemaProvider for CryptoOptionSpread {
    fn get_schema(metadata: Option<HashMap<String, String>>) -> Schema {
        ...
        final_metadata.insert("class".to_string(), "CryptoOptionSpread".to_string());
...
impl EncodeToRecordBatch for CryptoOptionSpread {
    fn encode_batch(
        ...
        for co in data {
            ...
            strategy_type_builder.append_value(co.strategy_type.as_str());
...
/// Helper function to decode CryptoOptionSpread from RecordBatch
/// (Cannot implement DecodeFromRecordBatch trait due to `Into<Data>` bound)
///
/// # Errors
///
/// Returns an `EncodingError` if the RecordBatch cannot be decoded.
pub fn decode_crypto_option_spread_batch(
    #[allow(unused)] metadata: &HashMap<String, String>,
    record_batch: &RecordBatch,
) -> Result<Vec<CryptoOptionSpread>, EncodingError> {
    ...
    for i in 0..num_rows {
        ...
        let underlying = super::decode_currency(
            ...
            "underlying",
            "crypto_option_spread.underlying",
            i,
        )?;
        let quote_currency = super::decode_currency(
            quote_currency_values.value(i),
            "quote_currency",
            "crypto_option_spread.quote_currency",
            i,
        )?;
        let settlement_currency = super::decode_currency(
            settlement_currency_values.value(i),
            "settlement_currency",
            "crypto_option_spread.settlement_currency",
            i,
        )?;
        let is_inverse = is_inverse_values.value(i);
        let strategy_type = Ustr::from(strategy_type_values.value(i));
        let activation_ns = vibe_core::UnixNanos::from(activation_ns_values.value(i));
        let expiration_ns = vibe_core::UnixNanos::from(expiration_ns_values.value(i));
        ...
        let ts_event = vibe_core::UnixNanos::from(ts_event_values.value(i));
        let ts_init = vibe_core::UnixNanos::from(ts_init_values.value(i));

        let tick_scheme = optional_ustr_value(tick_scheme_values, i);

        let crypto_option_spread = CryptoOptionSpread::new_checked(
            id,
            raw_symbol,
            underlying,
            quote_currency,
        ...
        .map_err(|e| super::instrument_validation_error::<CryptoOptionSpread>(i, e))?;

        result.push(crypto_option_spread);
    }
