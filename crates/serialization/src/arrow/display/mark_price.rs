//! Display-mode Arrow encoder for [`MarkPriceUpdate`].
...
use vibe_model::data::MarkPriceUpdate;

use super::{float64_field, price_to_f64, timestamp_field, unix_nanos_to_i64, utf8_field};

/// Returns the display-mode Arrow schema for [`MarkPriceUpdate`].
#[must_use]
...
/// Encodes mark price updates as a display-friendly Arrow [`RecordBatch`].
///
...
/// [`crate::arrow::mark_prices_to_arrow_record_batch_bytes`] for catalog storage.
///
...
mod tests {
    ...
    use rstest::rstest;
    use vibe_model::{identifiers::InstrumentId, types::Price};
