...
use vibe_model::{data::QuoteTick, identifiers::InstrumentId, types::fixed::PRECISION_BYTES};
...
impl EncodeToRecordBatch for QuoteTick {
    fn encode_batch(
        ...
        for quote in data {
            bid_price_builder.append_value(quote.bid_price.raw.to_le_bytes())?;
            ask_price_builder.append_value(quote.ask_price.raw.to_le_bytes())?;
            bid_size_builder.append_value(quote.bid_size.raw.to_le_bytes())?;
            ask_size_builder.append_value(quote.ask_size.raw.to_le_bytes())?;
...
mod tests {
    ...
    fn test_decode_batch_invalid_ask_size_returns_error() {
        use vibe_model::types::{fixed::FIXED_PRECISION, quantity::QUANTITY_RAW_MAX};
