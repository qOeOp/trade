...
The `vibe-event-store` crate captures commands, generated events, raw venue reports,
...
`vibe-event-store` is a single-node, embedded event store for one trading instance. It defines:
...
```bash
cargo run -p vibe-event-store --bin verify -- /path/to/run.redb
```
