...
impl EventStore for BlockingMemoryBackend {
    fn open_run(&mut self, _: RunManifest) -> Result<(), vibe_event_store::EventStoreError> {
    ...
    fn append_batch(
        &mut self,
        entries: &[AppendEntry],
    ) -> Result<u64, vibe_event_store::EventStoreError> {
    ...
    ) -> Result<Vec<EventStoreEntry>, vibe_event_store::EventStoreError> {
        self.inner
            .lock()
            .expect("inner poisoned")
    ...
    fn scan_seq(
        &self,
        seq: u64,
    ) -> Result<Option<EventStoreEntry>, vibe_event_store::EventStoreError> {
    ...
    fn lookup(
        ...
        kind: IndexKind,
        key: &str,
    ) -> Result<Option<u64>, vibe_event_store::EventStoreError> {
    ...
    fn iter_index_keys(
        &self,
        kind: IndexKind,
    ) -> Result<Vec<(String, u64)>, vibe_event_store::EventStoreError> {
        self.inner
            .lock()
            .expect("inner poisoned")
    ...
    fn seal(&mut self, status: RunStatus) -> Result<(), vibe_event_store::EventStoreError> {
        self.inner.lock().expect("inner poisoned").seal(status)
    }

    fn manifest(&self) -> Result<RunManifest, vibe_event_store::EventStoreError> {
        self.inner.lock().expect("inner poisoned").manifest()
    }

    fn high_watermark(&self) -> Result<u64, vibe_event_store::EventStoreError> {
...
fn writer_commits_drafts_durably_to_redb_file() {
    ...
    match err {
        vibe_event_store::EventStoreError::Backend(msg) => {
...
fn writer_preserves_payload_and_hash_round_trip() {
    ...
    for row in entries.iter().expect("iter") {
        ...
        let bytes = v.value();
        let entry =
            codec::decode_from_slice::<vibe_event_store::EventStoreEntry>(bytes).expect("decode");
...
fn writer_does_not_seal_when_dropped_without_close() {
    ...
    match err {
        vibe_event_store::EventStoreError::CrashedPredecessor => {}
...
fn writer_stamps_ts_publish_from_clock_at_submit() {
    ...
    let mut decoded: Vec<vibe_event_store::EventStoreEntry> = Vec::new();

    for row in entries.iter().expect("iter") {
        let (_, v) = row.expect("row");
        let entry = codec::decode_from_slice::<vibe_event_store::EventStoreEntry>(v.value())
            .expect("decode");
        decoded.push(entry);
    }
