...
const TIMEOUT_ENV: &str = "VIBE_EVENT_STORE_VERIFY_TIMEOUT_SECS";
#[cfg(debug_assertions)]
const WORKER_SLEEP_ENV: &str = "VIBE_EVENT_STORE_VERIFY_SLEEP_WORKER_MS";
...
fn print_index_drift(kind: vibe_event_store::IndexKind, key: &str, drift: IndexDrift) {
    match drift {
...
fn abort_worker_when_requested() {
    #[cfg(debug_assertions)]
    if env::var_os("VIBE_EVENT_STORE_VERIFY_ABORT_WORKER").is_some() {
        std::process::abort();
    }
}
