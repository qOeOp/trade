...
//! uses the positional codec; MessagePack inside the payload handles the upstream Vibe
...
/// [`BookResponse`] holds a [`vibe_model::orderbook::OrderBook`] without serde
...
mod tests {
    ...
    #[cfg(feature = "defi")]
    use vibe_model::defi::Blockchain;
    use vibe_model::{
    ...
    // (gated by the `stubs` feature on `vibe-model`) with the test client_order_id
