...
//!
//! ```bash
//! cargo test --release -p vibe-live --test stress \
...
//!
//! ```bash
//! cargo nextest run --release -p vibe-live --test stress --run-ignored=ignored-only
//! ```
//!
...
//! # Scale via env var
//!
//! `VIBE_STRESS_SCALE=N` multiplies the burst counts by `N`. Default 1 keeps
...
//! VIBE_STRESS_SCALE=100 cargo flamegraph --profile release-debugging \
//!     -p vibe-live --test stress -o target/flamegraph/trade_burst.svg \
//!     -- --ignored --nocapture --test-threads=1 stress_trade_burst
//! ```
...
// Scale factor for burst counts, read from `VIBE_STRESS_SCALE` (default 1).
...
// and produces ~75 samples spread across many frames. `VIBE_STRESS_SCALE=100`
// yields ~7.5 s of runtime and ~7500 samples.
fn stress_scale() -> usize {
    std::env::var("VIBE_STRESS_SCALE")
        .ok()
