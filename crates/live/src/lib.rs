...
//!
//! ```toml
//! vibe-live = { workspace = true, default-features = false }
//! ```
//!
//! With `node` disabled, this crate exposes only `emitter` and `runner`, and skips
//! the transitive dependencies on `vibe-system`, `vibe-trading`,
//! `vibe-portfolio`, `vibe-risk`, and `vibe-data`.
//!
//! # Plug-in support
//!
//! The live crate does not host dynamic plug-ins directly.
//! `vibe-plugin` is the public guest ABI crate, while host-side loading,
...
//!
//! ```toml
//! vibe-live = { workspace = true, default-features = false, features = ["node"] }
//! ```
//!
...
pub use vibe_common::factories::OrderEventFactory;
pub use vibe_execution::client::core::ExecutionClientCore;
