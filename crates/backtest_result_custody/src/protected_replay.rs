    ...
    let exact: bool = sqlx::query_scalar(
        "WITH expected_function AS (
         ...
         SELECT session_user=$2 AND current_user=$2
           AND (SELECT count(*)=1 AND bool_and(pg_catalog.pg_get_userbyid(procedure.proowner)='backtest_custodian' AND language.lanname='plpgsql' AND procedure.prokind='f' AND NOT procedure.proleakproof AND procedure.prorettype='jsonb'::pg_catalog.regtype AND procedure.pronargs=3 AND procedure.prosecdef AND procedure.proisstrict AND procedure.provolatile='v' AND procedure.proparallel='u' AND procedure.proconfig=ARRAY['search_path=pg_catalog, pg_temp']::text[] AND procedure.prosrc=$1) FROM pg_catalog.pg_proc procedure JOIN pg_catalog.pg_language language ON language.oid=procedure.prolang WHERE procedure.oid=(SELECT oid FROM expected_function))
           ...
           AND (SELECT count(*)=1 AND bool_and(pg_catalog.pg_get_userbyid(procedure.proowner)='backtest_custodian' AND language.lanname='plpgsql' AND procedure.prokind='f' AND NOT procedure.proleakproof AND procedure.prorettype='jsonb'::pg_catalog.regtype AND procedure.pronargs=2 AND procedure.prosecdef AND procedure.proisstrict AND procedure.provolatile='v' AND procedure.proparallel='u' AND procedure.proconfig=ARRAY['search_path=pg_catalog, pg_temp']::text[] AND procedure.prosrc=$3) FROM pg_catalog.pg_proc procedure JOIN pg_catalog.pg_language language ON language.oid=procedure.prolang WHERE procedure.oid=(SELECT oid FROM frontier_function))
