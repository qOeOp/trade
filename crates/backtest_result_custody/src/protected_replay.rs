...
use vibe_backtest_owner_contracts::{
    ...
    ProtectedResultOutboxDtoV1, ProtectedResultReceiptDtoV1, protected_result_custody_wires_v1,
...
pub async fn validate_protected_replay_result_writer_topology_v1(
    ...
    sqlx::query(
        "LOCK TABLE public.backtest_protected_replay_results_v1, public.backtest_protected_replay_result_receipts_v1, public.backtest_protected_replay_result_outbox_v1 IN ROW EXCLUSIVE MODE",
    )
    ...
    let exact: bool = sqlx::query_scalar(
        "WITH expected_function AS (
         ...
         SELECT session_user=$2 AND current_user=$2
           ...
           AND (SELECT count(*)=3 FROM pg_catalog.pg_proc sibling JOIN pg_catalog.pg_namespace sibling_namespace ON sibling_namespace.oid=sibling.pronamespace WHERE sibling_namespace.nspname='backtest_owner_api')
           ...
           AND NOT EXISTS (SELECT 1 FROM pg_catalog.pg_attribute attribute CROSS JOIN LATERAL pg_catalog.aclexplode(attribute.attacl) acl WHERE attribute.attrelid IN ('public.backtest_protected_replay_results_v1'::pg_catalog.regclass,'public.backtest_protected_replay_result_receipts_v1'::pg_catalog.regclass,'public.backtest_protected_replay_result_outbox_v1'::pg_catalog.regclass) AND attribute.attnum>0 AND NOT attribute.attisdropped)
        ",
    )
