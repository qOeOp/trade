...
/// # Implementation
///
/// Use the `vibe_strategy!` macro to generate the native runtime wiring
...
mod tests {
    ...
    vibe_strategy!(MarketExitHookTrackingStrategy, {
        fn on_market_exit(&mut self) {
    ...
    vibe_strategy!(FailingPostExitStrategy, {
        fn post_market_exit(&mut self) {
    ...
    vibe_strategy!(MacroTestWithHooks, {
        fn on_order_rejected(&mut self, _event: OrderRejected) {}
    });
    ...
    vibe_strategy!(MacroTestCustomField, inner, {
        fn external_order_claims(&self) -> Option<Vec<InstrumentId>> {
            None
        }
    });
    ...
    #[rstest]
    fn test_vibe_strategy_macro_forms() {
        let config = StrategyConfig {
