...
///
/// ```ignore
/// use vibe_trading::{vibe_strategy, strategy::StrategyCore};
///
...
/// }
///
/// vibe_strategy!(MyStrategy, strat_core, {
...
/// ```
#[macro_export]
macro_rules! vibe_strategy {
    ($ty:ty) => {
        $crate::vibe_strategy!($ty, core, {});
    };
    ($ty:ty, $field:ident) => {
        $crate::vibe_strategy!($ty, $field, {});
    };
    ($ty:ty, { $($extra:item)* }) => {
        $crate::vibe_strategy!($ty, core, { $($extra)* });
    };
...
///
/// ```ignore
/// use vibe_trading::{algorithm::ExecutionAlgorithmCore, vibe_execution_algorithm};
///
...
/// }
///
/// vibe_execution_algorithm!(MyExecutionAlgorithm, {
...
/// }
///
/// vibe_execution_algorithm!(MyExecutionAlgorithm, algorithm_core, {
///     fn on_order(&mut self, order: OrderAny) -> anyhow::Result<()> {
///         // custom handling
///         Ok(())
///     }
...
/// ```
#[macro_export]
macro_rules! vibe_execution_algorithm {
    ($ty:ty) => {
        compile_error!(
            "vibe_execution_algorithm! requires an `on_order` implementation block"
        );
    };
    ($ty:ty, $field:ident) => {
        compile_error!(
    ...
    ($ty:ty, { $($extra:item)* }) => {
        $crate::vibe_execution_algorithm!($ty, core, { $($extra)* });
    };
    ($ty:ty, $field:ident, { $($extra:item)* }) => {
        impl $crate::_macro_reexports::DataActorNative for $ty {
            fn core(&self) -> &$crate::_macro_reexports::DataActorCore {
