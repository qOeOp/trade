...
use vibe_execution::models::latency::StaticLatencyModel;
use vibe_indicators::{
...
use vibe_system::trader::Trader;
use vibe_trading::{
    ExecutionAlgorithm, ExecutionAlgorithmConfig, ExecutionAlgorithmCore, Strategy, StrategyConfig,
    StrategyCore, TwapAlgorithm, TwapAlgorithmConfig, vibe_execution_algorithm, vibe_strategy,
};
...
vibe_strategy!(FailingStartStrategy);
...
vibe_execution_algorithm!(EmptyExecAlgorithm, {
...
vibe_strategy!(TwapOrderStrategy);
...
vibe_strategy!(SnapshotNettingFlip);
...
vibe_strategy!(OpenOptionOnQuote);
...
fn create_inverse_funding_engine() -> (BacktestEngine, InstrumentId) {
    let instrument =
        InstrumentAny::CryptoPerpetual(vibe_model::instruments::stubs::xbtusd_bitmex());
    let instrument_id = instrument.id();
    let mut engine = BacktestEngine::new(BacktestEngineConfig::default()).unwrap();
...
vibe_strategy!(ShutdownBeforeFutureTimer);
...
vibe_strategy!(ShutdownFromTimer);
...
vibe_strategy!(ShutdownAndScheduleNewAlert, {
...
fn test_multi_venue_data_routing(crypto_perpetual_ethusdt: CryptoPerpetual) {
    ...
    let btc = InstrumentAny::CryptoPerpetual(vibe_model::instruments::stubs::xbtusd_bitmex());
    let btc_id = btc.id();
...
vibe_strategy!(CascadingStopStrategy, {
...
vibe_strategy!(EmulatedStopEntryOnQuote);
...
vibe_strategy!(EmulatedTrailingStopOnQuote);
...
vibe_strategy!(EmulatedBracketOnQuote);
...
vibe_strategy!(DualTimerStrategy);
...
vibe_strategy!(SameTimestampTimerObserver);
...
vibe_strategy!(BarSubscriberStrategy);
...
vibe_strategy!(OpenOnEveryQuote);
...
vibe_strategy!(MultiInstrumentCloseOnStop);
