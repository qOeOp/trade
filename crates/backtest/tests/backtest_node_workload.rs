...
//! [`CanonicalBacktestResult`](vibe_backtest::result::CanonicalBacktestResult).
//!
//! Run with:
//! ```bash
//! cargo test -p vibe-backtest --features streaming \
...
use ustr::Ustr;
use vibe_backtest::{
    config::{BacktestDataConfig, BacktestRunConfig, BacktestVenueConfig, VibeDataType},
    node::BacktestNode,
...
fn validate_canonical_result(result: &CanonicalBacktestResult) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        result.as_value()["schema"] == "vibe-backtest-result/v1",
...
mod tests {
    ...
    fn test_workload_writes_repeatable_bounded_canonical_result() {
        ...
        assert_eq!(value["schema"], "vibe-backtest-result/v1");
    ...
    fn generate_native_repeat_catalog() {
        let catalog_path = env::var_os("VIBE_NATIVE_REPEAT_CATALOG_PATH")
            .map(PathBuf::from)
            .expect("VIBE_NATIVE_REPEAT_CATALOG_PATH must be set");
        fs::create_dir(&catalog_path).unwrap();
        write_catalog(&catalog_path);
    }
