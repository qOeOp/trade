...
use vibe_system::config::{StreamingConfig, VibeKernelConfig};
...
impl Display for VibeDataType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        std::fmt::Debug::fmt(self, f)
    }
}

impl FromStr for VibeDataType {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self> {
        match s {
            ...
            _ => anyhow::bail!("Invalid `VibeDataType`: '{s}'"),
        }
    }
}
...
impl VibeKernelConfig for BacktestEngineConfig {
...
impl BacktestDataConfig {
    ...
    #[must_use]
    pub const fn data_type(&self) -> VibeDataType {
        self.data_type
    }
    ...
    pub fn query_identifiers(&self) -> Option<Vec<String>> {
        if self.data_type == VibeDataType::Bar {
