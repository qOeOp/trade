...
use vibe_persistence::backend::{catalog::ParquetDataCatalog, session::QueryResult};

use crate::{
    config::{BacktestDataConfig, BacktestRunConfig, SimulatedVenueConfig, VibeDataType},
            ...
            if needs_book_data {
                ...
                let has_book_data = config.data().iter().any(|dc| {
                    let is_book_type = matches!(
                        dc.data_type(),
                        VibeDataType::OrderBookDelta | VibeDataType::OrderBookDepth10
                    );
