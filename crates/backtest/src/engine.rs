...
use vibe_analysis::analyzer::PortfolioAnalyzer;
use vibe_common::{
...
use vibe_data::client::DataClientAdapter;
use vibe_execution::models::fill::FillModelHandle;
use vibe_model::{
...
impl BacktestEngine {
    ...
    /// # Errors
    ///
    /// Returns an error if the core `VibeKernel` fails to initialize.
    pub fn new(mut config: BacktestEngineConfig) -> anyhow::Result<Self> {
        ...
        let kernel = VibeKernel::new("BacktestEngine".to_string(), config.clone())?;
    ...
    pub fn kernel_mut(&mut self) -> &mut VibeKernel {
        &mut self.kernel
    }
    ...
    pub fn get_canonical_result(&self) -> anyhow::Result<CanonicalBacktestResult> {
        ...
        let diagnostics = self
            ...
            .into_iter()
            .collect();
        let statistics = vibe_analysis::PortfolioStatistics {
...
mod tests {
    ...
    use vibe_execution::engine::{SnapshotAnchorer, stubs::StubExecutionClient};
    use vibe_model::{
    ...
    use vibe_system::{KernelEventStore, RegisteredComponents};
    use vibe_testkit::{
    ...
    fn create_engine_with_replay_store(fail_restore: bool) -> BacktestEngine {
        ...
        engine.kernel = VibeKernel::new_with(
    ...
    fn test_new_forces_drop_instruments_on_reset_false(
        ...
        use vibe_common::cache::CacheConfig;
