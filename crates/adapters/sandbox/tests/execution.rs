...
use vibe_sandbox::{SandboxExecutionClient, SandboxExecutionClientConfig};
...
fn publish_expired_close(
    ...
    msgbus::publish_any(
        vibe_common::msgbus::switchboard::get_instrument_close_topic(instrument.id()),
        &close,
    );
}
...
fn setup_pending_resolution_harness(
    ...
    vibe_common::live::runner::replace_exec_event_sender(tx);
    client.start().unwrap();
...
fn test_paper_binary_option_pending_resolution_then_close_settlement(
    ...
    msgbus::publish_any(
        vibe_common::msgbus::switchboard::get_instrument_close_topic(harness.instrument.id()),
        &close,
    );
...
fn test_instrument_status_lazy_creates_but_close_requires_existing_engine(
    ...
    msgbus::publish_any(
        vibe_common::msgbus::switchboard::get_instrument_status_topic(instrument.id()),
        &status,
    );
    ...
    msgbus::publish_any(
        vibe_common::msgbus::switchboard::get_instrument_close_topic(second_instrument.id()),
        &close,
    );
...
fn test_process_bar_disabled(test_context: TestContext, instrument: InstrumentAny) {
    use vibe_model::data::{Bar, BarType};
...
// Regression test for source issue #3732
//
