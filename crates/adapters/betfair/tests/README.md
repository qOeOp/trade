...
- `harness/mod.rs`: the reusable harness, made up of:
  ...
  This module is the unit to extract into a shared `vibe-live-testkit` crate once a second
...
```bash
cargo nextest run -p vibe-betfair --test live
cargo nextest run -p vibe-betfair --test node
```
...
The `node` feature on `vibe-live` (which pulls the builder/config/node modules) is enabled for
