//! Parsing utilities that convert Betfair stream messages into Vibe domain models.
//!
...
///
/// Iterates `def.runners` and maps each runner's lifecycle to a Vibe status.
...
/// Parses a streaming [`UnmatchedOrder`] into a Vibe [`OrderStatusReport`].
///
/// Resolves the Vibe order status from the Betfair streaming status
...
mod tests {
    use rstest::rstest;
    use vibe_model::enums::{MarketStatusAction, OrderStatus, TimeInForce};
