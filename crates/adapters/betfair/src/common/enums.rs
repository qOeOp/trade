...
use vibe_model::enums::{
    MarketStatus as VibeMarketStatus, OrderSide, OrderStatus, OrderType, TimeInForce,
};
...
/// Resolves the Vibe `OrderStatus` for a Betfair order.
///
...
/// Resolves the Vibe `OrderStatus` for a streaming order update.
///
...
fn resolve_terminal_status(size_matched: Decimal, size_cancelled: Decimal) -> OrderStatus {
    if size_matched > Decimal::ZERO && size_cancelled <= Decimal::ZERO {
        ...
        // partially matched. PartiallyFilled is an open status in Vibe
        // and must not be used for ExecutionComplete orders.
        OrderStatus::Canceled
    }
}

impl From<MarketStatus> for VibeMarketStatus {
...
mod tests {
    ...
    #[rstest]
    #[case(MarketStatus::Open, VibeMarketStatus::Open)]
    #[case(MarketStatus::Closed, VibeMarketStatus::Closed)]
    #[case(MarketStatus::Suspended, VibeMarketStatus::Suspended)]
    #[case(MarketStatus::Inactive, VibeMarketStatus::NotAvailable)]
    fn test_market_status(#[case] input: MarketStatus, #[case] expected: VibeMarketStatus) {
        assert_eq!(VibeMarketStatus::from(input), expected);
    }
