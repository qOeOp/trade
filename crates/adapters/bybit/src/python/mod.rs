...
use crate::{
    common::{
        consts::{BYBIT, BYBIT_CLIENT_ID, BYBIT_VENUE, BYBIT_VIBE_BROKER_ID},
...
#[pyfunction]
#[pyo3_stub_gen::derive::gen_stub_pyfunction(module = "vibe_trader.adapters.bybit")]
#[pyo3(name = "bybit_extract_raw_symbol")]
fn py_bybit_extract_raw_symbol(symbol: &str) -> &str {
    extract_raw_symbol(symbol)
}

/// Converts a Vibe bar aggregation and step to a Bybit kline interval string.
///
/// # Errors
///
...
/// Exposed through `vibe_trader.adapters.bybit`.
///
/// # Errors
///
...
pub fn bybit(_: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add(stringify!(BYBIT_VIBE_BROKER_ID), BYBIT_VIBE_BROKER_ID)?;
