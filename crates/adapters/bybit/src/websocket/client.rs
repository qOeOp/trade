...
use vibe_core::{AtomicMap, AtomicSet, UUID4, consts::VIBE_USER_AGENT};
use vibe_model::{
    data::BarType,
...
use crate::{
    common::{
        consts::{BYBIT_VIBE_BROKER_ID, BYBIT_WS_TOPIC_DELIMITER},
...
impl BybitWebSocketClient {
    ...
    pub async fn place_order(&self, params: BybitWsPlaceOrderParams) -> BybitWsResult<String> {
        ...
        let referer = if self.include_referer_header(params.time_in_force) {
            Some(BYBIT_VIBE_BROKER_ID.to_string())
        } else {
            None
        };
    ...
    /// Submits an order using Vibe domain objects.
    ///
    /// # Errors
    ///
    ...
    /// Modifies an existing order using Vibe domain objects.
    ///
    /// # Errors
    ///
    ...
    /// Cancels an order using Vibe domain objects.
    ///
    /// # Errors
    ///
