...
use ustr::Ustr;
use vibe_common::{cache::quote::QuoteCache, live::get_runtime};
use vibe_core::{
    UUID4, UnixNanos,
...
use crate::{
    ...
    websocket::{
        ...
        messages::{
            ExecutionReport, OKXAlgoOrderMsg, OKXBookMsg, OKXOptionSummaryMsg, OKXOrderMsg,
            OKXRpiBookMsg, OKXWebSocketError, OKXWsMessage, VibeWsMessage, WsAttachAlgoOrdParams,
...
fn handle_channel_data(
    ...
    match parse_ws_message_data(
        ...
        Ok(Some(ws_msg)) => {
            dispatch_vibe_ws_msg_to_python(ws_msg, call_soon, callback, instruments_by_symbol);
        }
        Ok(None) => {}
        Err(e) => {
...
fn dispatch_vibe_ws_msg_to_python(
    msg: VibeWsMessage,
    call_soon: &Py<PyAny>,
    callback: &Py<PyAny>,
    instruments_by_symbol: &mut AHashMap<Ustr, InstrumentAny>,
    ...
    match msg {
        VibeWsMessage::Data(payloads) => Python::attach(|py| {
            for data in payloads {
                send_data_to_python(py, data, call_soon, callback);
            }
        }),
        VibeWsMessage::Deltas(deltas) => Python::attach(|py| {
            send_data_to_python(
                py,
        ...
        VibeWsMessage::FundingRates(updates) => {
            for data in updates {
                call_python_with_data(call_soon, callback, |py| data.into_py_any(py));
            }
        }
        VibeWsMessage::Instrument(instrument, status) => {
