//! Enumerations mapping OKX concepts onto idiomatic Vibe variants.
...
impl TryFrom<OrderStatus> for OKXOrderStatus {
    ...
    /// Converts a Vibe [`OrderStatus`] into the matching [`OKXOrderStatus`].
    ///
...
impl TryFrom<OKXOptionType> for OptionKind {
    ...
    /// Converts an OKX option type into the matching Vibe [`OptionKind`].
    ///
...
mod tests {
    ...
    use rstest::rstest;
    use vibe_model::enums::{GreeksConvention, OptionKind, OrderStatus, OrderType};
...
/// Converts Vibe conditional order types to OKX algo order type.
///
/// # Errors
///
