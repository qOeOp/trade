//! Parsing utilities that convert OKX payloads into Vibe domain models.
...
use ustr::Ustr;
pub use vibe_core::serialization::{
...
use vibe_core::{Params, UUID4, datetime::NANOSECONDS_IN_MILLISECOND, nanos::UnixNanos};
use vibe_model::{
    data::{
...
/// Maps an [`OKXInstrumentStatus`] to a Vibe [`MarketStatusAction`].
#[must_use]
...
/// Parses a Vibe instrument ID from the given OKX `symbol` value.
#[must_use]
pub fn parse_instrument_id(symbol: Ustr) -> InstrumentId {
    InstrumentId::new(Symbol::from_ustr_unchecked(symbol), *OKX_VENUE)
}

/// Parses a Vibe client order ID from the given OKX `clOrdId` value.
#[must_use]
...
pub fn parse_fee(value: Option<&str>, currency: Currency) -> anyhow::Result<Money> {
    ...
    // Negate to match Vibe convention: positive = cost, negative = rebate.
...
/// Parses OKX side to Vibe aggressor side.
...
/// Parses OKX execution type to Vibe liquidity side.
...
/// Parses quantity to Vibe position side.
...
/// Parses an OKX mark price record into a Vibe [`MarkPriceUpdate`].
///
/// # Errors
///
...
/// Parses an OKX index ticker record into a Vibe [`IndexPriceUpdate`].
///
/// # Errors
///
...
/// Parses an OKX trade record into a Vibe [`TradeTick`].
///
/// # Errors
///
...
/// Parses an OKX historical candlestick record into a Vibe [`Bar`].
///
/// # Errors
///
...
/// Parses an OKX order history record into a Vibe [`OrderStatusReport`].
///
/// # Errors
///
...
/// Parses an OKX position into a Vibe [`PositionStatusReport`].
///
...
pub fn parse_position_status_report(
    ...
    // Convert to absolute quantity (positions are always positive in Vibe)
    let quantity = Quantity::from_decimal_dp(quantity_dec, size_precision)?;
    ...
    let venue_position_id = match position.pos_side {
        ...
        OKXPositionSide::Net | OKXPositionSide::None => {
            // Net mode: None signals NETTING OMS (Vibe uses its own position IDs)
            None
        }
    };
...
/// Parses an OKX transaction detail into a Vibe `FillReport`.
///
/// # Errors
///
...
/// Parses an OKX spread order record into a Vibe [`OrderStatusReport`].
///
/// # Errors
///
...
/// Parses an OKX spread trade into a Vibe [`FillReport`].
///
/// # Errors
///
...
/// parsing each message, and wrapping results in Vibe Data enum variants.
///
/// # Errors
///
...
/// Converts a Vibe bar specification into the matching OKX candle channel.
///
/// # Errors
///
...
/// Converts Vibe bar specification to OKX mark price channel.
///
/// # Errors
///
...
/// Converts Vibe bar specification to OKX timeframe string.
///
/// # Errors
///
...
/// Converts OKX timeframe string to Vibe bar specification.
///
/// # Errors
///
...
/// This ensures the BarType uses canonical Vibe format instead of raw OKX strings.
///
/// # Errors
///
...
/// Parses an OKX instrument definition into a Vibe instrument.
///
/// # Errors
///
...
/// Parses an OKX spread definition into a Vibe crypto spread.
///
/// # Errors
///
...
/// Parses an OKX spot instrument definition into a Vibe currency pair.
///
/// # Errors
///
/// Returns an error if the instrument definition cannot be parsed.
...
/// Parses an OKX swap instrument definition into a Vibe crypto perpetual.
///
/// # Errors
///
/// Returns an error if the instrument definition cannot be parsed.
...
/// Parses an OKX futures instrument definition into a Vibe crypto future.
///
/// # Errors
///
/// Returns an error if the instrument definition cannot be parsed.
...
/// Parses an OKX option instrument definition into a Vibe option contract.
///
/// # Errors
///
/// Returns an error if the instrument definition cannot be parsed.
pub fn parse_option_instrument(
    ...
    let option_kind: OptionKind = OptionKind::try_from(definition.opt_type).map_err(|kind| {
        anyhow::anyhow!(
            "Unsupported `optType` '{kind:?}' for {}: cannot map to Vibe OptionKind",
            definition.inst_id
        )
    })?;
...
/// Parses an OKX event contract instrument definition into a Vibe binary option.
///
/// # Errors
///
/// Returns an error if the instrument definition cannot be parsed.
...
/// Parses an OKX account into a Vibe account state.
///
...
pub fn parse_account_state(
    ...
    // Ensure at least one balance exists (Vibe requires non-empty balances)
...
mod tests {
    ...
    use vibe_model::{identifiers::PositionId, instruments::Instrument};
    ...
    fn test_parse_instrument_any_passes_through_fees() {
        // parse_instrument_any receives fees already converted to Vibe format
        // (negation happens in HTTP client when parsing OKX API values)
        let json_data = load_test_json("http_get_instruments_spot.json");
        let response: OKXResponse<OKXInstrument> = serde_json::from_str(&json_data).unwrap();
        let okx_inst = response.data.first().unwrap();

        // Fees are already in Vibe convention (negated by HTTP client)
        let maker_fee = Some(dec!(-0.00025)); // Vibe: rebate (negative)
        let taker_fee = Some(dec!(0.00050)); // Vibe: commission (positive)
