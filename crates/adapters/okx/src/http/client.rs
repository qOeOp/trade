...
use vibe_core::{
    AtomicMap, AtomicTime, UnixNanos, consts::VIBE_USER_AGENT,
...
use crate::{
    common::{
        consts::{
            OKX_FIELD_SCODE, OKX_FIELD_SMSG, OKX_HTTP_URL, OKX_SUPPORTED_ORDER_TYPES,
            OKX_SUPPORTED_TIME_IN_FORCE, OKX_VIBE_BROKER_ID,
        },
...
impl OKXRawHttpClient {
    ...
    fn default_headers(environment: OKXEnvironment) -> HashMap<String, String> {
        let mut headers = HashMap::from([(USER_AGENT.to_string(), VIBE_USER_AGENT.to_string())]);
...
impl OKXHttpClient {
    ...
    pub async fn request_instruments(
        ...
        for inst in &resp {
            ...
            // Vibe convention: negative = rebate, positive = commission
    ...
    ///
    /// This is a convenience method that accepts Vibe domain types
    ...
    pub async fn place_order_with_domain_types(
        ...
        let request = OKXPlaceOrderRequest {
            ...
            tag: Some(OKX_VIBE_BROKER_ID.to_string()),
            side,
            pos_side,
            ord_type,
