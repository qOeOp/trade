//! Functions translating raw OKX WebSocket frames into Vibe data types.
...
use ustr::Ustr;
use vibe_core::{MUTEX_POISONED, UUID4, nanos::UnixNanos};
use vibe_model::{
    data::{
...
use crate::{
    ...
    websocket::messages::{ExecutionReport, OKXFundingRateMsg, VibeWsMessage},
};
...
/// Parses vector of OKX book messages into Vibe order book deltas.
///
/// # Errors
///
...
/// Parses RPI book messages into Vibe order book deltas.
///
/// # Errors
///
...
/// Parses vector of OKX ticker messages into Vibe quote ticks.
///
/// # Errors
///
...
/// Parses vector of OKX book messages into Vibe quote ticks.
///
/// # Errors
///
...
/// Parses vector of OKX trade messages into Vibe trade ticks.
///
/// # Errors
///
...
/// Parses vector of OKX mark price messages into Vibe mark price updates.
///
/// # Errors
///
...
/// Parses vector of OKX index price messages into Vibe index price updates.
///
/// # Errors
///
...
/// Parses vector of OKX funding rate messages into Vibe funding rate updates.
...
/// Parses vector of OKX candle messages into Vibe bars.
///
/// # Errors
///
...
/// Parses vector of OKX book messages into Vibe depth10 updates.
///
/// # Errors
///
...
/// Parses an OKX book message into Vibe order book deltas.
///
/// # Errors
///
...
/// Parses an RPI book message into Vibe order book deltas.
///
/// # Errors
///
...
/// Parses an OKX book message into a Vibe quote tick.
///
/// # Errors
///
...
/// Parses an OKX book message into a Vibe [`OrderBookDepth10`].
///
...
/// Parses an OKX ticker message into a Vibe quote tick.
///
/// # Errors
///
...
/// Parses an OKX trade message into a Vibe trade tick.
///
/// # Errors
///
...
/// Parses an OKX mark price message into a Vibe mark price update.
///
/// # Errors
///
...
/// Parses an OKX index price message into a Vibe index price update.
///
/// # Errors
///
...
/// Parses an OKX candle message into a Vibe bar.
///
/// # Errors
///
...
/// Parses vector of OKX order messages into Vibe execution reports.
///
/// # Errors
///
...
/// Parses an OKX algo order message into a Vibe execution report.
///
/// # Errors
///
...
/// Parses an OKX algo order message into a Vibe order status report.
///
/// # Errors
///
...
/// Parses an OKX order message into a Vibe order status report.
///
/// # Errors
///
...
pub fn parse_order_status_report(
    ...
    // matches the tracked order in Vibe.
...
/// Parses an OKX order message into a Vibe fill report.
///
/// # Errors
///
...
/// Parses OKX WebSocket message payloads into Vibe data structures.
///
/// # Errors
///
...
) -> anyhow::Result<Option<VibeWsMessage>> {
    match channel {
        OKXWsChannel::Instruments => {
            if let Ok(msg) = serde_json::from_value::<OKXInstrument>(data) {
                ...
                match parse_instrument_any(
                    ...
                    Ok(Some(inst_any)) => Ok(Some(VibeWsMessage::Instrument(
                        Box::new(inst_any),
                        Some(status),
                    ))),
                    Ok(None) => {
                        log::warn!("Empty instrument payload: {msg:?}");
                        Ok(Some(VibeWsMessage::InstrumentStatus(status)))
                    }
                    Err(e) => {
        ...
        OKXWsChannel::BboTbt => {
            ...
            Ok(Some(VibeWsMessage::Data(data_vec)))
        }
        ...
        OKXWsChannel::FundingRate => {
            ...
            Ok(Some(VibeWsMessage::FundingRates(data_vec)))
        }
        OKXWsChannel::EventContractMarkets => Ok(Some(VibeWsMessage::Raw(data))),
...
mod tests {
    ...
    fn test_websocket_instrument_update_preserves_cached_fees() {
        ...
        // These values are already in Vibe format (HTTP client negates OKX values)
        let initial_fees = (
            Some(Decimal::new(8, 4)),  // Vibe: 0.0008 (commission)
            Some(Decimal::new(10, 4)), // Vibe: 0.0010 (commission)
        );
        ...
        if let Some(VibeWsMessage::Instrument(boxed_inst, _status)) = result {
    ...
    fn test_quote_cache_complete_bbo_tbt_message() {
        use vibe_common::cache::quote::QuoteCache;
    ...
    fn test_parse_instruments_channel_produces_status() {
        ...
        match result {
            Some(VibeWsMessage::Instrument(inst, status)) => {
    ...
    fn test_parse_instruments_channel_returns_status_when_definition_invalid() {
        ...
        match result {
            Some(VibeWsMessage::InstrumentStatus(status)) => {
    ...
    fn test_parse_instruments_channel_suspend_status() {
        ...
        match result {
            Some(VibeWsMessage::Instrument(_, status)) => {
                let status = status.expect("Expected InstrumentStatus");
    ...
    fn test_parse_event_contract_markets_returns_raw_message() {
        ...
        match result {
            Some(VibeWsMessage::Raw(raw)) => assert_eq!(raw, data),
    ...
    fn test_parse_ws_message_data_spread_public_trades() {
        ...
        let Some(VibeWsMessage::Data(data_vec)) = result else {
