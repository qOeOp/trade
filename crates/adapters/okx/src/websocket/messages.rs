...
fn parse_book_data<E: serde::de::Error>(
    ...
    if arg.channel == OKXWsChannel::BooksRpi {
        let data: Vec<OKXRpiBookMsg> = serde_json::from_value(data_val)
...
mod tests {
    ...
    fn test_vibe_message_variants() {
        ...
        let error_msg = VibeWsMessage::Error(error);
        ...
        for raw_data in raw_scenarios {
            let raw_msg = VibeWsMessage::Raw(raw_data.clone());
