...
use vibe_common::{providers::InstrumentProvider, testing::wait_until_async};
use vibe_model::{identifiers::InstrumentId, instruments::Instrument};
use vibe_network::{http::HttpClient, retry::RetryConfig};
use vibe_polymarket::{
    common::{
...
async fn test_fetch_configured_instruments_uses_rust_event_slug_builder_result() {
    ...
    let instruments =
        vibe_polymarket::providers::fetch_configured_instruments(&http_client, &config, &[])
            .await
...
async fn test_request_trade_ticks_end_limit_counts_exact_boundary() {
    ...
    let ticks = client
        .request_trade_ticks(
            ...
            2,
            None,
            Some(vibe_core::UnixNanos::from(1_710_001_000_000_000_000_u64)),
            Some(2),
        )
        .await
        .unwrap();
...
async fn test_request_trade_ticks_honors_start_end_and_limit_after_token_filtering() {
    ...
    let addr = start_mock_server(state.clone()).await;
    let client = create_data_api_client(&addr);
    let start = vibe_core::UnixNanos::from(1_710_000_002_000_000_000_u64);
    let end = vibe_core::UnixNanos::from(1_710_000_004_999_999_999_u64);
...
async fn test_request_trade_ticks_rejects_start_at_offset_ceiling(#[case] limit: Option<u32>) {
    ...
    let error = client
        .request_trade_ticks(
            ...
            2,
            2,
            Some(vibe_core::UnixNanos::from(1_710_000_000_000_000_000_u64)),
            None,
            limit,
        )
        .await
