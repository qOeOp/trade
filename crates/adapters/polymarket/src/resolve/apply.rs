...
use vibe_common::messages::DataEvent;
use vibe_core::{AtomicMap, time::AtomicTime};
use vibe_model::{
    data::{Data as VibeData, InstrumentClose, InstrumentStatus},
...
pub(crate) fn apply_condition_resolution(
    ...
    for tracked in &tracked_instruments {
        ...
        if let Err(e) = ctx
            .data_sender
            .send(DataEvent::Data(VibeData::InstrumentClose(close)))
        {
            log::error!(
...
mod tests {
    ...
    use rstest::rstest;
    use vibe_common::messages::DataEvent;
    use vibe_core::{AtomicMap, UnixNanos, time::get_atomic_clock_realtime};
    use vibe_model::identifiers::{InstrumentId, PositionId};
