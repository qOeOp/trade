...
use ustr::Ustr;
use vibe_common::{live::get_runtime, messages::DataEvent};
use vibe_core::{AtomicMap, AtomicSet, time::AtomicTime};
use vibe_model::{
    data::{Data as VibeData, InstrumentStatus, OrderBookDeltas, OrderBookDeltas_API, QuoteTick},
                ...
                match parse_book_snapshot(
                    ...
                    Ok(deltas) => {
                        ...
                        if let Some(deltas) = emit {
                            let data: VibeData = OrderBookDeltas_API::new(deltas).into();
        ...
        MarketWsMessage::LastTradePrice(trade) => {
            ...
            if ctx.active_trade_subs.contains(&instrument_id) {
                ...
                match parse_trade_tick(
                    ...
                    Ok(tick) => {
                        if let Err(e) = ctx.data_sender.send(DataEvent::Data(VibeData::Trade(tick)))
                        {
...
fn emit_quote_if_changed(ctx: &WsMessageContext, instrument_id: InstrumentId, quote: QuoteTick) {
    ...
    if emit {
        ...
        if let Err(e) = ctx
            .data_sender
            .send(DataEvent::Data(VibeData::Quote(quote)))
        {
...
mod tests {
    ...
    use vibe_core::{Params, UUID4, UnixNanos, time::get_atomic_clock_realtime};
    use vibe_model::{
    ...
    use vibe_network::{retry::RetryConfig, websocket::TransportBackend};
    ...
    fn count_instrument_close_events(events: &[DataEvent]) -> usize {
        events
            .iter()
            .filter(|event| matches!(event, DataEvent::Data(VibeData::InstrumentClose(_))))
            .count()
    }
    ...
    fn market_resolved_emits_grouped_close_and_removes_watch_entry() {
        ...
        for event in events {
            if let DataEvent::Data(VibeData::InstrumentClose(close)) = event {
    ...
    fn price_change_emits_delta_when_not_pending() {
        ...
        assert!(
            events
                .iter()
                .any(|e| matches!(e, DataEvent::Data(VibeData::Deltas(_)))),
    ...
    fn price_change_batches_interleaved_changes_by_instrument() {
        ...
        let batches: Vec<_> = events
            ...
            .filter_map(|event| match event {
                DataEvent::Data(VibeData::Deltas(deltas)) => Some(deltas),
                _ => None,
            })
            .collect();
        let quote_instruments: Vec<_> = events
            .iter()
            .filter_map(|event| match event {
                DataEvent::Data(VibeData::Quote(quote)) => Some(quote.instrument_id),
                _ => None,
            })
            .collect();
        ...
        assert!(vibe_model::orderbook::analysis::book_check_integrity(&book_a).is_ok());
        assert!(vibe_model::orderbook::analysis::book_check_integrity(&book_b).is_ok());
    }
    ...
    fn quote_path_open_during_pending_window() {
        ...
        let emitted_quote = events
            .iter()
            .find_map(|e| match e {
                DataEvent::Data(VibeData::Quote(q)) => Some(q),
                _ => None,
            })
    ...
    fn price_change_missing_side_quote_drops_by_default() {
        ...
        assert!(
            !events
                .iter()
                .any(|e| matches!(e, DataEvent::Data(VibeData::Quote(_)))),
    ...
    fn collect_delta_batches(
        ...
        std::iter::from_fn(|| data_rx.try_recv().ok())
            .filter_map(|event| match event {
                DataEvent::Data(VibeData::Deltas(deltas)) => Some(deltas.into_inner()),
                _ => None,
            })
            .collect()
    }
