...
use vibe_core::{UnixNanos, time::AtomicTime};
use vibe_model::{
    data::{CustomData, Data as VibeData, DataType, custom::CustomDataTrait},
    types::Price,
};
use vibe_network::{
    RECONNECTED,
        ...
        T: CustomDataTrait + 'static,
    {
        for data_type in data_types {
            ...
            if let Err(e) = self
                .inner
                .data_sender
                .send(DataEvent::Data(VibeData::Custom(custom)))
            {
...
mod tests {
    ...
    use rstest::rstest;
    use serde_json::json;
    use vibe_common::{messages::DataEvent, testing::wait_until_async};
    use vibe_core::{Params, time::get_atomic_clock_realtime};
    ...
    fn collect_crypto_symbols(
        ...
        for _ in 0..expected_count {
            ...
            let DataEvent::Data(VibeData::Custom(custom)) = event else {
