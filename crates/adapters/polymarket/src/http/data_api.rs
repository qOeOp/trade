...
fn parse_trade_ticks(
    ...
    for t in data_api_trades {
        ...
        *occurrence += 1;
        let ts_event = vibe_core::UnixNanos::from(base_ns + tiebreaker);
...
mod tests {
    ...
    fn test_build_position_reports_filters_dust_and_zero() {
        ...
        let ts_now = vibe_core::UnixNanos::from(1_000_000_000u64);
    ...
    fn test_build_trade_ticks_filters_by_token_id() {
        ...
        let ticks: Vec<TradeTick> = trades
            ...
            .map(|t| {
                ...
                let ts_event = vibe_core::UnixNanos::from(t.timestamp as u64 * 1_000_000_000);
