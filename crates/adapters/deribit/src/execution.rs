...
use crate::{
    ...
    websocket::{
        ...
        messages::{DeribitOrderParams, VibeWsMessage},
...
impl DeribitExecutionClient {
    ...
    fn spawn_stream_handler(
        &mut self,
        stream: impl futures_util::Stream<Item = VibeWsMessage> + Send + 'static,
    ) {
...
fn dispatch_ws_message(message: VibeWsMessage, emitter: &ExecutionEventEmitter) {
    match message {
        VibeWsMessage::AccountState(state) => {
            emitter.send_account_state(state);
        }
        VibeWsMessage::OrderStatusReports(reports) => {
            log::debug!("Processing {} order status report(s)", reports.len());
            for report in reports {
                emitter.send_order_status_report(report);
            }
        }
        VibeWsMessage::FillReports(reports) => {
        ...
        VibeWsMessage::OrderRejected(event) => {
        ...
        VibeWsMessage::OrderExpired(event) => {
        ...
        VibeWsMessage::OrderCancelRejected(event) => {
            emitter.send_order_event(OrderEventAny::CancelRejected(event));
        }
        VibeWsMessage::OrderModifyRejected(event) => {
        ...
        VibeWsMessage::Authenticated(auth) => {
            log::debug!("WebSocket authenticated: scope={}", auth.scope);
        }
        VibeWsMessage::AuthenticationFailed(reason) => {
...
mod tests {
    use rstest::rstest;
    use vibe_common::messages::{ExecutionEvent, execution::ExecutionReport};
    use vibe_core::UUID4;
    use vibe_model::{
        enums::LiquiditySide,
        events::OrderFilled,
    ...
    fn dispatch_tracked_fill_uses_order_event_path() {
        ...
        dispatch_ws_message(VibeWsMessage::OrderFilled(filled), &emitter);
    ...
    fn dispatch_untracked_fill_keeps_report_path() {
        ...
        dispatch_ws_message(VibeWsMessage::FillReports(vec![report]), &emitter);
