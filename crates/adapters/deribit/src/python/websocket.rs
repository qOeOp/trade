...
use vibe_core::python::{call_python_threadsafe, to_pyruntime_err, to_pyvalue_err};
use vibe_model::{
...
impl DeribitWebSocketClient {
            ...
            get_runtime().spawn(async move {
                ...
                while let Some(msg) = stream.next().await {
                    match msg {
                        ...
                        VibeWsMessage::OptionGreeks(greeks) => {
                            call_python_with_data(&call_soon, &callback, |py| {
                                Py::new(py, greeks).map(|obj| obj.into_any())
                            });
                        }
                        // Execution events - route to Python callback
                        VibeWsMessage::OrderStatusReports(reports) => Python::attach(|py| {
                        ...
                        VibeWsMessage::FillReports(reports) => Python::attach(|py| {
                            for report in reports {
                                match Py::new(py, report) {
                                    Ok(py_obj) => call_python_threadsafe(
                                        py,
                        ...
                        VibeWsMessage::OrderFilled(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderRejected(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderAccepted(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderCanceled(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderExpired(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderUpdated(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderCancelRejected(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::OrderModifyRejected(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
                        VibeWsMessage::AccountState(msg) => {
                            call_python_with_data(&call_soon, &callback, |py| msg.into_py_any(py));
                        }
    ...
    fn py_submit_order<'py>(
        ...
        // Convert Vibe TimeInForce to Deribit format
