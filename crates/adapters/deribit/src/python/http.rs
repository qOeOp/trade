...
use vibe_core::{
    UnixNanos,
    python::{IntoPyObjectVibeExt, to_pyruntime_err, to_pyvalue_err},
...
impl DeribitHttpClient {
    ...
    ///
    /// This client wraps the raw HTTP client and provides methods that use Vibe
    ...
    /// Requests a specific instrument by its Vibe instrument ID.
    ///
    /// This is a high-level method that fetches the raw instrument data from Deribit
    /// and converts it to a Vibe `InstrumentAny` type.
    ///
    /// # Errors
    ///
    /// Returns an error if:
    ...
    /// and converts it to Vibe `AccountState` event.
    ///
    /// # Errors
    ///
    /// Returns an error if:
    ...
    ///
    /// Fetches trade ticks from Deribit and converts them to Vibe `TradeTick` objects.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Fetches the order book from Deribit and converts it to a Vibe `OrderBook`.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Fetches order statuses from Deribit and converts them to Vibe `OrderStatusReport`.
    ///
    /// # Strategy
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let reports = client
                .request_order_status_reports(
                    account_id,
                    instrument_id,
                    start.map(vibe_core::UnixNanos::from),
                    end.map(vibe_core::UnixNanos::from),
                    open_only,
                )
                .await
                .map_err(to_pyvalue_err)?;
    ...
    ///
    /// Fetches user trades from Deribit and converts them to Vibe `FillReport`.
    ...
    ///
    /// Fetches positions from Deribit and converts them to Vibe `PositionStatusReport`.
    ///
    /// # Strategy
    ...
    fn py_request_forward_prices<'py>(
        ...
        pyo3_async_runtimes::tokio::future_into_py(py, async move {
            let forward_prices = if let Some(inst_id) = instrument_id {
                ...
                let ts = vibe_core::UnixNanos::default();
