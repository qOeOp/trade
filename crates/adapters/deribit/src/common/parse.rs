//! Parsing functions for Deribit API responses into Vibe domain types.
...
/// Parses a Deribit instrument into a Vibe [`InstrumentAny`].
///
...
/// Parses Deribit account summaries into a Vibe [`AccountState`].
///
...
/// Parses a Deribit WebSocket portfolio message into a Vibe [`AccountState`].
///
/// This function converts real-time portfolio updates from the `user.portfolio.{currency}`
/// subscription channel into Vibe account state events.
///
/// # Returns
///
...
// Parses a Deribit public trade into a Vibe [`TradeTick`].
///
/// # Errors
///
/// Returns an error if:
...
/// Parses Deribit TradingView chart data into Vibe [`Bar`]s.
///
...
/// Parses Deribit order book data into a Vibe [`OrderBook`].
///
...
mod tests {
    ...
    use vibe_model::{instruments::Instrument, types::Money};
