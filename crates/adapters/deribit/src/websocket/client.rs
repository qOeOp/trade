...
use ustr::Ustr;
use vibe_common::{enums::LogColor, live::get_runtime, log_debug};
use vibe_core::{
    AtomicMap, AtomicSet, consts::VIBE_USER_AGENT, env::get_or_env_var_opt,
...
use super::{
    ...
    messages::{
        ...
        DeribitOrderParams, VibeWsMessage,
    },
};
use crate::common::{
    consts::{
...
pub struct DeribitWebSocketClient {
    ...
    out_rx: Option<Arc<tokio::sync::mpsc::UnboundedReceiver<VibeWsMessage>>>,
...
impl DeribitWebSocketClient {
    ...
    pub async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        let config = WebSocketConfig {
            url: self.url.clone(),
            headers: vec![(USER_AGENT.to_string(), VIBE_USER_AGENT.to_string())],
            ...
            loop {
                match handler.next().await {
                    Some(msg) => match msg {
                        ...
                        VibeWsMessage::Authenticated(result) => {
    ...
    pub fn stream(&mut self) -> DeribitWsResult<impl Stream<Item = VibeWsMessage> + 'static> {
