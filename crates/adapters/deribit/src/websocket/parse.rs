//! Parsing functions for converting Deribit WebSocket messages to Vibe domain types.
...
use ustr::Ustr;
use vibe_core::{UUID4, UnixNanos, datetime::NANOSECONDS_IN_MILLISECOND};
use vibe_model::{
    data::{
...
/// Parses a Deribit trade message into a Vibe `TradeTick`.
///
/// # Errors
///
...
/// Parses a vector of Deribit trade messages into Vibe `Data` items.
...
/// Parses a Deribit order book snapshot into Vibe `OrderBookDeltas`.
///
/// # Errors
///
...
/// Parses a Deribit order book change (delta) into Vibe `OrderBookDeltas`.
///
/// # Errors
///
/// Returns an error if the book data cannot be parsed.
...
/// Parses a Deribit order book message (snapshot or delta) into Vibe `OrderBookDeltas`.
///
/// # Errors
///
/// Returns an error if the book data cannot be parsed.
...
/// Parses a Deribit ticker message into a Vibe `QuoteTick`.
///
/// # Errors
///
...
/// Parses a Deribit quote message into a Vibe `QuoteTick`.
///
/// # Errors
///
...
/// Parses a Deribit ticker message into a Vibe `MarkPriceUpdate`.
///
/// # Errors
///
...
/// Parses a Deribit ticker message into a Vibe `IndexPriceUpdate`.
///
/// # Errors
///
/// Returns an error if the price cannot be converted to the required precision.
...
/// Parses a Deribit ticker message into a Vibe `FundingRateUpdate`.
///
...
/// Parses a Deribit ticker message into a Vibe `OptionGreeks`.
///
...
/// Parses a Deribit perpetual channel message into a Vibe `FundingRateUpdate`.
///
...
/// Converts a Deribit chart resolution and instrument to a Vibe BarType.
///
...
/// Parses a Deribit user order message into a Vibe `OrderStatusReport`.
///
/// # Errors
///
...
pub fn parse_user_order_msg(
    ...
    // Map Deribit order type to Vibe
    ...
    // Map Deribit order state to Vibe status
...
/// Parses a Deribit user trade message into a Vibe `FillReport`.
///
/// # Errors
///
...
/// Parses a Deribit position into a Vibe `PositionStatusReport`.
///
/// # Arguments
/// - `position` - The Deribit position data from `/private/get_positions`
/// - `instrument` - The corresponding Vibe instrument
...
mod tests {
    ...
    fn test_parse_order_stop_market_response() {
        // Regression for source issue #3925
    ...
    fn test_parse_order_stop_market_response_missing_filled_amount() {
        // Regression for source issue #3995
