...
use vibe_core::{AtomicSet, AtomicTime, UUID4, UnixNanos, time::get_atomic_clock_realtime};
use vibe_model::{
...
use super::{
    ...
    messages::{
        ...
        DeribitWsMessage, VibeWsMessage, parse_raw_message,
    },
    parse::{
...
pub struct DeribitWsFeedHandler {
    ...
    out_tx: tokio::sync::mpsc::UnboundedSender<VibeWsMessage>,
    ...
    pending_outgoing: VecDeque<VibeWsMessage>,
...
impl DeribitWsFeedHandler {
    ...
    async fn process_raw_message(&mut self, text: &str) -> Option<VibeWsMessage> {
        if text == RECONNECTED {
            ...
            return Some(VibeWsMessage::Reconnected);
        }
        ...
        match ws_msg {
            DeribitWsMessage::Response(response) => {
                    ...
                    && let Some(request_type) = self.pending_requests.remove(&request_id)
                {
                    match request_type {
                        PendingRequestType::Authenticate => {
                            if let Some(error) = &response.error {
                                ...
                                return Some(VibeWsMessage::AuthenticationFailed(reason));
                                ...
                                match serde_json::from_value::<DeribitAuthResult>(result.clone()) {
                                    Ok(auth_result) => {
                                        ...
                                        return Some(VibeWsMessage::Authenticated(Box::new(
                                            auth_result,
                                        )));
                                    }
                                    Err(e) => {
                        ...
                        PendingRequestType::Cancel {
                            ...
                            if let Some(result) = &response.result {
                                    ...
                                    Ok(order_msg) => {
                                                ...
                                                .contains_key(&venue_order_id)
                                        {
                                                ...
                                                && let Some(account_id) = self.account_id
                                            {
                                                ...
                                                return Some(VibeWsMessage::OrderCanceled(event));
                                            }
                                        }
                                    }
                                    Err(e) => {
                                ...
                                return Some(VibeWsMessage::OrderCancelRejected(
                        ...
                        | PendingRequestType::Sell {
                            ...
                            if let Some(result) = &response.result {
                                        ...
                                        if !order_response.trades.is_empty() {
                                                ...
                                                self.instruments_cache.get(&instrument_name_ustr)
                                            {
                                                if let Some(account_id) = self.account_id {
                                                    ...
                                                    return Some(VibeWsMessage::OrderAccepted(
                                                        event,
                                                    ));
                                                } else {
                                                    log::warn!(
                                ...
                                return Some(VibeWsMessage::OrderRejected(OrderRejected::new(
                                    trader_id,
                                    strategy_id,
                                    instrument_id,
                                    client_order_id,
                        ...
                        PendingRequestType::Edit {
                            ...
                            if let Some(result) = &response.result {
                                            ...
                                            self.instruments_cache.get(&instrument_name_ustr)
                                        {
                                            if let Some(account_id) = self.account_id {
                                                let Some(mut context) = self.find_order_context(
                                                    ...
                                                    return report
                                                    .map(|report| {
                                                        VibeWsMessage::OrderStatusReports(vec![
                                                            report,
                                                        ])
                                                    })
                                                    .map_err(|e| {
                                                ...
                                                self.pending_outgoing.extend(outgoing);
                                                return event.map(VibeWsMessage::OrderUpdated);
                                            } else {
                                                log::warn!(
                                ...
                                return Some(VibeWsMessage::OrderModifyRejected(
                                            ...
                                            self.instruments_cache.get(&instrument_name_ustr)
                                        {
                                            if let Some(account_id) = self.account_id {
                                                match parse_user_order_msg(
                                                    ...
                                                    Ok(report) => {
                                                        return Some(
                                                            VibeWsMessage::OrderStatusReports(
                                                                vec![report],
                                                            ),
                                                        );
                                                    }
                    ...
                    if let Some(error) = &response.error {
                        ...
                        return Some(VibeWsMessage::Error(DeribitWsError::DeribitError {
                            code: error.code,
                            ...
                            match serde_json::from_value::<Vec<DeribitTradeMsg>>(data.clone()) {
                                Ok(trades) => {
                                    ...
                                    if data_vec.is_empty() && !trades.is_empty() {
                                        ...
                                        return Some(VibeWsMessage::Data(data_vec));
                                    }
                                }
                                Err(e) => {
                                        ...
                                        self.instruments_cache.get(&book_msg.instrument_name)
                                    {
                                            ...
                                            && book_msg.msg_type == DeribitBookMsgType::Snapshot
                                        {
                                            ...
                                            match parse_book_msg(&book_msg, instrument, ts_init) {
                                                Ok(deltas) => {
                                                    return Some(VibeWsMessage::Deltas(deltas));
                                                }
                                                Err(e) => {
                                ...
                                Ok(ticker_msg) => {
                                        ...
                                        self.instruments_cache.get(&ticker_msg.instrument_name)
                                    {
                                                ...
                                                )
                                        {
                                            let _ = self
                                                .out_tx
                                                .send(VibeWsMessage::OptionGreeks(option_greeks));
                                        }
                                ...
                                Ok(perpetual_msg) => {
                                    ...
                                    if parts.len() >= 2 {
                                            ...
                                            self.instruments_cache.get(&instrument_name)
                                        {
                                            ...
                                            return Some(VibeWsMessage::FundingRates(vec![
                                                funding_rate,
                                            ]));
                                        } else {
                                            log::warn!(
                                ...
                                Ok(quote_msg) => {
                                        ...
                                        self.instruments_cache.get(&quote_msg.instrument_name)
                                    {
                                        match parse_quote_msg(&quote_msg, instrument, ts_init) {
                                            Ok(quote) => {
                                                return Some(VibeWsMessage::Data(vec![
                                                    Data::Quote(quote),
                                                ]));
                                            }
                                            Err(e) => {
                        ...
                        DeribitWsChannel::VolatilityIndex => {
                            match serde_json::from_value::<DeribitVolatilityIndexMsg>(data.clone())
                            {
                                Ok(msg) => {
                                    ...
                                    return Some(VibeWsMessage::Data(vec![Data::Custom(
                        ...
                        DeribitWsChannel::InstrumentState => {
                            match serde_json::from_value::<DeribitInstrumentStateMsg>(data.clone())
                            {
                                Ok(state_msg) => {
                                    ...
                                    return Some(VibeWsMessage::InstrumentStatus(status));
                                }
                                Err(e) => {
                                                    ...
                                                    Ok(new_bar) => {
                                                            ...
                                                            self.pending_bars.get(&channel_key)
                                                        {
                                                                ...
                                                                != pending_bar.ts_event
                                                            {
                                                                ...
                                                                return Some(VibeWsMessage::Data(
                                                                    vec![Data::Bar(closed_bar)],
                                                                ));
                                                            }
                            ...
                            match orders_result {
                                Ok(orders) => {
                                    ...
                                    let Some(account_id) = self.account_id else {
                                        ...
                                        return Some(VibeWsMessage::Raw(data.clone()));
                                    };
                                    ...
                                    for order in &orders {
                                        ...
                                        let Some(mut context) = self.find_order_context(
                                            ...
                                            match parse_user_order_msg(
                                                ...
                                                Ok(report) => outgoing.push(
                                                    VibeWsMessage::OrderStatusReports(vec![report]),
                                                ),
                                                Err(e) => log::warn!(
                                        ...
                                        match event_type {
                                            OrderEventType::Accepted => {
                                                ...
                                                outgoing.push(VibeWsMessage::OrderAccepted(event));
                                            }
                                            OrderEventType::Canceled => {
                                                ...
                                                if !context.accepted {
                                                    outgoing.push(VibeWsMessage::OrderAccepted(
                                                        parse_order_accepted_with_client_order_id(
                                                            order,
                                                            instrument,
                                                            account_id,
                                                ...
                                                self.finish_order_context(venue_order_id, &context);
                                                outgoing.push(VibeWsMessage::OrderCanceled(event));
                                            }
                                            OrderEventType::Expired => {
                                                ...
                                                self.finish_order_context(venue_order_id, &context);
                                                outgoing.push(VibeWsMessage::OrderExpired(event));
                                            }
                                            OrderEventType::Updated => {
                                                ...
                                                outgoing.push(VibeWsMessage::OrderUpdated(event));
                                            }
                                            OrderEventType::None => {
                                ...
                                Ok(portfolio) => {
                                    ...
                                    match parse_portfolio_to_account_state(
                                        ...
                                        Ok(account_state) => {
                                            ...
                                            return Some(VibeWsMessage::AccountState(
                                                account_state,
                                            ));
                                        }
                                        Err(e) => {
                ...
                match heartbeat.heartbeat_type {
                    DeribitHeartbeatType::TestRequest => {
                        ...
                        if let Err(e) = self.handle_heartbeat_test_request().await {
                            ...
                            return Some(VibeWsMessage::Error(DeribitWsError::Send(format!(
            ...
            DeribitWsMessage::Error(err) => {
                ...
                Some(VibeWsMessage::Error(DeribitWsError::DeribitError {
                    code: err.code,
                    message: err.message,
                }))
            }
            DeribitWsMessage::Reconnected => Some(VibeWsMessage::Reconnected),
        }
    }
    ...
    fn route_user_trades(
        ...
        for trade in trades {
            ...
            if !context.accepted {
                outgoing.push(VibeWsMessage::OrderAccepted(OrderAccepted::new(
                    context.trader_id,
                    context.strategy_id,
            ...
            outgoing.push(VibeWsMessage::OrderFilled(OrderFilled::new(
                context.trader_id,
                context.strategy_id,
                context.instrument_id,
                context.client_order_id,
        ...
        if !reports.is_empty() {
            outgoing.push(VibeWsMessage::FillReports(reports));
        }
        outgoing
    }
    ...
    pub async fn next(&mut self) -> Option<VibeWsMessage> {
        loop {
            if let Some(msg) = self.pending_outgoing.pop_front() {
                match msg {
                    VibeWsMessage::Reconnected
                    | VibeWsMessage::Authenticated(_)
                    | VibeWsMessage::AuthenticationFailed(_) => {
                        return Some(msg);
                    }
                    _ => {
            ...
            tokio::select! {
                ...
                Some(msg) = self.raw_rx.recv() => {
                    match msg {
                        Message::Text(text) => {
                            if let Some(vibe_msg) = self.process_raw_message(&text).await {
                                // Send data messages to user stream
                                match &vibe_msg {
                                    VibeWsMessage::Data(_)
                                    | VibeWsMessage::Deltas(_)
                                    | VibeWsMessage::Instrument(_)
                                    | VibeWsMessage::InstrumentStatus(_)
                                    | VibeWsMessage::OptionGreeks(_)
                                    | VibeWsMessage::Raw(_)
                                    | VibeWsMessage::Error(_) => {
                                        let _ = self.out_tx.send(vibe_msg);
                                    }
                                    VibeWsMessage::FundingRates(rates) => {
                                        let msg_to_send =
                                            VibeWsMessage::FundingRates(rates.clone());

                                        if let Err(e) = self.out_tx.send(msg_to_send) {
                                            log::error!("Failed to send funding rates: {e}");
                                        }
                                    }
                                    VibeWsMessage::OrderStatusReports(_)
                                    | VibeWsMessage::FillReports(_)
                                    | VibeWsMessage::OrderFilled(_)
                                    | VibeWsMessage::OrderAccepted(_)
                                    | VibeWsMessage::OrderCanceled(_)
                                    | VibeWsMessage::OrderExpired(_)
                                    | VibeWsMessage::OrderUpdated(_)
                                    | VibeWsMessage::OrderRejected(_)
                                    | VibeWsMessage::OrderCancelRejected(_)
                                    | VibeWsMessage::OrderModifyRejected(_)
                                    | VibeWsMessage::AccountState(_) => {
                                        let _ = self.out_tx.send(vibe_msg);
                                    }
...
mod tests {
    use rstest::rstest;
    use vibe_model::{enums::LiquiditySide, instruments::Instrument, types::Money};
    ...
    async fn tracked_fast_fill_synthesizes_accepted_before_filled() {
        ...
        assert!(matches!(
            ...
            VibeWsMessage::OrderAccepted(event)
                if event.client_order_id == client_order_id
        ));
        assert!(matches!(
            handler.pending_outgoing.pop_front().unwrap(),
            VibeWsMessage::OrderFilled(event)
                if event.client_order_id == client_order_id
    ...
    async fn tracked_terminal_order_synthesizes_accepted_first(#[case] order_state: &str) {
        ...
        assert!(
            matches!(
                ...
                ("cancelled", VibeWsMessage::OrderCanceled(_))
                    | ("expired", VibeWsMessage::OrderExpired(_))
            ),
    ...
    async fn late_fill_after_cancel_stays_on_tracked_event_path(#[case] trade_state: &str) {
        ...
        assert!(matches!(
            handler.pending_outgoing.pop_front().unwrap(),
            VibeWsMessage::OrderCanceled(event)
                if event.client_order_id == client_order_id
        ));
        assert!(matches!(
            handler.pending_outgoing.pop_front().unwrap(),
    ...
    async fn subscription_accept_before_submit_response_is_not_duplicated() {
        ...
        assert!(matches!(accepted, VibeWsMessage::OrderAccepted(_)));
    ...
    async fn untracked_live_order_and_fill_use_report_paths() {
        ...
        assert!(matches!(
            handler.pending_outgoing.pop_front().unwrap(),
            VibeWsMessage::OrderStatusReports(reports) if reports.len() == 1
        ));
        assert!(matches!(
            handler.pending_outgoing.pop_front().unwrap(),
            VibeWsMessage::FillReports(reports) if reports.len() == 1
        ));
        assert!(handler.pending_outgoing.is_empty());
    }
    ...
    async fn edit_response_without_tracked_context_uses_report_path() {
        ...
        assert!(matches!(
            message,
            Some(VibeWsMessage::OrderStatusReports(reports)) if reports.len() == 1
        ));
        assert!(matches!(
            fill,
            VibeWsMessage::FillReports(reports) if reports.len() == 1
    ...
    async fn tracked_edit_response_routes_fill_and_deduplicates_subscription_echo() {
        ...
        assert!(matches!(
            updated,
            Some(VibeWsMessage::OrderUpdated(event))
                if event.client_order_id == client_order_id
        ));
        assert!(matches!(
            filled,
    ...
    async fn subscription_edit_echo_before_response_is_not_duplicated() {
        ...
        assert!(matches!(
            updated,
            VibeWsMessage::OrderUpdated(event)
                if event.client_order_id == client_order_id
        ));
        assert!(duplicate.is_none());
        assert!(matches!(
