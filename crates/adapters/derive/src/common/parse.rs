...
use ustr::Ustr;
use vibe_core::{UnixNanos, datetime::NANOSECONDS_IN_SECOND, params::Params};
use vibe_model::{
...
/// Converts a Derive venue symbol to a Vibe instrument ID.
#[must_use]
...
/// Converts a Vibe Derive instrument ID back to the venue symbol.
///
/// # Errors
///
...
/// Maps a Vibe order side to the Derive direction string.
///
/// # Errors
///
/// Returns an error for ambiguous Vibe order sides
...
/// Maps a Vibe order type to the Derive order type string.
///
/// # Errors
///
...
/// Maps a supported Vibe trigger order type to the child Derive order type.
///
/// # Errors
///
...
/// Maps a Vibe trigger order type to Derive's stop-loss/take-profit flag.
///
/// # Errors
///
/// Returns an error for order types not supported by Derive trigger orders.
...
/// Maps Vibe trigger price source to Derive.
///
/// # Errors
///
...
/// Maps a Vibe time-in-force flag to the Derive TIF.
///
/// # Errors
///
...
/// Maps a Derive order side back to Vibe.
#[must_use]
pub fn derive_order_side_to_vibe(side: DeriveOrderSide) -> OrderSide {
    match side {
...
/// Maps a Derive order type back to Vibe.
///
...
#[must_use]
pub fn derive_order_type_to_vibe(order_type: DeriveOrderType) -> OrderType {
    match order_type {
...
/// Maps a Derive trigger order record back to the Vibe order type.
///
/// An unmodeled trigger type degrades to the plain order type; the trigger
/// price still rides on the report.
#[must_use]
pub fn derive_order_type_to_vibe_for_order(
    ...
    match (order_type, trigger_type) {
        ...
        (order_type, _) => derive_order_type_to_vibe(order_type),
    }
}

/// Maps a Derive trigger price source back to Vibe.
///
/// Unmodeled trigger price sources map to [`TriggerType::Default`].
#[must_use]
pub const fn derive_trigger_price_type_to_vibe(
...
/// Maps a Derive order status to the Vibe equivalent, given the current
/// filled quantity.
#[must_use]
pub fn derive_status_to_vibe(
...
/// Parses a Derive instrument definition into a Vibe instrument.
///
...
// Serializes the raw DeriveInstrument into the Vibe `info` slot so
...
mod tests {
    ...
    fn test_derive_order_type_to_vibe_for_order(
        #[case] order_type: DeriveOrderType,
        #[case] trigger_type: Option<DeriveTriggerType>,
        #[case] expected: OrderType,
    ) {
        assert_eq!(
            derive_order_type_to_vibe_for_order(order_type, trigger_type),
            expected,
        );
    }
    ...
    fn test_unknown_wire_variants_map_to_safe_defaults() {
        assert_eq!(
            derive_order_type_to_vibe(DeriveOrderType::Unknown),
            OrderType::Limit,
        );
        assert_eq!(
            derive_tif_to_vibe(DeriveTimeInForce::Unknown),
            TimeInForce::Gtc,
        );
        assert_eq!(
            derive_trigger_price_type_to_vibe(DeriveTriggerPriceType::Unknown),
            TriggerType::Default,
        );
    }
    ...
    #[rstest]
    fn test_derive_status_to_vibe_maps_untriggered_to_accepted() {
        assert_eq!(
            derive_status_to_vibe(DeriveOrderStatus::Untriggered, dec!(0), dec!(1)),
            OrderStatus::Accepted,
        );
    }
