...
use crate::{
    common::{
        ...
        parse::{
            derive_order_side_to_vibe, derive_order_type_to_vibe_for_order,
            derive_rejection_due_post_only, derive_status_to_vibe, derive_tif_to_vibe,
            derive_trigger_price_type_to_vibe,
        },
    },
...
/// # Errors
///
/// Returns an error when any decimal field cannot be converted to a Vibe
/// `Price` or `Quantity`.
pub fn parse_derive_order_to_report(
    ...
    let order_side = derive_order_side_to_vibe(order.direction);
    let order_type = derive_order_type_to_vibe_for_report(order);
    let post_only = matches!(order.time_in_force, DeriveTimeInForce::PostOnly);
    let time_in_force = derive_tif_to_vibe(order.time_in_force);
    let order_status = derive_status_to_vibe(order.order_status, order.filled_amount, order.amount);
    ...
    if let Some(trigger_price_type) = order.trigger_price_type {
        report = report.with_trigger_type(derive_trigger_price_type_to_vibe(trigger_price_type));
    }
...
fn derive_order_type_to_vibe_for_report(order: &DeriveOrder) -> OrderType {
    let order_type = derive_order_type_to_vibe_for_order(order.order_type, order.trigger_type);
...
pub fn parse_derive_trade_to_fill_report(
    ...
    let order_side = derive_order_side_to_vibe(trade.direction);
...
mod tests {
    ...
    use vibe_model::enums::{OrderSide, OrderStatus, OrderType, TimeInForce, TriggerType};
    ...
    use crate::{
        common::{
            ...
            parse::{
                derive_status_to_vibe, order_side_to_derive, order_type_to_derive,
    ...
    fn test_derive_status_partial_fill_classification() {
        assert_eq!(
            derive_status_to_vibe(DeriveOrderStatus::Open, dec!(0), dec!(10)),
            OrderStatus::Accepted,
        );
        assert_eq!(
            derive_status_to_vibe(DeriveOrderStatus::Open, dec!(4), dec!(10)),
            OrderStatus::PartiallyFilled,
        );
        assert_eq!(
            derive_status_to_vibe(DeriveOrderStatus::Filled, dec!(10), dec!(10)),
            OrderStatus::Filled,
        );
        assert_eq!(
            derive_status_to_vibe(DeriveOrderStatus::Cancelled, dec!(0), dec!(10)),
