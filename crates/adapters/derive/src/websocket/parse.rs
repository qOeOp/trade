...
/// Parses an order book snapshot message into Vibe snapshot deltas.
///
...
/// Parses a public trade message into a Vibe trade tick.
///
/// Pass price and size precision from the instrument definition rather than
/// inferring them from the wire values, since Derive may trim trailing zeroes.
///
...
/// Parses a ticker message into a Vibe top-of-book quote.
///
/// Pass price and size precision from the instrument definition rather than
/// inferring them from the wire values, since Derive may trim trailing zeroes.
///
...
/// Parses a REST `public/get_tickers` snapshot into a Vibe top-of-book quote.
///
/// # Errors
///
/// Returns an error when price, size, or timestamp conversion fails.
...
/// Parses a `public/get_tradingview_chart_data` record into a Vibe [`Bar`].
///
/// Pass price and size precision from the instrument definition rather than
...
/// Maps a Vibe bar aggregation and step to the Derive `period` enum value
