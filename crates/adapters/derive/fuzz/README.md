...
unstable compiler flags. The targets are binary targets of `vibe-derive` behind its `fuzz`
feature. The workspace pins `libfuzzer-sys`, and `vibe-live` owns the shared libFuzzer
integration.
...
3. Import `vibe_live::fuzz::fuzz_target`.
