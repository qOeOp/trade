...
setarch -R cargo bench -p vibe-derive --profile bench-lto \
...
Raw WS frame bytes -> Vibe domain type. Covers frame decode (single-pass
into a typed frame) + channel decode (raw payload bytes -> typed struct) +
parse + Vibe type construction. No I/O, no async runtime, no channel. The
...
raw-bytes -> typed-message cost; `parse_only` is the typed-message -> Vibe
