...
use ustr::Ustr;
use vibe_common::cache::fifo::FifoCache;
use vibe_core::{
...
use super::{
    ...
    messages::{
        CandleData, ExecutionReport, HyperliquidWsMessage, HyperliquidWsRequest, PostRequest,
        SubscriptionRequest, VibeWsMessage, WsActiveAssetCtxData, WsAllDexsAssetCtxsData,
        WsUserEventData,
    },
    parse::{
...
pub enum HandlerCommand {
    ...
    /// as [`VibeWsMessage::Depth10`] snapshots.
...
pub(super) struct FeedHandler {
    ...
    message_buffer: VecDeque<VibeWsMessage>,
...
impl FeedHandler {
                        ...
                        Message::Text(text) => {
                            ...
                            match serde_json::from_str::<HyperliquidWsMessage>(&text) {
                                Ok(msg) => {
                                    ...
                                    let vibe_msgs = Self::parse_to_vibe_messages(
                                        msg,
                                        &self.instruments,
                                        &self.cloid_cache,
                                    ...
                                    if !vibe_msgs.is_empty() {
                                        let mut iter = vibe_msgs.into_iter();
            ...
            HyperliquidWsMessage::AllMids { data } => {
                ...
                if !mids.is_empty() {
                    ...
                    for (i, data_type) in all_mids_data_types.iter().enumerate() {
                        ...
                        result.push(VibeWsMessage::CustomData(Data::Custom(CustomData::new(
                            Arc::new(all_mids),
                            data_type.clone(),
                        ))));
                    }
    ...
    fn handle_order_updates(
        ...
        if exec_reports.is_empty() {
            ...
            Some(VibeWsMessage::ExecutionReports(exec_reports))
        }
    }
    ...
    fn handle_trades(
        ...
        if !trade_ticks.is_empty() {
            result.push(VibeWsMessage::Trades(trade_ticks));
        }
        result.extend(public_trades.into_iter().map(|trade| {
            let instrument_id = trade.instrument_id;
            VibeWsMessage::CustomData(Data::Custom(CustomData::new(
                Arc::new(trade),
    ...
    fn handle_bbo(
        ...
        if let Some(instrument) = instruments.get(&data.coin) {
            match parse_ws_quote_tick(data, instrument, ts_init) {
                Ok(quote_tick) => Some(VibeWsMessage::Quote(quote_tick)),
                Err(e) => {
    ...
    fn handle_l2_book(
        ...
        match parse_ws_order_book_deltas(data, instrument, ts_init) {
            Ok(deltas) => out.push(VibeWsMessage::Deltas(deltas)),
        ...
        if depth10_subs.contains(&data.coin) {
            match parse_ws_order_book_depth10(data, instrument, ts_init) {
                Ok(depth) => out.push(VibeWsMessage::Depth10(Box::new(depth))),
            ...
            if let Some(bar_type) = bar_types.get(&key) {
                if let Some(instrument) = instruments.get(&data.s) {
                    match parse_ws_candle(&closed_data, instrument, bar_type, ts_init) {
                        Ok(bar) => return Some(VibeWsMessage::Candle(bar)),
                        Err(e) => {
                ...
                match parse_ws_asset_context(data, instrument, ts_init) {
                    Ok((mark_price, index_price, funding_rate)) => {
                                ...
                                .is_some_and(|s| s.contains(&AssetContextDataType::MarkPrice))
                        {
                            ...
                            result.push(VibeWsMessage::MarkPrice(mark_price));
                        }
                                ...
                                .is_some_and(|s| s.contains(&AssetContextDataType::IndexPrice))
                        {
                            ...
                            if let Some(index) = index_price {
                                result.push(VibeWsMessage::IndexPrice(index));
                            }
                        }
                                ...
                                .is_some_and(|s| s.contains(&AssetContextDataType::FundingRate))
                        {
                            ...
                            if let Some(funding) = funding_rate {
                                result.push(VibeWsMessage::FundingRate(funding));
                            }
                        }
                    }
                    Err(e) => {
    ...
    fn handle_all_dexs_asset_ctxs(
        ...
        Some(VibeWsMessage::CustomData(Data::Custom(CustomData::new(
            Arc::new(payload),
            data_type,
        ))))
    }
...
mod tests {
    ...
    use vibe_network::websocket::SubscriptionState;

    use super::{
        super::{
            client::{AssetContextDataType, CLOID_CACHE_CAPACITY, CloidCache},
            messages::{
                HyperliquidWsRequest, PerpsAssetCtx, PostRequest, SharedAssetCtx, SpotAssetCtx,
                SubscriptionRequest, VibeWsMessage, WsActiveAssetCtxData, WsAllDexsAssetCtxsData,
    ...
    impl Log for OutboundLogCapture {
        fn enabled(&self, metadata: &Metadata<'_>) -> bool {
            metadata.level() == Level::Debug
                && metadata.target() == "vibe_hyperliquid::websocket::handler"
        }
    ...
    fn handle_l2_book_emits_deltas_only_when_not_in_depth10_subs() {
        ...
        assert!(matches!(msgs[0], VibeWsMessage::Deltas(_)));
    }
    ...
    fn handle_l2_book_emits_deltas_and_depth10_when_coin_in_subs() {
        ...
        assert!(matches!(msgs[0], VibeWsMessage::Deltas(_)));
        assert!(matches!(msgs[1], VibeWsMessage::Depth10(_)));
    }
    ...
    fn handle_asset_context_emits_open_interest_custom_data_when_subscribed() {
        ...
        match &msgs[0] {
            VibeWsMessage::CustomData(Data::Custom(custom)) => {
