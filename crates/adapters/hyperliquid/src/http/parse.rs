...
use ustr::Ustr;
use vibe_core::{Params, UUID4, UnixNanos, datetime::unix_nanos_to_iso8601};
use vibe_model::{
    data::TradeTick,
    enums::{
...
use crate::{
    common::{
        ...
        parse::{
            format_outcome_vibe_symbol, is_conditional_order_data, make_fill_trade_id,
...
///
/// This deliberately avoids any tight coupling to Vibe' Cython types.
/// The InstrumentProvider can later convert this into Vibe `Instrument`s.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
// `x` so the value is safe to embed in a Vibe `InstrumentId`. HIP-3
...
/// modeled as binary outcome side tokens. The Vibe internal symbol uses
...
/// the fee is zero. The side token is not a Vibe currency and emitting it as
...
/// Converts a single Hyperliquid instrument definition into a Vibe `InstrumentAny`.
///
...
/// Convert a collection of Hyperliquid instrument definitions into Vibe instruments,
...
/// Convert owned definitions into Vibe instruments, consuming the input vector.
#[must_use]
