...
use crate::{
    ...
    common::{
        consts::{HYPERLIQUID_VENUE, VIBE_BUILDER_ADDRESS, exchange_url, info_url},
...
/// with Vibe domain types. It maintains an instrument cache and handles conversions
/// between Hyperliquid API responses and Vibe domain models.
#[derive(Debug, Clone)]
#[cfg_attr(
    feature = "python",
    pyo3::pyclass(module = "vibe_trader.adapters.hyperliquid", from_py_object)
...
impl HyperliquidHttpClient {
    ...
    pub fn builder_attribution(&self) -> Option<HyperliquidExecBuilderFee> {
        if !self.include_builder_attribution || self.has_vault_address() || self.is_testnet() {
            ...
            Some(HyperliquidExecBuilderFee {
                address: VIBE_BUILDER_ADDRESS.to_string(),
                fee_tenths_bp: 0,
            })
        }
    }
    ...
    pub async fn request_instrument_defs(&self) -> Result<Vec<HyperliquidInstrumentDef>> {
        ...
        // Drop defs whose Vibe-internal symbol collides with one already
    ...
    /// Converts instrument definitions into Vibe instruments.
    ...
    ///
    /// Fetches candle data from the Hyperliquid API and converts it to Vibe bars.
...
mod tests {
    ...
    use ustr::Ustr;
    use vibe_core::{MUTEX_POISONED, time::get_atomic_clock_realtime};
    use vibe_model::{
    ...
    use crate::{
        common::{
            consts::{HYPERLIQUID_VENUE, VIBE_BUILDER_ADDRESS},
    ...
    fn test_builder_attribution_defaults_to_mainnet_builder() {
        ...
        assert_eq!(builder.address, VIBE_BUILDER_ADDRESS);
    ...
    fn test_cache_instrument_by_raw_symbol() {
        ...
        // Vibe symbol is "vntls:vCURSOR-USDC-SPOT"
