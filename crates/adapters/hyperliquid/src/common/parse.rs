//! Parsing utilities that convert Hyperliquid payloads into Vibe domain models.
//!
...
pub fn millis_to_nanos(millis: u64) -> anyhow::Result<UnixNanos> {
    let value = vibe_core::datetime::millis_to_nanos(millis as f64)?;
...
/// Suffix shared by every Vibe outcome symbol, mirroring `-PERP` / `-SPOT`.
pub const OUTCOME_SYMBOL_SUFFIX: &str = "-OUTCOME";
/// Yes-side label on Vibe outcome symbols.
pub const OUTCOME_SIDE_YES: &str = "YES";
/// No-side label on Vibe outcome symbols.
pub const OUTCOME_SIDE_NO: &str = "NO";

/// Parses a Vibe outcome instrument symbol of the form
...
#[must_use]
pub fn parse_outcome_vibe_symbol(symbol: &str) -> Option<(u32, u8)> {
...
/// Formats an `(outcome_index, side)` pair into the Vibe outcome symbol
/// form `{outcome_index}-{YES|NO}-OUTCOME`.
#[must_use]
pub fn format_outcome_vibe_symbol(outcome_index: u32, side: u8) -> String {
...
/// Vibe outcome symbol, or `None` if the symbol is not an outcome.
#[must_use]
pub fn outcome_token_from_vibe_symbol(symbol: &str) -> Option<String> {
    let (outcome_index, side) = parse_outcome_vibe_symbol(symbol)?;
    let encoding = 10 * outcome_index + u32::from(side);
    Some(format!("+{encoding}"))
}

/// Returns the secondary cache-alias key for a Vibe instrument symbol.
///
...
pub fn cache_alias_for_symbol(symbol: &str) -> Option<String> {
    if let Some(token) = outcome_token_from_vibe_symbol(symbol) {
        return Some(token);
    }
...
/// Converts a Vibe `TimeInForce` to Hyperliquid TIF.
///
/// # Errors
///
...
/// Converts a Vibe `BarType` to a Hyperliquid bar interval.
///
/// # Errors
///
...
/// Converts a Vibe order to Hyperliquid request using a pre-resolved asset index.
///
...
/// Converts a Vibe order to Hyperliquid request with an explicit CLOID.
...
/// # Returns
///
/// The corresponding Vibe `OrderType`.
...
/// Parses Hyperliquid clearinghouse state into Vibe account balances and margins.
///
...
/// Parses Hyperliquid spot clearinghouse state into Vibe account balances.
///
...
/// # Errors
///
/// Returns an error if any balance cannot be converted to a Vibe `Money`.
...
mod tests {
    ...
    fn test_parse_outcome_vibe_symbol(
        #[case] symbol: &str,
        #[case] outcome_index: u32,
        #[case] side: u8,
    ) {
        let parsed = parse_outcome_vibe_symbol(symbol).unwrap();
    ...
    #[case("25-YES-outcome")]
    #[case("25-YES")]
    fn test_parse_outcome_vibe_symbol_rejects_invalid(#[case] symbol: &str) {
        assert!(parse_outcome_vibe_symbol(symbol).is_none());
    }
    ...
    fn test_parse_outcome_vibe_symbol_rejects_overflow(#[case] symbol: &str) {
        assert!(parse_outcome_vibe_symbol(symbol).is_none());
    }
    ...
    fn test_format_outcome_vibe_symbol(
        #[case] outcome_index: u32,
        #[case] side: u8,
        #[case] expected: &str,
    ) {
        assert_eq!(format_outcome_vibe_symbol(outcome_index, side), expected,);
    }
    ...
    #[case("+250", None)]
    fn test_outcome_token_from_vibe_symbol(#[case] symbol: &str, #[case] expected: Option<String>) {
        assert_eq!(outcome_token_from_vibe_symbol(symbol), expected);
    }
