...
/// Manages precision configuration and converts Hyperliquid data to standard Vibe formats
#[derive(Debug)]
...
///
/// This struct is used only for parsing API responses and converting to Vibe
/// PositionStatusReport events. The actual position tracking is handled by the
/// Vibe platform, not the adapter.
///
...
/// Used for converting to Vibe AccountBalance and AccountState events.
///
/// See Hyperliquid API documentation:
...
///
/// This tracks only the essential state needed for generating Vibe AccountState events.
/// Position tracking is handled by the Vibe platform, not the adapter.
///
/// See Hyperliquid API documentation:
/// - [User State Info](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/info-endpoint/perpetuals#retrieve-users-perpetuals-account-summary)
...
impl HyperliquidAccountState {
    ...
    /// tracked by the Vibe platform, not the adapter.
    pub fn account_value(&self) -> Decimal {
        self.balances.values().map(|balance| balance.total).sum()
    }

    /// Convert HyperliquidAccountState to Vibe AccountState event.
    ///
    /// This creates a standard Vibe AccountState from the Hyperliquid-specific account state,
    /// converting balances and handling the margin account type since Hyperliquid supports leverage.
    ///
    /// # Returns
    ///
    /// A Vibe AccountState event that can be processed by the platform
...
/// Vibe platform, so this only processes balance changes.
///
...
/// Parse Hyperliquid position data into a Vibe PositionStatusReport.
///
/// This function converts raw position data from Hyperliquid API responses into
/// the standardized Vibe PositionStatusReport format. The actual position
/// tracking and management is handled by the Vibe platform.
///
/// See Hyperliquid API documentation:
/// - [User State Info](https://hyperliquid.gitbook.io/hyperliquid-docs/for-developers/api/info-endpoint/perpetuals#retrieve-users-perpetuals-account-summary)
