...
//! the zero-fee Vibe builder attribution without ever charging a fee.
//!
...
use vibe_network::http::{HttpClient, Method};

use super::{
    consts::{HYPERLIQUID_CHAIN_ID, VIBE_BUILDER_ADDRESS, exchange_url},
...
/// Approves the Vibe builder fee using environment variables.
///
...
pub async fn approve_from_env(non_interactive: bool) -> bool {
    ...
    println!("Approving Vibe builder attribution on {network}");
    println!("Builder address: {VIBE_BUILDER_ADDRESS}");
    ...
    println!("Vibe builder address. The action must be signed by the master wallet.");
    println!();
...
/// Revokes the Vibe builder fee using environment variables.
///
/// Reads private key from environment:
/// - Testnet: `HYPERLIQUID_TESTNET_PK`
/// - Mainnet: `HYPERLIQUID_PK`
...
pub async fn revoke_from_env(non_interactive: bool) -> bool {
    ...
    println!("Revoking Vibe builder fee on {network}");
    println!("Builder address: {VIBE_BUILDER_ADDRESS}");
    println!();
...
/// Approves the Vibe builder fee for a wallet.
///
...
/// Revokes the Vibe builder fee approval for a wallet.
///
/// This signs an EIP-712 `ApproveBuilderFee` action with a 0% max fee rate and
...
async fn submit_builder_fee_update(
    ...
    Ok(BuilderFeeApprovalResult {
        ...
        message,
        wallet_address,
        builder_address: VIBE_BUILDER_ADDRESS.to_string(),
        is_testnet,
    })
}
...
fn approval_signing_hash(is_testnet: bool, nonce: u64, fee_rate: &str) -> Result<B256> {
    ...
    let builder_addr = Address::from_str(VIBE_BUILDER_ADDRESS)
...
mod tests {
    ...
    fn test_build_approval_action_payload() {
        ...
        assert_eq!(action["builder"], VIBE_BUILDER_ADDRESS);
