...
async fn set_report_payload(
    state: &common::server::TestServerState,
    family: ReportFamily,
    symbol: &str,
) {
    match family {
...
#[rstest]
#[tokio::test]
async fn test_domain_http_request_instruments_returns_vibe_types() {
    let addr = start_test_server().await;
    let base_url = format!("http://{addr}");
...
#[rstest]
#[tokio::test]
async fn test_domain_http_request_instrument_returns_vibe_type() {
    let addr = start_test_server().await;
    let base_url = format!("http://{addr}");
