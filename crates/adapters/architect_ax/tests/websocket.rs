...
impl Log for OutboundLogCapture {
    fn enabled(&self, metadata: &Metadata<'_>) -> bool {
        metadata.level() == Level::Trace
            && matches!(
                metadata.target(),
                "vibe_architect_ax::websocket::data::handler"
                    | "vibe_architect_ax::websocket::orders::handler"
            )
    }
...
async fn test_outbound_logs_omit_payload_bodies() {
    ...
    assert!(
        messages.iter().any(|(target, message)| {
            target == "vibe_architect_ax::websocket::data::handler"
    ...
    assert!(
        messages.iter().any(|(target, message)| {
            target == "vibe_architect_ax::websocket::orders::handler"
