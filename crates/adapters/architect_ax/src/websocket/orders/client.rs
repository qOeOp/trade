...
use ustr::Ustr;
use vibe_common::{cache::InstrumentLookupError, live::get_runtime};
use vibe_core::{
    AtomicMap,
...
impl AxOrdersWebSocketClient {
    ...
    /// Submits the AX priced order shape using Vibe domain types.
    ///
    /// This method handles conversion from Vibe domain types to AX-specific
    ...
    pub async fn submit_order(
        ...
        let order = AxWsPlaceOrder {
            ...
            tif: ax_tif,
            po: post_only,
            tag: Some(AX_VIBE_TAG.to_string()),
            cid: Some(cid),
        };
