//! Parsing functions to convert Ax HTTP responses to Vibe domain types.
...
use serde_json::json;
use ustr::Ustr;
use vibe_core::{Params, UUID4, datetime::datetime_to_unix_nanos, nanos::UnixNanos};
use vibe_model::{
...
/// Converts an Ax candle width to a Vibe bar specification.
#[must_use]
...
/// Parses an Ax candle into a Vibe Bar.
///
/// # Errors
///
...
/// Parses an Ax funding rate into a Vibe [`FundingRateUpdate`].
///
/// # Errors
///
...
/// Parses an Ax instrument into a Vibe perpetual or dated futures contract.
///
/// # Errors
///
...
/// Parses an Ax balances response into a Vibe [`AccountState`].
///
...
/// Parses an Ax open order into a Vibe [`OrderStatusReport`].
///
...
/// Parses an Ax historical order into a Vibe [`OrderStatusReport`].
///
/// The `cid_resolver` parameter is an optional function that resolves a `cid` (u64)
/// to a `ClientOrderId`. This is needed because orders submitted via WebSocket use
/// a hashed `cid` for correlation rather than storing the full `ClientOrderId` in the tag.
    ...
    F: Fn(u64) -> Option<ClientOrderId>,
{
    ...
    // The current AX wire shape maps to a Vibe limit order.
...
/// Parses an Ax fill into a Vibe [`FillReport`].
///
...
/// Parses an Ax position into a Vibe [`PositionStatusReport`].
///
/// # Errors
///
/// Returns an error if:
...
/// Parses an Ax REST trade into a Vibe [`TradeTick`].
///
/// # Errors
///
