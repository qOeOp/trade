...
use ustr::Ustr;
use vibe_core::{
    AtomicMap, AtomicTime, UUID4, consts::VIBE_USER_AGENT, nanos::UnixNanos,
...
impl AxRawHttpClient {
    ...
    fn default_headers() -> HashMap<String, String> {
        HashMap::from([
            (USER_AGENT.to_string(), VIBE_USER_AGENT.to_string()),
...
impl AxHttpClient {
    ...
    /// Requests an order book snapshot from Ax and builds a Vibe [`OrderBook`].
    ///
    ...
    /// Requests recent trades from Ax and parses them to Vibe [`TradeTick`].
    ///
    ...
    /// Requests historical bars from Ax and parses them to Vibe Bar types.
    ///
    ...
    /// Requests funding rates from Ax and parses them to Vibe types.
    ///
    ...
    /// is no pagination. The schedule has no Vibe domain equivalent, so
    ...
    /// Requests account state from Ax and parses to a Vibe [`AccountState`].
    ///
    /// # Errors
    ///
    ...
    /// Requests open orders from Ax and parses them to Vibe [`OrderStatusReport`].
    ///
    ...
    /// Requests historical orders from Ax and parses them to Vibe
    /// [`OrderStatusReport`].
    ///
    /// Missing instruments are requested from Ax and cached before parsing order details.
    ///
    ...
    /// Requests fills from Ax and parses them to Vibe [`FillReport`].
    ///
    ...
    /// Requests positions from Ax and parses them to Vibe [`PositionStatusReport`].
    ///
