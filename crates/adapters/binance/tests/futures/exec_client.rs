...
use vibe_binance::{
    common::{
        consts::{
            ...
            BINANCE_VENUE, BINANCE_VIBE_FUTURES_BROKER_ID,
        },
...
async fn test_mixed_batch_cancel_splits_id_lists_and_maps_rejections() {
    ...
    let expected_client_order_id = format!(
        "[\"{}\"]",
        encode_broker_id(&client_id_only, BINANCE_VIBE_FUTURES_BROKER_ID)
    );
...
fn assert_query_symbol_eq(query: &HashMap<String, String>, expected: &str, vibe_symbol: &str) {
    assert_eq!(query.get("symbol").map(String::as_str), Some(expected));
    assert!(!query.values().any(|value| value.contains(vibe_symbol)));
}
