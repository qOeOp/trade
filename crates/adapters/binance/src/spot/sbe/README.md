...
- Generic cursor/error utilities are shared from `vibe-serialization` behind the `sbe` feature and re-exported from this module for compatibility.
...
- Run `cargo +nightly fmt -p vibe-binance` to normalise the generator's whitespace.
...
- `cargo build -p vibe-binance` and `cargo test -p vibe-binance` must pass.
- `cargo clippy -p vibe-binance --all-targets` must pass.
