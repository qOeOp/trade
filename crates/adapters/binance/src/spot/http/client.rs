...
use vibe_core::{
    collections::AtomicMap, consts::VIBE_USER_AGENT, datetime::SECONDS_IN_DAY, hex,
...
use crate::{
    common::{
        consts::{
            BINANCE_API_KEY_HEADER, BINANCE_NO_SUCH_ORDER_CODE, BINANCE_SPOT_RATE_LIMITS,
            BINANCE_VENUE, BINANCE_VIBE_SPOT_BROKER_ID, BinanceRateLimitQuota,
        },
...
impl BinanceRawSpotHttpClient {
    ...
    fn default_headers(
        ...
        headers.insert(USER_AGENT.to_string(), VIBE_USER_AGENT.to_string());
        if json_responses {
...
/// - Complex types (instruments, orders): Transform to Vibe domain types.
...
impl BinanceSpotHttpClient {
    ...
    /// Requests Vibe instruments for all trading symbols.
    ///
    ...
    /// Requests configured Vibe instruments with populated maker and taker fees.
    ///
    /// # Errors
    ///
    ...
    /// Requests the account state with Vibe types.
    ///
    /// # Errors
    ///
    /// Returns an error if the request fails or SBE decoding fails.
    ...
    pub async fn request_order_status_report(
        ...
        let client_id_str =
            client_order_id.map(|id| encode_broker_id(&id, BINANCE_VIBE_SPOT_BROKER_ID));
    ...
    ///
    /// Converts Vibe domain types to Binance-specific parameters
        ...
        responses
            ...
            .map(|r| {
                Ok((
                    ...
                    decode_client_order_id(&r.orig_client_order_id, BINANCE_VIBE_SPOT_BROKER_ID)?,
                ))
            })
            .collect()
    }
...
mod tests {
    use rstest::rstest;
    use vibe_model::instruments::stubs::currency_pair_btcusdt;
