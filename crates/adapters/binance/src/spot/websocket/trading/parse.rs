//! Parse functions for converting Binance Spot venue types to Vibe reports.
//!
...
/// Converts a Binance Spot execution report to a Vibe order status report.
///
/// # Errors
///
...
pub fn parse_spot_exec_report_to_order_status(
    ...
    let client_order_id =
        decode_client_order_id(&msg.client_order_id, BINANCE_VIBE_SPOT_BROKER_ID)?;
...
/// Converts a Binance Spot execution report (Trade type) to a Vibe fill report.
///
/// # Errors
///
/// Returns an error if report construction fails.
...
/// Converts a Binance Spot account position update to a Vibe account state.
...
mod tests {
    use rstest::rstest;
    use vibe_model::{identifiers::ClientOrderId, types::Quantity};
