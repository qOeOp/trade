...
//! into Vibe domain objects such as instruments and market data.
...
/// Parses a USD-M Futures symbol definition into a Vibe futures instrument.
///
/// # Errors
///
/// Returns an error if:
...
/// Parses a COIN-M Futures symbol definition into a Vibe crypto futures instrument.
///
...
/// Parses a Binance Spot SBE symbol into a Vibe CurrencyPair instrument.
///
/// # Errors
///
/// Returns an error if:
...
/// Parses Binance SBE trades into Vibe TradeTick objects.
///
...
/// Maps Binance SBE order status to Vibe order status.
#[must_use]
...
/// Maps Binance SBE order type to Vibe order type.
#[must_use]
...
/// Maps Binance SBE order side to Vibe order side.
#[must_use]
...
/// Maps Binance SBE time in force to Vibe time in force.
#[must_use]
...
/// Parses a Binance SBE order response into a Vibe `OrderStatusReport`.
///
/// # Errors
///
...
/// Parses a Binance new order response (SBE) into a Vibe `OrderStatusReport`.
///
/// # Errors
///
/// Returns an error if any field cannot be parsed.
...
/// Parses a Binance SBE account trade into a Vibe `FillReport`.
///
/// # Errors
///
/// Returns an error if any field cannot be parsed.
...
/// Parses Binance klines (candlesticks) into Vibe Bar objects.
///
/// # Errors
///
...
/// Converts a Vibe bar specification to a Binance kline interval.
///
/// # Errors
///
...
mod tests {
    ...
    use super::*;
    use crate::common::{
        consts::BINANCE_VIBE_SPOT_BROKER_ID,
    ...
    fn test_parse_klines_to_bars() {
        use vibe_model::enums::{AggregationSource, PriceType};
