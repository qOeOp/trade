...
//! `^[\.A-Z\:/a-z0-9_-]{1,36}$`.
//!
//! Internal Vibe `ClientOrderId` values (O-format: 23+ chars, UUID: 32-36
...
//! Use `cargo test --release -p vibe-binance --lib -- bench_encode_decode_timing --nocapture`
...
mod tests {
    ...
    use super::{super::consts::BINANCE_VIBE_SPOT_BROKER_ID, *};

    const TEST_BROKER_ID: &str = BINANCE_VIBE_SPOT_BROKER_ID;
