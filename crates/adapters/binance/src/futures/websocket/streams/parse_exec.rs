//! Parse functions for converting Binance Futures venue types to Vibe reports.
//!
//! Pure functions that take venue message + precision + account_id + ts_init
//! and return Vibe report types.
...
/// Converts a Binance Futures order update to a Vibe order status report.
///
/// # Errors
///
...
/// Converts a Binance Futures order update (Trade type) to a Vibe fill report.
///
/// # Errors
///
/// Returns an error if report construction fails.
...
/// Converts a Binance Futures algo order update to a Vibe order status report.
///
...
/// Converts a Binance Futures account update to a Vibe account state.
...
pub fn decode_order_client_id(order: &OrderUpdateData) -> anyhow::Result<ClientOrderId> {
    decode_client_order_id(&order.client_order_id, BINANCE_VIBE_FUTURES_BROKER_ID)
}
...
pub fn decode_algo_client_id(algo: &AlgoOrderUpdateData) -> anyhow::Result<ClientOrderId> {
    decode_client_order_id(&algo.client_algo_id, BINANCE_VIBE_FUTURES_BROKER_ID)
}
...
mod tests {
    ...
    fn test_decode_order_client_id() {
        ...
        msg.order.client_order_id = encode_broker_id(&original, BINANCE_VIBE_FUTURES_BROKER_ID);
    ...
    fn test_decode_algo_client_id() {
        ...
        msg.algo_order.client_algo_id = encode_broker_id(&original, BINANCE_VIBE_FUTURES_BROKER_ID);
