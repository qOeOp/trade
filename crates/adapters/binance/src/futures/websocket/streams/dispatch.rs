...
use vibe_common::{cache::fifo::FifoCache, live::get_runtime};
use vibe_core::{AtomicSet, MUTEX_POISONED, UUID4, UnixNanos, time::AtomicTime};
...
pub(crate) fn dispatch_trade_lite(
    ...
    let client_order_id =
        match decode_client_order_id(&msg.client_order_id, BINANCE_VIBE_FUTURES_BROKER_ID) {
            Ok(client_order_id) => client_order_id,
            Err(e) => {
...
mod tests {
    ...
    fn make_status_report() -> OrderStatusReport {
        use vibe_model::enums::TimeInForce;
