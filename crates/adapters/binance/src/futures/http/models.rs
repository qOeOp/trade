...
use crate::{
    common::{
        consts::BINANCE_VIBE_FUTURES_BROKER_ID,
...
impl BinanceFuturesAccountInfo {
    /// Converts this Binance account info to a Vibe [`AccountState`].
    ///
    /// # Errors
    ///
...
impl BinanceFuturesOrder {
    /// Converts this Binance order to a Vibe [`OrderStatusReport`].
    ///
    /// # Errors
    ///
    ...
    pub fn to_order_status_report(
        ...
        let client_order_id =
            decode_client_order_id(&self.client_order_id, BINANCE_VIBE_FUTURES_BROKER_ID)?;
        ...
        let order_type = self.order_type.to_vibe_order_type();
        let time_in_force = self.time_in_force.to_vibe_time_in_force();
        let order_status = self.status.to_vibe_order_status(treat_expired_as_canceled);
...
impl BinanceFuturesOrderType {
    ...
    /// Converts to Vibe order type.
    #[must_use]
    pub fn to_vibe_order_type(&self) -> OrderType {
        match self {
...
impl BinanceTimeInForce {
    /// Converts to Vibe time in force.
    #[must_use]
    pub fn to_vibe_time_in_force(&self) -> TimeInForce {
        match self {
...
impl BinanceOrderStatus {
    /// Converts to Vibe order status.
    #[must_use]
    pub fn to_vibe_order_status(&self, treat_expired_as_canceled: bool) -> OrderStatus {
        match self {
...
impl BinanceUserTrade {
    /// Converts this Binance trade to a Vibe [`FillReport`].
    ///
    /// # Errors
    ///
...
impl BinanceFuturesAlgoOrder {
    /// Converts this Binance algo order to a Vibe [`OrderStatusReport`].
    ///
    /// # Errors
    ///
    ...
    pub fn to_order_status_report(
        ...
        let client_order_id =
            decode_client_order_id(&self.client_algo_id, BINANCE_VIBE_FUTURES_BROKER_ID)?;
        ...
        let time_in_force = self
            .time_in_force
            .as_ref()
            .map_or(TimeInForce::Gtc, |tif| tif.to_vibe_time_in_force());
...
mod tests {
    ...
    fn test_to_vibe_order_status_expired_respects_treat_as_canceled(
        #[case] status: BinanceOrderStatus,
        #[case] treat_expired_as_canceled: bool,
        #[case] expected: OrderStatus,
    ) {
        let result = status.to_vibe_order_status(treat_expired_as_canceled);
