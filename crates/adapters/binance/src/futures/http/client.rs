...
use vibe_core::{
    consts::VIBE_USER_AGENT, datetime::SECONDS_IN_DAY, nanos::UnixNanos, time::AtomicTime,
};
use vibe_model::{
...
use crate::{
    common::{
        ...
        consts::{
            ...
            BINANCE_FAPI_RATE_LIMITS, BINANCE_VIBE_FUTURES_BROKER_ID, BinanceRateLimitQuota,
        },
...
impl BinanceFuturesInstrument {
    ...
    /// Returns the Vibe-formatted instrument ID.
    #[must_use]
...
impl BinanceFuturesHttpClient {
    ...
    /// Fetches exchange information and returns parsed Vibe instruments.
    ///
    /// # Errors
    ///
    /// Returns an error if the request fails or the product type is invalid.
    ...
    pub async fn submit_order(
        ...
        let client_id_str = encode_broker_id(&client_order_id, BINANCE_VIBE_FUTURES_BROKER_ID);
    ...
    pub async fn modify_order(
        ...
        let params = BinanceModifyOrderParams {
            ...
            orig_client_order_id: client_order_id
                .map(|id| encode_broker_id(&id, BINANCE_VIBE_FUTURES_BROKER_ID)),
            side: binance_side,
    ...
    pub async fn cancel_algo_order(&self, client_order_id: ClientOrderId) -> anyhow::Result<()> {
        let params = BinanceAlgoOrderQueryParams {
            ...
            client_algo_id: Some(encode_broker_id(
                &client_order_id,
                BINANCE_VIBE_FUTURES_BROKER_ID,
            )),
            recv_window: None,
        };
    ...
    pub async fn request_order_status_report(
        ...
        let orig_client_order_id =
            client_order_id.map(|id| encode_broker_id(&id, BINANCE_VIBE_FUTURES_BROKER_ID));
...
/// Converts a Vibe order type to a Binance Futures order type.
...
mod tests {
    ...
    use vibe_network::http::{HttpStatus, StatusCode};
