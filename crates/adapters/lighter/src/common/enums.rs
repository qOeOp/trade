...
impl LighterCandleResolution {
    ...
    /// Returns the Vibe [`BarSpecification`] for this candle resolution (`Last` / `External`).
    #[must_use]
...
impl LighterOrderStatus {
    ...
    /// [`vibe_model::reports::OrderStatusReport::with_cancel_reason`].
    ///
    ...
    /// variants, which carry their meaning via the Vibe order status itself.
    #[must_use]
...
impl LighterOrderType {
    /// Returns the Vibe [`OrderType`] that this Lighter order type maps to.
    ///
    /// `StopLoss` / `StopLossLimit` map to Vibe stop-on-the-loss-side
    ...
    /// do not map to a single Vibe order type.
    pub fn as_vibe(self) -> anyhow::Result<OrderType> {
        match self {
            ...
            Self::Twap | Self::TwapSub | Self::Liquidation => Err(anyhow::anyhow!(
                "Lighter `{self:?}` has no Vibe order-type equivalent",
            )),
        }
    }
}
    ...
    fn try_from(value: OrderType) -> Result<Self, Self::Error> {
        match value {
            ...
            other => Err(anyhow::anyhow!(
                "Vibe `{other:?}` has no Lighter order-type equivalent",
            )),
        }
    }
}
...
// Conversions between `LighterTimeInForce` and Vibe `TimeInForce` are
...
/// Converts a Vibe [`OrderSide`] to the venue's `is_ask` boolean.
///
...
/// Converts the venue's `is_ask` boolean to a Vibe [`OrderSide`].
#[must_use]
...
mod tests {
    ...
    fn test_order_type_round_trip(#[case] lighter: LighterOrderType, #[case] vibe: OrderType) {
        assert_eq!(lighter.as_vibe().unwrap(), vibe);
        assert_eq!(LighterOrderType::try_from(vibe).unwrap(), lighter);
    }
    ...
    fn test_order_type_internal_variants_have_no_vibe_mapping(
        #[case] order_type: LighterOrderType,
    ) {
        let err = order_type.as_vibe().unwrap_err();
    ...
    fn test_order_type_unsupported_vibe_variants_error(#[case] vibe: OrderType) {
        let err = LighterOrderType::try_from(vibe).unwrap_err();
