...
//! Vibe [`Price`] and [`Quantity`] without a floating-point round-trip.
...
use vibe_model::types::{Price, Quantity, fixed::FIXED_PRECISION};

/// Maximum decimal places that fit into Vibe [`Price`] / [`Quantity`].
pub const MAX_DECIMALS: u8 = FIXED_PRECISION;

/// Converts a Lighter price tick count into a Vibe [`Price`].
///
...
/// Converts a Lighter base-amount tick count into a Vibe [`Quantity`].
///
/// Order sizes on the wire are signed `i64` multiples of `10^-decimals`
/// base-asset units. Vibe [`Quantity`] is non-negative, so a negative
...
/// Converts a decimal string into a Vibe [`Price`] at the requested precision.
///
/// # Errors
///
...
/// Converts a decimal string into a non-negative Vibe [`Quantity`].
///
...
/// Converts a [`Decimal`] into a Vibe [`Price`] at the requested precision.
///
...
/// Converts a [`Decimal`] into a non-negative Vibe [`Quantity`] at the
/// requested precision.
///
/// Zero is allowed because Lighter sends zero-size book levels to delete
/// existing orders.
...
mod tests {
    ...
    fn parse_quantity_rejects_oversized_ticks() {
        // i64::MAX at 0 decimals exceeds the Vibe Quantity range.
