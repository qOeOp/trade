//! Bidirectional mapping between Vibe `InstrumentId` and Lighter `market_index`.
//!
...
/// Suffix applied to perpetual instrument symbols on the Vibe side.
pub const PERP_SUFFIX: &str = "-PERP";

/// Suffix applied to spot instrument symbols on the Vibe side.
pub const SPOT_SUFFIX: &str = "-SPOT";

/// Builds a Vibe [`InstrumentId`] from a venue symbol and product type.
///
...
/// WebSocket subscriptions), the Vibe [`InstrumentId`] (used by the
