...
use vibe_core::{AtomicTime, MUTEX_POISONED, UnixNanos};
use vibe_model::{
...
/// has an originating Vibe order that should receive a typed
...
pub(crate) struct WsDispatchState {
    ...
    /// time back to the originating Vibe [`ClientOrderId`]. The venue
    ...
    /// Maps Vibe [`ClientOrderId`] to the venue-assigned
...
impl WsDispatchState {
    ...
    /// Hash a Vibe [`ClientOrderId`] into a stable positive `i64` for use
    ...
    /// Look up the venue-assigned [`VenueOrderId`] for a Vibe cloid.
...
/// Convert a Lighter HTTP `LighterOrder` into a Vibe
...
/// originating Vibe [`ClientOrderId`] when available, so HTTP-driven
...
/// Translate a Vibe [`TimeInForce`] into the venue's `LighterTimeInForce`.
///
...
pub(crate) fn vibe_to_lighter_tif(
    order_type: OrderType,
    tif: TimeInForce,
    post_only: bool,
    ...
    if is_conditional_market_order(order_type) {
        return match tif {
            ...
            TimeInForce::Ioc => anyhow::bail!(
                "Lighter conditional market orders require a positive expiry; Vibe IOC cannot be represented because the venue uses IOC for post-trigger execution",
            ),
            TimeInForce::Fok => anyhow::bail!(
                "Lighter has no fill-or-kill TIF; reject FOK at the strategy or use IOC explicitly",
            ),
...
/// Translate a Vibe [`OrderType`] into the venue's [`LighterOrderType`]
/// discriminant for use in `CreateOrder` tx bodies.
pub(crate) fn vibe_to_lighter_order_type(
    order_type: OrderType,
) -> anyhow::Result<LighterOrderType> {
    LighterOrderType::try_from(order_type)
        .map_err(|e| anyhow::anyhow!("unsupported Vibe order type for Lighter: {e}"))
}

/// Compute the venue-side `order_expiry` (millis) for a Vibe order.
///
...
/// Convert a Vibe [`Quantity`] to the venue's signed-i64 base-asset tick
...
/// Convert a Vibe [`Price`] to the venue's `u32` quote-asset tick
...
mod tests {
    ...
    // client id substitutes to the originating Vibe cloid only when the
    ...
    fn vibe_to_lighter_tif_supported_variants(
        #[case] input: TimeInForce,
        #[case] expected: LighterTimeInForce,
    ) {
        assert_eq!(
            vibe_to_lighter_tif(OrderType::Limit, input, false).unwrap(),
            expected
        );
    }

    #[rstest]
    fn vibe_to_lighter_tif_market_orders_use_ioc() {
        assert_eq!(
            vibe_to_lighter_tif(OrderType::Market, TimeInForce::Gtc, false).unwrap(),
            LighterTimeInForce::ImmediateOrCancel,
        );
        assert_eq!(
            vibe_to_lighter_tif(OrderType::Market, TimeInForce::Ioc, false).unwrap(),
            LighterTimeInForce::ImmediateOrCancel,
        );
    }
    ...
    fn vibe_to_lighter_tif_market_orders_reject_resting_tifs(#[case] tif: TimeInForce) {
        let err = vibe_to_lighter_tif(OrderType::Market, tif, false).unwrap_err();
    ...
    fn vibe_to_lighter_tif_conditional_market_orders_use_ioc_wire_tif(
        #[case] order_type: OrderType,
    ) {
        for tif in [TimeInForce::Gtc, TimeInForce::Day, TimeInForce::Gtd] {
            assert_eq!(
                vibe_to_lighter_tif(order_type, tif, false).unwrap(),
                LighterTimeInForce::ImmediateOrCancel,
            );
        }
    }
    ...
    #[rstest]
    fn vibe_to_lighter_tif_conditional_market_orders_reject_vibe_ioc() {
        let err = vibe_to_lighter_tif(OrderType::StopMarket, TimeInForce::Ioc, false).unwrap_err();
    ...
    fn vibe_to_lighter_tif_conditional_limit_orders_allow_ioc(#[case] order_type: OrderType) {
        assert_eq!(
            vibe_to_lighter_tif(order_type, TimeInForce::Ioc, false).unwrap(),
            LighterTimeInForce::ImmediateOrCancel,
        );
    }
    ...
    fn vibe_to_lighter_tif_unsupported_variants_error(#[case] tif: TimeInForce) {
        let err = vibe_to_lighter_tif(OrderType::Limit, tif, false).unwrap_err();
    ...
    // A precision-0 quantity at 1e10 is well inside the Vibe
    ...
    fn stub_pending_create(
        ...
        use vibe_model::orders::builder::OrderTestBuilder;
