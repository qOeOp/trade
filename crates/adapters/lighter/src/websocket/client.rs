...
impl LighterWebSocketClient {
            ...
            loop {
                match handler.next().await {
                    Some(VibeWsMessage::Reconnected { connection_epoch }) => {
                        ...
                        if handler
                            .send(VibeWsMessage::Reconnected { connection_epoch })
                            .is_err()
                        {
    ...
    pub async fn next_event(&mut self) -> Option<VibeWsMessage> {
    ...
    /// [`VibeWsMessage::Raw`]; once it is set the handler emits typed
    /// [`crate::websocket::messages::ExecutionReport`] and
    /// [`crate::websocket::messages::VibeWsMessage::AccountState`]
