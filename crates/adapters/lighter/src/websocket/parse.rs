//! Parsers from Lighter streaming payloads to Vibe domain types.
...
/// Parses a Lighter trade stream item into a Vibe [`TradeTick`].
///
/// # Errors
///
/// Returns an error if the trade cannot be converted into a Vibe tick.
...
/// Parses a Lighter order book update into Vibe deltas.
///
...
/// Parses a full Lighter order book payload into a Vibe [`OrderBookDepth10`].
///
...
/// Parses a Lighter ticker stream payload into a Vibe [`QuoteTick`].
///
...
/// not used for the streaming Vibe update. The accompanying
...
/// Parses a Lighter WebSocket candle into a Vibe [`Bar`] with `ts_event` set to the bar open.
///
/// # Errors
///
...
/// converted, or if the venue order kind has no Vibe equivalent.
pub fn parse_ws_order_status_report(
    ...
    let order_side = order
        .side
        .map_or_else(|| order_side_from_is_ask(order.is_ask), vibe_order_side);
    let order_type = vibe_order_type(order.order_type)?;

    let (time_in_force, expire_time) = vibe_time_in_force(order.time_in_force, order.order_expiry);
    ...
    let order_status = vibe_order_status(order.status, &filled_qty);
    ...
    // `order_id`), not the client namespace. Vibe
...
/// consumption loop maps these into [`vibe_model::events::OrderEventAny`]
...
pub fn parse_ws_position_status_report(
    ...
    Ok(PositionStatusReport::new(
        ...
        ts_init,
        Some(UUID4::new()),
        None, // venue_position_id: NETTING, Vibe identifies by instrument
        avg_px_open,
    ))
}
...
fn vibe_order_side(side: LighterOrderSide) -> OrderSide {
    match side {
...
fn vibe_order_type(kind: LighterOrderKind) -> anyhow::Result<OrderType> {
    match kind {
        ...
        LighterOrderKind::Twap | LighterOrderKind::TwapSub | LighterOrderKind::Liquidation => Err(
            anyhow::anyhow!("Lighter `{kind:?}` has no Vibe order-type equivalent",),
        ),
    }
}

fn vibe_time_in_force(
    tif: LighterOrderTimeInForce,
    order_expiry: i64,
) -> (TimeInForce, Option<UnixNanos>) {
    match tif {
        LighterOrderTimeInForce::ImmediateOrCancel => (TimeInForce::Ioc, None),
        // Lighter has no Vibe PostOnly TIF; Vibe models it as Gtc + post_only flag.
...
fn vibe_order_status(status: LighterOrderStatus, filled_qty: &Quantity) -> OrderStatus {
    match status {
...
mod tests {
    ...
    fn test_parse_ws_order_status_report_omits_parent_order_id() {
        ...
        // Vibe `OrderStatusReport::parent_order_id` (a `ClientOrderId`)
    ...
    // the post_only flag because Vibe has no PostOnly TIF. This matrix
    // pins each combination so silent regressions in vibe_time_in_force
    // surface immediately.
    #[rstest]
    #[case::ioc(
    ...
    fn test_parse_ws_order_status_report_rejects_twap() {
        ...
        assert!(err.to_string().contains("no Vibe order-type equivalent"));
    }
