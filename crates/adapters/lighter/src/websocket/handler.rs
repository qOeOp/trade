...
use vibe_core::{AtomicTime, nanos::UnixNanos, time::get_atomic_clock_realtime};
use vibe_model::{identifiers::AccountId, instruments::InstrumentAny};
use vibe_network::{
    RECONNECTED,
    error::SendError,
...
use super::{
    ...
    messages::{
        ...
        LighterWsOrderBook, LighterWsRequest, SendTxRejectionSource, VibeWsMessage,
    },
    parse::{
...
pub enum HandlerCommand {
    ...
    /// be emitted as [`VibeWsMessage::Deltas`].
    SetBookDeltasSub { market_index: i16, subscribed: bool },
    /// Toggle whether `update/order_book` frames for `market_index` should
    /// also be emitted as a [`VibeWsMessage::Depth10`] snapshot.
...
pub(super) struct FeedHandler {
    ...
    pending_messages: VecDeque<VibeWsMessage>,
...
impl FeedHandler {
    ...
    pub(super) fn send(&self, msg: VibeWsMessage) -> Result<(), String> {
        self.out_tx
            .send(msg)
    ...
    pub(super) async fn next(&mut self) -> Option<VibeWsMessage> {
                        ...
                        Message::Text(text) => {
                                ...
                                serde_json::from_str::<serde_json::Value>(&text)
                            {
                                ...
                                if !matched {
                                    ...
                                    return Some(VibeWsMessage::Raw(value));
                                }
                            } else {
    ...
    fn dispatch_results(&mut self, mut messages: Vec<VibeWsMessage>) -> Option<VibeWsMessage> {
    ...
    fn handle_control_value(&mut self, value: &serde_json::Value) -> (bool, Option<VibeWsMessage>) {
    ...
    fn handle_frame(&mut self, frame: LighterWsFrame, ts_init: UnixNanos) -> Vec<VibeWsMessage> {
        match frame {
            ...
            | LighterWsFrame::AccountAllOrders { ref orders, .. } => {
                ...
                msgs.push(VibeWsMessage::AccountStreamFirstFrame(
            ...
            LighterWsFrame::AccountAllTradesSnapshot { .. } => {
                ...
                vec![VibeWsMessage::AccountStreamFirstFrame(
    ...
    fn emit_cached_order_book_deltas_snapshot(&self, market_index: i16) -> Option<VibeWsMessage> {
        ...
        {
            Ok(deltas) => Some(VibeWsMessage::Deltas(deltas)),
            Err(e) => {
    ...
    fn emit_cached_order_book_depth10_snapshot(&self, market_index: i16) -> Option<VibeWsMessage> {
        let cached = self.book_states.get(&market_index)?.clone();
        let instrument = self.instruments.get(&market_index)?;
        let ts_init = self.clock.get_time_ns();
        match parse_ws_order_book_depth10(&cached.book, instrument, cached.timestamp, ts_init) {
            Ok(depth) => Some(VibeWsMessage::Depth10(Box::new(depth))),
            Err(e) => {
    ...
    fn order_book_messages(
        ...
        if self.book_delta_subs.contains(&market_index) {
            match parse_ws_order_book_deltas(book, instrument, timestamp, is_snapshot, ts_init) {
                Ok(deltas) => messages.push(VibeWsMessage::Deltas(deltas)),
        ...
        {
            match parse_ws_order_book_depth10(&cached.book, instrument, cached.timestamp, ts_init) {
                Ok(depth) => messages.push(VibeWsMessage::Depth10(Box::new(depth))),
    ...
    fn handle_ticker(
        ...
        match parse_ws_quote_tick(ticker, instrument, timestamp, ts_init) {
            Ok(Some(quote)) => vec![VibeWsMessage::Quote(quote)],
            Ok(None) => {
                log::debug!(
    ...
    fn handle_trades(
        ...
        if ticks.is_empty() {
            ...
            vec![VibeWsMessage::Trades(ticks)]
        }
    }
    ...
    fn handle_one_spot_market_stats(
        ...
        match parse_ws_spot_index_price_update(stats, instrument, timestamp, ts_init) {
            Ok(index_price) => Some(VibeWsMessage::IndexPrice(index_price)),
            Err(e) => {
            ...
            match previous {
                ...
                Some(prev) if candle.t > prev.t => {
                    ...
                    match parse_ws_bar(instrument, &prev, resolution, ts_init) {
                        Ok(bar) => emitted.push(VibeWsMessage::Bar(bar)),
    ...
    fn handle_account_orders(
        ...
        if reports.is_empty() {
            ...
            vec![VibeWsMessage::ExecutionReports(reports)]
        }
    }
    ...
    fn handle_account_positions(
        ...
        vec![VibeWsMessage::PositionSnapshot {
            reports,
            skipped_market_ids,
        }]
    }
    ...
    /// `VibeWsMessage`. Returns an empty vec when the second stream
    ...
    fn emit_unified_account_state(
        ...
        {
            Some(Ok(state)) => vec![VibeWsMessage::AccountState(Box::new(state))],
            Some(Err(e)) => {
...
fn raw_message(frame: &LighterWsFrame) -> Vec<VibeWsMessage> {
    let value = serde_json::to_value(frame).unwrap_or(serde_json::Value::Null);
    vec![VibeWsMessage::Raw(value)]
}
...
fn log_integrator_not_approved() {
    log::error!(
        "Lighter venue rejected with code {LIGHTER_ERROR_CODE_INTEGRATOR_NOT_APPROVED} \
         ...
         Tagged orders require Vibe integrator approval. \
...
mod tests {
    ...
    impl Log for OutboundLogCapture {
        fn enabled(&self, metadata: &Metadata<'_>) -> bool {
            metadata.level() == Level::Debug
                && metadata.target() == "vibe_lighter::websocket::handler"
        }
    ...
    fn handle_control_text(handler: &mut FeedHandler, text: &str) -> (bool, Option<VibeWsMessage>) {
    ...
    fn make_handler_with_account() -> FeedHandler {
        ...
        let (out_tx, _out_rx) = tokio::sync::mpsc::unbounded_channel::<VibeWsMessage>();
        let mut handler =
    ...
    fn strip_account_marker(mut msgs: Vec<VibeWsMessage>) -> Vec<VibeWsMessage> {
        if matches!(msgs.last(), Some(VibeWsMessage::AccountStreamFirstFrame(_)),) {
            msgs.pop();
        }
        msgs
    }
    ...
    fn handle_frame_routes_account_orders_to_execution_reports() {
        ...
        match &messages[0] {
            VibeWsMessage::ExecutionReports(reports) => {
    ...
    fn handle_frame_routes_account_positions_to_position_snapshot() {
        ...
        assert_eq!(messages.len(), 1);
        match &messages[0] {
            VibeWsMessage::PositionSnapshot {
                reports,
                skipped_market_ids,
            } => {
    ...
    fn handle_control_text_sendtx_success_emits_typed_ack() {
        ...
        match msg.expect("SendTxAck emitted") {
            VibeWsMessage::SendTxAck { tx_hash, code, .. } => {
                assert_eq!(code, 200);
    ...
    fn handle_control_text_sendtx_failure_carries_echoed_tx_hash() {
        ...
        match msg.expect("SendTxRejected emitted") {
            VibeWsMessage::SendTxRejected { tx_hash, .. } => {
                assert_eq!(tx_hash.as_deref(), Some("0000abcd"));
            }
            other => panic!("expected SendTxRejected, was {other:?}"),
        }
    ...
    fn handle_frame_routes_account_assets_and_user_stats_to_unified_state() {
        ...
        assert_eq!(messages.len(), 1);
        match &messages[0] {
            VibeWsMessage::AccountState(state) => {
    ...
    fn handle_frame_emits_account_stream_first_frame_marker_per_variant() {
        ...
        for (frame, expected) in cases {
            ...
            let marker = msgs
                .iter()
                .find(|m| matches!(m, VibeWsMessage::AccountStreamFirstFrame(_)))
                .unwrap_or_else(|| panic!("missing marker for {expected:?}"));
            match marker {
                VibeWsMessage::AccountStreamFirstFrame(stream) => {
            ...
            assert!(
                matches!(msgs.last(), Some(VibeWsMessage::AccountStreamFirstFrame(_)),),
    ...
    fn handle_frame_market_stats_all_emits_mark_index_and_funding_updates() {
        ...
        assert_eq!(messages.len(), 3);
        assert!(matches!(&messages[0], VibeWsMessage::MarkPrice(_)));
        assert!(matches!(&messages[1], VibeWsMessage::IndexPrice(_)));
        assert!(matches!(&messages[2], VibeWsMessage::FundingRate(_)));
    ...
    async fn reconnect_requeues_attempt_with_new_generation() {
        ...
        assert!(matches!(next, Some(VibeWsMessage::Reconnected { .. })));
