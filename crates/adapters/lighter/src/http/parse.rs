//! Parsers from Lighter REST payloads to Vibe domain types.
...
/// Parses Lighter order book metadata into Vibe instruments and registers their market ids.
///
/// # Errors
///
...
/// Parses Lighter order book metadata into Vibe instruments and market statuses.
///
/// # Errors
///
/// Returns an error if metadata is nonempty but no instrument definition can be converted.
...
/// Parses a Lighter trade into a Vibe [`TradeTick`].
///
/// # Errors
///
...
/// Parses a Lighter candle into a Vibe [`Bar`].
///
/// # Errors
///
...
/// Parses a Lighter historical funding row into a Vibe [`FundingRateUpdate`].
///
/// Lighter returns `rate` as a magnitude and `direction` as the side paying
/// the funding. Vibe uses the conventional signed rate: positive when
...
/// Parses an HTTP order book snapshot response into Vibe order book deltas.
///
/// # Errors
///
