...
use vibe_core::{AtomicTime, UnixNanos, consts::VIBE_USER_AGENT, time::get_atomic_clock_realtime};
use vibe_model::{
...
const MULTIPART_BOUNDARY: &str = "vibe-lighter-form-boundary";
...
/// to venue endpoints and return venue response models without converting to Vibe domain types.
#[derive(Clone, Debug)]
...
impl LighterRawHttpClient {
    ...
    fn default_headers() -> HashMap<String, String> {
        HashMap::from([(USER_AGENT.to_string(), VIBE_USER_AGENT.to_string())])
    }
}
...
/// Vibe domain data. Market metadata calls also populate the shared [`MarketRegistry`].
#[derive(Clone, Debug)]
...
impl LighterHttpClient {
    ...
    /// Requests historical candles and parses them into Vibe [`Bar`]s.
    ///
    /// # Errors
    ///
    ...
    /// Requests historical funding rates and parses them into Vibe updates.
    ///
    ...
    /// Requests an HTTP order book snapshot and parses it into Vibe order book deltas.
    ///
    /// # Errors
    ///
