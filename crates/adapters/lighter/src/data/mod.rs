...
use crate::{
    ...
    websocket::{
        ...
        messages::{LighterMarketSelection, LighterWsChannel, VibeWsMessage},
    },
};
            ...
            loop {
                tokio::select! {
                    ...
                    msg_opt = ws_client.next_event() => {
                        match msg_opt {
                            Some(VibeWsMessage::Trades(trades)) => {
                                for trade in trades {
                            ...
                            Some(VibeWsMessage::Quote(quote)) => {
                                if let Err(e) = data_sender
                            ...
                            Some(VibeWsMessage::Deltas(deltas)) => {
                            ...
                            Some(VibeWsMessage::Depth10(depth)) => {
                                if let Err(e) =
                            ...
                            Some(
                                ...
                                | VibeWsMessage::SendTxAck { .. }
                                | VibeWsMessage::SendTxRejected { .. }
                                | VibeWsMessage::AccountStreamFirstFrame(_),
                            ) => {}
                            Some(VibeWsMessage::Reconnected { .. }) => {
...
mod tests {
    ...
    use vibe_common::live::runner::replace_data_event_sender;
    use vibe_core::UUID4;
    use vibe_model::{
        data::{
    ...
    fn test_market_stats_ws_forwarding_requires_matching_subscription() {
        ...
        assert!(emit_market_stats_ws_message(
            &sender,
            &subscriptions,
            &VibeWsMessage::MarkPrice(MarkPriceUpdate::new(
                instrument_id,
                Price::from("2000.00"),
                UnixNanos::from(10),
                UnixNanos::from(1),
        ...
        assert!(emit_market_stats_ws_message(
            &sender,
            &subscriptions,
            &VibeWsMessage::IndexPrice(IndexPriceUpdate::new(
                instrument_id,
                Price::from("1999.50"),
                UnixNanos::from(11),
                UnixNanos::from(1),
        ...
        assert!(emit_market_stats_ws_message(
            &sender,
            &subscriptions,
            &VibeWsMessage::FundingRate(FundingRateUpdate::new(
                instrument_id,
                Decimal::new(12, 6),
                None,
