...
use crate::{
    common::{
        consts::{
            LIGHTER, LIGHTER_CLIENT_ID, LIGHTER_VENUE, LIGHTER_VIBE_INTEGRATOR_ACCOUNT_INDEX,
        },
...
async fn submit_integrator_revocation(environment: LighterEnvironment) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "integrator={LIGHTER_VIBE_INTEGRATOR_ACCOUNT_INDEX} account_index={} tx_hash={}",
        credential.account_index(),
        response.tx_hash,
    ))
}

/// Revoke the Vibe integrator approval when leaving the adapter.
///
