...
use crate::{
    common::{
        consts::{
            ...
            LIGHTER_VENUE, LIGHTER_VIBE_INTEGRATOR_ACCOUNT_INDEX,
        },
    ...
    websocket::{
        ...
        dispatch::{
            ...
            evict_terminal_mappings, lookup_order_status_report, order_expiry_for,
            parse_http_order_to_report, price_to_ticks, quantity_to_ticks, unwrap_reports_or_warn,
            vibe_to_lighter_order_type, vibe_to_lighter_tif,
        },
        messages::{
            AccountStream, ExecutionReport, LighterWsChannel, SendTxRejectionSource, VibeWsMessage,
        },
        parse::{
...
impl LighterExecutionClient {
    ...
    async fn submit_integrator_auto_approval(&self) -> anyhow::Result<()> {
        ...
        log::debug!(
            ...
            LIGHTER_VIBE_INTEGRATOR_ACCOUNT_INDEX,
            approval.nonce,
            approval.api_key_index,
    ...
    fn prepare_integrator_auto_approval(
        ...
        let tx = ApproveIntegratorTxInfo {
            context,
            integrator_account_index: LIGHTER_VIBE_INTEGRATOR_ACCOUNT_INDEX as i64,
            ...
            loop {
                tokio::select! {
                    ...
                    msg_opt = ws_client.next_event() => {
                        match msg_opt {
                            Some(VibeWsMessage::ExecutionReports(reports)) => {
                            ...
                            Some(VibeWsMessage::PositionSnapshot {
                                reports,
                                skipped_market_ids,
                            }) => {
                            ...
                            Some(VibeWsMessage::AccountState(state)) => {
                                log::debug!(
                            ...
                            Some(VibeWsMessage::Reconnected { connection_epoch }) => {
                            ...
                            Some(VibeWsMessage::SendTxRejected {
                                connection_epoch,
                                source,
                                code,
                                message,
                            ...
                            Some(VibeWsMessage::Raw(value)) => {
                                log::debug!("Unhandled Lighter raw frame on execution stream: {value}");
                            }
                            Some(VibeWsMessage::AccountStreamFirstFrame(stream)) => {
                            ...
                            Some(
                                VibeWsMessage::Trades(_)
                                | VibeWsMessage::Quote(_)
                                | VibeWsMessage::Deltas(_)
                                | VibeWsMessage::Depth10(_)
                                | VibeWsMessage::Bar(_)
                                | VibeWsMessage::MarkPrice(_)
                                | VibeWsMessage::IndexPrice(_)
                                | VibeWsMessage::FundingRate(_),
                            ) => {}
                            None => {
    ...
    fn prepare_create_order_plan(
        ...
        let order_kind = vibe_to_lighter_order_type(order.order_type())?;

        let tif = vibe_to_lighter_tif(
            order.order_type(),
            order.time_in_force(),
            order.is_post_only(),
        )?;
    ...
    ///
    /// Vibe does not expose a `set_leverage` command on the execution
...
fn integrator_attributes() -> L2TxAttributes {
    L2TxAttributes {
        integrator_account_index: LIGHTER_VIBE_INTEGRATOR_ACCOUNT_INDEX,
            ...
            for trade in &response.trades {
                ...
                match parse_ws_fill_report(
                    ...
                    Ok(Some(report)) => {
                        ...
                        // Vibe cloid, not the venue's numeric echo.
...
mod tests {
    ...
    #[rstest]
    fn integrator_attributes_tags_vibe_account_at_zero_fees() {
