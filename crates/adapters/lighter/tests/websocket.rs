...
//! [`VibeWsMessage`] stream and the recorded server-side state.
...
async fn test_order_book_update_before_snapshot_is_dropped() {
    ...
    let VibeWsMessage::Deltas(deltas) = event else {
...
async fn test_order_book_nonce_gap_drops_update_and_resubscribes() {
    ...
    let VibeWsMessage::Deltas(deltas) = snapshot_event else {
...
async fn test_order_book_depth10_nonce_gap_drops_update_and_resubscribes() {
    ...
    let VibeWsMessage::Depth10(depth) = snapshot_event else {
    ...
    let VibeWsMessage::Depth10(depth) = event else {
...
async fn test_order_book_second_frame_is_incremental_no_depth10() {
    ...
    assert!(matches!(snapshot_event, VibeWsMessage::Deltas(_)));
    ...
    let VibeWsMessage::Deltas(deltas) = second else {
    ...
    assert!(
        !matches!(next, Some(VibeWsMessage::Depth10(_))),
...
async fn test_order_book_depth10_emits_on_snapshot() {
    ...
    assert!(
        matches!(event, VibeWsMessage::Depth10(_)),
        "expected Depth10 on snapshot, was {event:?}",
    );

    let next = next_event_within(&mut harness.client, Duration::from_millis(200)).await;
    assert!(
        !matches!(next, Some(VibeWsMessage::Deltas(_))),
...
async fn test_order_book_depth10_emits_on_incremental_update() {
    ...
    assert!(matches!(event, VibeWsMessage::Depth10(_)));
...
async fn test_order_book_incremental_emits_deltas_and_depth10_when_both_subscribed() {
    ...
    assert!(matches!(event, VibeWsMessage::Deltas(_)));
...
async fn test_trade_frame_includes_liquidation_trades() {
    ...
    let event = next_event_within(&mut harness.client, Duration::from_secs(2))
        .await
        .expect("trades event");
    let VibeWsMessage::Trades(ticks) = event else {
...
async fn test_ticker_frame_resolves_via_channel_index() {
    ...
    let event = next_event_within(&mut harness.client, Duration::from_secs(2))
        .await
        .expect("quote event");
    let VibeWsMessage::Quote(quote) = event else {
...
async fn test_spot_market_stats_frame_emits_index_update() {
    ...
    let VibeWsMessage::IndexPrice(update) = event else {
...
async fn test_typed_snapshot_then_update_marks_only_first_with_f_snapshot() {
    ...
    let incremental_event = next_event_within(&mut harness.client, Duration::from_secs(2))
        .await
        .expect("incremental deltas");
    let VibeWsMessage::Deltas(deltas) = incremental_event else {
...
async fn test_unsubscribe_trade_does_not_clear_book_snapshot_state() {
    ...
    for _ in 0..6 {
        ...
        if let VibeWsMessage::Deltas(deltas) = event {
            assert!(
                !deltas
                    .deltas
                    .iter()
...
async fn test_height_subscription_routes_through_raw() {
    ...
    let VibeWsMessage::Raw(value) = event else {
...
async fn test_multi_market_book_state_isolation() {
    ...
    let VibeWsMessage::Deltas(deltas0) = market0_event else {
    ...
    let VibeWsMessage::Deltas(deltas1) = market1_event else {
    ...
    for _ in 0..6 {
        ...
        if let VibeWsMessage::Deltas(d) = event
            && d.instrument_id == id1
        {
            assert!(
                !d.deltas
...
async fn test_reconnect_replays_authenticated_and_public_subscriptions() {
    ...
    for _ in 0..20 {
        ...
        if let VibeWsMessage::Reconnected { connection_epoch } = event {
