...
The ordinary targets are binary targets of `vibe-lighter` behind its `fuzz` feature. The
workspace pins `libfuzzer-sys`, and `vibe-live` owns the shared libFuzzer integration. The
...
3. Import `vibe_live::fuzz::fuzz_target` and assert the deterministic invariants promised by
   the API.
