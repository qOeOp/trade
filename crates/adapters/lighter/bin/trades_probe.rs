...
//!
//! Run with `cargo run -p vibe-lighter --bin lighter-trades-probe --features examples`.
//!
//! Read-only: no orders are placed.

use std::error::Error as _;

use vibe_common::logging::{init_logging, logger::LoggerConfig};
use vibe_core::{UUID4, string::secret::mask_api_key};
use vibe_lighter::{
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _log_guard = init_logging(
        ...
        UUID4::new(),
        LoggerConfig {
            // INFO, not TRACE: vibe_network's HTTP client traces
