...
//! Usage:
//! ```bash
//! cargo run --bin lighter-flatten -p vibe-lighter
//! ```
//!
...
use vibe_lighter::{
    ...
    websocket::{LighterWebSocketClient, LighterWsChannel, VibeWsMessage},
};
use vibe_model::{
...
async fn collect_positions(
    ...
    while tokio::time::Instant::now() < deadline {
        ...
        if let Ok(Some(VibeWsMessage::PositionSnapshot {
            reports,
            skipped_market_ids,
        })) =
