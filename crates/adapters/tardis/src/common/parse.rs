...
use ustr::Ustr;
use vibe_core::{UnixNanos, datetime::NANOSECONDS_IN_MICROSECOND};
use vibe_model::{
...
/// Parses a Vibe instrument ID from the given Tardis `exchange` and `symbol` values.
#[must_use]
...
/// Parses a Vibe instrument ID with a normalized symbol from the given Tardis `exchange` and `symbol` values.
#[must_use]
...
/// Parses a Vibe price from the given `value`.
///
...
/// Parses a Vibe order side from the given Tardis string `value`.
#[must_use]
...
/// Parses a Vibe aggressor side from the given Tardis string `value`.
#[must_use]
...
/// Parses a Vibe option kind from the given Tardis enum `value`.
#[must_use]
...
/// Parses a Vibe book action inferred from the given Tardis values.
#[must_use]
...
/// Parses a Vibe bar specification from the given Tardis string `value`.
///
...
/// Converts a Vibe `BarSpecification` to the Tardis trade bar string convention.
///
/// # Errors
///
