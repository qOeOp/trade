...
use vibe_network::http::{HttpClient, USER_AGENT};
...
impl TardisHttpClient {
    ...
    /// Returns all Vibe instrument definitions for the given `exchange`, and filter params.
    ///
    /// # Errors
    ///
    ...
    /// for WS message parsing, and parses Vibe instrument definitions.
    ///
    /// Returns a tuple of `(instrument_map, vibe_instruments)`. The caller
    ...
    pub async fn bootstrap_instruments(
        ...
        let mut instrument_map: AHashMap<TardisInstrumentKey, Arc<TardisInstrumentMiniInfo>> =
            AHashMap::new();
        let mut vibe_instruments: Vec<InstrumentAny> = Vec::new();
        ...
        for exchange in exchanges {
            ...
            for inst in instruments_info {
                vibe_instruments.extend(parse_instrument_any(
                    &inst,
                    None,
                    None,
        ...
        Ok((instrument_map, vibe_instruments))
    }
}
