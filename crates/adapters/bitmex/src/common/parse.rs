//! Shared parsing helpers that transform BitMEX payloads into Vibe types.
...
use ustr::Ustr;
use vibe_core::{Params, nanos::UnixNanos, uuid::UUID4};
use vibe_model::{
    data::bar::BarType,
...
/// Parses a Vibe instrument ID from the given BitMEX `symbol` value.
#[must_use]
...
/// Vibe quantities back to venue units, so each instrument can have its own contract multiplier.
...
/// Converts a BitMEX contracts value into a Vibe quantity using instrument precision.
#[must_use]
...
/// size increment for Vibe instruments.
///
...
/// Converts an optional contract-count field (e.g. `lotSize`, `maxOrderQty`) into a Vibe
...
/// Converts a signed BitMEX contracts value into a Vibe quantity using instrument precision.
#[must_use]
...
/// Maps an optional BitMEX side to the corresponding Vibe aggressor side.
#[must_use]
...
/// Maps BitMEX liquidity indicators onto Vibe liquidity sides.
#[must_use]
...
/// Derives a Vibe position side from the BitMEX `currentQty` value.
#[must_use]
...
/// Maps BitMEX currency codes to standard Vibe currency codes.
///
...
/// Returns the Vibe account ID for a BitMEX account number.
#[must_use]
pub fn bitmex_account_id(account: i64) -> AccountId {
    AccountId::new(format!("BITMEX-{account}"))
}

/// Parses a BitMEX margin message into a Vibe account balance.
...
/// Parses a BitMEX margin message into a Vibe account state.
///
/// # Errors
///
...
mod tests {
    ...
    use rstest::rstest;
    use ustr::Ustr;
    use vibe_model::{instruments::CurrencyPair, types::fixed::FIXED_PRECISION};
