//! Conversion routines that map BitMEX REST models into Vibe domain structures.
...
use ustr::Ustr;
use vibe_core::{UnixNanos, uuid::UUID4};
use vibe_model::{
...
pub enum InstrumentParseResult {
    /// Successfully parsed into a Vibe instrument.
...
/// Attempts to convert a BitMEX instrument record into a Vibe instrument by type.
#[must_use]
...
/// Parse a BitMEX index instrument into a Vibe `InstrumentAny`.
///
...
/// Parse a BitMEX spot instrument into a Vibe `InstrumentAny`.
///
/// # Errors
///
/// Returns an error if values are out of valid range or cannot be parsed.
...
/// Parse a BitMEX perpetual instrument into a Vibe `InstrumentAny`.
///
/// # Errors
///
/// Returns an error if values are out of valid range or cannot be parsed.
...
/// Parse a BitMEX futures instrument into a Vibe `InstrumentAny`.
///
/// # Errors
///
/// Returns an error if values are out of valid range or cannot be parsed.
...
/// Parse a BitMEX futures spread instrument into a Vibe `InstrumentAny`.
///
/// # Errors
///
/// Returns an error if values are out of valid range or cannot be parsed.
...
/// Parse a BitMEX trade into a Vibe `TradeTick`.
///
/// # Errors
///
...
/// Converts a BitMEX trade-bin record into a Vibe [`Bar`].
///
/// # Errors
///
...
/// Parse a BitMEX order into a Vibe `OrderStatusReport`.
///
...
/// Parse a BitMEX execution into a Vibe `FillReport`.
///
/// # Errors
///
/// Currently this function does not return errors as all fields are handled gracefully,
/// but returns `Result` for future error handling compatibility.
///
/// Parse a BitMEX execution into a Vibe `FillReport` using instrument scaling.
///
/// # Errors
///
...
/// Parse a BitMEX position into a Vibe `PositionStatusReport`.
///
/// # Errors
///
/// Currently this function does not return errors as all fields are handled gracefully,
