//! Parsing utilities for converting dYdX v4 Indexer API responses into Vibe domain models.
//!
//! This module contains functions that transform raw JSON data structures
//! from the dYdX Indexer API into strongly-typed Vibe data types such as
...
/// Parses a dYdX [`Trade`] into a Vibe [`TradeTick`].
///
/// # Errors
///
...
/// Parses a dYdX [`Candle`] into a Vibe [`Bar`].
///
...
/// Parses a dYdX perpetual market into a Vibe [`InstrumentAny`].
///
...
pub fn parse_instrument_any(
    ...
    // Parse instrument ID with Vibe perpetual suffix and keep raw symbol as venue ticker
...
/// Map dYdX order status to Vibe OrderStatus.
...
/// Parse a dYdX Order into a Vibe OrderStatusReport.
///
/// # Errors
///
...
pub fn parse_order_status_report(
    ...
    // Track the dYdX-side type alongside the Vibe type so the TIF resolver
    ...
    // `DydxOrderType::TakeProfitLimit` (the deserialized form) maps to Vibe
...
/// Parse a dYdX Fill into a Vibe FillReport.
///
/// # Errors
///
/// Returns an error if required fields are missing or invalid.
pub fn parse_fill_report(
    ...
    // dYdX sign convention matches Vibe (positive = cost)
...
/// Parse a dYdX PerpetualPosition into a Vibe PositionStatusReport.
///
/// # Errors
///
/// Returns an error if required fields are missing or invalid.
...
/// Parse a dYdX subaccount info into a Vibe AccountState.
///
...
/// Parse a dYdX HTTP [`Subaccount`] response into a Vibe [`AccountState`].
///
