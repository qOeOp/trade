...
mod tests {
    ...
    fn test_sequence_mismatch_from_code_pattern() {
        ...
        let err = DydxError::Vibe(anyhow::anyhow!(
    ...
    fn test_non_sequence_error_not_matched() {
        let err = DydxError::Vibe(anyhow::anyhow!("insufficient funds"));
        assert!(!err.is_sequence_mismatch());
    }
    ...
    fn test_is_transient_sequence_mismatch() {
        let err = DydxError::Vibe(anyhow::anyhow!("account sequence mismatch"));
    ...
    #[rstest]
    fn test_benign_cancel_non_vibe_variant() {
        let err = DydxError::Order("order rejected".to_string());
        assert!(!err.is_benign_cancel_error());
    }
    ...
    fn test_definitive_broadcast_rejection_excludes_transport_errors() {
        ...
        let err = DydxError::Vibe(anyhow::anyhow!("connection reset by peer"));
        assert!(!err.is_definitive_broadcast_rejection());
    }
