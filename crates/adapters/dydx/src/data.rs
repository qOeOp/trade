...
use vibe_model::{
    data::{
        Bar, BarSpecification, BarType, BookOrder, Data as VibeData, FundingRateUpdate,
...
///
/// This client integrates with the Vibe DataEngine to provide:
...
impl DydxDataClient {
        ...
        if let Some(ref oracle_prices) = contents.oracle_prices {
            for (ticker, oracle_data) in oracle_prices {
                ...
                if ctx.active_mark_price_subs.contains(&instrument_id) {
                    ...
                    let data = VibeData::MarkPriceUpdate(mark_price);
                ...
                if ctx.active_index_price_subs.contains(&instrument_id) {
                    ...
                    let data = VibeData::IndexPriceUpdate(index_price);
                    if let Err(e) = ctx.data_sender.send(DataEvent::Data(data)) {
    ...
    fn emit_bar_guarded(bar: Bar, ctx: &WsMessageContext) {
        ...
        if bar.ts_event <= current_time_ns {
            ...
            if let Err(e) = ctx.data_sender.send(DataEvent::Data(VibeData::Bar(bar))) {
    ...
    fn handle_bar_message(
        ...
        if bar.ts_event <= current_time_ns {
            ...
            if let Err(e) = data_sender.send(DataEvent::Data(VibeData::Bar(bar))) {
                log::error!("Failed to emit completed bar: {e}");
            }
        } else {
    ...
    fn handle_deltas_message(
        ...
        let instrument = match instrument_cache.get(&instrument_id) {
            ...
            None => {
                ...
                if active_delta_subs.contains(&instrument_id)
                    && let Err(e) = data_sender.send(DataEvent::Data(VibeData::from(
        ...
        if active_quote_subs.contains(&instrument_id) {
            ...
            if let Some(quote) = quote_opt {
                ...
                if emit_quote {
                    ...
                    if let Err(e) = data_sender.send(DataEvent::Data(VibeData::Quote(quote))) {
        ...
        if active_delta_subs.contains(&instrument_id) {
            let data: VibeData = OrderBookDeltas_API::new(resolved_deltas).into();
