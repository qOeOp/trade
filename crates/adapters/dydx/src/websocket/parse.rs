...
//!
//! Converts WebSocket-specific message formats into Vibe domain types
...
pub fn parse_ws_order_report(
    ...
    // Look up the original Vibe client_order_id from the order context first,
    ...
    if let Some(client_id) = dydx_client_id {
        if let Some(ctx) = order_contexts.get(&client_id) {
            log::debug!(
                "[WS_ORDER_RECV] DECODE via order_contexts: dYdX u32={} -> Vibe '{}'",
                client_id,
                ctx.client_order_id
            );
        ...
        {
            log::debug!(
                "[WS_ORDER_RECV] DECODE via encoder fallback: dYdX u32={client_id} meta={dydx_client_metadata:#x} -> Vibe '{client_order_id}'"
            );
    ...
    // surface `PendingUpdate` to match Vibe semantics and existing dYdX
    // enum mapping.
...
mod tests {
    ...
    fn test_ws_position_report_emits_venue_side_for_mismatched_size() {
        use vibe_model::enums::PositionSideSpecified;
