//! Parsing utilities that convert dYdX payloads into Vibe domain models.

use std::str::FromStr;

use rust_decimal::Decimal;
use vibe_core::{UnixNanos, datetime::NANOSECONDS_IN_SECOND};
use vibe_model::{
...
/// Extracts the raw dYdX ticker from a Vibe symbol.
///
...
/// Converts Vibe `OrderSide` to dYdX proto `OrderSide`.
#[must_use]
...
/// Converts Vibe `TimeInForce` to dYdX proto `TimeInForce`.
///
...
/// Converts Vibe `TimeInForce` to dYdX proto `TimeInForce` with post_only flag support.
///
...
/// "BASE-QUOTE" (e.g., "BTC-USD"). Vibe standardizes perpetual
/// instrument symbols by appending the product suffix "-PERP".
///
/// This function converts a dYdX ticker into a Vibe `InstrumentId`
