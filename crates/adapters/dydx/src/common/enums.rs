//! Enumerations mapping dYdX v4 concepts onto idiomatic Vibe variants.
...
impl DydxOrderSide {
    /// Tries to convert from Vibe `OrderSide`.
    ///
    /// # Errors
    ///
...
impl DydxOrderType {
    /// Tries to convert from Vibe `OrderType`.
    ///
    /// # Errors
    ///
...
impl DydxCandleResolution {
    /// Maps a Vibe [`BarSpecification`] to a dYdX candle resolution.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    #[rstest]
    fn test_order_side_conversion_from_vibe() {
        assert_eq!(
    ...
    #[rstest]
    fn test_order_type_conversion_to_vibe() {
        assert_eq!(OrderType::from(DydxOrderType::Market), OrderType::Market);
        assert_eq!(OrderType::from(DydxOrderType::Limit), OrderType::Limit);
        assert_eq!(
            OrderType::from(DydxOrderType::StopMarket),
