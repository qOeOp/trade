...
//! # Retry Logic
//!
//! Uses the battle-tested [`RetryManager`] from `vibe-network` with exponential
...
impl TxBroadcaster {
    ...
    pub async fn broadcast_with_retry(
        ...
        let _permit = self
            ...
            .acquire()
            .await
            .map_err(|e| DydxError::Vibe(anyhow::anyhow!("Broadcast semaphore closed: {e}")))?;
        ...
        let create_error = |msg: String| -> DydxError { DydxError::Vibe(anyhow::anyhow!(msg)) };
