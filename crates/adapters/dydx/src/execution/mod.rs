...
/// consistent behavior across the Vibe ecosystem.
#[derive(Debug)]
...
impl ExecutionClient for DydxExecutionClient {
    ...
    ///
    /// dYdX requires u32 client IDs - Vibe ClientOrderId strings are hashed to fit.
    ///
    ...
    fn submit_order(&self, cmd: SubmitOrder) -> anyhow::Result<()> {
        ...
        log::debug!(
            "[SUBMIT_ORDER] Vibe '{}' -> dYdX u32={} meta={:#x} | instrument={} side={:?} qty={} type={:?}",
            client_order_id,
            client_id_u32,
            client_metadata,
            instrument_id,
        ...
        self.spawn_order_task(
            ...
            async move {
                ...
                let (msg, order_type_str) = match order.order_type() {
                    ...
                    // dYdX TakeProfitMarket maps to Vibe MarketIfTouched
                    OrderType::MarketIfTouched => {
                        let trigger_price = order.trigger_price().ok_or_else(|| {
                    ...
                    // dYdX TakeProfitLimit maps to Vibe LimitIfTouched
                    OrderType::LimitIfTouched => {
                        let trigger_price = order.trigger_price().ok_or_else(|| {
    ...
    fn cancel_order(&self, cmd: CancelOrder) -> anyhow::Result<()> {
        ...
        log::debug!(
            "[CANCEL_ORDER] Vibe '{client_order_id}' -> dYdX u32={client_id_u32} | instrument={instrument_id}"
        );
...
mod tests {
    ...
    use vibe_model::{
        enums::OrderSide as VibeOrderSide,
    ...
    fn test_submit_order_list_denies_unsupported_tif(
        ...
        use vibe_common::messages::execution::SubmitOrderList;
        use vibe_model::{identifiers::OrderListId, orders::OrderList};
