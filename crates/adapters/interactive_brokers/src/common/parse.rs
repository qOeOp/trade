//! Parsing utilities for converting Interactive Brokers data to Vibe types.

use std::{collections::HashMap, str::FromStr, sync::LazyLock};

use ibapi::contracts::{Contract, Currency, Exchange, OptionRight, SecurityType, Symbol};
use vibe_core::UnixNanos;
use vibe_model::identifiers::{InstrumentId, Symbol as VibeSymbol, TradeId, Venue};
...
pub fn ib_contract_to_instrument_id_simplified(
    ...
    let symbol = match contract.security_type {
        SecurityType::Stock => {
            ...
            VibeSymbol::from(symbol_str.as_str())
        }
        SecurityType::Index => {
            // IND: Prefix with ^
            ...
            VibeSymbol::from(format!("^{base}").as_str())
        }
        SecurityType::Option => {
        ...
        SecurityType::Future => {
            ...
            if contract.local_symbol.is_empty() {
                    ...
                    && !contract.last_trade_date_or_contract_month.is_empty()
                {
                    ...
                    VibeSymbol::from(contract.symbol.as_str())
                }
            } else {
                VibeSymbol::from(contract.local_symbol.as_str())
            }
        }
        ...
        SecurityType::CFD => {
            ...
            if !contract.local_symbol.is_empty() && contract.local_symbol.contains('.') {
                ...
                VibeSymbol::from(cash_like.as_str())
            } else {
...
pub fn ib_contract_to_instrument_id_raw(
    ...
    let symbol = VibeSymbol::from(symbol_str.as_str());
    Ok(InstrumentId::new(symbol, venue))
}
...
pub fn create_spread_instrument_id(
    ...
    let symbol = VibeSymbol::from(composite_symbol.as_str());
    ...
    for component in components {
            ...
            && let Some(pos) = rest.find("))")
        {
            ...
            if let Ok(ratio) = ratio_str.parse::<i32>() {
                let leg_instrument_id = InstrumentId::new(VibeSymbol::from(symbol_value), venue);
