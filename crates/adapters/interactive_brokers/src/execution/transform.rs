//! Order transformation utilities for converting Vibe orders to IB orders.
...
use vibe_model::{
    enums::{OrderSide, OrderType as VibeOrderType, TimeInForce as VibeTimeInForce, TriggerType},
...
/// Transform a Vibe order to an IB order.
///
/// # Errors
///
/// Returns an error if the transformation fails.
pub fn vibe_order_to_ib_order(
    order: &OrderAny,
    _contract: &Contract,
    ...
    // Note: Parent ID in Vibe is ClientOrderId, but IB expects order_id.
...
/// Transform Vibe order type to IB order type string and prices.
fn transform_order_type(
    order_type: VibeOrderType,
    time_in_force: VibeTimeInForce,
    price: Option<Price>,
    trigger_price: Option<Price>,
    price_magnifier: f64,
) -> (&'static str, Option<f64>, Option<f64>) {
    let ib_order_type = IbOrderType::from_vibe(order_type, time_in_force);
    let (limit_price, aux_price) = match order_type {
        VibeOrderType::Market | VibeOrderType::MarketToLimit => (None, None),
        VibeOrderType::Limit => (convert_price_opt(price, price_magnifier), None),
        VibeOrderType::StopMarket | VibeOrderType::MarketIfTouched => {
            (None, convert_price_opt(trigger_price, price_magnifier))
        }
        VibeOrderType::StopLimit | VibeOrderType::LimitIfTouched => (
            convert_price_opt(price, price_magnifier),
            convert_price_opt(trigger_price, price_magnifier),
        ),
        VibeOrderType::TrailingStopMarket => (None, None),
        VibeOrderType::TrailingStopLimit => (convert_price_opt(price, price_magnifier), None),
    };

    (ib_order_type.as_str(), limit_price, aux_price)
}

/// Transform Vibe time in force to IB time in force.
fn transform_time_in_force(
    tif: VibeTimeInForce,
    _expire_time: Option<vibe_core::UnixNanos>,
) -> TimeInForce {
    IbTimeInForce::from_vibe(tif).ibapi_time_in_force()
}
...
mod tests {
    ...
    use ustr::Ustr;
    use vibe_model::{
        enums::{OrderSide, OrderType, TimeInForce as VibeTimeInForce, TrailingOffsetType},
        identifiers::{InstrumentId, OrderListId, Symbol as VibeSymbol, Venue},
    ...
    fn create_test_order_with_tags(tags_json: &str) -> OrderAny {
        let instrument_id = InstrumentId::new(VibeSymbol::from("AAPL"), Venue::from("NASDAQ"));
    ...
    fn test_active_start_time_encoding() {
        ...
        let result = vibe_order_to_ib_order(&order, &contract, &provider, 1, "TEST-001");
        assert!(result.is_ok());
    ...
    fn test_at_the_open_maps_to_ib_opg() {
        let order = OrderTestBuilder::new(OrderType::Market)
            .instrument_id(InstrumentId::new(
                VibeSymbol::from("AAPL"),
                Venue::from("NASDAQ"),
            ))
            .side(OrderSide::Buy)
            .quantity(Quantity::from(100))
            .time_in_force(VibeTimeInForce::AtTheOpen)
            .build();
        let contract = Contract {
            contract_id: 0,
            symbol: Symbol::from("AAPL"),
        ...
        let ib_order = vibe_order_to_ib_order(&order, &contract, &provider, 1, "TEST-001")
    ...
    fn test_gtd_orders_encode_ib_timestamp_string() {
        ...
        let order = OrderTestBuilder::new(OrderType::Limit)
            ...
            .quantity(Quantity::from(100))
            .price(Price::from("150.00"))
            .time_in_force(VibeTimeInForce::Gtd)
    ...
    fn test_trailing_stop_market_rejects_unsupported_trailing_offset_type() {
        ...
        let err = vibe_order_to_ib_order(&order, &contract, &provider, 1, "TEST-001")
