...
pub async fn subscribe_pnl(client: &Arc<Client>, account_id: AccountId) -> anyhow::Result<()> {
    ...
    vibe_common::live::get_runtime().spawn(async move {
        while let Some(result) = subscription.next().await {
            match result {
                Ok(pnl) => {
                    tracing::debug!(
...
/// Parse IB account summary to Vibe AccountBalance.
...
mod tests {
    ...
    use vibe_model::types::{AccountBalance, Currency, MarginBalance, Money};
    ...
    /// Verifies the IB avg cost to Vibe price conversion formula used in position parsing.
