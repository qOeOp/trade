...
mod tests {
    ...
    fn create_test_option_spread(instrument_id: InstrumentId) -> OptionSpread {
        OptionSpread::new(
            instrument_id,
            VibeSymbol::from(instrument_id.symbol.as_str()),
            AssetClass::Equity,
            Some(Ustr::from("XNAS")),
            Ustr::from("SPY"),
            Ustr::from("SPY"),
    ...
    fn test_is_spread_instrument_id() {
        ...
        let spread_id =
            InstrumentId::new(VibeSymbol::from("(1)AAPL_((2))MSFT"), Venue::from("NASDAQ"));
        assert!(is_spread_instrument_id(&spread_id));

        // Test non-spread instrument ID
        let regular_id = InstrumentId::new(VibeSymbol::from("AAPL"), Venue::from("NASDAQ"));
    ...
    fn test_parse_spread_instrument_id_to_legs() {
        ...
        let leg_tuples = [
            (
                InstrumentId::new(VibeSymbol::from("MSFT"), Venue::from("NASDAQ")),
                -2,
            ),
            (
                InstrumentId::new(VibeSymbol::from("AAPL"), Venue::from("NASDAQ")),
                1,
            ),
        ];
