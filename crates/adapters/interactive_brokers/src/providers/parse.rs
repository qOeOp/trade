//! Instrument parsing utilities for converting IB ContractDetails to Vibe instruments.
...
/// Parse an IB ContractDetails to a Vibe instrument.
///
/// # Errors
///
...
fn ib_contract_info(details: &ibapi::contracts::ContractDetails) -> vibe_core::Params {
    let mut info = vibe_core::Params::new();
...
fn ib_contract_info_for_contract(contract: &ibapi::contracts::Contract) -> vibe_core::Params {
    let mut info = vibe_core::Params::new();
...
mod tests {
    ...
    use vibe_model::{
        enums::AssetClass,
        identifiers::{InstrumentId, Symbol as VibeSymbol, Venue},
    ...
    fn test_parse_option_contract_prefixes_index_underlying() {
        ...
        let instrument_id = InstrumentId::new(
            VibeSymbol::from("SPXW  260313P06630000"),
            Venue::from("SMART"),
        );
    ...
    fn test_parse_contract_preserves_price_magnifier_in_info() {
        ...
        let instrument_id = InstrumentId::new(VibeSymbol::from("AAPL"), Venue::from("XNAS"));
    ...
    fn test_parse_continuous_future_contract_uses_symbol_as_raw_symbol() {
        ...
        let instrument_id = InstrumentId::new(VibeSymbol::from("ES"), Venue::from("CME"));
...
fn parse_bond_contract(
    ...
    // Note: This is a limitation of the current Vibe Rust model, not the IB adapter
    let price_precision = tick_size_to_precision(details.min_tick);
    let timestamp = get_atomic_clock_realtime().get_time_ns();
