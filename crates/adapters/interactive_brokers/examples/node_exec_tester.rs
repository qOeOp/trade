...
//!
//! Run live smoke with:
//! `cargo run --example ib-exec-tester --package vibe-interactive-brokers --features examples`
//!
//! Run embedded config unit tests with:
//! `cargo test --example ib-exec-tester --package vibe-interactive-brokers --features examples`
//!
//! Edit the constants below to change the TWS/Gateway connection, target
//! instrument, order size, and exec spec profile.
//!
//! Required environment variable:
//! - `VIBE_IB_ACCOUNT_ID` is your IB account, for example `U1234567`.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let account_id_raw = env::var("VIBE_IB_ACCOUNT_ID")?;
...
fn parse_market_data_type(value: &str) -> MarketDataType {
    match value {
        ...
        value => panic!("invalid VIBE_IB_MARKET_DATA_TYPE={value}"),
    }
}
