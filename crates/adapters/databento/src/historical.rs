...
use vibe_core::{AtomicMap, UnixNanos, consts::VIBE_USER_AGENT, time::AtomicTime};
use vibe_model::{
...
use crate::{
    ...
    symbology::{
        MetadataCache, check_consistent_symbology, decode_vibe_instrument_id, infer_symbology_type,
    },
...
impl DatabentoHistoricalClient {
    ...
    pub async fn get_range_instruments(
        ...
        while let Some(msg) = decoder.decode_record::<dbn::InstrumentDefMsg>().await? {
            ...
            let mut instrument_id = decode_vibe_instrument_id(
                &record,
                &mut metadata_cache,
    ...
    pub async fn get_range_quotes(
        ...
        let mut process_record = |record: dbn::RecordRef| -> anyhow::Result<()> {
            let sym_map = self.symbol_venue_map.load();
            let instrument_id = decode_vibe_instrument_id(
                &record,
                &mut metadata_cache,
                &self.publisher_venue_map,
                &sym_map,
