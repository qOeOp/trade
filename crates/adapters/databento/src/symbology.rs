...
use vibe_model::identifiers::{InstrumentId, Symbol, Venue};
...
/// Decodes a Databento record into a Vibe `InstrumentId`.
///
/// # Errors
///
/// Returns an error if:
...
pub fn decode_vibe_instrument_id(
    ...
    let mut instrument_id = get_vibe_instrument_id_for_record(record, metadata, *venue)?;
...
/// Gets the Vibe `InstrumentId` for a Databento record.
///
/// # Errors
///
/// Returns an error if:
...
pub fn get_vibe_instrument_id_for_record(
    record: &dbn::RecordRef,
    metadata: &mut MetadataCache,
    venue: Venue,
) -> anyhow::Result<InstrumentId> {
