...
CARGO_BUILD_JOBS=16 setarch "$(uname -m)" -R cargo bench -p vibe-databento --profile bench-lto \
...
and DBN decode. Stops before Vibe instrument lookup, precision resolution,
...
Fixture file -> Vibe domain value through the public
`DatabentoDataLoader` API. Covers file open, zstd + DBN decode, instrument
resolution when needed, price precision resolution, Vibe type construction,
...
- MBP10 direct decode is the largest pure Vibe conversion row because it
