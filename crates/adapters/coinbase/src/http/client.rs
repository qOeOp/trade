...
//! - [`CoinbaseHttpClient`]: domain wrapper with instrument caching and Vibe type conversions.
...
impl CoinbaseRawHttpClient {
    ...
    /// this with [`parse_account_state`] to build a Vibe [`AccountState`].
...
/// and Vibe type conversions. This is the primary HTTP interface for the
...
impl CoinbaseHttpClient {
    ...
    ///
    /// Parses each supported product into a Vibe [`InstrumentAny`] and caches
    ...
    /// Submits a new order built from Vibe domain types.
    ///
    ...
    /// Fetches all CFM futures positions and returns Vibe position reports.
    ///
    /// # Errors
    ///
...
/// Maps a Vibe [`OrderSide`] to Coinbase's wire enum.
///
/// # Errors
///
...
/// Builds the Coinbase [`OrderConfiguration`] payload from Vibe order
/// parameters.
///
...
pub fn build_order_configuration(
    ...
    match order_type {
        OrderType::Market => {
            ...
            // MARKET orders. Vibe' default GTC is mapped to IOC (mirroring
...
mod tests {
    ...
    fn test_build_order_configuration_market_accepts_default_gtc() {
        // Vibe orders default to GTC; coerce to MARKET IOC silently for
