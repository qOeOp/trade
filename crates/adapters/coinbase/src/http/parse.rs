//! Parsing functions for converting Coinbase API responses to Vibe domain types.
...
/// Converts a Coinbase order side to a Vibe aggressor side.
...
/// zero, or values that exceed Vibe's `QUANTITY_RAW_MAX`.
...
/// Parses a Coinbase product into the appropriate Vibe instrument type.
...
/// Converts a Coinbase order side to the Vibe [`OrderSide`].
...
/// Converts a Coinbase order status to the Vibe [`OrderStatus`].
///
...
/// reconciler. `Open` also maps to `Accepted` because Vibe differentiates
...
/// Converts a Coinbase time-in-force to the Vibe [`TimeInForce`].
...
/// Converts a Coinbase liquidity indicator to the Vibe [`LiquiditySide`].
...
/// Converts a Coinbase order type to the Vibe [`OrderType`].
///
/// Coinbase uses `BRACKET` on history endpoints for multi-leg orders. Vibe
...
pub fn parse_order_status_report(
    ...
    // order is still open, matching Vibe' lifecycle.
...
/// to follow the Vibe convention where commissions are positive when
/// paid by the taker.
///
/// # Errors
///
...
/// Parses a list of Coinbase [`Account`] entries into a Vibe [`AccountState`].
///
...
pub fn parse_account_state(
    ...
    // currency before emitting balances: Vibe stores balances keyed by
...
/// Parses a single CFM position into a Vibe [`PositionStatusReport`].
///
...
mod tests {
    ...
    fn test_parse_ws_cfm_account_state_picks_whole_window_not_per_field_max() {
        use vibe_model::enums::AccountType;
    ...
    fn test_parse_cfm_position_drops_avg_px_when_entry_zero() {
        ...
        // before a fill lands; Vibe represents "no open price" as None.
        let report = parse_cfm_position_status_report(
