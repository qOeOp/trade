//! Feed handler for parsing Coinbase WebSocket messages into Vibe types.
...
use vibe_network::{RECONNECTED, websocket::WebSocketClient};
...
/// Vibe-typed messages produced by the feed handler.
#[derive(Debug, Clone)]
pub enum VibeWsMessage {
...
/// Processes raw WebSocket messages into Vibe domain types.
#[derive(Debug)]
...
impl FeedHandler {
    ...
    fn handle_text(&mut self, text: &str) -> Option<VibeWsMessage> {
        if text == RECONNECTED {
    ...
    fn handle_l2_events(
        ...
        let mut first: Option<VibeWsMessage> = None;
        ...
        for event in events {
            ...
            match result {
                Ok(deltas) => {
                    let msg = VibeWsMessage::Deltas(deltas);
            ...
            for trade in &event.trades {
                ...
                match parse_ws_trade(trade, instrument, ts_init) {
                    Ok(tick) => {
                        ...
                        self.buffer.reverse();
                        return Some(VibeWsMessage::Trade(tick));
                    }
    ...
    fn buffer_remaining_trades(
        ...
        for event in events {
            ...
            for trade in &event.trades {
                ...
                {
                    self.buffer.push(VibeWsMessage::Trade(tick));
                }
            }
        }
    }
        ...
        for event in events {
            for ticker in &event.tickers {
                ...
                match parse_ws_ticker(ticker, instrument, ts_event, ts_init) {
                    Ok(quote) => {
                        let msg = VibeWsMessage::Quote(quote);
    ...
    fn emit_user_event_messages(
        ...
        ts_event: UnixNanos,
        ts_init: UnixNanos,
        first: &mut Option<VibeWsMessage>,
    ) {
        ...
        let msg = VibeWsMessage::UserOrder(Box::new(UserOrderUpdate {
    ...
    fn handle_status_events(
        ...
        for event in events {
            for product in &event.products {
                ...
                let msg = VibeWsMessage::InstrumentStatus(Box::new(status));
    ...
    fn handle_futures_balance_summary(
        ...
        for event in events {
            let msg = VibeWsMessage::FuturesBalanceSummary(Box::new(event.fcm_balance_summary));
...
mod tests {
    ...
    fn test_handle_user_event_emits_user_order_update() {
        ...
        match msg {
            VibeWsMessage::UserOrder(carrier) => {
    ...
    fn test_handle_text_emits_instrument_status_from_status_channel() {
        use vibe_model::enums::MarketStatusAction;
        ...
        let VibeWsMessage::InstrumentStatus(status) = first else {
        ...
        let VibeWsMessage::InstrumentStatus(status) = handler.buffer.pop().unwrap() else {
    ...
    fn test_handle_text_emits_futures_balance_summary_snapshot() {
        ...
        match msg {
            VibeWsMessage::FuturesBalanceSummary(summary) => {
    ...
    fn test_handle_text_routes_reconnected_sentinel() {
        ...
        assert!(matches!(result, Some(VibeWsMessage::Reconnected)));
    }
