...
//! parsed Vibe messages through the [`FeedHandler`].
...
use crate::{
    ...
    websocket::{
        handler::{FeedHandler, HandlerCommand, VibeWsMessage},
...
/// Spawns a [`FeedHandler`] task that parses raw messages into Vibe types.
#[derive(Debug)]
#[cfg_attr(
    feature = "python",
...
pub struct CoinbaseWebSocketClient {
    ...
    out_rx: Option<tokio::sync::mpsc::UnboundedReceiver<VibeWsMessage>>,
...
impl CoinbaseWebSocketClient {
    ...
    pub async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        let (out_tx, out_rx) = tokio::sync::mpsc::unbounded_channel::<VibeWsMessage>();
            ...
            loop {
                match handler.next().await {
                    Some(VibeWsMessage::Reconnected) => {
                        ...
                        if let Err(e) = out_tx.send(VibeWsMessage::Reconnected) {
    ...
    pub async fn next_message(&mut self) -> Option<VibeWsMessage> {
    ...
    pub fn take_out_rx(&mut self) -> Option<tokio::sync::mpsc::UnboundedReceiver<VibeWsMessage>> {
        self.out_rx.take()
    }
...
fn resubscribe_all(
    ...
    out_tx: Option<&tokio::sync::mpsc::UnboundedSender<VibeWsMessage>>,
) {
                ...
                Err(e) => {
                    if channel_enum.requires_auth() {
                        ...
                        if let Some(tx) = out_tx {
                            let _ = tx.send(VibeWsMessage::Error(msg));
                        }
                        continue;
                    }
                    None
