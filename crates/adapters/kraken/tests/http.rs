...
use vibe_testkit::events::drain_data_events;
...
async fn test_spot_request_account_state_margin_does_not_lock_free_margin() {
    ...
    // with no separate maintenance figure. Vibe `MarginAccount::recalculate_balance`
...
async fn test_spot_raw_get_open_positions() {
    use vibe_kraken::http::SpotOpenPositionsParams;
...
async fn test_spot_margin_position_bails_on_unknown_pair_when_cache_missing() {
    use vibe_model::{enums::AccountType, identifiers::AccountId};
