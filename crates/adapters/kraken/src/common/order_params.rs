//! Pure builder functions that convert Vibe execution commands into Kraken WS param structs.

use jiff::tz::Offset;
use vibe_common::messages::execution::{CancelOrder, ModifyOrder, SubmitOrder};
...
/// Builds WebSocket `add_order` parameters from a Vibe submit command and the cached order.
///
/// # Errors
///
/// Returns an error if:
...
/// Builds WebSocket `amend_order` parameters from a Vibe modify command.
///
...
/// Builds WebSocket `cancel_order` parameters from a Vibe cancel command.
///
/// Prefers `venue_order_id` (as `order_id`) over `client_order_id` (as `cl_ord_id`),
...
mod tests {
    ...
    fn test_build_amend_order_params_with_venue_id() {
        use vibe_model::types::{Price, Quantity};
