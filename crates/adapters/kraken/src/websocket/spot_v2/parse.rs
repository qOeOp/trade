//! WebSocket message parsers for converting Kraken streaming data to Vibe domain models.

use anyhow::Context;
use jiff::Timestamp;
use rust_decimal::Decimal;
use vibe_core::{UUID4, nanos::UnixNanos};
use vibe_model::{
...
/// Parses Kraken WebSocket ticker data into a Vibe quote tick.
///
/// # Errors
///
/// Returns an error if:
...
/// Parses Kraken WebSocket trade data into a Vibe trade tick.
///
/// # Errors
///
/// Returns an error if:
...
/// Parses Kraken WebSocket book data into Vibe order book deltas.
///
...
/// Parses Kraken WebSocket OHLC data into a Vibe bar.
///
...
/// Converts a Kraken OHLC interval (minutes) to a Vibe bar specification.
...
/// Parses Kraken execution type and order status to Vibe order status.
...
/// Parses Kraken order type to Vibe order type.
...
/// Parses Kraken order side to Vibe order side.
...
/// Parses Kraken time-in-force to Vibe time-in-force.
...
mod tests {
    ...
    use ustr::Ustr;
    use vibe_model::{identifiers::Symbol, types::Currency};
    ...
    fn create_mock_instrument() -> InstrumentAny {
        use vibe_model::instruments::currency_pair::CurrencyPair;
