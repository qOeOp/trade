...
use vibe_execution::engine::config::ExecutionEngineConfig;
...
use crate::{
    ...
    kernel::{VibeKernel, VibeKernelDependencies},
};

/// Builder for constructing a [`VibeKernel`] with a fluent API.
///
/// Provides a convenient way to configure and build a kernel instance with
/// optional components and settings.
pub struct VibeKernelBuilder {
    name: String,
    trader_id: TraderId,
...
impl Debug for VibeKernelBuilder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(stringify!(VibeKernelBuilder))
...
impl VibeKernelBuilder {
    /// Creates a new [`VibeKernelBuilder`] with required parameters.
    #[must_use]
    ...
    ///
    /// The adapter is passed straight to [`vibe_common::cache::Cache::new`] so
    ...
    /// construction lives outside this crate to keep `vibe-system` decoupled
    ...
    /// (typically in `vibe-event-store`); callers build it inside the closure.
    #[must_use]
    ...
    /// Build the [`VibeKernel`] with the configured settings.
    ///
    /// # Errors
    ///
    /// Returns an error if kernel initialization fails.
    pub fn build(self) -> anyhow::Result<VibeKernel> {
        ...
        {
            anyhow::bail!(
                "VibeKernelBuilder cannot consume external message bus streams; \
        ...
        let kernel = VibeKernel::new_with_dependencies(
            self.name,
            config,
        ...
        if let Some(external_egress) = external_egress {
            vibe_common::msgbus::get_message_bus()
                .borrow_mut()
...
mod tests {
    ...
    fn test_builder_default() {
        let builder = VibeKernelBuilder::default();
        assert_eq!(builder.name, "VibeKernel");
    ...
    fn test_builder_fluent_api() {
        ...
        let builder =
            VibeKernelBuilder::new("TestKernel".to_string(), trader_id, Environment::Live)
    ...
    fn test_builder_build() {
        let result = VibeKernelBuilder::default().build();
        assert!(result.is_ok());

        let kernel = result.unwrap();
        assert_eq!(kernel.name(), "VibeKernel".to_string());
    ...
    fn test_builder_with_cache_database() {
        let builder = VibeKernelBuilder::default().with_cache_database(Box::new(NoopAdapter));
    ...
    fn test_builder_with_external_msgbus_factory_rejects_external_streams() {
        ...
        let error = VibeKernelBuilder::default()
    ...
    fn test_builder_with_clock_factory_drives_kernel_and_component_clocks() {
        use vibe_common::clock::TestClock;

        let calls = Rc::new(Cell::new(0usize));
        let calls_in_closure = calls.clone();

        let kernel = VibeKernelBuilder::new(
    ...
    fn test_builder_without_clock_factory_uses_live_clock_default() {
        use vibe_common::live::clock::LiveClock; // vibe-import-ok
    ...
    fn test_builder_with_event_store_propagates_factory_error() {
        let result = VibeKernelBuilder::default()
