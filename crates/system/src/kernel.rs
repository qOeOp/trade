...
pub struct VibeKernel {
    ...
    pub config: Box<dyn VibeKernelConfig>,
...
/// Optional construction-time dependencies for [`VibeKernel`].
#[derive(Default)]
pub struct VibeKernelDependencies {
...
impl Debug for VibeKernelDependencies {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct(stringify!(VibeKernelDependencies))
...
impl VibeKernel {
    /// Create a new [`VibeKernelBuilder`] for fluent configuration.
    #[must_use]
    pub const fn builder(
        ...
        VibeKernelBuilder::new(name, trader_id, environment)
    }

    /// Create a new [`VibeKernel`] instance.
    ///
    /// # Errors
    ///
    /// Returns an error if the kernel fails to initialize.
    pub fn new<T: VibeKernelConfig + 'static>(name: String, config: T) -> anyhow::Result<Self> {
        Self::new_with(name, config, None, None)
    }

    /// Create a new [`VibeKernel`] instance with an injected cache database adapter.
    ///
    ...
    ///
    /// Returns an error if the kernel fails to initialize.
    pub fn new_with_cache_database<T: VibeKernelConfig + 'static>(
        name: String,
        config: T,
        cache_database: Option<Box<dyn CacheDatabaseAdapter>>,
    ) -> anyhow::Result<Self> {
    ...
    /// Create a new [`VibeKernel`] instance with optional cache database and event store
    /// injections.
    ///
    ...
    pub fn new_with<T: VibeKernelConfig + 'static>(
        name: String,
        config: T,
        cache_database: Option<Box<dyn CacheDatabaseAdapter>>,
        event_store_factory: Option<EventStoreFactory>,
        ...
        Self::new_with_dependencies(
            name,
            config,
            VibeKernelDependencies::default()
                .with_cache_database(cache_database)
                .with_event_store_factory(event_store_factory),
        )
    }

    /// Create a new [`VibeKernel`] instance with construction-time dependencies.
    ///
    /// # Errors
    ///
    /// Returns an error if the kernel fails to initialize or an injected factory fails.
    pub fn new_with_dependencies<T: VibeKernelConfig + 'static>(
        name: String,
        config: T,
        dependencies: VibeKernelDependencies,
    ) -> anyhow::Result<Self> {
        let VibeKernelDependencies {
            clock_factory,
            cache_database,
            event_store_factory,
        } = dependencies;
    ...
    fn initialize_logging(
                ...
                if e.downcast_ref::<log::SetLoggerError>().is_some() {
                    if let Some(guard) = LogGuard::new() {
                        ...
                        return Err(e.context(
                            "A non-Vibe logger is already registered; \
                             cannot initialize Vibe logging",
                        ));
                    }
                } else {
                    return Err(e);
        ...
        #[cfg(feature = "tracing-bridge")]
        if use_tracing && !vibe_common::logging::bridge::tracing_is_initialized() {
            vibe_common::logging::bridge::init_tracing()?;
        }
    ...
    /// Starts the Vibe system kernel synchronously (for backtest use).
    ...
    /// Starts the Vibe system kernel asynchronously.
    #[expect(
        clippy::unused_async,
    ...
    /// [`VibeKernelBuilder::with_event_store`](crate::builder::VibeKernelBuilder::with_event_store);
    ...
    /// Resets the Vibe system kernel to its initial state.
    ...
    /// Disposes of the Vibe system kernel, releasing resources.
...
mod tests {
    use rstest::*;
    use ustr::Ustr;
    use vibe_common::messages::system::ShutdownSystem;
    use vibe_core::UUID4;

    use super::*;
    use crate::builder::VibeKernelBuilder;

    #[rstest]
    fn test_shutdown_system_sets_kernel_flag() {
        let kernel = VibeKernelBuilder::default().build().unwrap();
...
mod lifecycle_tests {
    ...
    use vibe_execution::engine::SnapshotAnchorer;
    use vibe_model::{
    ...
    fn finalize(kernel: &mut VibeKernel) -> anyhow::Result<()> {
        kernel
            .finalize_stop()
            .now_or_never()
    ...
    fn test_state_persistence_orders_restore_load_start_stop_save_seal_and_dispose() {
        ...
        let mut kernel = VibeKernelBuilder::default()
    ...
    fn test_state_persistence_skips_callbacks_without_cache_backing() {
        ...
        let mut kernel = VibeKernelBuilder::default().build().unwrap();
