...
//!
//! The [`KernelEventStore`] trait is the surface [`crate::kernel::VibeKernel`] uses to wire
//! a durable event-sourcing session into its boot, snapshot, and seal flow. The concrete
//! implementation lives in `vibe-event-store` so that crate can be developed and versioned
//! independently of `vibe-system`; callers inject an implementation through the builder
//! (see [`crate::builder::VibeKernelBuilder::with_event_store`]).

use std::{cell::RefCell, fmt::Debug, path::PathBuf, rc::Rc, time::Duration};

use indexmap::IndexMap;
use serde::{Deserialize, Serialize};
use vibe_common::{cache::Cache, clock::Clock, enums::Environment};
...
///
/// `VibeKernel` drives the open/restore/seal sequence through this trait so the concrete
/// event-store machinery (writers, readers, bus tap, redb backend) lives outside
/// `vibe-system`. Implementations are typically built by the caller and injected via
/// [`crate::builder::VibeKernelBuilder::with_event_store`].
