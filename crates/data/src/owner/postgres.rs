...
const MIGRATION_STATEMENTS: &[&str] = &[
    ...
    "CREATE OR REPLACE FUNCTION market_data_private.resolve_pit_request_heads_v1(p_request_identity BYTEA) RETURNS TABLE(snapshot_identity BYTEA, request_digest BYTEA) LANGUAGE SQL STABLE SECURITY DEFINER SET search_path = pg_catalog, pg_temp AS $function$ SELECT DISTINCT h.snapshot_identity, hf.request_digest FROM market_data_private.pit_snapshot_facts_v1 AS f JOIN market_data_private.pit_snapshot_heads_v1 AS h ON h.lineage_root = f.lineage_root JOIN market_data_private.pit_snapshot_facts_v1 AS hf ON hf.snapshot_identity = h.snapshot_identity WHERE f.request_identity = p_request_identity ORDER BY 1 $function$",
    ...
    "REVOKE ALL ON FUNCTION market_data_private.resolve_pit_request_heads_v1(BYTEA) FROM PUBLIC",
