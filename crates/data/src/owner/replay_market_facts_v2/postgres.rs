...
pub(crate) const REPLAY_MARKET_RD_CUT_API_SCHEMA_V1: &[&str] = &[
    ...
    "CREATE OR REPLACE FUNCTION market_data_rd_api.lock_replay_composition_binding_v1(p_binding_identity BYTEA) RETURNS TABLE(binding_identity BYTEA,binding_digest BYTEA,receipt_identity BYTEA,record_bytes BYTEA,receipt_bytes BYTEA,outbox_identity BYTEA,outbox_binding_identity BYTEA,outbox_receipt_identity BYTEA,outbox_bytes BYTEA) LANGUAGE SQL VOLATILE SECURITY DEFINER SET search_path=pg_catalog, pg_temp AS $function$ SELECT b.binding_identity,b.binding_digest,b.receipt_identity,b.record_bytes,r.receipt_bytes,o.outbox_identity,o.binding_identity,o.receipt_identity,o.payload_bytes FROM market_data_private.replay_composition_bindings_v1 AS b JOIN market_data_private.replay_composition_binding_receipts_v1 AS r ON r.binding_identity=b.binding_identity JOIN market_data_private.replay_composition_binding_outbox_v1 AS o ON o.binding_identity=b.binding_identity WHERE b.binding_identity=p_binding_identity FOR SHARE OF b,r,o $function$",
