...
/// The universe-member entry runs through the same reader/Owner protocol as the first corpus.
///
/// Offered the exact Design's own Composer attestation and locators, it opens both transactions
/// and their challenges, reaches the exact-instrument declarations, refuses them by name as a
/// composition shape mismatch, and leaves both transactions terminal with nothing written. The
/// positive issuance, its readback and the Instrument Master V2 cut it keys are proven in the
/// Market Data owner scenario: this shared database's Research request already has its one
/// Composer operation, and a second Research request over the same instrument has no Market
/// Semantics fact it could declare against.
fn universe_member_entry_refuses_an_exact_design_v1<'a>(
    owner_url: &'a str,
    reader_url: &'a str,
    market_pool: &'a sqlx::PgPool,
    exact: &'a super::ReplayCompositionLocatorOnlyIssuanceRequestV1,
) -> ChainEntryStepV1<'a, ()> {
    Box::pin(universe_member_entry_refuses_an_exact_design_step_v1(
        owner_url,
        reader_url,
        market_pool,
        exact,
...
async fn universe_member_entry_refuses_an_exact_design_step_v1(
    owner_url: &str,
    reader_url: &str,
    market_pool: &sqlx::PgPool,
    ...
    use crate::owner::replay_market_facts_v2::{
        ReplayCompositionBindingErrorV1, ReplayCompositionLocatorOnlyIssuanceRequestV1,
    ...
    let exact_design = ReplayCompositionLocatorOnlyIssuanceRequestV1::new(
        vibe_data_binding_digest_for_test(0x79),
    ...
    assert_eq!(
        replay_positive_state(market_pool).await,
        before,
        "the refusal writes nothing"
    );
}
...
async fn replay_composition_after_base_fixture_step_v1(
    ...
    universe_member_entry_refuses_an_exact_design_v1(
        owner_url,
        reader_url,
        market_mutation_pool,
        &command,
