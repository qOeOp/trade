...
/// One Research request's own PIT snapshot, with the Reference Fact R0 record and Market
/// Semantics readback bound to it, over the replay composition base fixture's Source Binding,
/// Instrument Master, Universe Selection and clock.
pub(crate) struct ResearchPitLineageFixtureV1 {
    pub(crate) pit: crate::owner::pit_snapshot::PitSnapshotCommitAggregate,
    pub(crate) r0: crate::owner::reference_fact_coordinates::r0::ReferenceFactR0ReadbackV1,
    pub(crate) semantics: crate::owner::market_semantics::MarketSemanticsReadbackV1,
}

/// Commits a PIT snapshot requested for `research`, and the R0 record and Market Semantics bound
/// to it, through the Owner's own writers.
///
...
pub(crate) async fn persist_research_pit_lineage_v1(
    ...
    let base_request = base.pit.fact().request();
    let mut pit_proposal = UntrustedPitSnapshotProposal {
        request: UntrustedPitSnapshotRequest {
            claimed_request_identity: d(0),
            claimed_request_digest: d(0),
            correlation_identity: d(seed),
            requester_identity: crate::owner::pit_snapshot::research_pit_requester_identity_v1(
                research,
            ),
            scope_digest: base_request.scope_digest,
            source_binding: source.receipt().locator().clone(),
    ...
    let observation = UntrustedPitObservationBatchProposal {
        rows: base
            ...
            .map(|row| UntrustedPitObservation {
                symbolic_key: row.symbolic_key().into(),
                member_key: row.member_key().into(),
                instrument: row.instrument().into(),
                channel: row.channel().into(),
                data_kind: row.data_kind().into(),
                timeframe: row.timeframe().into(),
                field: row.field().into(),
    ...
    let source_locator_bytes = serde_json::to_vec(source.receipt().locator())
        ...
        .into_boxed_slice();
    let mut r0_request = UntrustedReferenceFactR0RequestV1 {
        request_identity: d(seed.wrapping_add(1)),
        request_meaning_digest: d(0),
        pit_locator_bytes: pit_locator_bytes.clone(),
        source_binding_locator_bytes: source_locator_bytes.clone(),
        replay_start_event_ns: 50,
    ...
    let mut instrument_locator_bytes = Vec::with_capacity(64);
    instrument_locator_bytes.extend_from_slice(base.instrument.request_identity.as_bytes());
    instrument_locator_bytes.extend_from_slice(base.instrument.request_meaning_digest.as_bytes());
    let mut r0_locator_bytes = Vec::with_capacity(64);
    r0_locator_bytes.extend_from_slice(r0_request.request_identity.as_bytes());
    r0_locator_bytes.extend_from_slice(r0_request.request_meaning_digest.as_bytes());
    let mut semantics_proposal = UntrustedMarketSemanticsProposalV1 {
    ...
    ResearchPitLineageFixtureV1 { pit, r0, semantics }
}
...
async fn native_replay_quote_cut_census_oracle(owner: &MarketDataOwnerPostgres) {
    ...
    let original_quote_cut = async |scope: BindingDigest, correlation_byte: u8, time: u64| {
        ...
        let committed = owner
            ...
            .await
            .expect("an original quote cut");
        (proposal, committed)
    };
    let same_instant = d(152);
    let moved_out = d(153);
    let moved_in = d(154);
    let rebound = d(155);
    let scenarios = [
    ...
    let (moved_out_proposal, moved_out_original) = original_quote_cut(moved_out, 92, 15).await;
    let (moved_in_proposal, moved_in_original) = original_quote_cut(moved_in, 93, 5).await;
    let (rebound_proposal, rebound_original) = original_quote_cut(rebound, 94, 15).await;
    ...
    let successor = owner
        ...
        .await
        .expect("a corrected Source Binding in the same lineage");
    let other_lineage = owner
        .commit_source_initial(
            source_proposal(14, 50),
            OwnerSourceBindingDecision {
                blockers: BTreeSet::new(),
            },
            &clock(50, 2),
        ...
        .await
        .expect("a Source Binding of another lineage");
    assert_ne!(
        other_lineage.fact().lineage_root(),
        successor.fact().lineage_root()
    );
    let correct = async |original_proposal: &UntrustedPitSnapshotProposal,
                         original: &PitSnapshotCommitAggregate,
                         source: &SourceBindingCommit,
                         event: u64| {
        let mut proposal = pit_correction(original_proposal, source);
        ...
        proposal.evidence.source_frontier = source.receipt().locator().source_frontier.clone();
        proposal.evidence.correction_frontier =
            source.receipt().locator().correction_frontier.clone();
        let observation = observation_batch_of(source, &proposal, &[BTC], &["QUOTE"]);
        proposal.evidence.normalized_records_digest =
            derive_observation_batch_digest(&observation).unwrap();
        refresh_request_claims(&mut proposal.request);
        ...
        Ok::<_, PitSnapshotError>(correction)
    };
    let same_instant_correction = correct(
        &same_instant_proposal,
        &same_instant_original,
        &successor,
        15,
    )
    .await
    .expect("a correction on the quote cut's own instant");
    let moved_out_correction = correct(&moved_out_proposal, &moved_out_original, &successor, 25)
        .await
        .expect("a correction that moves the quote cut later");
    let moved_in_correction = correct(&moved_in_proposal, &moved_in_original, &successor, 15)
        .await
        .expect("a correction that moves the quote cut into the interval");
    // (c) A correction cannot move a quote cut onto another Source Binding lineage: the Owner
    // admits a correction only on the exact successor of its original's binding
    // (`validate_source_binding_successor`, pinned without a database by
    // `correction_rejects_a_different_source_binding_lineage_without_write`). So that shape never
    // reaches the census; the selection rule would still refuse it
    // (`a_superseded_original_is_never_the_frames_quote_cut`), and here the original simply stays
    // its lineage's latest.
    assert!(
        correct(&rebound_proposal, &rebound_original, &other_lineage, 15)
            .await
            .is_err(),
