...
pub(super) async fn resolve_corporate_action_in_transaction_v1(
    ...
    sqlx::query("INSERT INTO market_data_private.corporate_action_state_v1(singleton,store_generation_identity,append_sequence) VALUES(TRUE,$1,0) ON CONFLICT(singleton) DO NOTHING").bind(generation.as_bytes().as_slice()).execute(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
    let sequence:i64=sqlx::query_scalar("UPDATE market_data_private.corporate_action_state_v1 SET append_sequence=append_sequence+1 WHERE singleton AND store_generation_identity=$1 RETURNING append_sequence").bind(generation.as_bytes().as_slice()).fetch_optional(&mut **transaction).await.map_err(|cause| store_error(&cause))?.ok_or(CorporateActionErrorV1::StoreUntrusted)?;
...
async fn persist_readback(
    ...
    for fact in readback.facts() {
        ...
        let stored:Vec<u8>=sqlx::query_scalar("SELECT fact_bytes FROM market_data_private.corporate_action_facts_v1 WHERE fact_identity=$1 FOR UPDATE").bind(fact.identity().as_bytes().as_slice()).fetch_one(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
        if stored != fact.canonical_bytes() {
            return Err(CorporateActionErrorV1::StoreUntrusted);
        }
        sqlx::query("INSERT INTO market_data_private.corporate_action_heads_v1(action_identity,fact_identity) VALUES($1,$2) ON CONFLICT(action_identity) DO UPDATE SET fact_identity=EXCLUDED.fact_identity").bind(fact.action_identity().as_bytes().as_slice()).bind(fact.identity().as_bytes().as_slice()).execute(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
    }
    sqlx::query("INSERT INTO market_data_private.corporate_action_cuts_v1(request_identity,request_meaning_digest,cut_identity,cut_bytes) VALUES($1,$2,$3,$4)").bind(readback.cut().request_identity.as_bytes().as_slice()).bind(readback.cut().request_meaning_digest.as_bytes().as_slice()).bind(readback.cut().identity().as_bytes().as_slice()).bind(readback.cut().canonical_bytes()).execute(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
    sqlx::query("INSERT INTO market_data_private.corporate_action_receipts_v1(request_identity,receipt_identity,receipt_bytes,readback_identity,readback_bytes,append_sequence) VALUES($1,$2,$3,$4,$5,$6)").bind(readback.cut().request_identity.as_bytes().as_slice()).bind(readback.receipt().identity().as_bytes().as_slice()).bind(readback.receipt().canonical_bytes()).bind(readback.identity().as_bytes().as_slice()).bind(readback.canonical_bytes()).bind(i64::try_from(readback.receipt().append_sequence).map_err(|_|CorporateActionErrorV1::CapacityExceeded)?).execute(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
    sqlx::query("INSERT INTO market_data_private.corporate_action_outbox_v1(outbox_identity,request_identity,payload) VALUES($1,$2,$3)").bind(readback.outbox_identity().as_bytes().as_slice()).bind(readback.cut().request_identity.as_bytes().as_slice()).bind(readback.receipt().canonical_bytes()).execute(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
    Ok(())
}
...
async fn load_readback(
    ...
    let state:Option<(Vec<u8>,i64)>=sqlx::query_as("SELECT store_generation_identity,append_sequence FROM market_data_private.corporate_action_state_v1 WHERE singleton").fetch_optional(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
    let stored_facts:Vec<Vec<u8>>=sqlx::query_scalar("SELECT fact_bytes FROM market_data_private.corporate_action_facts_v1 WHERE fact_identity=ANY($1) ORDER BY fact_identity").bind(readback.facts().iter().map(|f|f.identity().as_bytes().to_vec()).collect::<Vec<_>>()).fetch_all(&mut **transaction).await.map_err(|cause| store_error(&cause))?;
...
#[track_caller]
fn store_error(cause: &impl Debug) -> CorporateActionErrorV1 {
