...
use crate::owner::{
    ...
    strategy_design_role_intent_v1::InitialPitRequestLocatorV1,
...
///
/// It is a synchronous dispatcher that returns the chosen resolution boxed. The exact-instrument
/// path runs on the ordered chain's 2 MiB test stack, and a debug build reserves room in an async
/// frame for every future it could await; boxing each path keeps the caller's frame to a pointer.
pub(super) fn resolve_role_snapshot_v1<'a, 'b: 'a>(
    transaction: &'a mut Transaction<'b, Postgres>,
    design: AuthenticatedDesignIdentityV1,
    role: &'a StrategyDesignRoleEntryV1,
    initial_pit_request: Option<InitialPitRequestLocatorV1>,
) -> RoleSnapshotFutureV1<'a> {
    match (classify_role_v1(role), initial_pit_request) {
        (Err(e), _) => Box::pin(std::future::ready(Err(e))),
        (Ok((true, _)), None) => Box::pin(std::future::ready(Err(
            PitRoleResolutionErrorV1::InitialPitRequestUnnamed,
        ))),
        (Ok((true, _)), Some(named)) => Box::pin(resolve_initial_pit_request_snapshot_v1(
            transaction,
            design,
            named,
        )),
        (Ok((false, semantic)), _) => {
            Box::pin(resolve_coordinate_snapshot_v1(transaction, role, semantic))
        }
    }
}

/// A role's snapshot resolution, boxed so its caller's frame holds only a pointer.
pub(super) type RoleSnapshotFutureV1<'a> = std::pin::Pin<
    Box<
...
/// Resolves an exact-instrument role to the one snapshot whose rows carry its coordinates.
async fn resolve_coordinate_snapshot_v1(
    transaction: &mut Transaction<'_, Postgres>,
    role: &StrategyDesignRoleEntryV1,
    semantic: MarketDataFieldSemantic,
...
async fn resolve_initial_pit_request_snapshot_v1(
    transaction: &mut Transaction<'_, Postgres>,
    design: AuthenticatedDesignIdentityV1,
    named: InitialPitRequestLocatorV1,
) -> Result<ResolvedRoleSnapshotV1, PitRoleResolutionErrorV1> {
    let rows = sqlx::query(
...
mod tests {
    ...
    use super::{
        BindingDigest, InitialPitRequestLocatorV1, PitRoleResolutionErrorV1,
    ...
    fn named() -> InitialPitRequestLocatorV1 {
