...
fn is_missing_custody_object(error: &anyhow::Error) -> bool {
    #[cfg(not(unix))]
    {
        let _ = error;
        false
...
/// Reads an optional bounded regular file, returning `None` only for a missing object.
///
/// Unsafe paths, symlinks, non-regular or oversized objects, permission failures, and every
/// non-`ENOENT` error remain fail-closed.
///
/// # Errors
///
/// Returns an error when the path is unsafe or the object exists but cannot be opened and read as
/// a bounded regular file.
pub fn read_optional_bounded_regular_at(
    root: &CustodiedDirectory,
    path: &Path,
    limit: u64,
...
mod tests {
    ...
    #[cfg(unix)]
    #[rstest]
    fn optional_read_distinguishes_only_missing_objects() {
        let root = tempfile::tempdir().unwrap();
        fs::write(root.path().join("object"), b"bound").unwrap();
        fs::create_dir(root.path().join("directory")).unwrap();
        std::os::unix::fs::symlink("object", root.path().join("link")).unwrap();
        let directory = open_custodied_directory(&fs::canonicalize(root.path()).unwrap()).unwrap();

        assert_eq!(
            read_optional_bounded_regular_at(&directory, Path::new("object"), 5).unwrap(),
            Some(b"bound".to_vec())
        );
        assert_eq!(
            read_optional_bounded_regular_at(&directory, Path::new("missing"), 5).unwrap(),
            None
        );
        assert!(read_optional_bounded_regular_at(&directory, Path::new("link"), 5).is_err());
        assert!(read_optional_bounded_regular_at(&directory, Path::new("directory"), 5).is_err());
        assert!(read_optional_bounded_regular_at(&directory, Path::new("object"), 4).is_err());
        assert!(!is_missing_custody_object(&anyhow::Error::new(
    ...
    fn custody_fails_closed_when_no_follow_open_is_unavailable() {
        ...
        assert!(
            read_optional_bounded_regular_at(&CustodiedDirectory {}, Path::new("object"), 1)
                .is_err()
        );
    }
}
