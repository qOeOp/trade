...
mod tests {
    ...
    async fn dashboard_owner_readback_acceptance_snapshot(
        ...
        for (pool, relations) in [
            ...
            for (relation, order) in relations {
                ...
                .fetch_all(pool)
                .await
                .unwrap_or_else(|error| panic!("snapshot of {relation} must read: {error}"));
    ...
    async fn strategy_source_browser_acceptance_reads_canonical_terminal_owner_custody() {
        ...
        let source_intake_owner = SourceIntakeOwnerV1::sealed_acceptance(
        ...
        let read_state = dashboard_read_api::compose_state(&DashboardReadApiConfigV1 {
