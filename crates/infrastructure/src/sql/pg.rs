...
fn get_schema_dir() -> anyhow::Result<String> {
    std::env::var("SCHEMA_DIR").or_else(|_| {
        let vibe_git_repo_name = "vibe_trader";
        let binding = std::env::current_dir().unwrap();
        let current_dir = binding.to_str().unwrap();
        match current_dir.find(vibe_git_repo_name){
            Some(index) => {
                let schema_path = current_dir[0..index + vibe_git_repo_name.len()].to_string() + "/schema/sql";
                Ok(schema_path)
            }
...
"#,
        ...
        assert_eq!(config.username, "vibe");
        assert_eq!(config.database, "vibe");
    }
