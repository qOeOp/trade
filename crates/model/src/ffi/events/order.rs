...
use ustr::Ustr;
use vibe_core::{UUID4, UnixNanos, ffi::string::cstr_to_ustr};
...
/// Represents an event where an order has been denied by the Vibe system.
///
...
/// Represents an event where an order has become emulated by the Vibe system.
#[repr(C)]
#[derive(Clone, Copy, Debug)]
...
/// Represents an event where an order was released from the `OrderEmulated` by the Vibe system.
#[repr(C)]
#[derive(Clone, Copy, Debug)]
