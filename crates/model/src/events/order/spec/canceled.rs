...
impl<S: order_canceled_spec_builder::IsComplete> OrderCanceledSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderCanceled`] through its production constructor.
    #[must_use]
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderCanceledSpec::builder().build();
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderCanceledSpec::builder().build();
        let b = OrderCanceledSpec::builder().build();
        let c = OrderCanceledSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderCanceledSpec::builder().build().event_id)
            .collect();
