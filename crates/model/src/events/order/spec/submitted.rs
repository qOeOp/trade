...
impl<S: order_submitted_spec_builder::IsComplete> OrderSubmittedSpecBuilder<S> {
    /// Builds the spec and constructs an [`OrderSubmitted`] through its production constructor.
    #[must_use]
...
mod tests {
    ...
    fn defaults_are_sensible() {
        ...
        let event = OrderSubmittedSpec::builder().build();
    ...
    fn overrides_apply_through_constructor() {
        ...
        assert_eq!(event.account_id, AccountId::from("SIM-002"));
    ...
    fn event_ids_are_unique_within_a_run() {
        reset_test_uuid_rng();
        let a = OrderSubmittedSpec::builder().build();
        let b = OrderSubmittedSpec::builder().build();
        let c = OrderSubmittedSpec::builder().build();
    ...
    fn event_id_sequence_is_reproducible() {
        ...
        let first_run: Vec<_> = (0..3)
            .map(|_| OrderSubmittedSpec::builder().build().event_id)
            .collect();
