...
pub fn order_filled(
    ...
    OrderFilledSpec::builder()
        ...
        .last_qty(Quantity::from_str("0.561").unwrap_or_else(|error| panic!("{error}")))
        .last_px(Price::from_str("22000").unwrap_or_else(|error| panic!("{error}")))
        .currency(Currency::from_str("USDT").unwrap_or_else(|error| panic!("{error}")))
        .event_id(uuid4)
...
pub fn order_initialized_buy_limit(
    ...
    OrderInitializedSpec::builder()
        ...
        .client_order_id(client_order_id)
        .order_type(OrderType::Limit)
        .quantity(Quantity::from_str("0.561").unwrap_or_else(|error| panic!("{error}")))
        .post_only(true)
        .reduce_only(true)
        .event_id(uuid4)
        .price(Price::from_str("22000").unwrap_or_else(|error| panic!("{error}")))
...
pub fn order_released(
    ...
    OrderReleasedSpec::builder()
        ...
        .instrument_id(instrument_id_btc_usdt)
        .client_order_id(client_order_id)
        .released_price(Price::from_str("22000").unwrap_or_else(|error| panic!("{error}")))
        .event_id(uuid4)
        .build()
}
