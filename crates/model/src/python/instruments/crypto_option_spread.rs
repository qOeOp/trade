...
use vibe_core::{
    from_pydict,
    python::{IntoPyObjectVibeExt, to_pyvalue_err},
};
...
impl CryptoOptionSpread {
    ...
    fn py_to_dict(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        ...
        dict.set_item("type", stringify!(CryptoOptionSpread))?;
...
mod tests {
    ...
    use crate::instruments::{CryptoOptionSpread, stubs::*};

    #[rstest]
    fn test_dict_round_trip(crypto_option_spread_btc_deribit: CryptoOptionSpread) {
        Python::initialize();
        Python::attach(|py| {
            let original = crypto_option_spread_btc_deribit;
            let values = original.py_to_dict(py).unwrap();
            let values: Py<PyDict> = values.extract(py).unwrap();
            let restored = CryptoOptionSpread::py_from_dict(py, values).unwrap();
