//! Static plug-in metadata returned from `vibe_plugin_init`.
...
use crate::{
    PLUGIN_BUILD_ID_VERSION, VIBE_PLUGIN_ABI_VERSION, boundary::BorrowedStr, host::HostVTable,
};

/// Signature of the single `extern "C"` entry symbol every plug-in exports
/// under the name [`crate::VIBE_PLUGIN_INIT_SYMBOL`].
...
///
/// The fields identify the Vibe plug-in crate and build environment that
...
pub struct PluginBuildId {
    ...
    /// Version of the `vibe-plugin` crate used to build the plug-in.
    pub vibe_plugin_version: BorrowedStr<'static>,
...
impl PluginBuildId {
    /// Returns the build identifier for the compiled `vibe-plugin` crate.
    #[must_use]
    pub const fn current() -> Self {
        Self {
            schema_version: PLUGIN_BUILD_ID_VERSION,
            vibe_plugin_version: BorrowedStr::from_str(env!("CARGO_PKG_VERSION")),
            rustc_version: BorrowedStr::from_str(env!("VIBE_PLUGIN_BUILD_RUSTC_VERSION")),
            target_triple: BorrowedStr::from_str(env!("VIBE_PLUGIN_BUILD_TARGET")),
            build_profile: BorrowedStr::from_str(env!("VIBE_PLUGIN_BUILD_PROFILE")),
...
pub struct PluginManifest {
    /// ABI version. Must equal [`VIBE_PLUGIN_ABI_VERSION`] or the host
...
impl PluginManifest {
    ...
    pub fn matches_compiled_abi(&self) -> bool {
        self.abi_version == VIBE_PLUGIN_ABI_VERSION
    }
    ...
    pub fn validate(&self) -> Result<(), PluginManifestValidationErrors> {
        ...
        if self.abi_version != VIBE_PLUGIN_ABI_VERSION {
            errors.push(format!(
...
mod tests {
    ...
    fn valid_manifest() -> PluginManifest {
        PluginManifest {
            abi_version: VIBE_PLUGIN_ABI_VERSION,
            plugin_name: BorrowedStr::from_str("test-plugin"),
            plugin_vendor: BorrowedStr::from_str("vibe"),
    ...
    fn matches_compiled_abi_rejects_mismatch() {
        let manifest = PluginManifest {
            abi_version: VIBE_PLUGIN_ABI_VERSION.wrapping_add(1),
            ..valid_manifest()
        };
    ...
    fn validate_rejects_mismatched_abi() {
        ...
        assert_eq!(
            ...
            &[format!(
                "abi_version {} does not match supported ABI {}",
                VIBE_PLUGIN_ABI_VERSION.wrapping_add(1),
                VIBE_PLUGIN_ABI_VERSION
            )]
        );
    }
