...
//!
//! Call [`vibe_serialization::ensure_custom_data_registered::<T>()`] once per type for JSON
//! and Arrow registration; for `no_arrow` types call
//! [`vibe_model::data::ensure_custom_data_json_registered::<T>()`] instead. For Python
//! bindings also call [`vibe_model::data::register_rust_extractor::<T>()`].
//!
//! # Requirements
//!
//! - Struct must have named fields
//! - Must include `ts_event` and `ts_init` fields (e.g. `vibe_core::UnixNanos`)
    ...
    if json {
        return Some(quote! {
            serde_json::from_str::<#ty>(#col.value(i)).map_err(|e| {
                vibe_serialization::arrow::EncodingError::ParseError(
                    stringify!(#name),
                    format!("row {i}: {e}"),
                )
            })?
    ...
    match (outer.as_str(), inner.as_str()) {
        ...
        ("Vec", "f64") => Some(quote! {
            {
                ...
                let float_arr = arr.as_any().downcast_ref::<arrow::array::Float64Array>()
                    .ok_or_else(|| vibe_serialization::arrow::EncodingError::ParseError(
                        stringify!(#name),
        ...
        _ if outer == inner => match outer.as_str() {
            "InstrumentId" | "AccountId" | "Currency" | "BarType" => Some(quote! {
                std::str::FromStr::from_str(#col.value(i)).map_err(|e| vibe_serialization::arrow::EncodingError::ParseError(
                    stringify!(#name),
                    format!("expected valid identifier/type, was '{}'", e),
                ))?
            }),
            "Params" => Some(quote! {
                serde_json::from_str::<vibe_core::Params>(#col.value(i)).map_err(|e| {
                    vibe_serialization::arrow::EncodingError::ParseError(
                        stringify!(#name),
                        format!("row {i}: {e}"),
                    )
...
fn py_field_init(ident: &syn::Ident, ty: &Type, json: bool) -> Option<TokenStream> {
    ...
    if json {
        if let Some(map_kind) = typed_json_map_kind(ty) {
            ...
            return Some(quote! {
                pyo3::Python::attach(|py| -> pyo3::PyResult<#ty> {
                    ...
                    vibe_core::python::serialization::#helper::<_, _>(py, value)
                        .map_err(|e| vibe_core::python::to_pyvalue_err(format!("failed to deserialize JSON field '{}': {e}", stringify!(#name))))
                })?
            });
        }
        ...
        return Some(quote! {
            pyo3::Python::attach(|py| -> pyo3::PyResult<#ty> {
                let value = #name.bind(py);
                vibe_core::python::serialization::from_pyobject_pyo3::<#ty>(py, value)
                    .map_err(|e| vibe_core::python::to_pyvalue_err(format!("failed to deserialize JSON field '{}': {e}", stringify!(#name))))
            })?
        });
    }
    ...
    if outer == inner && outer == "Params" {
        return Some(quote! {
            pyo3::Python::attach(|py| vibe_core::from_pydict(py, &#name))?.unwrap_or_default()
        });
    }
...
fn py_getter_body(ident: &syn::Ident, ty: &Type, json: bool) -> Option<TokenStream> {
    ...
    if json {
        if let Some(map_kind) = typed_json_map_kind(ty) {
            ...
            return Some(quote! {
                pyo3::Python::attach(|py| {
                    vibe_core::python::serialization::#helper(py, &self.#name)
                        .map_err(|e| vibe_core::python::to_pyvalue_err(format!("failed to serialize JSON field '{}': {e}", stringify!(#name))))
                })
            });
        }
        ...
        return Some(quote! {
            pyo3::Python::attach(|py| {
                vibe_core::python::serialization::to_pyobject_pyo3(py, &self.#name)
                    .map_err(|e| vibe_core::python::to_pyvalue_err(format!("failed to serialize JSON field '{}': {e}", stringify!(#name))))
            })
        });
    }
...
fn gen_repr_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    let repr_args: Vec<TokenStream> = field_list
        ...
        .map(|f| {
            ...
            let s = ident.to_string();
            if s == "ts_event" || s == "ts_init" {
                quote! { vibe_core::datetime::unix_nanos_to_iso8601(self.#ident) }
            } else {
                quote! { self.#ident }
            }
        })
...
fn gen_ts_init_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    let generics = ctx.generics;
    quote! {
        impl #generics vibe_model::data::HasTsInit for #name #generics {
...
fn gen_custom_data_trait_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    let to_pyobject_impl = if ctx.options.pyo3 {
        quote! {
            ...
            fn to_pyobject(&self, py: pyo3::Python<'_>) -> pyo3::PyResult<pyo3::Py<pyo3::PyAny>> {
                vibe_model::data::custom::clone_pyclass_to_pyobject(self, py)
            }
        }
    } else {
        quote! {}
    ...
    quote! {
        impl #generics vibe_model::data::CustomDataTrait for #name #generics {
            fn type_name(&self) -> &'static str {
                ...
                fn clone_arc(&self) -> std::sync::Arc<dyn vibe_model::data::CustomDataTrait> {
                ...
                fn from_json(value: serde_json::Value) -> anyhow::Result<std::sync::Arc<dyn vibe_model::data::CustomDataTrait>> {
...
fn gen_custom_data_serialize_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    let name_str = ctx.name_str;
    quote! {
        impl #generics vibe_serialization::arrow::custom::CustomDataSerialize for #name #generics {
            fn schema(&self) -> anyhow::Result<arrow::datatypes::Schema> {
                Ok(<Self as vibe_serialization::arrow::ArrowSchemaProvider>::get_schema(
                    Some(vibe_serialization::arrow::EncodeToRecordBatch::metadata(self))
                ).into())
            }
            fn encode_record_batch(
                &self,
                items: &[std::sync::Arc<dyn vibe_model::data::CustomDataTrait>],
                ...
                let metadata = vibe_serialization::arrow::EncodeToRecordBatch::metadata(self);
                vibe_serialization::arrow::EncodeToRecordBatch::encode_batch(&metadata, &typed).map_err(Into::into)
            }
        }
    }
}
...
fn gen_arrow_schema_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    let arrow_schema_fields: Vec<TokenStream> = field_list
        ...
        .collect();
    quote! {
        impl #generics vibe_serialization::arrow::ArrowSchemaProvider for #name #generics {
...
fn gen_encode_batch_impl(ctx: &ExpansionContext<'_>) -> TokenStream {
    ...
    quote! {
        impl #generics vibe_serialization::arrow::EncodeToRecordBatch for #name #generics {
            fn encode_batch(
                ...
                arrow::record_batch::RecordBatch::try_new(
                    <Self as vibe_serialization::arrow::ArrowSchemaProvider>::get_schema(Some(metadata.clone())).into(),
                    vec![ #(#col_names),* ],
                )
            }
    ...
    let decode_extracts: Vec<TokenStream> = field_list
        ...
        .map(|(idx, f)| {
            ...
            if use_string_extract(ty, f.options.serde) {
                quote! {
                    let #col_name = vibe_serialization::arrow::extract_column_string(
                ...
                quote! {
                    let #col_name = vibe_serialization::arrow::extract_column::<#array_ty>(
                        record_batch.columns(),
                        #fn_str,
                        #idx,
                        #arrow_dt,
        ...
        .collect();
    quote! {
        impl #generics vibe_serialization::arrow::DecodeDataFromRecordBatch for #name #generics {
            fn decode_data_batch(
                _metadata: &std::collections::HashMap<String, String>,
                record_batch: arrow::record_batch::RecordBatch,
            ) -> std::result::Result<Vec<vibe_model::data::Data>, vibe_serialization::arrow::EncodingError> {
                #(#decode_extracts)*
                ...
                for i in 0..num_rows {
                    ...
                    results.push(vibe_model::data::Data::Custom(vibe_model::data::CustomData::from_arc(std::sync::Arc::new(row))));
                }
                Ok(results)
            }
        }
...
fn gen_catalog_path_and_conversions(
    ...
    let catalog_path_prefix_impl = quote! {
        impl #generics vibe_model::data::CatalogPathPrefix for #name #generics {
    ...
    let from_impl = quote! {
        impl #generics std::convert::From<#name #generics> for vibe_model::data::Data {
            fn from(value: #name #generics) -> Self {
                vibe_model::data::Data::Custom(vibe_model::data::CustomData::from_arc(std::sync::Arc::new(value)))
            }
        }
    };
    let try_from_impl = quote! {
        impl #generics std::convert::TryFrom<vibe_model::data::Data> for #name #generics {
            type Error = anyhow::Error;
            fn try_from(value: vibe_model::data::Data) -> std::result::Result<Self, Self::Error> {
                match value {
                    vibe_model::data::Data::Custom(custom) => {
    ...
    let record_batch_methods = if ctx.options.no_arrow {
        ...
        quote! {
            ...
            fn decode_record_batch_py(
                ...
                let schema = std::sync::Arc::new(arrow::datatypes::Schema::try_from(&ffi_schema).map_err(vibe_core::python::to_pyvalue_err)?);
                let struct_array_data = unsafe { arrow::ffi::from_ffi_and_data_type(ffi_array, arrow::datatypes::DataType::Struct(schema.fields().clone())).map_err(vibe_core::python::to_pyvalue_err)? };
                ...
                let data_list = <#name as vibe_serialization::arrow::DecodeDataFromRecordBatch>::decode_data_batch(
                    &metadata,
                    batch,
                ...
                for d in data_list {
                    if let vibe_model::data::Data::Custom(custom) = d {
            ...
            fn encode_record_batch_py(
                ...
                let metadata = <#name as vibe_serialization::arrow::EncodeToRecordBatch>::metadata(self);
                let batch = <#name as vibe_serialization::arrow::EncodeToRecordBatch>::encode_batch(
                    &metadata,
                    &typed,
                ).map_err(vibe_core::python::to_pyvalue_err)?;
                ...
                let mut ffi_schema = arrow::ffi::FFI_ArrowSchema::try_from(arrow::datatypes::DataType::Struct(batch.schema().fields().clone())).map_err(vibe_core::python::to_pyvalue_err)?;
    ...
    quote! {
        ...
        impl #generics #name #generics {
            ...
            fn to_json(&self) -> pyo3::PyResult<String> {
                <#name as vibe_model::data::CustomDataTrait>::to_json_py(self)
                    .map_err(vibe_core::python::to_pyvalue_err)
            }
            ...
            fn from_json(
                ...
                let json_module = py.import("json")
                    .map_err(|e| vibe_core::python::to_pyvalue_err(format!("import json failed: {e}")))?;
                let json_str: String = json_module
                    .call_method1("dumps", (data,))
                    .map_err(|e| vibe_core::python::to_pyvalue_err(format!("json.dumps failed: {e}")))?
                    .extract()?;
                let value: serde_json::Value = serde_json::from_str(&json_str)
                    .map_err(|e| vibe_core::python::to_pyvalue_err(format!("serde_json::from_str failed: {e}")))?;
                let arc = <#name as vibe_model::data::CustomDataTrait>::from_json(value)
                    .map_err(vibe_core::python::to_pyvalue_err)?;
                let inner = arc.as_any().downcast_ref::<#name>()
                    .ok_or_else(|| vibe_core::python::to_pyvalue_err("from_json downcast failed"))?;
...
pub(crate) fn expand_custom_data(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if ts_init_field.is_none() || ts_event_field.is_none() {
        return syn::Error::new_spanned(
            input,
            "#[custom_data] requires fields ts_event and ts_init (e.g. vibe_core::UnixNanos)",
        )
        .to_compile_error();
    }
...
mod tests {
    ...
    fn expand_preserves_existing_imported_serde_derives_without_duplicates() {
        ...
        let item = quote! {
            ...
            pub struct TestData {
                pub value: f64,
                pub ts_event: vibe_core::UnixNanos,
                pub ts_init: vibe_core::UnixNanos,
            }
        };
