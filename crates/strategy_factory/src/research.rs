...
const PREDECESSOR_REPRESENTATIVE_INTENT_BYTES: &[u8] =
    include_bytes!("../assets/representative_intent_v2.jcs");

pub const REPRESENTATIVE_INTENT_ID: &str = "researchintent-strategy-factory-representative-v3";
pub const REPRESENTATIVE_EXPERIMENT_ID: &str = "btc-eth-perpetual-cross-asset-multitimeframe-v3";
pub const REPRESENTATIVE_INTENT_SHA256: &str =
    "b8dc3a9b68745b5203b24d86805c9a8a64a6b26f5e5e36ab670d220541d6469f";
...
impl ResearchIntent {
    ...
    fn validate_invariants(&self) -> Result<(), ResearchIntentError> {
        ...
        if self.document.identity != REPRESENTATIVE_INTENT_ID
            ...
            || self.document.revision != "3"
            || self.document.schema_version != 6
        ...
        if admission.formation != "NOT_ADMITTED_NO_CONTENT_ADDRESSED_MULTI_SOURCE_MANIFEST"
            || admission.holdout != "NOT_ADMITTED_NO_PHYSICAL_CUSTODY_OR_UNSEEN_PARTITION"
            || admission.live != "FORBIDDEN"
            || admission.runtime != "DESIGN_FROZEN_EXECUTION_NOT_ADMITTED"
        {
        ...
        if self.document.payload.predecessor.disposition
            != "REPRESENTATIVE_V2_SUPERSEDED_USER_REQUIRED_GOLD_INTERMARKET_CONTEXT_BEFORE_RESULTS"
            || self.document.payload.predecessor.intent_sha256
                != "sha256:b4c796543bc22b212f9bce933976f0ca44603afc8142ecf02854b373ac6e71f9"
            || self.document.payload.predecessor.locator
                != "crates/strategy_factory/assets/representative_intent_v2.jcs"
            || format!(
                "sha256:{:x}",
...
mod tests {
    ...
    #[rstest]
    fn frozen_representative_intent_binds_authority_and_non_admission() {
        ...
        assert_eq!(intent.deletion_variants().len(), 9);
    ...
    #[rstest]
    fn economic_calendar_remains_policy_only_until_an_existing_owner_is_bound() {
        let intent = ResearchIntent::frozen_representative().unwrap();
        assert_eq!(
