...
impl NativeProducerEvidence {
    ...
    pub(crate) const fn allows_test_or_attested_execution(&self) -> bool {
        ...
        #[cfg(test)]
        {
            matches!(&self.record, NativeProducerRecord::TestOnly { .. })
        }
        #[cfg(not(test))]
        {
            false
    ...
    #[cfg(test)]
    pub(crate) fn test_only_for_execution() -> Self {
        Self {
            record: NativeProducerRecord::TestOnly {
                authority: "cfg-test-only".to_string(),
                accepted_public_authority: false,
                reason: "FULL_CHAIN_TEST_EXECUTION_ONLY_NO_PUBLIC_RECEIPT_AUTHORITY".to_string(),
            },
        }
    }
}
