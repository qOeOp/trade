...
from vibe_trader.analysis.config import TearsheetChart
from vibe_trader.analysis.config import TearsheetConfig
...
if TYPE_CHECKING:
    ...
    from vibe_trader.backtest import BacktestResult
...
def _require_pandas():
    try:
        ...
        raise ImportError(
            "pandas is required for visualization; install the visualization extras "
...
def _write_figure(fig: go.Figure, output_path: str) -> None:
    # Static image export uses Kaleido (install via `vibe_trader[visualization]`).
...
def create_tearsheet(
    ...
    title: str = "VibeTrader Backtest Results",
    currency=None,
    config=None,
    ...
    title : str, default "VibeTrader Backtest Results"
    ...
    if not PLOTLY_AVAILABLE:
        msg = "plotly is required; install the visualization extras from python/pyproject.toml"
    ...
    if title == "VibeTrader Backtest Results":
        ...
        title = f"<b>VibeTrader</b> v{VIBE_VERSION} - Backtest Results"
...
def _create_tearsheet_figure(
    ...
    fig.update_layout(
        title_text=config.title if config.title != "VibeTrader Backtest Results" else title,
        title_font_size=20,
        template=theme_config["template"],
        height=config.height,
        showlegend=benchmark_returns is not None,
