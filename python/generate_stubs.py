...
# Classes to relocate from _libvibe to their target modules
...
def generate_stubs() -> bool:
    ...
    if "vibe-interactive-brokers/gateway" not in cargo_features:
        cargo_features.append("vibe-interactive-brokers/gateway")
    ...
    if root.exists():
        ...
        relocate_classes_from_libvibe(root)
...
def relocate_classes_from_libvibe(root: Path) -> None:
    """
    Extract specific classes from _libvibe and merge into target modules.
    """
    lib_stub = find_libvibe_stub(root)
    if lib_stub is None:
        return
    ...
    # Clean up the remaining _libvibe content
...
def find_libvibe_stub(root: Path) -> Path | None:
    """
    Find the _libvibe stub file (handles both old and new formats).
    """
    # New format: _libvibe/__init__.pyi (pyo3-stub-gen >= 0.18.0)
    new_path = root / "_libvibe" / "__init__.pyi"
    if new_path.exists():
        return new_path

    # Old format: _libvibe.pyi
    old_path = root / "_libvibe.pyi"
    if old_path.exists():
        return old_path
