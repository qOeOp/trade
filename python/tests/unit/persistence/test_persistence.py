...
from vibe_trader.persistence import BarDataWrangler
from vibe_trader.persistence import DataBackendSession
from vibe_trader.persistence import OrderBookDeltaDataWrangler
from vibe_trader.persistence import OrderBookDepth10DataWrangler
from vibe_trader.persistence import ParquetDataCatalog
from vibe_trader.persistence import QuoteTickDataWrangler
from vibe_trader.persistence import StreamingFeatherWriter
from vibe_trader.persistence import TradeTickDataWrangler
from vibe_trader.persistence import VibeDataType
...
def _data_path(name: str) -> str:
    ...
    return str(TEST_DATA_DIR / "vibe" / subdir / name)
...
def test_backend_session_add_file_and_query_quotes():
    session = DataBackendSession()
    session.add_file(VibeDataType.QuoteTick, "quotes", _data_path("quotes.parquet"))
...
def test_backend_session_add_file_and_query_trades():
    session = DataBackendSession()
    session.add_file(VibeDataType.TradeTick, "trades", _data_path("trades.parquet"))
...
def test_backend_session_add_file_and_query_bars():
    session = DataBackendSession()
    session.add_file(VibeDataType.Bar, "bars", _data_path("bars.parquet"))
...
def test_backend_session_vibe_data_type_variants():
    assert VibeDataType.OrderBookDelta is not None
    assert VibeDataType.OrderBookDepth10 is not None
    assert VibeDataType.QuoteTick is not None
    assert VibeDataType.TradeTick is not None
    assert VibeDataType.Bar is not None
    assert VibeDataType.MarkPriceUpdate is not None
