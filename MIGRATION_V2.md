...
See [Installation](docs/getting_started/installation.md) for platform support and source setup.
...
- Postgres-backed deployments must run `vibe database init` before starting a v2 node. The
...
The remaining post-cutover work is tracked as repository migration work and must stay aligned with
the current Rust, PyO3, and Python consumers described above.
