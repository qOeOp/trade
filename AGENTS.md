...
- 每条根用户消息都使用 `.agents/skills/run-bounded-mission/SKILL.md`，严格经过 `Mission-Start → Contract → Plan → Build → Evaluate → Handoff → Mission-Terminate`；简单问答、机械修改和具体 skill 只能把无工作的阶段记为 `noop`，不得跳过。仓库候选默认以 merged PR 收口；无候选时 Build / Evaluate / Handoff 为 `noop`
