...
export type ArtifactBuildExecutionRuntimeV1 = {
  owner_url: string
  owner_token: string | undefined
  provider_url: string
  provider_api_key: string | undefined
...
export type ArtifactBuildExecutionRequestV1 = {
  action: Action
...
const DEFAULT_OWNER_URL = "http://rd-owner-api:8080"
const DEFAULT_PROVIDER_URL = "https://api.deepseek.com/chat/completions"
...
async function resolveS1Context(
  runtime: ArtifactBuildExecutionRuntimeV1,
...
export type ArtifactBuildPreflightV1 =
  ...
  | {
      ...
      unavailable_reason: "OWNER_CURRENT_RESEARCH_UNAVAILABLE"
      context: null
    }

export async function preflightArtifactBuildV1(
  researchRequestIdentity: string,
  runtime: ArtifactBuildExecutionRuntimeV1,
): Promise<ArtifactBuildPreflightV1> {
  if (!nonEmpty(researchRequestIdentity) || !runtime.owner_token) {
    return {
      availability: "unavailable",
      unavailable_reason: "OWNER_CURRENT_RESEARCH_UNAVAILABLE",
      context: null,
  ...
  try {
    const context = await resolveS1Context(runtime, researchRequestIdentity)
    if (context) {
      return { availability: "available", unavailable_reason: null, context }
    }
  } catch {}
  return {
    availability: "unavailable",
...
export function projectOwnerResolution(
  ...
  if (
    result.owner_result
    && typeof result.owner_result === "object"
    && !Array.isArray(result.owner_result)
    && Object.keys(result).length === 2
    && Object.hasOwn(result, "provider_invocation")
  ) {
    return {
      ...result.owner_result,
      provider_invocation: result.provider_invocation ?? null,
    }
  }
  return result
}

function ownerArtifactOperationRequest(request: Record<string, any>) {
  return {
    build_request_identity: request.build_request_identity,
    attempt_identity: request.attempt_identity,
    intent_identity: request.intent_identity,
...
export async function invocationStateDigestV1(value: Record<string, unknown>): Promise<string> {
  return providerInvocationStateDigestV1({
    schema_version: Number(value.schema_version),
    claim_identity: String(value.claim_identity),
    admission_identity: String(value.admission_identity),
    attempt_identity: String(value.attempt_identity),
    claim_digest: String(value.claim_digest),
    state: value.state as "CLAIMED" | "INVOCATION_STARTED",
    updated_at_epoch_ms: Number(value.updated_at_epoch_ms),
  })
}
...
async function generateCandidate(
  ...
  try {
    response = await runtime.fetcher(runtime.provider_url, {
      method: "POST",
      headers: {
        authorization: `Bearer ${runtime.provider_api_key}`,
        "content-type": "application/json",
      },
      body: JSON.stringify({
...
async function runOwnerOperation(
  ...
  if (!runtime.owner_token) return finish(unknown(build_request_identity, attempt_identity))
  if (action === "RESOLVE") {
    try {
  ...
  if (freshAdmissionAllowed) {
    existing = unknown(build_request_identity, attempt_identity)
  } else {
    try {
  ...
  if (!runtime.provider_api_key) {
    if (Object.keys(request).length === 0) return finish(existing)
    return finish(await fail(runtime, request, "NOT_CONFIGURED"))
  }
  ...
  try {
    generated = await generateCandidate(runtime, canonicalIntentBytes)
  } catch (error) {
    ...
    const code = error instanceof Error && ["PROVIDER_EMPTY", "CANDIDATE_MALFORMED"].includes(error.message)
      ? error.message
      : "PROVIDER_ERROR"
    return finish(await fail(runtime, request, code))
  }
...
export async function main(
  ...
  return executeArtifactBuildV1(
    ...
    {
      owner_url: process.env.RD_OWNER_API_URL || DEFAULT_OWNER_URL,
      owner_token: process.env.RD_OWNER_API_TOKEN,
      provider_url: process.env.RD_EXECUTION_AGENT_PROVIDER_URL || DEFAULT_PROVIDER_URL,
      provider_api_key: process.env.DEEPSEEK_API_KEY,
      provider_model: process.env.RD_EXECUTION_AGENT_MODEL || "deepseek-chat",
      dispatcher: "WINDMILL",
      fetcher: fetch,
    },
  )
...
export async function executeArtifactBuildV1(
  request: ArtifactBuildExecutionRequestV1,
  runtime: ArtifactBuildExecutionRuntimeV1,
) {
  const {
    action,
