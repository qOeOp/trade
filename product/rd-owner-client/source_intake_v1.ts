// Trade-owned Source Intake orchestration and strict Owner projection shared by
// the Windmill compatibility adapter and the first-party Dashboard adapter.
...
function validInterpretation(value: unknown): value is SourceInterpretationV1 {
  ...
  const alternatives = value.plausible_alternatives
  if (!validBoundedText(value.bounded_explanation)
    ...
    || !alternatives.every(validBoundedText)) return false
  return alternatives
    .slice(1)
    .every((item, index) => compareUtf8(alternatives[index], item) < 0)
}
...
export function unknownSourceIntakeProjectionV1(requestIdentity: string) {
