...
import { projectSourceIntakeJourneyV1 } from "../lib/source-intake-journey";
...
function TerminalReadback({
  ...
  terminal: SourceIntakeTerminalReadbackV1;
}) {
  const journey = projectSourceIntakeJourneyV1(projection);
  if (!journey) return null;
  return (
    <div className={styles.readback}>
      <JourneyProgress eyebrow="Intake journey" summary={journey.summary} stages={journey.stages} />
      <FactGroupGrid>
...
export function SourceIntakeReadbackWorkbench({
  ...
  const journey = projection ? projectSourceIntakeJourneyV1(projection) : null;
        ...
        <div className={styles.result} aria-live="polite">
          {status === "loading" ? (
            ...
            : status === "available" && projection?.state === "terminal" && projection.terminal
              ? <TerminalReadback projection={projection} terminal={projection.terminal} />
              : status === "available" && projection?.state === "no_verified_terminal"
                ? <div className={styles.readback}>
                  {journey ? <JourneyProgress eyebrow="Intake journey" summary={journey.summary} stages={journey.stages} /> : null}
