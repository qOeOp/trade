...
test("Artifact directory uses the shared compact read-only table surface", async () => {
  const [component, route, shell, css] = await Promise.all([
    ...
    readFile(new URL("../components/artifact-directory.module.css", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(component, /placeholder="Artifact, intent, or request"/u);
