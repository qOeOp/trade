...
{ skip: !browserAcceptance, timeout: 20 * 60_000 }, async (t) => {
  ...
  try {
    ...
    await t.test("strategy source renders the exact Owner bytes in a read-only viewer", async () => {
      ...
      // Folding shortens the document, which moves the scroller off the end; the editor only
      // renders the lines in view, so bring the end back into view before reading the gutter.
      const lastGutterNumber = `[...document.querySelectorAll('[data-slot="strategy-read-only-code"] .cm-lineNumbers .cm-gutterElement')]
        ...
        .filter(Number.isInteger).at(-1)`;
      await waitForBrowserExpressionWithRefresh(browser, `(() => {
        const scroller = document.querySelector('[data-slot="strategy-read-only-code"] .cm-scroller');
        if (scroller) scroller.scrollTop = scroller.scrollHeight;
        return ${lastGutterNumber} === ${sourceLineCount};
      })()`, { label: "code viewer document end", attempts: 1, attemptTimeoutMs: 15_000 });
    ...
    await t.test("research directory, readback and saved question all show the committed request", async () => {
      ...
      // The result column is filled by the outcome inventory, which reads every candidate
      // request through the read API after the row list arrived; wait for this row to settle.
      await waitForBrowserExpressionWithRefresh(browser,
        `[...document.querySelectorAll('table[aria-label="Research question history"] strong')]
          .some((cell) => cell.textContent === ${JSON.stringify(researchHypothesis)}
            && !/^(Checking…|Not checked)$/u.test(cell.closest('tr')?.querySelector('.status-badge')?.textContent ?? 'Checking…'))`,
        { label: "research history row", attemptTimeoutMs: 40_000, endpoints: [
          "/api/rd/historical-custodies/", "/api/rd/research/outcome-inventory/", "/api/rd/research/questions/",
          `/api/rd/research/${encodeURIComponent(researchRequestIdentity)}/`,
        ] });
      assert.deepEqual(await readBrowserValue(browser, `(() => {
      ...
      // The outcome and the question arrive from two reads, so the page is settled only once both
      // have landed; waiting on the outcome alone would sample the page mid-composition.
      await waitForBrowserExpressionWithRefresh(browser,
        `${factExpression("Result", "Decision")} !== null
          && document.querySelector('[aria-label="Research question"]') !== null`,
        { label: "research readback", endpoints: [
          `/api/rd/research/${encodeURIComponent(researchRequestIdentity)}/`,
          "/api/rd/research/questions/",
        ] });
      assert.deepEqual(await readBrowserValue(browser, `(() => ({
      ...
      await waitForBrowserExpression(browser,
        `[...document.querySelectorAll('table[aria-label="Saved research questions"] strong')]
          .some((cell) => cell.textContent === ${JSON.stringify(researchHypothesis)}
            && Object.keys(cell.closest('button') ?? {}).some((key) => key.startsWith('__reactFiber')))`,
      ...
      await waitForBrowserExpression(browser,
        `[...document.querySelectorAll('a')].some((link) => link.textContent === 'Open research record'
          && link.getAttribute('href').replace(/\\/$/u, '') === ${JSON.stringify(`/rd/research/${encodeURIComponent(researchRequestIdentity)}`)})`,
        { label: "saved question detail" });
    });

    await t.test("iteration decisions compose the formation catalog with every family timeline", async () => {
      // The directory is a composition: the catalog names the families, and one timeline read per
      ...
      const acceptanceFamily = catalog.body.families
        .find((family) => family.research.request_identity === researchRequestIdentity);
      ...
      for (const family of catalog.body.families) {
        ...
        assert.equal(timeline.body.trial_budget, family.research.trial_budget);
        assert.equal(timeline.body.consumed_trial_budget, family.research.consumed_trial_budget);
        ownerDecisions.push(...timeline.body.decisions);
      }
      const decisionLabels = {
        REPAIR_INPUTS: "Repair inputs",
        SUCCESSOR_EXPERIMENT: "Successor experiment",
        READY_FOR_SELECTION: "Ready for qualification",
        TERMINAL_STOP: "Research stopped",
      };
      const expectedLabels = [...new Set(ownerDecisions
        .map((decision) => decisionLabels[decision.outcome.outcome]))].sort();
      assert.ok(expectedLabels.every(Boolean), JSON.stringify(ownerDecisions));

      await navigate(browser, `${origin}/rd/decisions/`);
      await waitForBrowserExpressionWithRefresh(browser,
        `[...document.querySelectorAll('details p')].map((line) => line.textContent)
          .includes(${JSON.stringify(`Catalog completeness: ${catalog.body.completeness}`)})`,
        { label: "decision catalog completeness", endpoints: ["/api/rd/formation-catalog/"] });
      assert.deepEqual(await readBrowserValue(browser, `(() => {
        const panel = document.querySelector('[aria-labelledby="rd-decision-directory-title"]');
        const rows = [...document.querySelectorAll('table[aria-label="Iteration decisions"] tbody tr[aria-expanded]')];
        return {
          unavailable: panel?.querySelector('.unavailable-state details code')?.textContent ?? null,
          rows: rows.length,
          labels: [...new Set(rows.map((row) => row.querySelector('.status-badge')?.textContent))].sort(),
        };
      })()`), {
        unavailable: null,
        rows: Math.min(ownerDecisions.length, 20),
    ...
    await t.test("overview family composes the Owner reads and reports the RunStore as unavailable", async () => {
      ...
      // The committed Research is accepted, so a settled panel always counts at least one ready
      // result; anything else means the composed read has not answered yet.
      await waitForBrowserExpressionWithRefresh(browser,
        `(() => {
          const panel = document.querySelector('section[aria-label="Workspace activity"]');
          if (panel?.getAttribute('aria-busy') !== 'false') return false;
          const ready = [...panel.querySelectorAll('.compact-status-item')]
            .find((item) => item.querySelector('dt')?.textContent === 'results ready');
          return /^[1-9][0-9]*$/u.test(ready?.querySelector('dd')?.childNodes[0]?.textContent?.trim() ?? '');
        })()`,
        { label: "overview activity", attemptTimeoutMs: 40_000, endpoints: [
          "/api/rd/historical-custodies/", "/api/rd/research/outcome-inventory/",
          "/api/rd/artifacts/review-inventory/",
        ] });
      const activity = await readBrowserValue(browser, `(() => ({
        ...Object.fromEntries(
          [...document.querySelectorAll('section[aria-label="Workspace activity"] .compact-status-item')]
            .map((item) => [item.querySelector('dt')?.textContent, item.querySelector('dd')?.childNodes[0]?.textContent?.trim()]),
        ),
        scope: Object.fromEntries([...document.querySelectorAll('.panel-info-facts div')]
          .map((fact) => [fact.querySelector('dt')?.textContent, fact.querySelector('dd')?.textContent])),
      }))()`);
      // The committed Research is accepted with a frozen intent, so at least one result is
      // ready. `reviewable` counts historical build outcomes only (failed, rejected, unknown);
      // a chain that produced only successful builds legitimately reports zero of them.
      ...
      assert.match(activity.reviewable, /^[0-9]+$/u, JSON.stringify(activity));
      ...
      // Recent outcomes is a composition of three independent Owner reads over one custody cut,
      // and it reports nothing rather than a partial count when they disagree. Compare them here,
      // so a disagreement is named instead of arriving as a missing row.
      const sources = await readBrowserValue(browser, `(async () => {
        ...
        const research = custody.projection?.research ?? [];
        const questionByRequest = new Map((questions.items ?? [])
          .map((item) => [item.requestIdentity, item]));
        const outcomeIdentities = new Set((outcomes.items ?? [])
          .map((item) => item.requestIdentity));
        return {
          statuses,
          custodyResolution: custody.projection?.resolution,
          custodyCompleteness: custody.projection?.completeness,
          custodyResearchTotal: custody.projection?.researchTotal,
          custodyResearchListed: research.length,
          outcomeAvailability: outcomes.availability,
          outcomeCandidateTotal: outcomes.candidateTotal,
          outcomeScanned: outcomes.scannedCandidateCount,
          outcomeItems: (outcomes.items ?? []).length,
          questionsTotal: questions.total,
          questionsItems: (questions.items ?? []).length,
          reviewAvailability: reviews.availability,
          reviewCandidateTotal: reviews.candidateTotal,
          missingFromQuestions: research
            .filter((item) => !questionByRequest.has(item.requestIdentity))
            .map((item) => item.requestIdentity),
          questionTimeMismatch: research
            .filter((item) => questionByRequest.has(item.requestIdentity)
              && questionByRequest.get(item.requestIdentity).committedAtEpochMs !== item.committedAtEpochMs)
            .map((item) => item.requestIdentity),
      ...
      assert.deepEqual(sources.statuses, [200, 200, 200, 200], sourceReport);
      assert.equal(sources.custodyResolution, "RETRIEVED", sourceReport);
      assert.equal(sources.custodyCompleteness, "COMPLETE", sourceReport);
      assert.equal(sources.outcomeAvailability, "available", sourceReport);
      assert.deepEqual(sources.missingFromQuestions, [], sourceReport);
      assert.deepEqual(sources.questionTimeMismatch, [], sourceReport);
      assert.deepEqual(sources.missingFromOutcomes, [], sourceReport);
      assert.equal(sources.questionsTotal, sources.custodyResearchTotal, sourceReport);
      assert.equal(sources.questionsItems, sources.custodyResearchListed, sourceReport);
