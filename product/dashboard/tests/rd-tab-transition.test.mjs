...
test("every R&D top tab owns one stable card-level route header", async () => {
  ...
  assert.match(shell, /const researchDecisionDirectory = current === "\/rd\/decisions";/u);
  assert.match(shell, /const rdPlaceholderRoute = current === "\/rd\/hypotheses";/u);
  assert.match(shell, /const embedsRouteChrome = [\s\S]*researchDecisionDirectory[\s\S]*;/u);
