...
test("DataWorkspaceTable owns one reusable controlled row-detail disclosure", async () => {
  const [table, entity, css, directoryCss] = await Promise.all([
  ...
  assert.doesNotMatch(table, /scrollIntoView|scrollTop|window\.scrollBy|requestAnimationFrame/u);
  assert.doesNotMatch(entity, /scrollIntoView|scrollTop|window\.scrollBy|requestAnimationFrame/u);
