...
function unavailable(
  ...
  return {
    ...
    requestIdentity: validExploratoryReplayOpaqueIdentityV2(requestIdentity)
      ? requestIdentity : "INVALID_REQUEST_IDENTITY",
...
export async function readExploratoryReplayReadbackGatewayV1({
  ...
  if (!validExploratoryReplayOpaqueIdentityV2(requestIdentity) || !validDigest(meaningDigest)) {
    return {
      status: 400,
...
export function parseExploratoryReplayBrowserProjectionV1(
  ...
  if (value.availability === "unavailable") {
    const validRequest = validExploratoryReplayOpaqueIdentityV2(value.requestIdentity)
  ...
  if (value.availability !== "available"
    || !validExploratoryReplayOpaqueIdentityV2(value.requestIdentity)
    ...
    || !validUnsignedDecimal(value.request.deterministicSeed)
    || !record(value.custody) || !exactKeys(value.custody, [
      "receiptIdentity", "sealDigest", "committedAt", "ownerObservedAt",
    ]) || !validExploratoryReplayOpaqueIdentityV2(value.custody.receiptIdentity)
    || !validDigest(value.custody.sealDigest)
