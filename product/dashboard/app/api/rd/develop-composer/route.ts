...
} from "@/lib/develop-composer-action-contract";
import { handleDevelopComposerActionV2 } from "@/lib/dashboard-operation-handler";
import {
...
export async function POST(request: Request) {
  ...
  if (!Number.isSafeInteger(declaredLength) || declaredLength < 0
    || declaredLength > MAX_REQUEST_BYTES) return unavailable("EXECUTION_REQUEST_INVALID", 400);
  let body: DevelopComposerRunRequestV2;
  try {
    const text = await request.text();
    if (new TextEncoder().encode(text).byteLength > MAX_REQUEST_BYTES) {
      return unavailable("EXECUTION_REQUEST_INVALID", 400);
    }
    const canonical = canonicalDevelopComposerRunRequestV2(JSON.parse(text));
