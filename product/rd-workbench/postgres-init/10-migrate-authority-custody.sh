...
BEGIN
  IF EXISTS (
    ...
    OR EXISTS (SELECT 1 FROM pg_catalog.pg_proc routine WHERE routine.oid IN (pg_catalog.to_regprocedure('public.product_edge_reject_admission_event_mutation_v1()'),pg_catalog.to_regprocedure('public.product_edge_reject_admission_assignment_mutation_v1()')) AND (routine.prorettype<>pg_catalog.to_regtype('trigger') OR routine.prolang<>(SELECT oid FROM pg_catalog.pg_language WHERE lanname='plpgsql') OR routine.pronargs<>0 OR routine.proisstrict OR routine.prosecdef OR routine.provolatile<>'v' OR routine.proparallel<>'u' OR routine.proconfig IS NOT NULL OR pg_catalog.md5(routine.prosrc)<>CASE routine.proname WHEN 'product_edge_reject_admission_event_mutation_v1' THEN '014d48cfd5b330c37330996eef7e211e' ELSE '143548e90171aa95dc9ae403d7ca6ee1' END)) THEN RAISE EXCEPTION 'R&D/Product Edge closed relation or programmable dependency manifest mismatch'; END IF;
  FOR object IN
