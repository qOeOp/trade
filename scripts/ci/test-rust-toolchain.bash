...
expect_target_success() {
  local config="$1"
  local expected="$2"

  printf '%s\n' "$config" > "${test_root}/rust-toolchain.toml"
  actual="$(bash "${test_root}/scripts/rust-toolchain.sh" --target)"
  if [[ "$actual" != "$expected" ]]; then
    echo "Expected Rust target '$expected', was '$actual'" >&2
    exit 1
  fi
}
...
expect_target_failure() {
  ...
  printf '%s\n' "$config" > "${test_root}/rust-toolchain.toml"
  if bash "${test_root}/scripts/rust-toolchain.sh" --target > "${test_root}/output" 2>&1; then
    echo "Expected Rust target config to fail:" >&2
    printf '%s\n' "$config" >&2
    exit 1
  fi
  grep -Fq "[toolchain].targets must contain exactly one target" "${test_root}/output"
}
...
expect_target_success $'[toolchain]\nchannel = "1.97.1"\ntargets = ["wasm32v1-none"]' "wasm32v1-none"
expect_target_failure $'[toolchain]\nchannel = "1.97.1"'
expect_target_failure $'[toolchain]\nchannel = "1.97.1"\ntargets = ["wasm32v1-none", "x86_64-unknown-linux-gnu"]'
