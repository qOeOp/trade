...
readonly -A unselected_reason=(
  ...
  ["exact_origin_terminal_legacy_is_read_only_and_nonterminal_blocks_activation"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ...
  ["migration_materializes_private_runtime_kernel_request_custody"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ["official_holdout_integrity_probe_is_deterministic"]="frozen external dataset; the chain provisions a database, not a corpus"
  ["opaque_legacy_success_is_classified_but_never_promoted"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ...
  ["postgres_sealed_success_atomically_reads_back_distinct_time_heads_and_rejects_mismatches"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ["readback_accepts_only_declared_exact_acl_topologies"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ...
  ["stored_attempt_catalog_is_exactly_classifiable_without_writes"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ["transaction_bound_read_rejects_wrong_owner_acl_and_stale_custody"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
  ["transaction_bound_read_uses_the_borrowed_backend_locks_and_writes_nothing"]="addable: the chain compiles it and nothing selects it; awaiting its own chain placement"
