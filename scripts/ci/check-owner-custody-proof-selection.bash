...
readonly -A unselected_reason=(
  ...
  ["composer_unlogged_drift_is_unavailable_to_migration_and_runtime"]="needs sealed-source-intake-composer-acceptance, which the chain's archive does not enable; the feature compiles clean now, so enabling it is a question of whether these four pass"
  ...
  ["forged_v3_admission_fails_without_replay_transition_or_outbox_write"]="needs sealed-source-intake-composer-acceptance, which the chain's archive does not enable; the feature compiles clean now, so enabling it is a question of whether these four pass"
  ...
  ["postgres_every_transaction_write_boundary_fault_leaves_zero_positive_rows"]="needs sealed-source-intake-composer-acceptance, which the chain's archive does not enable; the feature compiles clean now, so enabling it is a question of whether these four pass"
  ...
  ["sealed_run_and_restarted_resolve_return_the_same_public_receipt"]="needs sealed-source-intake-composer-acceptance, which the chain's archive does not enable; the feature compiles clean now, so enabling it is a question of whether these four pass"
