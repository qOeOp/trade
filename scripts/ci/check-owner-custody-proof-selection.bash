...
readonly -A unselected_reason=(
  ...
  ["composer_unlogged_drift_is_unavailable_to_migration_and_runtime"]="compiled by the ordered chain since sealed-source-intake-composer-acceptance joined its union, but not selected and never run. The earlier reason - the chain refusing rd_research_view_transitions_v3 as having incompatible custody - no longer holds: that table's runtime reader now arrives at cutover like every other table's, and 10-migrate installs the Composer acceptance functions when the union asks for them. Selecting it is its own change"
  ...
  ["sealed_run_and_restarted_resolve_return_the_same_public_receipt"]="compiled by the ordered chain since sealed-source-intake-composer-acceptance joined its union, but not selected and never run. The earlier reason - the chain refusing rd_research_view_transitions_v3 as having incompatible custody - no longer holds: that table's runtime reader now arrives at cutover like every other table's, and 10-migrate installs the Composer acceptance functions when the union asks for them. Selecting it is its own change"
