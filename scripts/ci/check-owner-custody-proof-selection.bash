...
readonly -A unselected_reason=(
  ...
  ["postgres_successor_is_append_only_replay_safe_and_preserves_history"]="uses DedicatedPostgresTestDatabase, whose marker validation requires every role to be named vibe_test_role_*. That is the dedicated per-Owner harness naming (crates/data/tests/run_market_data_owner_postgres.bash creates vibe_test_role_market_data_owner); the ordered chain exports its canonical role names, so admission refuses with ExpectedIdentityMismatch before the proof runs. Joining this chain would need both a canonical topology and no materialization, since that store is past the cutover"
  ["postgres_history_mutations_fail_closed_and_restore_exactly"]="uses DedicatedPostgresTestDatabase, whose marker validation requires every role to be named vibe_test_role_*. That is the dedicated per-Owner harness naming (crates/data/tests/run_market_data_owner_postgres.bash creates vibe_test_role_market_data_owner); the ordered chain exports its canonical role names, so admission refuses with ExpectedIdentityMismatch before the proof runs. Joining this chain would need both a canonical topology and no materialization, since that store is past the cutover"
  ["shared_resolver_blocks_revoke_update_lock"]="uses DedicatedPostgresTestDatabase, whose marker validation requires every role to be named vibe_test_role_*. That is the dedicated per-Owner harness naming (crates/data/tests/run_market_data_owner_postgres.bash creates vibe_test_role_market_data_owner); the ordered chain exports its canonical role names, so admission refuses with ExpectedIdentityMismatch before the proof runs. Joining this chain would need both a canonical topology and no materialization, since that store is past the cutover"
  ["select_only_consumer_resolve_serializes_with_revoke"]="uses DedicatedPostgresTestDatabase, whose marker validation requires every role to be named vibe_test_role_*. That is the dedicated per-Owner harness naming (crates/data/tests/run_market_data_owner_postgres.bash creates vibe_test_role_market_data_owner); the ordered chain exports its canonical role names, so admission refuses with ExpectedIdentityMismatch before the proof runs. Joining this chain would need both a canonical topology and no materialization, since that store is past the cutover"
