...
for file in $changed_files; do
  if [[ "$file" =~ ^crates/adapters/([^/]+)/ ]]; then
    pkg="vibe-${BASH_REMATCH[1]}"
    pkg="${pkg//_/-}"
  elif [[ "$file" =~ ^crates/persistence/macros/ ]]; then
    pkg="vibe-persistence-macros"
...
# When 'defi' is enabled on vibe-common, Cargo feature unification adds the
# DeFi variant to DataEvent for all consumers. vibe-live matches on DataEvent
