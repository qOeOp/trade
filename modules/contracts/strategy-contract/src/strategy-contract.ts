...
interface StrategyContractCandidateInput {
  candidateId: string
  description?: string
  family?: string
...
interface StrategyContractCompiled {
  ...
  candidate?: StrategyContractCandidateInput
...
function candidateFromStrategyContract(path: string, candidateParamOverrides: JSONRecord = {}): StrategyContractCandidateInput {
  const compiled = compileStrategyContract(path, candidateParamOverrides)
  if (compiled.engine !== "rnd_family_v1" || !compiled.candidate) {
    throw new Error("strategy contract requires a rnd_family_v1 engine")
  }
...
export {
  ...
  parseYamlSubset,
  type StrategyContractCandidateInput,
