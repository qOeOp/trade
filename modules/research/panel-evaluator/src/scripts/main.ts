...
export function run(argv: string[]): JSONRecord {
  ...
  try {
    ...
    return successResponse(runStrategyPanelRnd(strategyPanelRndInputFromJson(parseArgs(argv).input)))
  } catch (error) {
...
function successResponse(data: unknown): JSONRecord {
  return { ok: true, schema_version: "panel-evaluator.script-response.v1", data }
}

function errorResponse(error: unknown): JSONRecord {
  const message = error instanceof Error ? error.message : String(error)
  return { ok: false, schema_version: "panel-evaluator.script-response.v1", error: message }
}
