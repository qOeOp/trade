...
export function reduceFlow(dbPath: string, chainId: string): JSONRecord {
  return asRecord(runFlowProjectorSync(["--reduce-flow", "--db", dbPath, "--chain-id", chainId]))
}

export function readLatestSlowObserve(dbPath: string, chainId: string): JSONRecord | null {
  const data = runFlowProjectorSync(["--latest-slow-observe", "--db", dbPath, "--chain-id", chainId])
