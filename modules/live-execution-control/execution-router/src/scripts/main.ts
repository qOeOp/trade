...
export function run(argv: string[]): JSONRecord {
  try {
    ...
    return {
      ok: true,
      schema_version: "execution-router.result.v1",
      data: buildExecutionCommandSpec(input),
    }
  } catch (error) {
    return { ok: false, schema_version: "execution-router.result.v1", error: error instanceof Error ? error.message : String(error) }
  }
}
