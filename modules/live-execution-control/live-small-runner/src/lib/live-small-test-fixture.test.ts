...
import { appendPlanEvent, readFlowEvents, readLatestOrderFill } from "../../../../portfolio-execution-state/event-store/src/lib/event-store"
...
export function createStateRuntime(
  db: Database,
  flowStateReader: (chainId: string) => JSONRecord,
): ExecutionStateRuntime {
  return {
    eventReader: (_dbPath, chainId) => readFlowEvents(db, chainId) as unknown as JSONRecord[],
...
export function successTool(data: JSONRecord): Awaited<ReturnType<Runner>> {
  return { ok: true, data: { ok: true, data }, stdout: "{}", stderr: "" }
}
