...
export async function run(argv: string[]): Promise<JSONRecord> {
  ...
  try {
    ...
    return successResponse(await runLiveSmall(config.dbPath, config.input, config.yes))
  } catch (error) {
...
function successResponse(data: unknown): JSONRecord {
  return { ok: true, schema_version: "live-small-runner.script-response.v1", data }
}

function errorResponse(error: unknown): JSONRecord {
  const message = error instanceof Error ? error.message : String(error)
  return { ok: false, schema_version: "live-small-runner.script-response.v1", error: message }
}
