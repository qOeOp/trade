...
- GitHub lifecycle worktrees outside the managed run/slot roots may be removed only when immutable `src/scripts/worktree-cleanup.ts` created both the worktree and branch, bound its owner commit, then refreshed the same generation after the final commit. Removal compares tracked raw bytes without clean filters and refuses tracked, untracked, ignored, special-entry, empty-directory, submodule, content-hash, executable-mode, or live-use ambiguity. It uses Git non-force without widening the manager's run/slot GC. A partially failed creation is preserved for explicit recovery; the owner does not guess how to roll it back.
