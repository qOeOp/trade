...
export async function runSlowTrackWorkflowDryRun(input: SlowTrackWorkflowInput): Promise<JSONRecord> {
  ...
  const [accountSnapshot, marketScan] = await Promise.all([
    callTool(runner, ["bun", "src/scripts/main.ts", "--timeout", "10"], join(input.repoRoot, "modules/binance/account-snapshot")),
    callTool(
      runner,
      ["bun", "src/scripts/main.ts", "--direction", "both", "--limit-per-side", String(candidateLimit)],
      join(input.repoRoot, "modules/binance/market-scan"),
    ),
  ])
...
async function runTechnicalAnalysis(
  ...
  const ohlcvToolDir = join(input.repoRoot, "modules/ohlcv-fetch")
  ...
  const indicators = await callTool(
    ...
    join(input.repoRoot, "modules/analytics/tech-indicators"),
  )
  if (!indicators.ok) {
    return {
      ...indicators,
...
async function fetchSymbolSnapshots(
  ...
  const entries = await Promise.all(unique.map(async (symbol) => {
    const result = await callTool(
      ...
      join(repoRoot, "modules/binance/symbol-snapshot"),
    )
