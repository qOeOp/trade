...
test("slow track workflow dry-run builds real watchlist without live action", async () => {
  ...
  const runner: Runner = async (command, options) => {
    ...
    if (options?.cwd?.endsWith("analytics/tech-indicators")) {
      return jsonOk({
  ...
  try {
    ...
    const indicatorCall = calls.find((call) => call.cwd?.endsWith("analytics/tech-indicators") && call.command.includes("--manifest"))
