...
| `research/strategy-rd` | OHLCV manifest、market feature artifact、candidate JSON、strategy contract、R&D state | R&D report、R&D state update、gated draft candidate、catalog metadata | 策略研发、forward holdout、R&D learning memory | 写 `trade.db`、触发 Binance、策略升格、单策略 replay CLI、latest signal CLI、panel CLI、data split CLI、benchmark/calibration/funding governance CLI、strategy contract compile/lint CLI |
