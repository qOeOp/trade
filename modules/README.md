...
| `research/candidate-batch` | OHLCV manifest、candidate JSON、optional feature report | batch report | 单批候选评估、negative controls、failure summary | artifact 写入、RD memory、review、`trade.db` |
...
| `research/strategy-rd` | OHLCV manifest、market feature artifact、candidate JSON、strategy contract、R&D state path | R&D report、optional R&D state writeback、gated draft candidate、catalog metadata | loop / campaign artifact writeback | 写 `trade.db`、触发 Binance、策略升格、candidate batch、forward tracker、RD state init/read/update/plan_next、supervisor CLI、单策略 replay CLI、latest signal CLI、panel CLI、data split CLI、benchmark/calibration/funding governance CLI、strategy contract compile/lint CLI |
