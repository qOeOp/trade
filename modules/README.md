...
| `research/strategy-rd` | OHLCV manifest、market feature artifact、candidate JSON、strategy contract、R&D state path | R&D report、optional R&D state writeback、gated draft candidate、catalog metadata | 策略研发、campaign、forward holdout | 写 `trade.db`、触发 Binance、策略升格、RD state init/read/update/plan_next、supervisor CLI、单策略 replay CLI、latest signal CLI、panel CLI、data split CLI、benchmark/calibration/funding governance CLI、strategy contract compile/lint CLI |
