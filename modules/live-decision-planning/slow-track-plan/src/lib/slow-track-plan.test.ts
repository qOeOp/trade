...
test("slow track workflow dry-run builds real watchlist without live action", async () => {
  ...
  const runner: Runner = async (command, options) => {
    ...
    if (options?.cwd?.endsWith("market-data-products/binance-read/market-scan")) {
      return jsonOk({
    ...
    if (options?.cwd?.endsWith("market-data-products/tech-indicators")) {
      return jsonOk({
  ...
  try {
    ...
    assert.equal(ohlcvCall?.command[ohlcvCall.command.indexOf("--ohlcv-db") + 1], join(dataDir, "ohlcv.db"))
