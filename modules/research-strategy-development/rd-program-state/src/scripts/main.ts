...
export function run(argv: string[]): JSONRecord {
  ...
  try {
    ...
    return successResponse(runRdProgramStateCommand({ path: config.statePath, input: config.input, catalogDbPath: config.catalogDbPath }))
  } catch (error) {
...
function successResponse(data: unknown): JSONRecord {
  return { ok: true, schema_version: "rd-program-state.script-response.v1", data }
}

function errorResponse(error: unknown): JSONRecord {
  const message = error instanceof Error ? error.message : String(error)
  return { ok: false, schema_version: "rd-program-state.script-response.v1", error: message }
}

function printHelp(): void {
  console.log(`Usage:
  bun src/scripts/main.ts --state ./data/rd/program.json --json '{"action":"plan_next"}'
`)
}
