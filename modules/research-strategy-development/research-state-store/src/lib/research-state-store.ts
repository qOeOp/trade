...
export function ensureResearchStateSchema(db: Database): void {
  ...
  db.run("CREATE INDEX IF NOT EXISTS idx_rd_trial_program_time ON rd_trial(program_id, created_at DESC)")
}
...
export function recordRdTrial(db: Database, trial: RdTrial): void {
  ...
  db.query(`
    INSERT INTO rd_trial(trial_id, program_id, hypothesis_id, run_id, status, result_ref, trial_count, created_at, result_json)
