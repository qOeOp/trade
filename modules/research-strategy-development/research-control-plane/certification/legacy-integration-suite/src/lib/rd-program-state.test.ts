...
test("rd program state command plans the next campaign from the hypothesis queue", () => {
  ...
  try {
    ...
    assert.equal(backlog.doc_ref, "docs/rd-strategy-universe-design.md")
...
test("rd program state next plan blocks when no active queue can run", () => {
  ...
  try {
    ...
    assert.equal(asRecord(blocked.next_plan?.strategy_universe_backlog).doc_ref, "docs/rd-strategy-universe-design.md")
