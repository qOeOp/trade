...
test("rd program state command plans the next campaign from the hypothesis queue", () => {
  ...
  try {
    ...
    assert.equal(backlog.doc_ref, "docs/research/strategy/rd-strategy-universe-design.md")
    assert.equal(backlog.machine_backlog_ref, "docs/research/strategy/strategy-universe-family-backlog.json")
    assert.equal(backlog.p0_certificate_ref, "docs/research/strategy/strategy-universe-p0-family-certificates.json")
...
test("rd program state next plan blocks when no active queue can run", () => {
  ...
  try {
    ...
    assert.equal(asRecord(blocked.next_plan?.strategy_universe_backlog).doc_ref, "docs/research/strategy/rd-strategy-universe-design.md")
...
function readSchema(name: string): JSONRecord {
  if (name === "rd-program-state-result") {
    return JSON.parse(readFileSync(new URL("../../../../program-control/src/schemas/rd-program-state-result.schema.json", import.meta.url), "utf8")) as JSONRecord
  }
