...
function parseArgs(argv: string[]): {
  ...
  if (argv.length !== 2 || argv[0] !== "--json") {
    throw new Error("Reviewer Agent cycle requires --json '<payload>'")
  }
  ...
  return {
    ...
    deadline_at: utc(
      string(value.deadline_at)
        || new Date(Date.parse(requestedAt) + 15 * 60_000).toISOString(),
      "deadline_at",
    ),
...
function assertInside(root: string, path: string): void {
  if (path !== root && !path.startsWith(`${root}${sep}`)) {
    throw new Error("Reviewer Agent DB escaped data root")
  }
}
