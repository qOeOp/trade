...
export function hashIdentityPayload(value: unknown): string {
  return createHash("sha256")
    .update(canonicalizeIdentityPayload(value), "utf8")
    .digest("hex")
}
...
function canonical(value: unknown): string {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      throw new Error("identity payload numbers must be finite")
    }
  ...
  if (typeof value === "object") {
    ...
    if (new Set(entries.map((entry) => entry.normalized)).size
        !== entries.length) {
      throw new Error(
        "identity payload contains keys that collide after NFC normalization",
      )
    }
    return `{${entries.map((entry) =>
      `${JSON.stringify(entry.normalized)}:${canonical(record[entry.source])}`
    ).join(",")}}`
  }
