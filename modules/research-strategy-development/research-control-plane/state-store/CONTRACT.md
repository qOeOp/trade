...
- Expose a read-only Current Attempt Lease Observation Receipt v1 from one SQLite transaction keyed by Trial. Only the unique `claimed|running` Lease inside `[heartbeat_at, lease_expires_at)` can produce a receipt；terminal/missing/early/expired observations fail closed. The receipt is deterministic and self-hashed but not persisted, and caller-supplied UTC is not an external clock attestation.
