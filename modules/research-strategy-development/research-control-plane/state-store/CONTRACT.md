...
- Own append-only Replay Checkpoint Receipt v1 registration. A receipt is accepted only inside the exact active Attempt lease generation, binds the immutable versioned diagnostic commit/payload hashes, and advances `next_source_offset` monotonically. Repeating an already committed receipt is idempotent even after Attempt expiry; new evidence from a stale or terminal lease is rejected.
