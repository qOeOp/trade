...
function writeRdProgramState(locator: string, state: RdProgramState, _catalogDbPath?: string, dbPath = DEFAULT_RD_STATE_DB): { ref: string; db_path: string } {
  ...
  try {
    ...
    const universeCount = (db.query("SELECT COUNT(*) AS count FROM rd_universe_node").get() as { count: number }).count
    if (universeCount === 0) seedDefaultResearchControlPlane(db, state.updated_at)
...
function runRdProgramStateCommand(input: RdProgramStateCommandInput): RdProgramStateCommandResult {
  ...
  if (action === "plan_next") {
    const state = readRdProgramState(stateRef, dbPath)
    const db = new Database(dbPath, { readonly: true })
    let controlPlaneContext: JSONRecord
    try {
      controlPlaneContext = readPlannerControlPlaneContext(db)
    } finally {
      db.close()
    }
    return {
      ...commandResult("plan_next", stateRef, state),
      next_plan: buildRdSupervisorNextPlan(state, stateRef, {
        ...input.input, rd_state_db: dbPath, control_plane_context: controlPlaneContext,
      }),
    }
  }
...
function safeID(value: string): string {
  return value.trim().toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+/, "").replace(/-+$/, "") || "rd-program"
}
