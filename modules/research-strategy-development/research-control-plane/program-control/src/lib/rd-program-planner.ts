...
function buildRdSupervisorNextPlan(state: RdProgramState, statePath: string, input: JSONRecord = {}): RdSupervisorNextPlan {
  ...
  const controlPlaneContext = asRecord(input.control_plane_context)
  const strategyUniverseBacklog = buildStrategyUniverseBacklog(controlPlaneContext)
  const base = {
    ...
    scout_subagent_plan: buildScoutSubagentPlan(state, statePath, null, controlPlaneContext),
  ...
  return {
    ...
    scout_subagent_plan: buildScoutSubagentPlan(state, statePath, hypothesis, controlPlaneContext),
  }
}

function buildScoutSubagentPlan(
  state: RdProgramState, statePath: string, hypothesis: JSONRecord | null, controlPlaneContext: JSONRecord,
): JSONRecord {
  ...
  return {
    ...
    state_ref: statePath,
    control_plane_context: controlPlaneContext,
    ...
    scouts: [
      {
        ...
        agent_type: "explorer",
        purpose: "Map the selected hypothesis or empty queue to the RD strategy universe design, then propose the next edge, mechanism family, data surface, and implementation gap.",
        inputs: ["control_plane_context.active_canonicals", "control_plane_context.capabilities", "selected_hypothesis", "rd_program_state.universe_lessons", "rd_program_state.rejected_mechanisms"],
...
function buildStrategyUniverseBacklog(controlPlaneContext: JSONRecord): JSONRecord {
  ...
  return {
    source_of_truth: "research_state_store.read_planning_context",
    control_plane_context: controlPlaneContext,
    doc_ref: docRef,
...
function commonRndPayloadFields(hypothesis: JSONRecord, input: JSONRecord, remaining: RdProgramBudget): JSONRecord {
  ...
  return compactRecord({
    ...
    control_plane: optionalRecord(hypothesis.control_plane),
