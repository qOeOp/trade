...
function admission(): ForwardAdmissionRequest {
  const authorization = {
  ...
  return {
    ...
    frozen_at: "2026-07-14T08:00:00Z", data_watermark: "2026-07-14T20:00:00Z", forward_dataset_hash: HASH,
    ...
    replay_request: {
      ...
      identity_hash_policy_version: "identity-v1", experiment_contract_hash: HASH, dataset_manifest_ref: "dataset://forward", dataset_hash: HASH,
...
test("Forward rejects pre-freeze data instead of silently backfilling", () => {
  const result = runForwardEvidenceSession({
    admission: admission(),
    bars: [{ open_time: "2026-07-14T04:00:00Z", close_time: "2026-07-14T08:00:00Z", open: 100, high: 101, low: 99, close: 100, volume: 10, closed: true }],
  })
