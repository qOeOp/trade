...
import { CONTROL_PLANE_IDENTITY_SCHEMA_VERSION, REPLAY_ATTEMPT_LEASE_SCHEMA_VERSION, REPLAY_INSTRUMENT_STATUS_PROVIDER_CERTIFICATION_SCHEMA_VERSION, TRIAL_RESERVATION_SNAPSHOT_SCHEMA_VERSION, createReplayInstrumentStatusProviderCertificationSnapshot, hashTrialReservationSnapshot, type ReplayAttemptLeaseSnapshot, type TrialReservationSnapshot } from "../../../../../research-control-plane/contracts/src/lib/control-plane-contracts"
...
test("legacy replay runner adapts Trial-bound requests to Replay Execution Plane", () => {
  ...
  const certification = providerCertification(hash)
  ...
  const statusSnapshot = { schema_version: REPLAY_INSTRUMENT_STATUS_SNAPSHOT_SCHEMA_VERSION, snapshot_id: "status-1", venue_id: "binance-usdm", symbol: "BTCUSDT", status: "trading" as const, effective_at: "2020-01-01T00:00:00Z", valid_until: null, observed_at: "2026-07-13T00:00:00Z", source_ref: "fixture:status-1", source_hash: hash }
  const statusProvenance = createReplayInstrumentStatusProvenance({ producer_domain: "market-data-products", producer_id: "fixture-status-producer", producer_version: "v1", producer_build_hash: hash, provider_capability_hash: hash, provider_certification_ref: certification.certification_ref, provider_certification_hash: certification.certification_hash, source_owner: "binance-usdm", source_kind: "venue_status_event_archive", normalization_policy_version: "fixture-status-normalization-v1", normalization_policy_hash: hash, completeness: "complete_history", coverage_start: "2020-01-01T00:00:00Z", coverage_end: "2026-07-15T00:00:00Z", source_observed_through: "2026-07-13T00:00:00Z", produced_at: "2026-07-13T00:00:00Z", source_ref: "fixture:status-source", source_hash: hash, source_record_count: 1, status_epochs: [statusSnapshot] })
  ...
  const executionRequest = {
    ...
    instrument_status_schedule_hash: canonicalHash([statusSnapshot]),
...
function authorize(request: ReplayExecutionRequest): TrialReservationSnapshot {
  const reservation: TrialReservationSnapshot = {
    ...
    bindings: { replay_idempotency_key: request.idempotency_key, execution_spec_hash: replayExecutionSpecHash(request), dataset_manifest_ref: request.dataset_manifest_ref, dataset_hash: request.dataset_hash, liquidity_capacity_attestation_hash: request.order.entry_execution.order_type === "limit" ? request.order.entry_execution.liquidity_capacity_attestation_hash : null, supplemental_facts_hash: request.supplemental_facts_hash, supplemental_requirement_set_hash: request.supplemental_requirement_set_hash, venue_risk_policy_schedule_hash: request.venue_risk_policy_schedule_hash, instrument_spec_schedule_hash: request.instrument_spec_schedule_hash, instrument_status_schedule_hash: request.instrument_status_schedule_hash, instrument_status_provenance_hash: request.instrument_status_provenance_hash, instrument_status_provider_capability_hash: request.instrument_status_provider_capability_hash, instrument_status_provider_certification_hash: request.instrument_status_provider_certification_hash, harness_hash: request.harness_hash, assumptions_hash: request.assumptions_hash, cost_policy_hash: canonicalHash(request.cost_policy), margin_policy_hash: canonicalHash(request.margin_policy), simulator_policy_version: request.simulator_policy.version, execution_mode: "step" }, instrument_status_provider_certification: providerCertification(request.instrument_status_provider_capability_hash), required_capabilities: [...REPLAY_CERTIFIED_CAPABILITIES],
  }
