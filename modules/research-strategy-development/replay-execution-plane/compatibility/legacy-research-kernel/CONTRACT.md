...
- Legacy strategy replay orchestration、trade fill ordering、cost and funding semantics。
- Legacy result shell、trade facts and next-open fill materialization。
- Legacy contracts/data/decision/feature/evaluation/provenance/identity/order-lane 仅兼容重导出。
...
- 所有直接消费者迁出 legacy `ReplayResult`、fill/signal semantics；contracts/data/decision/feature/evaluation/provenance/identity/order-lane consumers 独立退役。
