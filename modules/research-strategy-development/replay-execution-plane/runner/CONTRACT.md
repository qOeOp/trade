...
Artifact Manifest v35 is the success commit marker. Runner 对 Result v33/OHLCV Evidence v3 首次发布与幂等读取执行两层校验：先绑定 SourceEvent、protection OrderEvent 与 terminal Fill identity，再从 Request、Dataset accounting 和 entry Fill 重算 path economics，并要求 canonical price/fee 等于实际 terminal Fill。Fingerprint 绑定完整 evidence collection；Run Outcome v31 只有 completed 可以携带 Result/Artifact。Checkpoint 保持 v16，因为 envelope 是恢复后重建的派生证据。
