...
import {
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_ARTIFACT_MANIFEST_SCHEMA_VERSION,
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_ARTIFACT_ROLES,
  REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_OUTCOME_SCHEMA_VERSION,
  ...
  assertReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome,
  replayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifestHash,
  replayPortfolioProtectiveTakeProfitCancelTerminalOutcomeHash,
  type ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest,
  type ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactRole,
  type ReplayPortfolioProtectiveTakeProfitCancelTerminalEvidence,
  type ReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome,
} from "../../../contracts/src/lib/replay-portfolio-protective-take-profit-cancel-terminal-contracts"
import {
  executeReplayPortfolioProtectiveTakeProfitCancelTerminal,
  type ReplayPortfolioProtectiveTakeProfitCancelTerminalLane,
} from "../../../engine/src/lib/replay-portfolio-protective-take-profit-cancel-terminal-engine"
...
const MANIFEST = "portfolio-protective-take-profit-cancel-terminal-artifact-manifest.json"
const NAMES: Record<ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactRole, string> = {
...
export interface ReplayPortfolioProtectiveTakeProfitCancelTerminalRunInput
  extends ReplayPortfolioProtectiveTerminalRunInput {
  execute_cancel_terminal?: typeof executeReplayPortfolioProtectiveTakeProfitCancelTerminal
}

export function runReplayPortfolioProtectiveTakeProfitCancelTerminal(
  input: ReplayPortfolioProtectiveTakeProfitCancelTerminalRunInput,
): ReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome {
  const projected = { ...input, allow_predeclared_take_profit_cancel_projection: true as const }
  ...
  let lanes: ReplayPortfolioProtectiveTakeProfitCancelTerminalLane[]
  let evidence: ReplayPortfolioProtectiveTakeProfitCancelTerminalEvidence
  try {
    lanes = materializeReplayPortfolioProtectiveTakeProfitCancelTerminalLanes(input)
    evidence = (input.execute_cancel_terminal ?? executeReplayPortfolioProtectiveTakeProfitCancelTerminal)({
  ...
  try {
    ...
    const body: Omit<ReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome, "outcome_hash"> = {
      schema_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_OUTCOME_SCHEMA_VERSION,
    ...
    const outcome = { ...body, outcome_hash: replayPortfolioProtectiveTakeProfitCancelTerminalOutcomeHash(body) }
    assertReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome(outcome)
    return outcome
  } catch (error) {
...
export function materializeReplayPortfolioProtectiveTakeProfitCancelTerminalLanes(
  input: Pick<ReplayPortfolioProtectiveTakeProfitCancelTerminalRunInput, "risk_plan" | "lanes">,
): ReplayPortfolioProtectiveTakeProfitCancelTerminalLane[] {
  return materializeReplayPortfolioProtectiveMutationTerminalLanes(input, {
    mutation_effect: "authorized_take_profit_cancel", select_intent: (entry) => entry.authorized_take_profit_cancel,
    mutation_key: "cancel",
  })
}

export function readReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactEvidence(
  input: ReplayPortfolioProtectiveTakeProfitCancelTerminalRunInput,
  expectedEvidence: ReplayPortfolioProtectiveTakeProfitCancelTerminalEvidence,
  expectedManifest: ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest,
): {
  ...
  assertReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest(manifest)
  if (canonicalHash(manifest) !== canonicalHash(expectedManifest)) {
    throw new Error("Cancel Terminal Artifact manifest read drift")
  }
  const values = new Map<ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactRole, unknown>()
  ...
  const evidence = values.get("cancel_terminal_evidence") as
    ReplayPortfolioProtectiveTakeProfitCancelTerminalEvidence | undefined
...
export interface ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactPublishInput {
  ...
  evidence: ReplayPortfolioProtectiveTakeProfitCancelTerminalEvidence
  authority_frozen_at: string
  artifact_store: ReplayPortfolioProtectiveTakeProfitCancelTerminalRunInput["artifact_store"]
}
export function publishReplayPortfolioProtectiveTakeProfitCancelTerminalArtifact(
  input: ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactPublishInput,
) {
  const sourceManifest = input.source_manifest; const sourceEvidence = input.source_evidence
  const evidence = input.evidence
  const values: Record<ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactRole, unknown> = {
  ...
  const files = REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_ARTIFACT_ROLES.map((role) => {
    const name = NAMES[role]
    return { role, name, ...namespace.writeImmutable(name, encode(values[role])) }
  })
  const body: Omit<ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest, "manifest_hash"> = {
    schema_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_ARTIFACT_MANIFEST_SCHEMA_VERSION,
    artifact_id: `replay-portfolio-protective-take-profit-cancel-terminal:${evidence.fingerprint.fingerprint_hash}`,
    ...
    completeness: { authoritative_result: true,
      required_roles: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_ARTIFACT_ROLES,
  ...
  const manifest = { ...body, manifest_hash: replayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifestHash(body) }
  assertReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest(manifest)
...
function readCommitted(namespace: ReplayArtifactNamespace, sourceManifestHash: string, sourceEvidenceHash: string,
  evidence: ReplayPortfolioProtectiveTakeProfitCancelTerminalEvidence,
  values: Record<ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactRole, unknown>) {
  const manifest = JSON.parse(new TextDecoder().decode(namespace.read(MANIFEST).bytes)) as
    ReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest
  assertReplayPortfolioProtectiveTakeProfitCancelTerminalArtifactManifest(manifest)
  if (manifest.cancel_terminal_evidence_hash !== evidence.evidence_hash
    ...
    throw new Error("Portfolio take-profit cancel terminal manifest identity drift")
  }
  for (const file of manifest.files) {
    const read = namespace.read(file.name)
    if (file.name !== NAMES[file.role] || file.ref !== read.ref || file.sha256 !== sha256(read.bytes)
        || canonicalHash(JSON.parse(new TextDecoder().decode(read.bytes))) !== canonicalHash(values[file.role])) {
      throw new Error("Portfolio take-profit cancel terminal Artifact payload binding mismatch")
    }
  }
  return manifest
}

function failed(input: ReplayPortfolioProtectiveTakeProfitCancelTerminalRunInput,
  code: NonNullable<ReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome["failure"]>["code"], error: unknown) {
  const body: Omit<ReplayPortfolioProtectiveTakeProfitCancelTerminalOutcome, "outcome_hash"> = {
    schema_version: REPLAY_PORTFOLIO_PROTECTIVE_TAKE_PROFIT_CANCEL_TERMINAL_OUTCOME_SCHEMA_VERSION,
