...
R4.90 的 schedule-observation binder 只定位外部冻结 Decision Schedule 的一个 sequence，并要求该 entry 的 `decision_time` 与已验证 Projection 的 `as_of_time` 完全相等。它只检查 schedule reference 的结构、顺序与 frozen hash；effect 保持 opaque，Request-aware authority 仍由既有合同 owner 负责。Binder 不调用 Harness、不解释 effect、不生成 Decision/Signal/Order，也不进入 Source Reducer、Runner 或经济事件链。

R4.91 的 binding-set builder 按 frozen Schedule entry count 与 sequence 构造完整 R4.90 member 集合。输入少一项、多一项、重复 sequence、乱序或 member Schedule hash 不同均在 Set 前失败；发布后每个 member 继续重验完整 observation lineage。Set 只关闭 boundary coverage 的闭世界基数，不调用 Harness、不执行 effect，也不进入 Source Reducer、Runner、Checkpoint、Result 或 Artifact。

R4.92 的 decision-observation bundle builder 只从已通过完整 Set lineage 的输入复制实际 Projection payload。每个 payload 必须与同序 binding 的 projection identity、as-of time、count、View/Cut hash 全等；Bundle 的 portability 不替代 Wire→Projection parent verification。Builder 不生成 Feature/Context/Decision/Signal/Order，也不进入 Source Reducer、Harness、Runner、Checkpoint、Result 或 Artifact。
...
R4.102 冻结 **Engine-owned Position-open State Input Materialization**。Builder 只接受一个既有正式 Decision State Snapshot v3，并重验 Request v30、Schedule v7、R4.96 Context Binding 的对应 `position_open` entry 及完整 SourceEvent prefix；输出仅保存 prefix hash/count/terminal EventKey 与 Snapshot/hash。它不重算 Position、保护单、Cash、Fee、Funding、PnL 或 Equity，也不证明任意外部 Snapshot 的认证 Engine provenance；离开完整 SourceEvent prefix 不能独立复验。该对象尚未进入 R4.101 Assembly v2，不创建 Worker Request、不调用 Harness、不接入 Runner，也不产生 Decision/Signal/Order/economic authority。

R4.103 冻结 **Engine-owned Decision Worker Input Assembly v3**。Builder 内嵌自校验的 R4.101 v2，并要求其至少有一个 position-open State 缺口；随后按 Schedule 顺序为每个缺口恰好接入一个 R4.102 parent，逐项核对 Request/Trial/Candidate、Context Binding、entry 与 State identity，使全部 entry 成为完整但不可执行的 tuple。v3 只证明 embedded parent schema/hash 与 cross-binding；R4.101/R4.102 更上游 lineage、完整 SourceEvent prefix 仍需外部 parents。它不创建 Worker Request、不调用 Harness、不进入 Source Reducer/Runner，也不产生 Decision/Signal/Order/economic authority。

R4.104 不把 code admission owner 放入 Engine。Engine 的终点仍是不可变 R4.103 v3；Runner 才能将其与 Source Bundle/Build Attestation 组合并执行 deterministic rebuild。Engine 不读取 artifact、不验证本机 runtime、不创建 Worker Request。
