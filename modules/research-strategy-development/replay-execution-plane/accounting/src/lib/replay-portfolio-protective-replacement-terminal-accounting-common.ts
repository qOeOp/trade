...
type JournalAccount = "wallet_cash" | "isolated_margin_collateral" | "position_valuation"
  | "opening_equity" | "realized_pnl_income" | "realized_pnl_loss" | "funding_income"
  | "funding_expense" | "fee_expense" | "liquidation_fee_expense"
  | "unrealized_pnl_income" | "unrealized_pnl_loss"
type PostingKind = "opening_cash" | "collateral_reserve" | "entry_fee" | "funding"
  | "realized_pnl" | "terminal_trading_fee" | "liquidation_fee" | "collateral_release"
...
function createJournal(
  ...
  post(null, "opening_cash", "wallet_cash", "opening_equity",
    input.replacement_terminal_evidence.shared_initial_cash, null)
  events.forEach((event, index) => {
    const ordinal = index + 1
    if (event.kind === "entry") {
...
type Post = (event: AccountingEvent | null, kind: PostingKind, debit: JournalAccount,
  credit: JournalAccount, amount: number, ordinal: number | null) => void

function postSigned(event: AccountingEvent, kind: "funding" | "realized_pnl", amount: number,
  income: "funding_income" | "realized_pnl_income", expense: "funding_expense" | "realized_pnl_loss",
  ordinal: number, post: Post): void {
  if (amount > 0) post(event, kind, "wallet_cash", income, amount, ordinal)
  if (amount < 0) post(event, kind, expense, "wallet_cash", -amount, ordinal)
}
