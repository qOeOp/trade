import {
  ...
  REPLAY_PORTFOLIO_PROTECTIVE_STOP_REPLACEMENT_CYCLE_SEQUENCE_LIMITATIONS,
  REPLAY_PORTFOLIO_PROTECTIVE_STOP_REPLACEMENT_CYCLE_SEQUENCE_POLICY_VERSION,
  assertReplayPortfolioProtectiveStopReplacementCycleSequenceEvidence,
  replayPortfolioProtectiveStopReplacementCycleCommitHash,
  ...
  replayPortfolioProtectiveStopReplacementCycleSequenceLedgerEntryHash,
  replayPortfolioProtectiveStopReplacementCycleSequenceTrialBalanceHash,
  type ReplayPortfolioProtectiveStopReplacementCycleCommit,
  type ReplayPortfolioProtectiveStopReplacementCycleSequenceEvidence,
  type ReplayPortfolioProtectiveStopReplacementCycleSequenceFingerprint,
  type ReplayPortfolioProtectiveStopReplacementCycleSequenceJournalEntry,
  type ReplayPortfolioProtectiveStopReplacementCycleSequenceLedgerEntry,
  type ReplayPortfolioProtectiveStopReplacementCycleSequenceTrialBalance,
  type ReplayPortfolioProtectiveStopReplacementCycleSequenceAuthority,
...
const ACCOUNTS: ReplayPortfolioProtectiveStopReplacementTerminalAccountingJournalAccount[] = [
...
const CREDIT_NORMAL = new Set<ReplayPortfolioProtectiveStopReplacementTerminalAccountingJournalAccount>([
  "opening_equity", "realized_pnl_income", "funding_income", "unrealized_pnl_income",
])

export interface ReplayPortfolioProtectiveStopReplacementCycleSequenceAccountingInput {
  plan: ReplayPortfolioCycleSequencePlan
  reservation: ReplayPortfolioProtectiveStopReplacementCycleSequenceAuthority
  cycles: ReplayPortfolioProtectiveStopReplacementCycleSource[]
}

export function createReplayPortfolioProtectiveStopReplacementCycleSequenceEvidence(
  input: ReplayPortfolioProtectiveStopReplacementCycleSequenceAccountingInput,
): ReplayPortfolioProtectiveStopReplacementCycleSequenceEvidence {
  const commits = createCommits(input.cycles)
  const ledger: ReplayPortfolioProtectiveStopReplacementCycleSequenceLedgerEntry[] = []
  const journal: ReplayPortfolioProtectiveStopReplacementCycleSequenceJournalEntry[] = []
  for (const cycle of input.cycles) {
    for (const cycleEntry of cycle.accounting_evidence.ledger) {
      const body: Omit<ReplayPortfolioProtectiveStopReplacementCycleSequenceLedgerEntry,
        "sequence_entry_hash"> = {
      ...
      ledger.push({ ...body,
        sequence_entry_hash: replayPortfolioProtectiveStopReplacementCycleSequenceLedgerEntryHash(body) })
    }
    ...
    for (const cycleEntry of cycle.accounting_evidence.journal) {
      ...
      const body: Omit<ReplayPortfolioProtectiveStopReplacementCycleSequenceJournalEntry,
        "sequence_entry_hash"> = {
      ...
      journal.push({ ...body,
        sequence_entry_hash: replayPortfolioProtectiveStopReplacementCycleSequenceJournalEntryHash(body) })
    }
  }
  ...
  const fingerprintBody: Omit<ReplayPortfolioProtectiveStopReplacementCycleSequenceFingerprint,
    "fingerprint_hash"> = {
      ...
      policy_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_REPLACEMENT_CYCLE_SEQUENCE_POLICY_VERSION,
  ...
  const fingerprint = { ...fingerprintBody,
    fingerprint_hash:
      replayPortfolioProtectiveStopReplacementCycleSequenceFingerprintHash(fingerprintBody) }
  const body: Omit<ReplayPortfolioProtectiveStopReplacementCycleSequenceEvidence, "evidence_hash"> = {
    schema_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_REPLACEMENT_CYCLE_SEQUENCE_EVIDENCE_SCHEMA_VERSION,
    policy_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_REPLACEMENT_CYCLE_SEQUENCE_POLICY_VERSION,
    experiment_id: input.reservation.experiment_id,
    trial_group_id: input.reservation.trial_group_id,
    trial_group_hash: input.reservation.trial_group_hash,
    ...
    limitations: REPLAY_PORTFOLIO_PROTECTIVE_STOP_REPLACEMENT_CYCLE_SEQUENCE_LIMITATIONS,
    fingerprint,
  }
  const evidence = { ...body,
    evidence_hash: replayPortfolioProtectiveStopReplacementCycleSequenceEvidenceHash(body) }
  assertReplayPortfolioProtectiveStopReplacementCycleSequenceEvidence(evidence, input)
  return evidence
}

function createCommits(
  cycles: ReplayPortfolioProtectiveStopReplacementCycleSource[],
): ReplayPortfolioProtectiveStopReplacementCycleCommit[] {
  return cycles.map((cycle) => {
    ...
    const body: Omit<ReplayPortfolioProtectiveStopReplacementCycleCommit, "cycle_commit_hash"> = {
      cycle_index: cycle.cycle_index,
    ...
    return { ...body, cycle_commit_hash: replayPortfolioProtectiveStopReplacementCycleCommitHash(body) }
  })
}
...
function createTrialBalance(
...
): ReplayPortfolioProtectiveStopReplacementCycleSequenceTrialBalance {
  const raw = Object.fromEntries(ACCOUNTS.map((account) => [account, 0])) as
    Record<ReplayPortfolioProtectiveStopReplacementTerminalAccountingJournalAccount, number>
  let totalDebits = 0
  let totalCredits = 0
  ...
  ])) as Record<ReplayPortfolioProtectiveStopReplacementTerminalAccountingJournalAccount, number>
  const body: Omit<ReplayPortfolioProtectiveStopReplacementCycleSequenceTrialBalance,
    "trial_balance_hash"> = {
  ...
  return { ...body,
    trial_balance_hash: replayPortfolioProtectiveStopReplacementCycleSequenceTrialBalanceHash(body) }
}
