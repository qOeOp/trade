import {
  REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_EVIDENCE_SCHEMA_VERSION,
  REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_JOURNAL_POLICY_VERSION,
  REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_LIMITATIONS,
  REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_POLICY_VERSION,
  assertReplayPortfolioProtectiveStopCancelCycleSequenceEvidence,
  replayPortfolioProtectiveStopCancelCycleCommitHash,
  ...
  replayPortfolioProtectiveStopCancelCycleSequenceLedgerEntryHash,
  replayPortfolioProtectiveStopCancelCycleSequenceTrialBalanceHash,
  type ReplayPortfolioProtectiveStopCancelCycleCommit,
  type ReplayPortfolioProtectiveStopCancelCycleSequenceAuthority,
  type ReplayPortfolioProtectiveStopCancelCycleSequenceEvidence,
  type ReplayPortfolioProtectiveStopCancelCycleSequenceFingerprint,
  type ReplayPortfolioProtectiveStopCancelCycleSequenceJournalEntry,
  type ReplayPortfolioProtectiveStopCancelCycleSequenceLedgerEntry,
  type ReplayPortfolioProtectiveStopCancelCycleSequenceTrialBalance,
...
const ACCOUNTS: ReplayPortfolioProtectiveStopCancelTerminalAccountingJournalAccount[] = [
...
const CREDIT_NORMAL = new Set<ReplayPortfolioProtectiveStopCancelTerminalAccountingJournalAccount>([
  "opening_equity", "realized_pnl_income", "funding_income", "unrealized_pnl_income",
])
export interface ReplayPortfolioProtectiveStopCancelCycleSequenceAccountingInput {
  plan: ReplayPortfolioCycleSequencePlan
  reservation: ReplayPortfolioProtectiveStopCancelCycleSequenceAuthority
  cycles: ReplayPortfolioProtectiveStopCancelCycleSource[]
}

export function createReplayPortfolioProtectiveStopCancelCycleSequenceEvidence(
  input: ReplayPortfolioProtectiveStopCancelCycleSequenceAccountingInput,
): ReplayPortfolioProtectiveStopCancelCycleSequenceEvidence {
  const commits = createCommits(input.cycles)
  const ledger: ReplayPortfolioProtectiveStopCancelCycleSequenceLedgerEntry[] = []
  const journal: ReplayPortfolioProtectiveStopCancelCycleSequenceJournalEntry[] = []
  for (const cycle of input.cycles) {
    for (const cycleEntry of cycle.accounting_evidence.ledger) {
      ...
      ledger.push({ ...body,
        sequence_entry_hash: replayPortfolioProtectiveStopCancelCycleSequenceLedgerEntryHash(body) })
    }
    ...
    for (const cycleEntry of cycle.accounting_evidence.journal) {
      ...
      journal.push({ ...body,
        sequence_entry_hash: replayPortfolioProtectiveStopCancelCycleSequenceJournalEntryHash(body) })
    }
  }
  ...
  const fingerprintBody: Omit<ReplayPortfolioProtectiveStopCancelCycleSequenceFingerprint,
    "fingerprint_hash"> = {
      ...
      policy_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_POLICY_VERSION,
      ...
      limitations_hash: canonicalHash(REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_LIMITATIONS),
    }
  const fingerprint = { ...fingerprintBody,
    fingerprint_hash: replayPortfolioProtectiveStopCancelCycleSequenceFingerprintHash(fingerprintBody) }
  const body: Omit<ReplayPortfolioProtectiveStopCancelCycleSequenceEvidence, "evidence_hash"> = {
    schema_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_EVIDENCE_SCHEMA_VERSION,
    policy_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_POLICY_VERSION,
    experiment_id: input.reservation.experiment_id, trial_group_id: input.reservation.trial_group_id,
    trial_group_hash: input.reservation.trial_group_hash, portfolio_id: input.reservation.portfolio_id,
    sequence_plan_hash: input.plan.plan_hash, sequence_reservation_hash: input.reservation.reservation_hash,
    ...
    limitations: REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_LIMITATIONS, fingerprint,
  }
  const evidence = { ...body,
    evidence_hash: replayPortfolioProtectiveStopCancelCycleSequenceEvidenceHash(body) }
  assertReplayPortfolioProtectiveStopCancelCycleSequenceEvidence(evidence, input)
  return evidence
}

function createCommits(cycles: ReplayPortfolioProtectiveStopCancelCycleSource[]):
ReplayPortfolioProtectiveStopCancelCycleCommit[] {
  return cycles.map((cycle) => {
    ...
    const body: Omit<ReplayPortfolioProtectiveStopCancelCycleCommit, "cycle_commit_hash"> = {
    ...
    return { ...body, cycle_commit_hash: replayPortfolioProtectiveStopCancelCycleCommitHash(body) }
  })
}
function createTrialBalance(reservation: ReplayPortfolioProtectiveStopCancelCycleSequenceAuthority,
  endingCash: number, journal: ReplayPortfolioProtectiveStopCancelCycleSequenceJournalEntry[]):
ReplayPortfolioProtectiveStopCancelCycleSequenceTrialBalance {
  const raw = Object.fromEntries(ACCOUNTS.map((account) => [account, 0])) as
    Record<ReplayPortfolioProtectiveStopCancelTerminalAccountingJournalAccount, number>
  ...
  const balances = Object.fromEntries(ACCOUNTS.map((account) => [account,
    ...
    Record<ReplayPortfolioProtectiveStopCancelTerminalAccountingJournalAccount, number>
  const body: Omit<ReplayPortfolioProtectiveStopCancelCycleSequenceTrialBalance, "trial_balance_hash"> = {
    settlement_asset: reservation.settlement_asset,
    journal_policy_version: REPLAY_PORTFOLIO_PROTECTIVE_STOP_CANCEL_CYCLE_SEQUENCE_JOURNAL_POLICY_VERSION,
  ...
  return { ...body, trial_balance_hash: replayPortfolioProtectiveStopCancelCycleSequenceTrialBalanceHash(body) }
}
