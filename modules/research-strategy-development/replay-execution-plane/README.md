...
runner/     Attempt lease fencing、幂等、transport-neutral coordinator/pre-terminal local outbox/recovery-first no-replay ack、typed failure、Harness worker 与完整 Artifact commit
...
当前是 Trial Reservation v8、Reservation/Attempt Cancellation v1、Attempt Cancellation Observation v1、Cancellation Coordination Result v1、Request v24、Result v36、Artifact v38、Run Outcome v35、Dataset Manifest v10、Instrument Status Provenance v2、Provider Certification/Termination v1、OHLCV Resolution Evidence v3、Partial Reduce Intent v1、Reduce-only Exit Intent v1、Protective Stop Replace Intent v1、Decision Schedule v5、Boundary v7、Timeline v8、State Snapshot v3、Harness Context v6、Registry Capability v7、Harness Capability/Receipt v9、Worker Protocol v7、Engine Checkpoint v17、Simulator v10、Margin v7、Journal v4 的受限认证纵切。Control Plane 分别冻结 status schedule、provenance、provider capability 与 certification hash；Dataset、Request、Reservation 与 Fingerprint 必须绑定同一认证收据。Termination/Cancellation/Observation 只改变或证明 Control Plane runtime authority，不进入 Replay 经济 wire；Runner 通过注入 port poll/ack，SQLite adapter 留在 Control Plane，authority cancel 后不暴露 resumable checkpoint并删除本地 diagnostic 文件。remote transport/SLA 未认证。当前 lane 可选择一次全仓 stop tighten，或一次 fixed-quantity partial reduce 后按剩余仓位原子重建双保护，并可追加一次末位 full exit。
