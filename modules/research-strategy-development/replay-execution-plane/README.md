...
当前是 Trial Reservation v8、Reservation/Attempt Cancellation v1、Request v24、Result v36、Artifact v38、Run Outcome v34、Dataset Manifest v10、Instrument Status Provenance v2、Provider Certification/Termination v1、OHLCV Resolution Evidence v3、Partial Reduce Intent v1、Reduce-only Exit Intent v1、Protective Stop Replace Intent v1、Decision Schedule v5、Boundary v7、Timeline v8、State Snapshot v3、Harness Context v6、Registry Capability v7、Harness Capability/Receipt v9、Worker Protocol v7、Engine Checkpoint v17、Simulator v10、Margin v7、Journal v4 的受限认证纵切。Control Plane 分别冻结 status schedule、provenance、provider capability 与 certification hash；Dataset、Request、Reservation 与 Fingerprint 必须绑定同一认证收据。Termination 与 Cancellation 只改变 Control Plane admission/runtime authority，不进入 Replay 经济 wire。当前 lane 可选择一次全仓 stop tighten，或一次 fixed-quantity partial reduce 后按剩余仓位原子重建双保护，并可追加一次末位 full exit。
