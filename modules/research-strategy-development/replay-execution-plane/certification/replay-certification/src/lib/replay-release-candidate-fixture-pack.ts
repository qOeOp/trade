...
const EXPECTED_COMPONENTS: Array<Pick<ReplayReleaseCandidateComponent,
  "role" | "path" | "authority_hash_field">> = [
  {
    ...
    path: "modules/research-strategy-development/replay-execution-plane/tests/src/fixtures/certified-single-position-v24.json",
  ...
  },
  {
    ...
    path: "modules/research-strategy-development/replay-execution-plane/certification/legacy-portfolio-cycle-certification/fixtures/historical-artifact-read-migration-v1.json",
...
const EXPECTED_PROFILES: Array<Omit<ReplayReleaseCandidateProfile, "golden_source_sha256">> = [
  {
    ...
    golden_path: "modules/research-strategy-development/replay-execution-plane/runner/src/lib/replay-independent-lane-batch-runner.test.ts",
  ...
  },
  {
    ...
    golden_path: "modules/research-strategy-development/replay-execution-plane/runner/src/lib/replay-trial-runner.test.ts",
...
export function assertReplayReleaseCandidateFixturePack(
  ...
  pack.components.forEach((component, index) => {
    ...
    const source = assertSource(
      repoRoot, component.path, component.content_sha256, `component ${component.role}`,
    )
  ...
  pack.profiles.forEach((profile, index) => {
    ...
    const source = assertSource(
      repoRoot, profile.golden_path, profile.golden_source_sha256, `${profile.profile} golden`,
    )
  ...
  for (const profile of pack.profiles) {
    ...
    profiles.push({
      ...
      golden_assertion_hash: sha256(stableJson({
        ...
        test_name: profile.golden_test_name,
        source_sha256: profile.golden_source_sha256,
        outcome: "passed",
      })),
    })
  }
...
function assertSource(repoRoot: string, path: string, expectedHash: string, role: string): string {
  ...
  if (sha256(source) !== expectedHash) throw new Error(`Replay release candidate ${role} source drifted`)
  return source
}
