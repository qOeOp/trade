...
- 通过 compatibility `legacy-replay-identity` 对 kernel harness、OHLCV manifest/data 与 assumptions 重算。
...
- 底层 legacy identity/kernel 退役时必须显式迁移或终止该 certification；不得静默承接新 Replay 语义。
