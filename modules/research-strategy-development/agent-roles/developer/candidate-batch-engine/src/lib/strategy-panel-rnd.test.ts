...
test("candidate batch panel mode pools samples but keeps per-asset evidence", () => {
  ...
  try {
    ...
    const candidate = (report.candidates as Array<{
      negative_controls: { method: string; asset_count: number }
      panel_negative_controls: { status: string; passed: boolean }
      assets: Array<{ negative_control_passed: boolean; funding_event_coverage: { status: string }; funding_event_count: number; funding_events_hash: string | null }>
    }>)[0]
