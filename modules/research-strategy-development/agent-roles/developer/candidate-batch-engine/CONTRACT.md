...
- Uses compatibility `legacy-research-kernel` for execution、`legacy-research-decision` for bounded decision integrity、`legacy-research-contracts` for compile-time shapes、`legacy-research-data` for Candle/manifest loading、`legacy-replay-identity` for frozen hashes, plus `agent-roles/developer/strategy-family-engine`; formulas are not duplicated.
