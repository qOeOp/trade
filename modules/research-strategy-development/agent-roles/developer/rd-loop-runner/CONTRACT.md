...
- Optional artifact root, ledger path, catalog DB path, run id, and deployment `environment_id`.
...
- Catalog registration, run-id checks, holdout checks, and ledger append must all use the same deployment environment identity.
