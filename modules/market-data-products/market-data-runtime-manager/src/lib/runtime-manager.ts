...
export function buildManagedL2Commands(input: {
  ...
  const ownerScript = resolve(
    input.root,
    "modules/market-data-products/l2-order-book-service/src/scripts/foreground.ts",
  )
  const consumerScript = resolve(
    input.root,
    "modules/orchestration-ops/l2-current-book-probe/src/scripts/consumer-foreground.ts",
  )
  return {
    owner: [
      input.bun_path,
