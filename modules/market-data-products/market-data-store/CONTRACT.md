...
- Commit bounded raw Binance funding pages、decimal-string events and a self-hashed exact-window coverage audit into append-only formal archive tables. Existing upsertable calibration funding rows cannot satisfy formal funding coverage.
- Resolve only an exact active funding demand through the current owner subscription plan; missing evidence stays pending and multiple same-window archives stay conflict. Export a create-or-identical Replay-ready JSON slice only when every archived event has a finite rate and positive mark price, preserving archive/audit/raw-normalized hashes alongside the numeric artifact hash.
- Run the funding demand resident as a separate process under this owner: fixed public endpoint、bounded pagination / timeout / backoff、exact-window resolution and redacted state. Process separation does not create a second data authority.
- Admit feature manifests derived from immutable source manifests create-or-identical; admission and owner read both parse the bounded artifact bytes and close source slice、feature set、symbol、timeframe and content hash. The same manifest identity may not drift in source、feature set、symbol、timeframe、content hash、path or generation time.
...
- Serve acquisition receipt metadata、an archive by id and the latest successful current snapshot inside an explicit completion-time window through owner read ports; raw BLOB reads remain an integrity/audit library surface, not a default CLI payload.
