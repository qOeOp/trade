...
test("foreground L2 plan keeps one exact supervisor child and repository refs", () => {
  const plan = buildL2ForegroundRuntimePlan({
    root: "/repo",
    module_root: "/repo/modules/market-data-products/l2-order-book-service",
  ...
  assert.deepEqual(plan.supervisor_command.slice(0, 2), [
    "/usr/bin/bun",
    "/repo/modules/market-data-products/l2-order-book-service/src/scripts/runtime-supervisor.ts",
  ])
  ...
  assert.equal(receipt.service_binary, "modules/market-data-products/l2-order-book-service/target/release/l2-order-book-service")
})
