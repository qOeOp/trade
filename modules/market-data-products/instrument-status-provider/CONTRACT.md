...
- Require Archive v2 source-batch/hash-chain closure, a passed batch-window continuity audit, an anchor event, strictly ordered state transitions, full requested-window coverage, and a finality watermark at or beyond `coverage_end`.
...
- Emit Replay status epochs, certification-bound provenance, archive/audit/batch-chain hashes, explicit `external_completeness=not_verified`, supersession ref, provider capability, and a self-hashed Evidence v3 envelope.
