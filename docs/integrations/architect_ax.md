...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/architect_ax/)
- [Rust examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/architect_ax/examples/)
...
instrument with an expiration as `FuturesContract`. The venue category determines the Vibe
asset class. The adapter uses `MARGIN` account type and `NETTING` order management.

## Symbology

The adapter preserves each AX symbol and appends the Vibe venue identifier `.AX`. Perpetual
...
smallest stream that covers the active Vibe subscriptions:
...
- If multiple Vibe data types are active for a symbol, the adapter resubscribes only when the
...
estimated-funding request. Vibe currently exposes settled funding-rate updates through HTTP
...
a price, which the adapter maps to a Vibe `LIMIT` order. The adapter simulates a Vibe
...
Vibe can still emulate a stop-limit order locally. The common order emulator waits for the
