...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/bitmex/)
- [Rust examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/bitmex/examples/)
...
accordingly, so quantities in Vibe are always expressed in base units (SOL, ETH, etc.).
...
in the [Python execution tester](https://github.com/qOeOp/trade/blob/main/examples/live/bitmex/exec_tester.py).
...
The BitMEX execution adapter maps Vibe contingent order lists to the exchange's
...
The adapter does not map Vibe `ContingencyType::Ouo` to BitMEX. For bracket
...
When defining strategies, continue to use Vibe `OrderList`/`ContingencyType`
