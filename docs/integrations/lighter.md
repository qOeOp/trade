...
- `LighterHttpClient`: domain client which parses instruments, trades, books, orders, and account
  state into Vibe model types.
- `LighterWebSocketClient`: reconnecting WebSocket client for public market and private account streams.
- `LighterDataClient`: Vibe data client for instruments, trades, quotes, and L2 MBP books.
- `LighterExecutionClient`: Vibe execution client for account streams, order submission,
...
For emergency account cleanup, `cargo run --bin lighter-flatten -p vibe-lighter` cancels open
...
batches in-progress updates for the open bar every ~500 ms; the adapter emits a Vibe `Bar`
...
negative. These rows cannot form valid Vibe bars and do not stop later valid rows from loading.
...
The adapter derives a 31‑bit index from the Vibe `ClientOrderId` and probes forward on a
...
a Vibe client order ID can query active orders by its derived client index. Client-index-only
...
The adapter denies Vibe `IOC` for conditional market orders because Lighter reserves IOC for
