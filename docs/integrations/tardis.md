...
- **Data pipeline functions**: replay historical data from Tardis Machine and write Vibe
  Parquet catalog files.

:::info
A `TARDIS_API_KEY` is required for Vibe instrument metadata calls. Tardis Machine uses
...
- Tardis `option_summary` messages include best bid/offer fields. Vibe always maps this feed to
...
The adapter converts Tardis trade bar intervals and suffixes to Vibe `BarType`s.
...
- Original exchange symbols are preserved in the Vibe instrument definitions `raw_symbol` field.

### Exchange-specific normalizations

- **Binance**: Vibe appends the suffix `-PERP` to all perpetual symbols.
- **Bybit**: Vibe uses product category suffixes, including `-SPOT`, `-LINEAR`,
  `-INVERSE`, and `-OPTION`.
- **dYdX**: Vibe appends the suffix `-PERP` to all perpetual symbols.
- **Gate.io**: Vibe appends the suffix `-PERP` to all perpetual symbols.
...
in Vibe Parquet format, using either Python or Rust. Since the function is implemented in Rust,
...
This format is compatible with Vibe data catalog queries, consolidation, and management.
...
This process is optimized for direct output to a Vibe Parquet data catalog.
Set `VIBE_PATH` to the parent directory that contains the `catalog/` subdirectory. Parquet
files are written under `<VIBE_PATH>/catalog/data/` in subdirectories by data type and
instrument.

If no `output_path` is specified and `VIBE_PATH` is unset, output defaults to the current
working directory.
...
- `normalize_symbols` (`bool | null`): applies Vibe symbol normalization. Defaults to `true`.
- `output_path` (`str | null`): output directory for Parquet data. Defaults to `VIBE_PATH`,
...
backtest also needs option BBO quotes. Vibe still writes `OptionGreeks` from every
...
An option-chain backtest starts after the Tardis replay has written data to the Vibe
...
Vibe catalog data. This path does not call Tardis Machine or the instrument metadata API, so
...
from disk and parses it into Vibe data. Since the loader is implemented in Rust, performance
...
and parse instrument metadata into Vibe instruments.
