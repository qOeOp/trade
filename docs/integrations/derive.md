...
[`crates/adapters/derive/examples/`](https://github.com/qOeOp/trade/tree/main/crates/adapters/derive/examples/).
...
Vibe-native stop and if-touched order types listed below. Unsupported Vibe order
...
therefore rejects Vibe modify requests for trigger orders with an `OrderModifyRejected`
...
adapter rejects Vibe values with no Derive equivalent before signing. Derive exposes
...
`11024 Reduce only not supported with this time in force`. As a result a Vibe bracket whose
