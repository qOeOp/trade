...
Vibe quantities map to Deribit's `amount` field, not the optional `contracts` field. Deribit
...
`amount` and contract count; the adapter does not apply it again as the Vibe multiplier.
...
unsubscribing that combo trade stream, Vibe also closes the leg subscriptions opened by
this opt-in.
...
The Vibe adapter supports both feed types via subscription parameters:
...
The Vibe adapter uses **reject mode** (`reject_post_only=true`) for deterministic behavior.
...
kind. Each response item includes the Vibe instrument ID, implied volatility, open interest,
...
:::note
The Vibe adapter uses WebSocket for order submission (not HTTP) for lower latency.
...
The Vibe adapter implements broad token bucket rate limiters configured as:
