...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/binance/)
- [Rust spot examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/binance/examples/spot/)
- [Rust futures examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/binance/examples/futures/)
...
Vibe appends `-PERP` to USD-M perpetual symbols. For example, the Binance
...
`BTCUSDT_260925` and `BTCUSD_260925` remain unchanged within Vibe. USD-M
...
part. Vibe rejects an expiry that is not on a whole-second boundary rather than silently
...
then warns and sends `GTC`, while Vibe cancels the order at its local expiry.
...
1. Vibe sends the order to Binance with the `priceMatch` parameter but omits the limit price from the API request.
2. Binance accepts the order and determines the actual working price.
3. Vibe generates an `OrderAccepted` event.
4. If the Binance-accepted price differs from the reference price, Vibe generates an `OrderUpdated` event with the actual working price.
5. The order price in the Vibe cache now matches the Binance-accepted price.
...
:::info
Vibe omits `quantity` and `reduceOnly` from the API request when `close_position` is set.
...
`BinanceFuturesHttpClient`) decode automatically when returning Vibe
...
update rates differ between Spot and Futures, with Vibe using the highest
available rate:
...
`BinanceFuturesMarkPriceUpdate`. Vibe also emits standard mark-price, index-price, and
...
Some Binance instruments cannot be parsed into Vibe objects if they contain
