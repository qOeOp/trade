...
- Loading historical data from DBN files and decoding to Vibe objects for backtesting or catalog storage.
- Requesting historical data decoded to Vibe objects for live trading and backtesting.
- Subscribing to real-time data feeds decoded to Vibe objects for live trading and sandbox environments.
...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/databento/)
...
The adapter decodes DBN data to Vibe objects. The same Rust decoder handles:
...
the schemas listed above to Vibe data types. Daily Databento OHLCV uses
...
FX spot definitions with currencies that Vibe cannot map are also skipped.
Index-emitting publishers include CGIF.TITANIUM (110), IEX Options (108), and
MEMX MX2 (109). Open an issue if you need Vibe modeling for these.
...
Databento dataset IDs are separate from Vibe venue identifiers. The adapter
...
Use `EQUS` as the Vibe venue for US Equities Mini instruments:
...
that Vibe subscription method. The selected dataset must also support the
...
day. This differs from the Vibe `InstrumentId`, a string of symbol + venue
separated by a period: `"{symbol}.{venue}"`.

The decoder maps the Databento `raw_symbol` to the Vibe `symbol`. Publisher
IDs map to the default Vibe venue through `publishers.json`. Subscription
...
For historical requests and live subscriptions, the adapter sends the Vibe
...
Vibe data requires at least two timestamps (per the `Data` contract):
...
Quote and trade-like schemas map Databento `ts_recv` to Vibe `ts_event`
...
whether Vibe bars use the interval open or close timestamp. `InstrumentStatus`
...
This section maps Databento schemas to Vibe data types.
...
The `vibe_databento::arrow` module provides Arrow record batch encoding and
...
- Store data as DBN (`.dbn.zst`) files and decode to Vibe objects every run.
- Convert DBN files to Vibe objects once and write to the data catalog (Vibe Parquet format).

The DBN decoder is optimized Rust, but writing to the catalog once gives the
best backtest performance.

[DataFusion](https://arrow.apache.org/datafusion/) streams Vibe Parquet data
...
For live data, decoded delivery from the feed handler to Vibe is
...
`DatabentoDataLoader` decodes DBN files directly into Vibe objects. It exposes a method for
...
[Python example](https://github.com/qOeOp/trade/blob/main/examples/live/databento/data_tester.py)
...
[contributing guide](https://github.com/qOeOp/trade/blob/main/CONTRIBUTING.md).
:::
