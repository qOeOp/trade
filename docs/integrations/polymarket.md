...
[`crates/adapters/polymarket/examples`](https://github.com/qOeOp/trade/tree/main/crates/adapters/polymarket/examples)
for Rust. For Python, use the Rust‑native [data tester](https://github.com/qOeOp/trade/blob/main/examples/live/polymarket/data_tester.py),
[execution tester](https://github.com/qOeOp/trade/blob/main/examples/live/polymarket/exec_tester.py),
or [Up/Down smoke tester](https://github.com/qOeOp/trade/blob/main/examples/live/polymarket/updown_smoke_tester.py).
...
- `PolymarketWebSocketClient`: Low‑level WebSocket API connectivity built on the Vibe Rust
  `WebSocketClient`.
...
Every order signed by the adapter carries the hard‑coded Vibe builder code. Its builder fee
...
- Compare these reports with Vibe execution state.
- Generate missing orders to bring Vibe execution state in line with positions reported by
  Polymarket.
...
in `vibe_polymarket::common::consts`.
...
The `PolymarketWebSocketClient` is built on top of the high-performance Vibe `WebSocketClient` base class, written in Rust.
...
The close type is `InstrumentCloseType.ContractExpired`. This event closes Vibe exposure and
...
path to claim funds; it only publishes market-outcome close events into Vibe.
