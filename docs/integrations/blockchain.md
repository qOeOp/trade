...
The DeFi domain model lives in `vibe_model::defi`.
...
stable Vibe instrument ID. The token pair, fee tier, tick spacing, and creation block remain
pool metadata.
...
Cached block timestamps load into Vibe data objects as UNIX nanoseconds. Cache rows written with
...
- Built CLI: `cargo build -p vibe-cli --features defi --bin vibe`.
