...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/dydx/)
- [Rust examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/dydx/examples/)
...
- Quoted in USD
- Settled in USDC
- Use the `.DYDX` venue suffix in Vibe
...
adapter submits Vibe `MARKET_IF_TOUCHED` as a take‑profit with the price set to the 5%
...
original Vibe order type. If the price is within the pay‑through band of the oracle, the
...
is a Vibe convenience that the adapter implements as an aggressive IOC limit order
...
dYdX requires `u32` client IDs on-chain, but Vibe uses string-based `ClientOrderId` values
...
On restart, Vibe resets the internal order counter based on the number of reconciled orders,
...
No manual conversion is needed when submitting orders through Vibe.
