...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/coinbase/)
- [Rust examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/coinbase/examples/)
...
Coinbase uses the venue's native `product_id` field directly as the Vibe
...
Examples of full Vibe instrument IDs:
...
<Step>
Enter a nickname (e.g. `vibe-trading`).
</Step>
<Step>
...
The matrix lists order types as exposed through the Vibe model. The
...
This section documents how `CoinbaseExecutionClient` translates Vibe
order commands and Coinbase venue events into Vibe execution events.
...
- `MARKET` -> `market_market_ioc`. Only `TimeInForce::Ioc` and `Gtc` (the
  Vibe default) are accepted; any explicit `Fok`, `Day`, or `Gtd` on a
...
- **MARKET orders default to IOC.** A `MarketOrder` constructed with the
  Vibe default `TimeInForce::Gtc` is mapped to `market_market_ioc` at
...
our [contributing guide](https://github.com/qOeOp/trade/blob/main/CONTRIBUTING.md).
:::
