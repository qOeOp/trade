...
- [Python examples](https://github.com/qOeOp/trade/tree/main/examples/live/okx/)
- [Rust examples](https://github.com/qOeOp/trade/tree/main/crates/adapters/okx/examples/)
...
band percentages, so they are not parsed as static Vibe `min_price` or `max_price`
values.
...
suffix when referencing instruments in Vibe, for example `BTC-USDT.OKX`.
...
endpoint. The adapter maps each OKX `sprdId` to a Vibe spread instrument ID
...
The adapter represents these markets as Vibe `BinaryOption` instruments.
...
Vibe order book, order, and lifecycle types. RPI routing is opt‑in, so standard subscriptions and
...
Vibe `OrderBookDeltas` and `OrderBook` use `totalQty` as the level quantity. The typed raw
...
orders as Vibe `LIMIT` orders with `post_only=True`.
...
If you need GTD functionality, use Vibe's strategy-managed GTD feature. It handles
...
these instruments as Vibe `BinaryOption` instruments and preserves OKX metadata
