...
本文定义 Binance 写 skill 的成功输出如何被 `trade-flow` 记录成 `plan_event(kind=order_fill)`。

它只锁最小外壳，不冻结 Binance 原始 `result` 的内部字段；交易所返回仍由各执行 skill 持有。
