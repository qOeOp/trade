...
Shell scripts in this repository use **bash** (not POSIX sh) and must be portable across **Linux** and **macOS**. Any user-facing script must also work on **Windows** via Git Bash or WSL.
...
established Vibe `load_*` vocabulary:
...
Each package under `python/vibe_trader/adapters/` is a thin facade over the private
`_libvibe` extension. Every adapter `__init__.py` declares a deterministic `__all__` that is
