...
allow VibeTrader to redistribute the data through the public repo or the public R2 bucket.
...
New datasets should be stored as **Vibe Parquet** (not raw vendor formats).
This ensures:
...
User-fetched datasets should also end up as Vibe Parquet after the local transform step.
...
2. The function should: download, parse, filter, convert to VibeTrader types, write Parquet.
...
For datasets that VibeTrader cannot redistribute:
...
3. Convert the vendor data locally into Vibe Parquet.
...
Several tutorials and guides load user-provided market data. The `VIBE_DATA_DIR` environment
...
The HISTDATA EURUSD.SIM Parquet files above replace the former external catalog layout.
Raw HISTDATA CSV files remain user-fetched.

Large fixtures that are not checked into Git require `VIBE_TEST_DATA_BASE_URL`. The value must
identify the configured fixture host root; the testkit appends `/large/<filename>` and verifies the
download against `test_data/large/checksums.json`. No default external host is embedded.
