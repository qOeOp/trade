...
- `postgres`: PostgreSQL with `POSTGRES_USER=vibe`, `POSTGRES_PASSWORD=pass`, and
  `POSTGRES_DB=vibe` by default.
- `redis`: Redis server.
...
You can install the Vibe CLI using the below Makefile target, which uses `cargo install` under the hood.
This places the `vibe` binary in Cargo's binary directory. Windows source installs require GNU
...
You can run `vibe --help` to view the CLI structure and available command groups:
...
- `--database` or `POSTGRES_DATABASE` for both the database **name and the new user** with privileges to that database
    (e.g., if you provide `vibe` as the value, a new user named vibe will be created with the password from `POSTGRES_PASSWORD`, and the `vibe` database will be bootstrapped with this user as the owner).
...
1. `vibe database init`: Will bootstrap schema, roles, and all sql files located in `schema` root directory (like `tables.sql`).
2. `vibe database drop`: Will drop all tables, roles, and data in target Postgres database.
