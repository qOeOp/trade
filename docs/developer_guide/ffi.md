...
Other workspace crates use Rust APIs or PyO3 bindings. The separate `vibe-plugin` crate defines
...
must route their implementation through `vibe_core::ffi::abort_on_panic`, which logs the panic
...
- Keep the implementation in `vibe-core` or `vibe-model`.
