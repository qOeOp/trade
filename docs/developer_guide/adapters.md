...
The [Vibe conventions hook](../../.pre-commit-hooks/check_vibe_conventions.sh) treats the
...
clearly, and the data client emits or returns complete Vibe instruments.
...
Repository‑wide import policy applies to adapter code: import Vibe types and use their short
names instead of fully qualifying them at call sites. The
[Vibe conventions hook](../../.pre-commit-hooks/check_vibe_conventions.sh) enforces this
...
Separate venue symbols from Vibe `InstrumentId` values. A symbol module commonly owns:

- Parsing and formatting venue symbols.
- Product or contract suffixes required for a unique Vibe symbol.
...
- Keep raw models separate from Vibe domain objects. Convert at one auditable boundary.
...
Do not infer support from a venue API alone. Implement and test the Vibe command and event
...
`place_order`. Name domain methods after Vibe semantics, such as `request_instruments`,
...
1. At a domain boundary, validate Vibe inputs and build typed venue parameters.
...
1. Allocate any required request identity, timestamp, or nonce. Sign the exact wire representation
   when needed, then send it through the shared `vibe_network::http::HttpClient` with the
...
`vibe_common::live::get_runtime().spawn()` for adapter production tasks so native Rust and
...
Assert both sides of the boundary: the exact venue request and the resulting Vibe event or
report.
