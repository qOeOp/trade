...
- Separate dependency groups with a blank line and alphabetize each group. Manifests normally group
  internal `vibe-*` crates, required external crates, and optional external crates, but preserve
...
- Obtain `libfuzzer-sys` in adapter crates through `vibe-live`; do not add it directly to an
  adapter manifest.
...
Place optional `build` and `include` fields after `repository.workspace`.
...
- Propagate `high-precision` to dependent Vibe crates that store or construct fixed‑point
  domain values.
...
Use `// vibe-import-ok` only where a macro, generated path, or conditional import requires a
fully qualified Vibe type. Put the marker on the affected line or directly above the narrow
block.
...
- Import `get_runtime` through `vibe_common::live`, not `live::runtime`.
...
- Keep venue wire enums distinct from Vibe domain enums. Use idiomatic Rust variant names,
...
updates the allowlist in `check_vibe_conventions.sh`.
...
interpreter or with `vibe_core::python::clone_py_object`. An extra `Arc<Py<T>>` is normally
...
Only `vibe-core` and `vibe-model` expose an `ffi` feature. Check both crates directly when
...
- Test the serialization crate with `make cargo-test-crate-vibe-serialization`.
