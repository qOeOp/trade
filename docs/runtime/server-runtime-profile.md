...
launchd | systemd | Docker
  ...
  -> formal Replay worker -> fenced durable queue -> registered Replay owners
```
...
- 容器例外是一个透明 foreground group：只管理五个 exact 顶层 child、readiness 顺序与反向 drain；任一 child 意外退出即让组失败，由 Docker 重启整个 group，不在内部重启 sibling。
...
最新 SiliconFlow capability probe 已证明 Chat JSON、stream、single / multi tool 与 tool-result continuation 可用；`/responses` 返回 unsupported，因此 Direct Model / 后续 chat-compatible Host 可用，但当前 Codex custom provider wire 不可用。真实 R&D owner 子进程现已补齐“Trial/Result 提交后、调用方确认前 `SIGKILL`”演练：重启时即使源 manifest 已移除也只读回同一 Result，库内保持单一 completed Trial / Result。Darwin 本机门据此闭合，但 release gate 已修正平台边界：macOS 证据不得输出 `no_live_server_shadow`，远程目标仍以 `linux_server_rehearsal_not_run` 阻断；catalog canary、live-small canary 与 exchange write 仍需各自显式授权。
