---
title: Module Structure Refactor Plan
...
> 本文保留重构动机、迁移顺序和实施记录，不再充当当前目录清单。物理模块重构已经完成；当前布局看 [tool-layout.md](../architecture/tool-layout.md) 与 [architecture-manifest.json](../architecture/architecture-manifest.json)，RD 四子树及仍在推进的语义替换看 [rd-architecture-migration-plan.md](../research/architecture/rd-architecture-migration-plan.md)。文中的旧路径、目标模块和“尚未完成”只代表当时施工上下文。
...
顶层责任域以 [design-architecture.md](../architecture/design-architecture.md) 为准；目录只是实现承载，不反向定义域边界。
