...
> 本文保留重构前的详细需求快照，不再定义当前产品范围。当前合同见 [PRD](../product/prd.md)；若两者冲突，以当前合同为准。
...
产品形态不是多条长期 automation，也不是一块巨大的 job line；而是一个总控入口生成一份带编号的 job graph，再把 job ticket 派给对应 subagent / handler：
...
R&D supervisor 可以临时分发 read-only scout subagent 来做历史失败审计、数据可用性盘点和新 edge 草拟；这些 scout 只产出 proposal，不写 `rd_program_state`、不消耗 trial、不打开 holdout。R&D 只有一个 state writer 边界：`research.rd-program-state` 负责显式 state command，`research.rd-supervisor` 只通过该边界 / R&D payload writeback 更新 queue、usage 与 stop status。

持续 R&D 的事实源是 `research_state_store.rd_program`，而不是临时对话记忆或文件 artifact。它保存 objective、budget、usage、stop status、失败摘要、reliability gate、被拒机制、universe lesson、下一轮 hypothesis queue 与 artifact refs；状态为 `active` 时总控才继续派发 `rd_strategy_supervisor`，进入 `shadow_candidate_found / budget_exhausted / data_or_tool_blocked` 后自动停线。该 state 只属于 research memory，不是 strategy evidence。

`rd_program_state` 的写入必须显式：`research.rd-program-state` 负责 init/read/update/plan_next；`plan_next` 只读 state，把 `next_hypothesis_queue` 编译为下一轮携带 `rd_program_ref + rd_state_db` 的 `research.rd-loop-runner` 或 `research.rd-campaign-runner` payload 草案。`research.rd-supervisor` 串起 `plan_next -> loop/campaign -> state writeback`，让 R&D 进入后自主循环到候选、预算耗尽或数据/工具阻断。`research.rd-loop-runner` / `research.rd-campaign-runner` 只有在 payload 传入 `rd_program_ref` 时才把本轮 artifact、usage、失败机制或 validated candidate 写回；`strategy-review` 只输出 execution attribution、成本反馈和 replay-to-shadow/live decay 诊断，不直接写 RD memory，后续由 R&D supervisor 显式消费。
...
- `research.rd-program-state`：初始化、读取、更新或 `plan_next` R&D learning memory store；`plan_next` 只生成下一轮 R&D payload，不写 `trade.db`，不产生 strategy evidence
- `research.rd-supervisor`：执行自主 R&D supervisor loop；每轮先从 state 规划下一条 hypothesis，再运行 loop/campaign，并依赖写回结果推进 queue、usage 与 stop status
...
- `research.forward-holdout`：对已冻结 candidate 做只读 forward holdout 验收；主数据与 benchmark / supplemental 数据都必须晚于机器可读 `frozen_at`，输出 `status / next_action / frozen_candidate hash`，只作为 shadow/review 前置观察，不直接产生 promotion evidence；缺 `frozen_at` 的“frozen”描述一律不验收
...
- R&D tracker 事件链设计见 [rd-event-chain-design.md](../research/architecture/rd-event-chain-design.md)；该链只存在于 R&D artifact，不进入 `trade.db.plan_event`
