...
> 本文保留重构前的执行层细节，不再定义当前技术合同。当前入口见 [Technical Contract](../runtime/tech-spec.md)、[Event and Flow Contract](../runtime/event-flow-contract.md) 与 [Risk Control Contract](../runtime/risk-control-contract.md)。

## 1. 范围

- 本文件只讨论 `Binance USDM` 执行层 + `trade.db` 持久化。
- 项目层固定：只做 USDM 永续 4H+ swing；外部 cron（Claude routines / Codex schedule）双轨触发：**慢轨** 1H / 4H 整点跑战略层全流程；**快轨** 5m / 15m 偏移点（如 :05/:20/:35/:50）守护执行层。设计细节见 [design-architecture.md §双轨](../architecture/design-architecture.md)。
...
- 没有消费正式 plan 结构。重构方向见 [design-architecture.md](../architecture/design-architecture.md) 的 `Plan 设计`：执行层应消费 `current_plan`（含 `symbol / side / stop_price / risk_budget_usdt / strategy_ref / entry_intent / exit_intent`）+ 同一条 observe 的证据段（含微结构 / 账户事实），产出写成 `order_fill` 事件；不再直接吃零散参数。
...
写 tool 成功输出到 `trade-flow` 记账的最低契约见 [execution-tool-contract.md](../runtime/execution-tool-contract.md)。
...
- [design-architecture.md](../architecture/design-architecture.md) 回答"为什么是 event-sourcing、Plan 怎么设计、cron 周期怎么走"
...
具体 body shape 三种，定义见 [design-architecture.md](../architecture/design-architecture.md)：
...
统一交易配置设计见 [trading-config.md](../runtime/trading-config.md)。执行、preflight、R&D 后续应消费编译后的 `runtime_policy`，不再各自散读 `account_config` 字段。
...
- `order_fill.body_json` shape 见 [design-architecture.md §order_fill.body shape](../architecture/design-architecture.md)；`source: trade_flow | reconcile` 标识来源（主动执行 vs 对账补录）；可选 `source_observe_event_key` 引用本笔 fill 对应的决策 observe（慢轨或快轨写的 observe 都可被引用）
...
- `review.body_json` 由某次仓位 / plan 阶段性闭合时写入；单条 flow 默认只写 1 条 terminal `review`；同一 lane 会跨多条历史 flow 累积多条 `review`
  ...
  - 完整 shape 与口径见 [design-architecture.md §REVIEW → review.body shape](../architecture/design-architecture.md)
...
- **数据库规格放本文件**，[design-architecture.md](../architecture/design-architecture.md) 只保留设计图、状态流和模型边界
