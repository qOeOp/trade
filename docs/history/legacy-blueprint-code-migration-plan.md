---
title: Legacy Blueprint Code Migration Plan
...
# Legacy Blueprint Code Migration Plan

> 本文保留 v2 蓝图施工过程和阶段性盘点，正文的“当前”“剩余”不再具有时效性。当前迁移边界见 [Blueprint Code Migration](../architecture/migrations/blueprint-code-migration-plan.md)，当前实现投影见 [Architecture Drift Report](../architecture/generated/architecture-drift-report.md)。
...
- `bun scripts/architecture-drift-audit.ts --write` 生成 `docs/architecture/generated/code-architecture-current.mmd` 和 `docs/architecture/generated/architecture-drift-report.md`。
...
- `docs/architecture/architecture-manifest.json` 要求 `J01...J07`。
...
### 3.4 Store owner 已清楚，源码飞线剩两条登记债务

当前状态：

- `architecture-drift-audit` 显示生产源码跨域 import 为 2：`aggregate-trade-provider` 与 `instrument-status-provider` 仍直接依赖 Replay Plane contracts。
...
1. 现有两条跨域 contract import 不得扩张；后续迁为共享 contract / ref rail，新增飞线必须被 drift audit 阻断。
...
5. `docs/product/prd.md` 已同步到 v2，产品文档不再宣称旧编号。
...
当前生产源码有 2 条已登记 contract 飞线；其余项不再是直接源码 import debt，而是后续 rail / resolver 化的结构优化。

| 飞线 | 当前原因 | 目标 |
| --- | --- | --- |
| `aggregate-trade-provider / instrument-status-provider -> replay-execution-plane/contracts` | Market Data producer 直接消费 Replay Plane 输入合同 | 上提共享 manifest/ref contract，或改走 market fact rail |
