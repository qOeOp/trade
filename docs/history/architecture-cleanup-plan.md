---
title: Architecture Cleanup Plan
...
> 本文已完成使命，保留为 2026-07-08 至 2026-07-10 的施工图与决策记录，不再描述当前目录。当前架构以 [README](../../README.md)、[tool-layout.md](../architecture/tool-layout.md)、[design-architecture.md](../architecture/design-architecture.md)、[architecture-manifest.json](../architecture/architecture-manifest.json) 和生成的 [architecture-drift-report.md](../architecture/generated/architecture-drift-report.md) 为准；后续不得从本文“目标目录形态”恢复已退役路径。
...
NOFX 可借鉴的是 runtime discipline，不是产品形态；详细记录见 [nofx-design-absorption.md](./nofx-design-absorption.md)。
...
- P0 inventory：新增 `docs/architecture/audits/architecture-inventory.md`，记录 tool、command、热点文件、测试基线。
...
- 研究侧 campaign 拆分：
  - `rd-campaign-runner/src/lib/rd-campaign-runner.ts`：R&D campaign orchestration、calibration gate、discovery/external validation non-overlap；locked holdout 保持独立预算与独立证据语义。
...
- 执行 tool contract 收口：
  - `docs/runtime/execution-tool-contract.md` 落地 Binance 写 tool 成功输出到 `trade-flow` 记账的最低契约。
...
- Artifact / data hygiene：
  ...
  - `docs/engineering/data-hygiene.md` 落地 Git 边界与 data 留存规则；`data/*.db`、`tmp/panels/`、`tmp/artifacts/`、runtime DB、ledger、artifact 与本地 profile config 默认不进 Git。
...
- P6 命令与测试契约：
  - `docs/engineering/check-contract.md` 落地项目级检查契约，按 docs / CLI / schema / runtime / execution / recovery / observe / research / artifact / cron / 外部 tool 改动域给出最小检查命令。
  - `docs/architecture/audits/architecture-inventory.md` 的测试入口基线改为指向 `check-contract.md`，避免 P0 盘点与当前执行契约分叉。
...
| P6 检查契约 | 完成 | `docs/engineering/check-contract.md` 已覆盖 helper scripts 与各改动域 |
...
- `docs/engineering/code-quality.md`：定义“干净提交”的项目级品位线。
- `docs/engineering/check-contract.md`：新增 `project-quality`，准备提交、跨语言改动、新增脚本或修复 warning/error 后必须跑。
