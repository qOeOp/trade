...
files and parses each line into Vibe domain objects:
...
[`crates/trading/src/examples/actors/imbalance/actor.rs`](https://github.com/qOeOp/trade/tree/main/crates/trading/src/examples/actors/imbalance/actor.rs).
...
2. `vibe_actor!(YourType)` to wire up the core, plus a `Debug`
   implementation.
...
runtime actors. The `vibe_actor!` macro supplies the native runtime wiring
...
using the `vibe_dark` tearsheet theme.
...
[`crates/adapters/betfair/examples/betfair_backtest.rs`](https://github.com/qOeOp/trade/tree/main/crates/adapters/betfair/examples/betfair_backtest.rs).
