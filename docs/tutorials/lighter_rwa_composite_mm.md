...
through the Vibe Databento adapter.
...
- A Cargo project with the Vibe, Lighter, and Databento crates as
...
The `examples` feature on `vibe-trading` exposes the `CompositeMarketMaker`
strategy, and `high-precision` is required for Lighter's crypto-native pricing.
For the general crate layout and feature flags, see the Rust
...
`vibe_dark` Plotly theme and deterministic replay data so docs builds do not
