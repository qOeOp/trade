...
order lifecycle, then writes four PNG panels using the ``vibe_dark``
tearsheet theme.
...
SUBMIT = re.compile(
    rf"{TS}.*\[SUBMIT_ORDER\] Vibe '(?P<id>[^']+)' .*side=(?P<side>Buy|Sell) qty=(?P<qty>[\-0-9.]+)",
)
