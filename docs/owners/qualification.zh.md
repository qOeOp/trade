...
- **CURRENT_PARTIAL - Candidate Intake：** `crates/qualification/src/postgres.rs` 的
  `submit_candidate_intake_v1` 在 Candidate advisory lock 下写入回执，并对已 intake 的 Candidate 的第二次评审请求
  返回类型化冲突。它没有生产调用者：本 Owner 之外的每一处调用都位于
  `crates/strategy_factory/src/iteration_decision_postgres.rs` 的 `sealed-develop-composer-acceptance` 测试模块里。
...
- **TARGET - Eligibility State：** 该模块拥有 `INELIGIBLE` `QUALIFIED` `EXPIRED` 与 `REVOKED`，其中只有前两个有实现。
  `crates/strategy_governance/src/model.rs` 的 `EligibilityState::Expired` 与 `::Revoked` 在全仓没有任何生产者，
...
- **TARGET - 需要部署授权的终端：** `DEPLOYMENT_STORE_ADMISSION_MODE` 保持 `disabled`，它在等什么记录在上文
  `Eligibility 终端状态` 一节。
- **CURRENT，且永久不可证 - 特定事故 Owner 重建：** 机器已合入 - `crates/qualification/src/recovery.rs`，
  经 `run_owner_recovery_cli` 导出，并以 `owner-recovery` 特性后面的 `qualification-owner-recovery` 二进制交付
