...
- **TARGET / IMPLEMENTATION_ADMITTED - `PAPER` 场所绑定：** 准入的切片是一次失败关闭的解析，它回答"对某个
  ...
  索要一个场所。接上第一个调用方的人，应当先建立一条场所腿的对照：这条路径上从来没有任何东西
  接触过场所，所以第一次尝试会在「本次解析有缺陷」与「场所腿本来就不通」之间二义，而事后把两者分开
  要花一轮，事前分开不花。`crates/adapters/binance` 可以自己对着 testnet 回答这个问题。那条对照是诊断
  而不是里程碑 - 它所经的 adapter 是继承来的、未准入、且与本 Owner 没有任何接线，所以那里的绿只说明
  场所腿是活的，对本次解析、对任何通向生产的路径，什么都没说。现在准入它，是因为它是 adapter 边界上唯一不依赖任何更上游东西的一格：它据以解析的
