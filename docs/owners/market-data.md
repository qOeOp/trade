...
- Data vendors and trading venues provide raw market and reference records through Data Clients, and every time
  ...
  publication the venue never claimed. On a submitted PIT Snapshot Request or Instrument Master admission every
  coordinate is stated by the submitter and carries the clock identity and epoch it claims; this Owner assigns
  none of them, and admits them only by comparing each claimed clock against its own sealed head and refusing on
  mismatch. On the live market channel this Owner states the Owner-observation coordinate from that head itself.
  Binding a coordinate to the head is therefore admission, not attribution: a claimed instant remains the
  submitter's claim after the head admits it, and is never compared to an Owner-stated instant as though one
  clock produced both. A record that states no coordinate of its own yields none for it, and this Owner never
  substitutes its own instant, the event instant, or a neighbouring record's stamp for a coordinate the source
  did not state.
