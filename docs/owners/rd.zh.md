...
逐字节与原先相同，所以没有任何关于某个程序的东西是活在写它的那个生成器里的。**停在推导之前的理由
比「别写重复代码」更硬：端口身份是对 `InputRoleV2` 求的摘要，因而依赖那个结构体的字段顺序，
而没有任何契约文字陈述过这个顺序，也没有承诺保持它。** 一个自己算身份的编写层，会把一个 Owner
随时可改的事实钉死；一个产出 `meaning` 的编写层则是继承它。
