...
- 向 [Backtest](./backtest/) 提供绑定请求 PIT 范围和 snapshot/correction rule 的准确 PIT Market Snapshot
  ...
  **实测 2026-09-22，而本段的第一版把形状判错了。**一个 PIT 快照是一个 as-of 切面，所以 N 根 bar 就是 N 个
  ...
  严格连续、无缺口、无重复，耗时 655.4 秒。主导这段时间的**不是**场馆：把每坐标的往返减半只省了 15%，
  而 256 坐标那一轮里全部 SQL 执行时间加起来只占 1.2%。
  ...
  语句，正好把实测时钟填满。所以总成本是二次的，**在测量主机上 t 约等于 0.0025 乘以 N 的平方秒**。
  按该曲线外推（这是外推，且假设库从空开始）：**一年的日线约五分半，一年的分钟线约二十二年**，
  而不是第一次测量按线性读出来的那个"天"。实际后果是：几百个坐标的有界窗口很便宜；长跨度的分钟级历史
  这条路径根本到不了；而且**一个从不重置的库会让此后每一个快照对每一个写入者都更慢**，
  所以在共享链路库里累积快照，花的是一笔不会回来的预算。
