...
| `bounded-mission-scripts` | `.agents/skills/run-bounded-mission` | `python3 .agents/skills/run-bounded-mission/scripts/mission_lifecycle_test.py`；合并时再运行 `python3 .agents/skills/run-bounded-mission/scripts/github_handoff_barrier.py --repo <owner/repository> --pr <number> --head <sha> --base <sha>` | 验证每个 turn 的 admission、七阶段顺序、noop、blocked 单调性、trusted boundary、结构化 carrier、merge attestation 与 terminal Terminate；PR barrier 消费一次 opening review，并只在稳定 closure 后执行 guarded direct squash merge |
