...
| `bounded-mission-scripts` | `.agents/skills/run-bounded-mission` | `python3 .agents/skills/run-bounded-mission/scripts/mission_handoff_stop_test.py`；`python3 .agents/skills/run-bounded-mission/scripts/github_handoff_barrier.py --repo <owner/repository> --pr <number> --head <sha> --base <sha>` | lifecycle hook 覆盖 root turn admission、阶段化写门、无可信 structured-output signal 时 fail-closed 与一次 Stop closure；handoff barrier 一次性消费 opening review，绑定 final PR head/base 后执行 guarded direct squash merge |
