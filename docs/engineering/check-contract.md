...
| `doc-contract-check` | repo root | `bun scripts/check-doc-contracts.ts` | 文档元数据、current/history 一级标题结构、role→status、可解析 owner、`last_verified` CST 日历日期、current index 精确闭包与 ID 命名空间、历史状态、Markdown 相对路径、risk Guard ID 对齐；不验证 freshness SLA、外部 URL 或页面 anchor |
