...
| `bounded-mission-scripts` | `.agents/skills/run-bounded-mission` | 测试：`python3 -m unittest discover scripts -p 'test_*.py'`；真实 Handoff：`python3 scripts/github_handoff_barrier.py --repo <owner/repository> --pr <number> --head <sha> --base <sha>` | skill 自包含 Stop/Handoff helper；重放 late review/thread race、绑定 original mission 与 final PR head/base，并只在稳定 closure 后执行 guarded direct squash merge |
