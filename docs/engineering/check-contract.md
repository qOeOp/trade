...
| `github-handoff-barrier` | repo root | 测试：`bun test scripts/github-handoff-barrier.test.ts scripts/codex-mission-stop-hook.test.ts`；真实 Handoff：`bun scripts/github-handoff-barrier.ts --repo <owner/repository> --pr <number> --head <sha> --base <sha>` | 重放 late review/thread race、绑定 original mission 与 final PR head/base，并只在稳定 closure 后启用 guarded squash auto-merge |
