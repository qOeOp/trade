...
| `bounded-mission-scripts` | `.agents/skills/run-bounded-mission` | `python3 .agents/skills/run-bounded-mission/scripts/mission_handoff_stop_test.py`；PR 合并使用 `python3 .agents/skills/run-bounded-mission/scripts/github_handoff_barrier.py --repo <owner/repository> --pr <number> --head <sha> --base <sha>` | lifecycle hook 定向回归；Handoff barrier 消费一次 opening review，绑定 original mission 与 final PR head/base，并只在稳定 closure 后执行 guarded direct squash merge |
