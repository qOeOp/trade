...
| `pr-lifecycle-gate` | GitHub Actions + ruleset | default branch `workflow_dispatch` → `bun scripts/check-pr-review.ts` | base-owned workflow 对 exact head/live base 两次读取 GitHub 原生 trigger、reaction、review、thread、fix reply 与 resolution；只读 verifier 通过后才向 exact head 写 required status；现有 quality、CodeQL 和 thread-resolution rules 仍独立执行 |
