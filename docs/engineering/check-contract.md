...
| `bounded-mission-scripts` | `.agents/skills/run-bounded-mission` | `python3 .agents/skills/run-bounded-mission/scripts/github_handoff_barrier.py --repo <owner/repository> --pr <number> --head <sha> --base <sha>` | skill 包只含运行 helper；消费一次 opening review，绑定 original mission 与 final PR head/base，并只在稳定 closure 后执行 guarded direct squash merge |
