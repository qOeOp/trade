...
| `doc-contract-check` | repo root | `bun scripts/check-doc-contracts.ts` | 文档元数据唯一性、current/history 一级标题结构、role→status、可解析 owner、`last_verified` CST 日历日期、current index 精确闭包、ID/implementation ref、本地 Markdown 链接仓库边界、历史状态、risk Guard ID 对齐；不验证 freshness SLA、外部 URL 或页面 anchor |
