...
代码 mission 应明确交付终点。若终点是 GitHub merge，Handoff 只能在 Authority 明确允许 direct merge 时调用 `.agents/skills/run-bounded-mission/scripts/github_handoff_barrier.py`；脚本必须绑定 final head/base、终止精确 head Codex attempt、拒绝并发 trigger、等待 review/thread 活动稳定、验证必需检查与 conversation 清零，并在最终复验后使用 exact-head guard 执行 direct squash merge。脚本不得使用 `--auto` / `--admin`，返回非零不得另走 `gh pr merge` 绕过；只有 GitHub 返回 expected head 的 merge metadata 后才能 `accept`。该权限不含修改 ruleset、review dismissal / approval或普通评论；精确 head Codex trigger、回复、重新请求 review、conversation resolution 与 direct merge 必须分别冻结写权限，并在精确 head 上验证后执行。远程等待和修订仍消耗同一 Stop。
