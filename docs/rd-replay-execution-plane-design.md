...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification/termination、Reservation/Attempt cancellation/fencing、durable Artifact/outbox、checkpoint resume、Dataset Manifest v11、PIT supplemental/status/provenance、closed-candle/next-open、simple bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market；GTC 可由冻结 closed-bar Cancel 终止，Stop range 触发后路径不可证明时 typed-fail 且无 Result。R4.77–R4.116 已锁定四源 pre-worker evidence 链：跨源 ordering、PIT observation/derivation、正式 phase-required inputs、deterministic code、process-local registry exact lookup、Worker v9 compatibility identity、Logical Request Identity v2、不可执行 Worker Request v10、零实例 Worker Response v10 Contract、Attempt-bound Execution Envelope、Dispatch Lease Admission 与 Control Plane Current Lease Observation Receipt/Authority Binding。R4.113 持久化 Control Plane receipt；R4.114 持久化完整 Envelope→Admission→Binding；R4.115 以 CAS 选出单一 opaque claimant；R4.116 在再次重验同代 Lease 后，先 CAS 提交不可重放 Process Launch Attempt，再由 Runner 以 exact attested Bun runtime/artifact 启动零 Worker Request 字节的 EOF probe，持久化本地 child handle/PID、exit/output hash 与 Process Launch Receipt。Attempt/Receipt 重启可读；相同完成态幂等，竞争、过期、续租漂移、孤儿/篡改失败，Attempt 后无 Receipt 必须停在 indeterminate 且不得自动 relaunch。该 Receipt 证明 Runner 本地观察到 exact argv 的 child start，不证明 PID 全局唯一、kernel start time、远端进程、OS sandbox、Worker Request transport、Harness invocation 或 Response；`dispatch_occurrence=not_materialized_zero_worker_request_bytes`。续租后旧 Envelope 必须失败并先生成 successor，跨 Attempt retry 必须使用新 root。Control Plane registry-read provenance 与独立时钟证明仍未进入 Replay wire，v10 Request dispatch、Response admission、Schedule/Harness Receipt 与 v9→v10 cutover 均未物化。正式结果继续写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45；真实 queue/depth partial、FOK/GTD、runtime Cancel/amend、多 pending order、Runner exact source、remote transport、停牌结算、cross/shared portfolio、tick/L2、generic matching 与 step/fast parity 仍缺，因此不升到 M3。
...
**当前状态：完成第一百一十六子集，R3 未完成。** R4.43–R4.76 已认证受限订单经济链，R4.77–R4.103 锁定跨源 PIT evidence 与完整 inputs，R4.104–R4.116 绑定 code、registry observation、v9 compatibility ID、v2 logical identity、不可执行 v10 Request、零实例 Response Contract、Attempt Envelope、半开 Lease freshness、Control Plane receipt lineage、durable registry、local claim 与 process probe Attempt/Receipt。双 Request hash、lease successor、retry root、observation-time freshness、SQLite current-authority source、restart read、first-claimant-wins、exact runtime/artifact child-start observation 与 orphan no-relaunch 已决；实际 Worker Request dispatch、Response instance、Schedule admission、Control Plane registry-read provenance、kernel start/remote process attestation、Harness Receipt 和激活门尚未物化。terminal preemption、capacity causality、zero-execution accounting、EventKey race、ambiguity、continuity fence 与 clean/resume parity 已锁定。仍缺 rolling supplemental、Worker/economic Wire execution admission、legacy parity、Runner exact-trigger、runtime Cancel、真实 liquidity partial、多订单、halt settlement、generic resolver 与 step/fast parity。
...
**当前状态：完成第一百一十六子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat 与三种 pending-entry 零成交终态。R4.77–R4.116 pre-worker evidence 不改变经济投影；R4.112–R4.116 只增加 Lease observation lineage、持久化、pre-dispatch recovery、claimant exclusivity 与零请求 process probe，既不派发 Worker Request、不接受 DecisionOutput，也不执行策略或重算账本。partial PnL/Fee、Funding、cash/equity、Margin、EOD/Cancel valuation 与零成交 cash conservation 已守恒。现有 lane 仍是单 settlement asset、单 isolated Position；borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.116 已冻结四源 pre-integration evidence 到不可执行 Worker Request/零实例 Response Contract/Attempt Envelope/Dispatch Lease Admission/Control Plane Observation Binding，并持久化 Observation Receipt、pre-dispatch bundle、local claim 与 Process Launch Attempt/Receipt。双 Request hash、Attempt/worker/ordinal、精确 generation、successor/retry root、`[heartbeat, expiry)` freshness、SQLite current-Attempt source、restart read、post-registration/post-claim revalidation、first-claimant-wins、exact runtime/artifact child start 与 orphan no-relaunch 已决定；`worker_id`、opaque claimant、local process receipt 三者仍是不同身份，PID 不可单独作为 authority，旧 generation 不能凭未届旧 expiry 继续准入，v9 execution path 不变。尚未决定并实现的是 Worker Request dispatch occurrence、claim/launch crash 的人工恢复制度、Control Plane registry-read provenance、独立 clock/kernel-start/remote process attestation、Response reproducibility pair、Schedule admission/Harness Receipt、transport/Worker certification 与 v9→v10 activation/cutover gate。Runner local CAS 不是 OS-enforced immutability 或 remote durability；Code Admission 仍无独立 durable registry、stable instance、signature 或 future lookup guarantee。R4.85–R4.116 已机器拒绝仅凭 schema、claim 或 process start 开放 Runner exact-trigger。
