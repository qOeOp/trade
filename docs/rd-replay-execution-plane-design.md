...
| execution authority | Reservation scheduled/immediate cancellation、active Attempt cancellation、terminal race、observation ack、ack registry failure | receipt/observation self-hash、CAS/单目标；新 claim 在 effective boundary 阻断；active cancellation 精确命中 lease generation；directive 只接受未过期 exact lease；terminal 后无新 checkpoint/Result authority；三段 latency 非负守序 | 生效前 active claim 的幂等重送不变；Reservation-only cancellation 不改变 active Attempt；Attempt-only cancellation 不暗中禁止 retry；重复 observation 不变；authority cancel 清除无权 diagnostic 文件但普通 cooperative resume 不变 | Control Plane claim/renew/finalize/Receipt fence + injected coordinator port + SQLite directive → Runner Run Outcome v35 Observation → Control Plane registry/latency projection；remote transport/polling/SLA 未认证 |
...
**当前状态：完成第五十八子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50 阻断 missing-grid 被误当 observed gap；R4.51 将 complete halt gap 与普通缺口分离；R4.52 阻断 current/periodic snapshot 被包装成 complete history；R4.53 增加 imported archive/provider 内容闭包；R4.57–R4.60 补齐 provider certification 未来 admission、显式 cancellation authority、observation/ack 与 transport-neutral coordinator。terminal preemption、long/short lane symmetry、economic-policy tamper rejection、status/provenance/continuity fence 与 clean/resume parity 已锁定。仍缺外部 venue completeness、multiple partial、真实 historical liquidity partial、通用 cancel/amend、limit/TIF、add/reversal、partial liquidation、halt settlement、generic OHLC resolver 与 step/fast parity。
...
**当前状态：完成第五十八子集，未完成统一组合账本。** Request/Result 为 v24/v36，Artifact/Run Outcome 为 v38/v35；Simulator v10 的 accounting 算术不变，但缺失执行区间不得产生任何后续账本事实。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation 与 EOD valuation 已守恒；R4.51 允许 exact Funding/Mark 在 frozen halt 内继续进入账本/风险观察，R4.52–R4.60 只增强这些状态事实的来源、采集、准入、取消、observation 与协调闭包，不改变经济执行。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
