...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 的 entry 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market。Limit 以 OHLC open/strict-cross、PIT capacity attestation 与 queue limitation 解析；Stop open gap 取 observed open，range trigger 取 trigger reference 后施加不利滑点。GTC 到数据边界仍未成交/触发则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`；Cancel 可固定执行或由 Harness `pending_entry` 相位重算。Stop range 触发 bar 若同时触达保护价，OHLC 无法证明触发后的路径，必须产生 self-hashed typed ambiguity 且无 Result。R4.77–R4.82 已冻结 exact-trade primitive、离线 archive/provider、Control Plane sidecar、cross-source ordering 与 lossless projection；R4.83 选择并行 candidate SourceEvent Wire v2，而非原地扩展已进入 Result/Checkpoint hash 的 legacy SourceEvent。Wire v2 可携带四源 typed inline payload、effective/availability、native identity 和新 ordering key，但仍固定 `runner_compatibility=not_bound`、legacy parity 未认证；外部 completeness、insurance/ADL、Fill/queue/cost 均未被证明。IOC 仍只属于 Limit earliest-open。正式结果写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、venue 历史 archive 外部穷尽审计、Runner exact source、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.68 已贯通 status imported archive/provider 与执行 authority；R4.79 建立 aggregate-trade certification/termination 和 Reservation-bound admission；R4.80 冻结跨源 envelope/key/ambiguity attestation；R4.81 再由 Control Plane 将真实 Attestation、aggregate admission、Reservation authority 与四源 hash 锁为不可变 sidecar；R4.82 认证 hash-only projection 的无损 lineage；R4.83 选择并冻结 parallel candidate Wire v2 schema。历史 archive 仍只能显式导入，两类 admission、projection 与 Wire v2 都不进入 Request/Runner。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、Projection-bound materializer、legacy parity、worker supervisor/pool identity、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第八十三子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链；R4.77–R4.81 冻结 aggregate-trade resolver/archive/provider/admission、跨源 Ordering Attestation 与 Control Plane sidecar；R4.82 完成 Admission-bound 无损 projection；R4.83 冻结不改写 legacy hash 的 parallel Wire v2 contract。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺 Projection-bound payload materializer、legacy parity、ambiguity execution gate、外部 complete-history certification、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
8. R4.80–R4.83 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、hash-only lossless projection 与 parallel candidate Wire v2，明确 source rank 只负责确定性而不证明 venue 顺序，且 legacy SourceEvent 不原地修改；尚未纳入 Mark，也未决定 Wire v2 何时绑定 Request/Reservation。Projection-bound payload materializer、legacy semantic parity 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留，Runner exact-trigger 不能仅凭 sidecar/projection/wire schema 开放。
