...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 的 entry 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market。Limit 以 OHLC open/strict-cross、PIT capacity attestation 与 queue limitation 解析；Stop open gap 取 observed open，range trigger 取 trigger reference 后施加不利滑点。GTC 到数据边界仍未成交/触发则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`；Cancel 可固定执行或由 Harness `pending_entry` 相位重算。Stop range 触发 bar 若同时触达保护价，OHLC 无法证明触发后的路径，必须产生 self-hashed typed ambiguity 且无 Result。R4.77 已以 pre-integration primitive 冻结 aggregate-trade event/coverage 与 ordered Stop path：连续 id 序列可区分同一 OHLC 内的 stop-first/target-first，但尚未进入 Runner，且外部 completeness、insurance/ADL、Fill/queue/cost 均不由此证明。IOC 仍只属于 Limit earliest-open。正式结果写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、权威 exact-trade archive、venue 历史 archive 外部穷尽审计、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
| OHLC/exact path | stop/target、open gap、single touch、long/short high-first/low-first collision、aggregate-trade target-first/stop-first、path/economics tamper | P1/P2 order/digest、bar/bracket/EventKey 自洽；aggregate-trade id 连续、半开 coverage/hash-bound，entry trigger event 不反向触发保护；独立 BigInt oracle 覆盖 cost arithmetic；重算 evidence hash 不能掩盖篡改 | price/cash 同比缩放不改 path role；轨迹 densification 不改 OHLC oracle；long/short reflection 保持 exact ordered owner；未来补数据不改已终止证据 | 已认证 synthetic ordered-price/economic oracle ↔ simple-bracket two-path parity，以及 aggregate-trade pre-integration resolver；真实 archive completeness、Runner exact source、真实成本拟合尚未完成 |
...
**当前状态：完成第七十七子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链、PIT capacity authority、GTC data-boundary、IOC first-open expiry、fixed/scheduled contract Cancel evidence，以及 Stop same-bar path ambiguity failure；R4.77 冻结 aggregate-trade coverage 与 ordered Stop pre-integration resolver。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺权威 complete-history exact source、跨 source EventKey merge、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
2. R4.71 已冻结 capacity attestation 的内部 schema、PIT 因果与跨合同哈希闭包，R4.72 已冻结数据边界 active/unfilled，R4.73 已冻结 IOC first-open expired/unfilled，R4.74–R4.75 已冻结 fixed/scheduled contract GTC Cancel，R4.76 已冻结 OHLCV Stop-market GTC、Cancel v2 与 same-bar path ambiguity，R4.77 已冻结 aggregate-trade event/coverage 与 ordered price-trigger resolver；production capacity source/校准算法、aggregate-trade complete-history producer/certification、外部真实性/完整性认证仍未决。两类 attestation 都不是 maker queue/depth 证明；真实 queue/depth/partial model，以及未冻结运行时 Cancel、IOC partial remainder与多订单何时进入 executable capability，须由真实数据能力和独立订单终态协议驱动。
...
8. R4.77 已冻结 aggTrades 内部以连续 aggregate-trade id 排序，同 timestamp 亦如此；但 aggTrades 与 mark、funding、instrument status、bar open/close 的跨 source EventKey phase/sequence 映射尚未冻结。缺 exchange sequence、archive gap 或跨源时钟关系时必须保留 limitation，不能用 stable id 猜时序。
