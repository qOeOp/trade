...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification/termination、Reservation/Attempt cancellation/fencing、durable Artifact/outbox、checkpoint resume、Dataset Manifest v11、PIT supplemental/status/provenance、closed-candle/next-open、simple bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market；GTC 可由冻结 closed-bar Cancel 终止，Stop range 触发后路径不可证明时 typed-fail 且无 Result。R4.77–R4.137 已锁定四源 pre-worker evidence 链及 Worker authority cutover。R4.128–R4.136 依次冻结 denied Gate、Frame v2、authority-stdio artifact、Authority Transport/Command/Intent/Capsule、capsule-bound current-Attempt revalidation与 fresh child；R4.137 由 process-bound CAS Attempt 占据唯一写槽，把从耐久 authority lineage 唯一重建的 Request Frame v2 写入 exact live handle、关闭 stdin，并在进程退出后只持久化 stdout/stderr 原始字节。当前 Request Frame/write Receipt 均为 1，decode Receipt、Response Frame/read/admission 均为零；raw stdout 即使来自 exit `0` 也只是 opaque candidate，不是 Response 或 DecisionOutput。正式结果继续写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45；真实 queue/depth partial、FOK/GTD、runtime Cancel/amend、多 pending order、Runner exact source、remote transport、停牌结算、cross/shared portfolio、tick/L2、generic matching 与 step/fast parity 仍缺，因此不升到 M3。
...
8. R4.80–R4.137 已冻结四源 pre-integration evidence 到不可执行 v10 Request/零实例 Response/Transport Contracts/Attempt Envelope/Lease authority，并持久化 receipt、bundle、claim、旧 process probe、blocked Gate、v10 Decoder/Stdio/Activated Stdio Capability、registry/clock provenance、Authority Frame Build Contract、Authority Transport/Command/Intent/Capsule、spawn revalidation、fresh process 与 Request dispatch Attempt/Receipt。R4.128–R4.136 关闭旧 artifact/frame、Frame v2、activated artifact、新 Transport、fresh Command/Intent、capsule、pre-spawn authority 与 process-start 缺口；R4.137 只有 durable dispatch Attempt 创建者和 exact live handle 可写一次 Request Frame，Receipt retry 不重写，并把 stdout/stderr 保持为 opaque raw bytes。尚未实现的是 raw Response fatal decode/echo/inner-response validation/admission、orphan Attempt 人工恢复、kernel/remote process attestation、Response reproducibility pair、Schedule admission/Harness Receipt 与 transport certification；local spawn/write observations 不消除 revalidation 后 cancellation/fencing race。Runner local CAS 不是 OS-enforced immutability 或 remote durability，base64 receipt carrier 也不是最终 Artifact 格式；Control Plane/Runner local process clock不是 signed TSA/NTP/remote trusted time。Request Frame/write Receipt=1，decode Receipt、Response Frame/read/admission=0；R4.85–R4.137 已机器拒绝仅凭 schema、claim、旧 clock/start evidence、artifact build、negative probe、zero-instance contract、durable Receipt 无 live handle、非可执行 intent 或 raw stdout 开放 Runner exact-trigger。
