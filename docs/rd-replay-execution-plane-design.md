...
| execution authority | Reservation scheduled/immediate cancellation、active Attempt cancellation、lease renewal、terminal race、observation ack、pre-terminal outbox、restart recovery-first admission、local namespace discovery | receipt/observation/outbox self-hash、CAS/单目标；active cancellation 精确命中未过期 current lease；outbox constructor 冻结 Attempt identity/generation floor，v2 persist/record 绑定 exact current lease 与 derived namespace；Engine boundary 返回 cancel 前提交且必须等于 terminal outcome；startup 先全量验证 discovery set | 生效前 active claim 幂等重送不变；同 Attempt generation 单调推进后仍可取消；already-registered 不重投；旧 generation、rollback、worker/Attempt 漂移、misplaced namespace、symlink、tamper 均 fail-before-delivery/poll/Engine；persistence failure 仍 cancel 但不 ack | Control Plane claim/renew/finalize/Receipt fence + injected coordinator + Attempt-scoped local outbox + SQLite directive → current-generation pre-terminal Outbox/Observation → local discovery + Control Plane pending/already inspection → no-replay ack；remote discovery/transport/polling/SLA 未认证 |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.66 已贯通 imported archive/provider、registry admission、source-batch audit、current-snapshot acquisition receipt、非追溯 rotation/revocation、emergency cancellation authority、transport-neutral coordinator、SQLite reference observation/ack/latency、no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission、renewal-aware binding 与 local namespace discovery；历史 archive 仍只能显式导入。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第六十四子集，未完成统一组合账本。** Request/Result 为 v24/v36，Artifact/Run Outcome 为 v38/v35；Simulator v10 的 accounting 算术不变，但缺失执行区间不得产生任何后续账本事实。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation 与 EOD valuation 已守恒；R4.51 允许 exact Funding/Mark 在 frozen halt 内继续进入账本/风险观察，R4.52–R4.66 只增强这些状态事实的来源、采集、准入、取消、observation、协调、ack retry 与 operational outbox/discovery 闭包，不改变经济执行。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
5. Reservation/Attempt Cancellation v1、transport-neutral coordinator、进程内 no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission、monotonic renewal binding 与 local namespace discovery 已冻结；Outbox v2 可独立 rehydrate cancellation-generation lease，SQLite reference path 可裁决 pending/already 并测三段 latency，authority cancel 删除无权 diagnostic 文件但保留 operational outbox。Control Plane poll 返回与 local commit 之间仍无跨存储事务；remote discovery/index、outbox retention/GC/quarantine、remote store、跨进程实现究竟用 push、poll、lease-renew response、IPC 或网络、startup/停止确认 SLA、worker 不响应时的进程 watchdog，以及 provider build/capability receipt 是否需要外部签名，仍需结合部署模型决定，不能让 Replay 动态查询 registry 代替。
