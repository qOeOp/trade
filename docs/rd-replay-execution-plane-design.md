...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 的 entry 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market。Limit 以 OHLC open/strict-cross、PIT capacity attestation 与 queue limitation 解析；Stop open gap 取 observed open，range trigger 取 trigger reference 后施加不利滑点。GTC 到数据边界仍未成交/触发则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`；Cancel 可固定执行或由 Harness `pending_entry` 相位重算。Stop range 触发 bar 若同时触达保护价，OHLC 无法证明触发后的路径，必须产生 self-hashed typed ambiguity 且无 Result。R4.77–R4.99 已冻结 exact-trade primitive、离线 archive/provider、Control Plane sidecar、cross-source ordering、lossless projection、parallel candidate SourceEvent Wire v2、typed materialization、受限 legacy parity、非经济 gate/reducer、availability-aware dual-clock cursor、closed-world Visibility Cut、Cut-bound PIT Payload View、Decision Observation Projection、frozen Schedule-entry/as-of binding、closed-world Binding Set、portable Observation Bundle、Control Plane Bundle Admission、Replay-owned full derivation attestation、Control Plane Derivation Admission、observation-to-Harness Context identity binding、正式 market/initial-signal supplemental input snapshot materialization与非执行型 Worker Input Assembly；effective timeline 不被可见性重排，迟到事实只能在 `availability_at` 后出现且禁止追溯执行，as-of cut 不能漏掉已可见事实，未来 payload 不得物化，bar-open 不得泄漏 range/close 字段，每个 selected decision time 必须等于其 observation as-of time，完整 Schedule 不得缺失、重复、乱序或混入其他 Schedule binding，携带的 Projection payload 必须与引用逐项一致且不能脱离 parent lineage 自证。R4.97 认证空 supplemental 与 admitted closed-bar Market Snapshot；R4.98 另以 Dataset Manifest/Request 绑定的独立通道认证非空 `signal_time_complete` 初始 Signal Snapshot；R4.99 将同一 Request 的 Context 与已认证 Snapshot 组成机器可审计 tuple：空模式可达到 `complete_non_executable_build_unbound`，非空模式因 R4.97 的空模式边界必须诚实标记 `incomplete_market_snapshot`，position-open 则标记 `incomplete_runtime_state_snapshot`。Assembly 不绑定 source bundle/build/invocation，不创建 Worker Request，不调用 Harness，也不进入 Runner；不覆盖 legacy payload/EventKey/cross-source-order parity。外部 completeness、insurance/ADL、Fill/queue/cost 均未被证明。IOC 仍只属于 Limit earliest-open。正式结果写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、venue 历史 archive 外部穷尽审计、Runner exact source、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
| 已正式实现并测试 | 单个 pre-entry Limit GTC/IOC 或 Stop-market GTC executable lane；fixed/scheduled contract-owned GTC Cancel | Limit 可推导 Fill/expiry/active/cancelled，Stop 可推导 triggered-and-filled/active/cancelled；Stop range 触发且同 bar 触达保护价时 typed-fail、无 Result。R4.77–R4.99 resolver、archive/provider、Control Plane sidecar、cross-source ordering、projection/materialization/parity、非经济 candidate trace、dual-clock cursor、visibility cut、PIT payload view、decision observation projection、schedule-entry/as-of binding、closed-world Binding Set、portable Observation Bundle、Bundle/Derivation Admission、derivation attestation、Harness Context identity binding、formal market/empty-supplemental snapshots、独立 initial-signal non-empty supplemental materialization 与非执行型 Worker Input Assembly 仅形成 pre-worker evidence seam，不是 Runner capability。amend、FOK/GTD、真实 queue/partial、多订单 allocation、multi-entry/reversal 尚未设计 |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt、Resume Authorization、Bundle Admission 与 Derivation Admission；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.68 已贯通 status imported archive/provider 与执行 authority；R4.79 建立 aggregate-trade certification/termination 和 Reservation-bound admission；R4.80 冻结跨源 envelope/key/ambiguity attestation；R4.81 再由 Control Plane 将真实 Attestation、aggregate admission、Reservation authority 与四源 hash 锁为不可变 sidecar；R4.82 认证 hash-only projection 的无损 lineage；R4.83 选择并冻结 parallel candidate Wire v2 schema；R4.84 补齐 Projection-bound typed materialization 与受限 legacy shared-schedule parity；R4.85–R4.92 增加只放行非经济 schedule trace 的 gate/reducer、availability-aware dual-clock cursor、closed-world as-of cut、Cut-bound PIT payload view、decision observation field projection、selected schedule-entry/as-of binding、完整 Binding Set 与 portable Observation Bundle；R4.93–R4.95 依次锁定 Bundle 治理 identity、Replay derivation 与 Control Plane derivation admission；R4.96–R4.99 再把 admitted boundaries 对齐 Harness Context v7，物化受限正式 market/empty-supplemental snapshots，以独立 Dataset-bound 通道物化 initial-signal non-empty supplemental Snapshot，并生成不会伪造 Worker Request 的输入装配状态。历史 archive 仍只能显式导入，projection、Wire v2、candidate trace、cursor/cut/view/observation/binding/set/bundle/admission/derivation/derivation-admission/context-binding/input-materialization/worker-input-assembly 都不进入 Runner；这些对象不替代 raw dataset validation、position-open State Snapshot、Worker Request 或 Harness Receipt。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、legacy payload/EventKey/cross-source-order parity、non-empty Request 的 Market materialization、滚动 supplemental、runtime State、economic Wire consumer、worker supervisor/pool identity、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental requirement/window 仍未完成。
...
**当前状态：完成第九十九子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链；R4.77–R4.81 冻结 aggregate-trade resolver/archive/provider/admission、跨源 Ordering Attestation 与 Control Plane sidecar；R4.82–R4.84 完成 projection、parallel Wire v2、typed materialization 与受限 legacy parity；R4.85–R4.92 锁定双时钟可见性、PIT payload、observation、Schedule binding 与 portable Bundle；R4.93–R4.95 锁定 Bundle/derivation 治理闭包；R4.96–R4.99 将每个 boundary 对齐 frozen Harness Context，生成受限正式 market/empty-supplemental snapshots、initial-signal non-empty supplemental materialization 与非执行 Worker input tuple 状态。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺 non-empty Request 的 Market Snapshot、滚动 supplemental 与 runtime State materialization、Worker Request/economic Wire consumer admission、legacy payload/EventKey/cross-source-order parity、外部 complete-history certification、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
**当前状态：完成第九十九子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.99 pre-integration/pre-worker evidence/admission/ordering/Wire candidate trace/cursor/cut/view/observation/binding/set/bundle/admission/derivation/context-binding/input-materialization/worker-input-assembly 不改变已发布结果的经济投影；R4.85–R4.99 更机器禁止这些对象产生 execution effect、追溯执行、Signal/Order 或 decision authority。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.99 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、projection、parallel candidate Wire v2、typed materialization、legacy shared-schedule parity、非经济 gate/reducer、dual-clock cursor、closed-world cut、Cut-bound PIT payload view、observation field whitelist、frozen schedule-entry/as-of binding、完整 Binding Set、portable Observation Bundle、Request/Reservation-aware Bundle Admission、full-chain Derivation Attestation、Derivation Admission、Harness Context identity binding、受限 formal input snapshot materialization 与 Worker input tuple assembly，明确 source rank 只负责确定性、availability 决定最早可见且迟到事实不得追溯执行、as-of view 不得少报已可见事实、未来 payload/未闭合 bar 字段不得物化、selected decision time 不得读取错位 prefix、Schedule member 不得缺失/重复/乱序/混装、portable payload 不得偏离 binding 且仍需 parent lineage，legacy SourceEvent 不原地修改。R4.94 在 Replay 内重放完整 parents 并拒绝 mixed root；R4.95 只由 Control Plane 验证 Attestation schema/hash 与 admitted Bundle binding，不重复执行父链，也不把接纳升级为执行权威；R4.96 对齐 canonical Harness Context；R4.97 认证空 supplemental 与 admitted closed-bar Market Snapshot；R4.98 通过独立 Dataset-bound 通道复验 initial-signal non-empty supplemental；R4.99 只按同一 Request 装配已认证输入并机器暴露 non-empty Market 与 position-open State 缺口，不接受裸 Snapshot 补洞。全部对象均不创建 Worker Request、不调用 Harness，尚未纳入 Mark。non-empty Request Market materialization、滚动 supplemental/runtime State、legacy payload/EventKey/cross-source-order parity、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.99 已机器拒绝仅凭 sidecar/projection/wire/cursor/cut/view/observation/binding/set/bundle/admission/derivation/derivation-admission/context-binding/input-materialization/worker-input-assembly 开放 Runner exact-trigger。
