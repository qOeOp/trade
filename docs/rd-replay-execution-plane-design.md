...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v29 / Schedule v6 的 entry 可冻结 next-open market，或单个 pre-entry GTC/IOC Limit。GTC 以 OHLC open/strict-cross、PIT liquidity-capacity attestation 与 queue limitation 解析；到数据边界仍未成交则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`。Cancel 可使用 R4.74 fixed-intent compatibility，或由 Schedule/Harness 在 `pending_entry` 相位以 PIT closed-bar Snapshot 重算；更早 Fill 使该决策 not-reached，same-close strict-cross 仍 Fill 胜。IOC 只观察 earliest-executable `bar_open`：全成或独立 `expired_unfilled`。四类结果均写入 Checkpoint v21、Result v42/Fingerprint 与 Artifact v44。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、venue 历史 archive 外部穷尽审计、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
| 部分设计 | Limit GTC/IOC 已有 executable lane；Stop-market pending 与单 Observation/Cancel race 只有 resolver；amend、FOK/GTD、真实 queue/partial、多订单 allocation、multi-entry/reversal 尚未设计 | 只有 GTC/IOC entry Result 可推导 Fill 或 expiry；其余 resolution reference 不等于 executable Runner capability |
...
当前 certified wire id 为 Control Plane `trade-flow.rd-experiment-contract.v3`、Trial Reservation v9、Provider Certification/Termination v1、Cancellation/Attempt Lease/Checkpoint Receipt/Resume Authorization，以及 Replay Request v29、Replay Entry Cancel Intent v1、Dataset Manifest v11、Liquidity Capacity Attestation v1、Decision Schedule v6/Timeline v9、Harness Context v7/Worker v8/Receipt v10、Pending Order Resolution v2、OHLCV Resolution Evidence v3、Result v42、Artifact v44、Engine Checkpoint v21、Run Outcome v35。Request/Reservation/Manifest/Result/Artifact 显式绑定 Limit capacity authority；Request/Execution Spec hash 同时绑定可选 Cancel Intent，Schedule/Timeline 可再绑定其 pending-entry Harness decision，Checkpoint 通过 Request/Manifest/Timeline hash 冻结恢复闭包。Simulator 为 v15，Journal/Equity 为 v5/v3；Storage/Numeric/Margin Policy 不变。
...
```json
{
  "schema_version": "trade.rd-replay-execution-request.v29",
  "run_id": "...",
  "idempotency_key": "...",
  "identity": {
    "experiment_id": "...",
  ...
  "decision_schedule": {
    "schema_version": "trade.rd-replay-decision-schedule.v6",
    ...
    "entries": [
      ...
      {"decision_sequence": 3, "decision_time": "...", "expected_effect": "authorized_entry_cancel", "authorized_entry_cancel": {"schema_version": "trade.rd-replay-entry-cancel-intent.v1", "intent_id": "...", "effective_at": "...", "intent_hash": "sha256"}, "authorized_order_hash": "sha256"},
  ...
  "order": {
    ...
    "entry_execution": {"order_type": "limit", "limit_price": 99.5, "time_in_force": "gtc | ioc", "liquidity_model": "ohlcv-cross-through-full-fill-bounded-v1", "full_fill_capacity": 1, "liquidity_capacity_attestation_hash": "sha256"},
    "entry_cancel_intent": {"schema_version": "trade.rd-replay-entry-cancel-intent.v1", "intent_id": "...", "authority": "experiment_contract", "target_order_role": "entry", "target_order_type": "limit", "target_time_in_force": "gtc", "requested_at": "...", "effective_at": "...", "effective_boundary": "after_bar_range", "reason_code": "experiment_contract_cancel", "intent_hash": "sha256"}
  },
  "executable_candidate": {"harness_bundle_hash": "sha256", "candidate_hash": "sha256"},
  "policies": {
    "simulator_policy_version": "rd-replay-simulator-v15",
    "assumptions_ref": "...",
...
```json
{
  "schema_version": "trade.rd-replay-result.v42",
  "result_id": "...",
  "run_id": "...",
  "attempt_id": "...",
  "idempotency_key": "...",
...
当前 Simulator v15 的 transition 是 append-only `OrderEvent`；conditional strategy fill 必须先 `triggered`。source vocabulary 为 `instrument_delisted|instrument_halted|instrument_resumed|funding|mark|bar_open|bar_range`。Mark 不触发 stop/TP；exact maintenance breach 只有在 trading 时才能创建 `liquidation`，halted 时必须 typed-fail。`authorized_entry_cancel` 是独立 `pending_entry` 决策相位，不使用 Position State；`authorized_protective_stop_replace` 保持 tighten-only。`authorized_partial_reduce` 在决策边界提交 `strategy_partial_reduce`，仅在严格更晚 eligible `bar_open` 全成 fixed quantity；同刻 exact risk、stop gap、target gap 先执行。partial Fill 位于 phase 20，旧 stop/target cancel 与剩余仓位 protection submit/activate 位于同 source 的 phase 90；无 SourceEvent 可插入。后续 source、Funding、Margin、State Snapshot 与 terminal owner 只读当前 Position/protection。非终止 `bar_range` 后必须先通过 continuity/status fence，才能发布 checkpoint 或消费未来 source。EOD 或更早 terminal 会取消 pending partial/final exit；`rd-replay-number-v3` 不变。

`Pending Order Resolution v2` 接入 pre-entry Limit GTC/IOC。Order 在 signal 后 active；GTC 从 next-open 起按 EventKey 逐 `bar_open/bar_range` 观察，未触达或 exact-touch 保持 resting，open marketable/strict-cross 生成 resolution-limited full Fill。IOC 只观察 earliest-executable `bar_open`，marketable 则全成，否则 `expired`，不进入 range。可选 `ReplayEntryCancelIntent v1` 只属于 GTC：R4.74 可固定执行，R4.75 可由 Schedule v6/Harness `pending_entry` 重算。scheduled lane 在 range 已可见后验证 exact output，State Snapshot 必须为空；更早 Fill 令 Cancel not-reached，同 close strict-cross Fill 胜，确定 non-fill Cancel 胜，exact touch typed-fail。Fill/expiry/Cancel EventKey 均严格晚于 decisive SourceEvent；Fill 价经 frozen adverse slippage 后仍不得劣于 limit，stop/target 再于 phase `90` 激活，因此不得回看已消费 boundary。解析链与 Decision Timeline 进入 Checkpoint/Result/Artifact。Stop pending、未冻结运行时 Cancel、amend/cancel-replace 和多单 allocation 仍不是 Runner capability。
...
| Cancel（受限） | Experiment Contract 预冻结的 pre-entry GTC Limit intent；指定 `bar_range` phase `20` 先于 phase `90` Cancel。strict-cross Fill 胜、确定 non-fill Cancel 胜、exact touch typed-fail；不是运行时命令、IOC Cancel 或通用 cancel API |
...
Observed-price gap policy 固定外壳：expected interval grid 完整，或 complete PIT status schedule 证明中间全程 halted 且当前 open 已 resumed 时，Market 使用 activation 后第一条 executable price；Stop/TP-market 在该 observed open 已越过 trigger 时以 open/quote 触发并成交，不能回填 trigger price；Limit 仍不得差于 limit。普通缺失 grid bar 的下一 open 不是“缺口期间首个可安全执行事实”，Simulator v15 在跨越未知区间前 typed-fail。具体 spread/slippage/impact 继续由冻结 cost/fill policy 决定。
...
当前 Result v42 延续单 settlement-asset Cash Ledger，并使用 Valuation/Equity v3 与 Journal v5。Limit entry 的实际 Fill fee/Position/Ledger 只使用 limit-respecting execution price；`pending_order_resolutions`、Decision Timeline 与 capacity attestation 是解析/authority 证据，不重复入账。partial Fill 的手续费和 realized PnL 以同一 EventKey 入账；Funding 只按 SourceEvent 前已生效 Position 计提。未成交 GTC EOD、IOC first-open expiry 与 contract-cancelled GTC 都是 `never_opened` 完成结果：无 Fill/Position/Funding/Margin、现金不变、PnL 为零；三者分别保留 active Order、绑定 expired OrderEvent/`bar_open` valuation、绑定 cancelled OrderEvent/`bar_close` valuation。Margin v7、exact-risk execution 与 OHLCV failure fallback 不变；cancel exact-touch ambiguity、data gap、halted breach仍由 Run Outcome v35 typed failure 表达且不发布 partial Result。
...
当前 Result v42 认证基础 PnL/cost、observed margin、`total_liquidation_fees`、Schedule v6/Timeline v9、OHLCV Resolution Evidence v3、Pending Order Resolution v2、nullable liquidity-capacity-attestation hash 与 typed `entry_outcome`。`pending_order_resolution_limited_count` 只计解析链中的 limitation；它不反写经济账本。Limit/partial 的经济效果都只能由 Fill → Position → Ledger → Equity/Metrics 派生。exact breach Result 含 simulated liquidation execution；`unfilled_at_data_end`、`expired_unfilled` 与 `cancelled_unfilled` 都有正式零交易 metrics，分别绑定 active GTC/data boundary、expired IOC/first open 与 cancelled GTC/frozen closed-bar boundary。scheduled Cancel receipt/not-reached 是 decision authority evidence，不产生账本现金流。OHLCV maintenance、halted maintenance 或 cancel-touch ambiguity 仍无正式 Result metrics。
...
当前 Artifact v44 通过 Artifact Store port 在 `logical idempotency key / attempt_id` namespace 中提交并绑定 storage policy；Fingerprint 分别绑定 status authority、liquidity-capacity attestation、Decision Timeline v9 与 `pending_order_resolutions_hash`。Runner 首次发布和幂等复读都重验 attestation、解析链、SourceEvent 与 `entry_outcome`：filled 分支必须绑定 limit-respecting entry Fill；GTC data-boundary 分支必须只有 resting chain 与 active Order；IOC expiry 分支必须只有一条 earliest-open expired resolution、独立 expired OrderEvent；GTC Cancel 分支必须绑定 frozen intent、terminal range source、phase-`90` Cancel key、cancelled OrderEvent 与 bar-close valuation。scheduled Cancel 还必须是 evaluated Harness receipt，或在更早 Fill 时是无 receipt 的 not-reached entry。三种 zero-execution 分支均无 Fill/Position。Checkpoint v21 通过 Request/Manifest/Timeline hash 与解析前缀冻结恢复闭包，恢复后必须重建同一 Result hash。local CAS 已认证，remote target 仍未认证。
...
| orders | market/bracket；pre-entry Limit GTC/IOC；fixed 或 scheduled contract-owned GTC Cancel；primitive 的 stop GTC/cancel race | fill 不早于 active/earliest boundary；bounded capacity；limit 不劣价；pending-entry Harness 无 Position State；range phase `20` 先于 Cancel phase `90`；touch ambiguity 无 Result；解析前缀严格有序 | buy/sell 镜像；clean/resume Result hash；未成交跨 bar 保持 active；更早 Fill→Cancel not-reached；same-close strict-cross Fill 胜、non-fill Cancel 胜 | Limit GTC/IOC/fixed+scheduled contract Cancel 已贯通 Runner/Timeline/Fill-or-zero-ledger/Artifact；Stop pending、未冻结运行时 Cancel、amend、多单与 fast 仍未认证 |
...
| decision boundary/timeline | evaluation/cutoff/earliest time、lookback 不足/gap/future-visible、Worker Order/Cancel 泄漏、pending-entry State 非空、exit/stop-replace/partial/cancel intent 篡改、stop 放宽/穿 target、state/source-prefix/protection generation 篡改、terminal/stop-gap 碰撞、resume 漂移、缺失/重复授权 | Timeline v9 恰有一条语义入场授权、可选一条匹配 Request 的 pending-entry Cancel、至多一次 tighten-only stop replace 或 fixed partial 和一个末位全量退出；更早 Fill 使 Cancel not-reached；State/Boundary/Entry/Timeline 自哈希，Checkpoint v21 另验 OrderEvent 末态、Intent、protection generation 与 status prefix | long/short 价格镜像及 partial terminal lane 保持 Fill/PnL/OrderEvent parity；scheduled Cancel fixed/scheduled 经济结果一致；重算 hash 后的 intent/trigger/generation 篡改仍拒绝；resume 不重跑已消费 Harness/Fill | Contract/Adapter/Worker/Engine/Runner/Artifact/Checkpoint 的 schedule/market/state/boundary/timeline/active-protection/pending-order hash 一致 |
...
- 冻结版本化 authority/evidence schema：Trial Reservation v8、Provider Certification/Termination v1、Reservation/Attempt Cancellation v1、Attempt Cancellation Observation v1、Attempt Lease v1、Checkpoint Receipt v2、Resume Authorization v1、Request v29、Dataset Manifest v10、Instrument Status Snapshot v1/Provenance v2、Supplemental Requirement Set v1、Decision Input/Market Snapshot v1、Decision State Snapshot v3、Entry Cancel/Partial Reduce/Reduce-only Exit/Protective Stop Replace Intent v1、Decision Schedule v6、Harness Context v7、Source Bundle v1、Build Attestation v2/Worker Protocol v8、Registry Capability v8、Harness Capability/Receipt v10、Boundary v8、Timeline v9、OHLCV Resolution Evidence v3、Data Gap Failure Evidence v1、Result v42、Artifact v44、Artifact Store Capability v1、Engine Checkpoint v21、Diagnostic Commit v2、Run Outcome v35、Cancellation Coordination Result v1；Simulator v15，Storage/Numeric/Journal/Equity/Margin Policy 不变。
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.68 已贯通 imported archive/provider、registry admission、source-batch audit、current-snapshot acquisition receipt、非追溯 rotation/revocation、emergency cancellation authority、transport-neutral coordinator、SQLite reference observation/ack/latency、no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission、renewal-aware binding、local namespace discovery、startup recovery job 与 recovery-before-claim gate；历史 archive 仍只能显式导入。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、worker supervisor/pool identity、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第七十五子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.75 完成单 pending-order resolver、pre-entry GTC/IOC Limit 经济链、PIT capacity authority、GTC data-boundary、IOC first-open expiry、fixed contract Cancel 与 scheduled pending-entry Cancel evidence。terminal preemption、capacity causality、zero-execution accounting、Fill/Cancel EventKey race、touch ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺 Stop pending、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order OHLC resolver 与 step/fast parity。
...
**当前状态：完成第七十五子集，未完成统一组合账本。** Request/Result 为 v29/v42，Artifact/Run Outcome 为 v44/v35；Simulator v15、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel 只增加 decision evidence，不改变零成交经济投影。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
2. R4.71 已冻结 capacity attestation 的内部 schema、PIT 因果与跨合同哈希闭包，R4.72 已冻结数据边界 active/unfilled，R4.73 已冻结 IOC first-open expired/unfilled，R4.74 已冻结 fixed contract GTC Cancel，R4.75 已冻结可选 Schedule/Harness pending-entry Cancel；production capacity source/校准算法、外部真实性/完整性认证仍未决。该 attestation 不是 maker queue/depth 证明；真实 queue/depth/partial model，以及未冻结运行时 Cancel、IOC partial remainder、Stop pending 与多订单何时进入 executable capability，须由真实数据能力和独立订单终态协议驱动。
