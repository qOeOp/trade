...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 的 entry 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market。Limit 以 OHLC open/strict-cross、PIT capacity attestation 与 queue limitation 解析；Stop open gap 取 observed open，range trigger 取 trigger reference 后施加不利滑点。GTC 到数据边界仍未成交/触发则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`；Cancel 可固定执行或由 Harness `pending_entry` 相位重算。Stop range 触发 bar 若同时触达保护价，OHLC 无法证明触发后的路径，必须产生 self-hashed typed ambiguity 且无 Result。R4.77–R4.79 冻结 exact-trade primitive、Market Data 离线归档/provider 与 Control Plane Reservation-sidecar admission；R4.80 已补 pre-integration cross-source EventKey/Ordering Attestation，明确同刻无全局序号时只能 resolution-limited。两者均尚未进入 Runner，且外部 completeness、insurance/ADL、production 跨源顺序、Fill/queue/cost 均不由此证明。IOC 仍只属于 Limit earliest-open。正式结果写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、venue 历史 archive 外部穷尽审计、Runner exact source、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
| cross-source ordering | status/funding/aggregate-trade/OHLCV 同刻与错峰 fixture；effective/availability 分离 | 各源 native order、半开 window、content/event/attestation hash；同刻跨源必须生成 ambiguity group | 相同输入复读 hash 不变；无跨源同刻时仅按 declared timestamp exact；source rank 漂移与 limitation overclaim fail-closed | pre-integration Contract/Data Adapter 已认证；既有 SourceEvent、Runner 与经济 EventKey 尚未消费 |
...
| exact-source admission | actual provider capability/build/policy certification；Reservation-bound archive/receipt/audit/evidence/attestation；revoke cutover | certification/admission self-hash、create-or-identical、每 Reservation hash 单 admission、reserved Trial 与半开有效窗；scope/completeness 不可扩张 | cutover 前 admission 不受后续 revoke 回写；相同输入重送不变，capability/evidence 漂移失败 | Control Plane registry/admission ↔ actual Market Data provider capability 已锁定；Reservation v9、Request/Runner 未消费，故不构成 execution parity |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.68 已贯通 status imported archive/provider 与执行 authority；R4.79 另为 aggregate-trade provider 建立不兼容语义隔离的 certification/termination registry 和 Reservation-bound pre-integration admission；R4.80 只在 Contract/Data Adapter 冻结跨源 envelope/key/ambiguity attestation。历史 archive 仍只能显式导入，aggregate admission 与 cross-source ordering 都不进入 Request/Runner。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、production cross-source merge、worker supervisor/pool identity、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第八十子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链；R4.77–R4.79 冻结 aggregate-trade coverage/ordered resolver、离线 archive/provider 与 Control Plane sidecar admission；R4.80 冻结不具经济 authority 的跨源 EventKey/ambiguity evidence。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺外部 complete-history certification、production cross-source SourceEvent merge、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
**当前状态：完成第八十子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.80 pre-integration evidence/admission/ordering 不改变已发布结果的经济投影。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey 与 ambiguity attestation，明确 source rank 只负责确定性而不证明 venue 顺序；尚未纳入 Mark，也未决定何时以哪个 Request/Reservation epoch 将其接入 production SourceEvent。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留，Runner exact-trigger 不能仅凭该 attestation 开放。
