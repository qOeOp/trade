...
| execution authority | Reservation scheduled/immediate cancellation、active Attempt cancellation、lease renewal、terminal race、observation ack、pre-terminal outbox、restart recovery-first admission | receipt/observation/outbox self-hash、CAS/单目标；active cancellation 精确命中未过期 current lease；outbox constructor 冻结 Attempt identity/generation floor，persist 绑定 exact current lease hash/generation；Engine boundary 返回 cancel 前提交且必须等于 terminal outcome；startup record 与 exact rehydrated invocation 全绑定 | 生效前 active claim 幂等重送不变；同 Attempt generation 单调推进后仍可取消；旧 generation、rollback、worker/Attempt 漂移、tamper 均不增加 poll/Engine execution；persistence failure 仍 cancel 但不 ack；authority cancel 清除无权 diagnostic 文件但保留 outbox | Control Plane claim/renew/finalize/Receipt fence + injected coordinator + Attempt-scoped local outbox + SQLite directive → current-generation pre-terminal Outbox/Observation → recovery-first ack；namespace discovery/remote transport/polling/SLA 未认证 |
...
**当前状态：完成第六十三子集，未完成统一组合账本。** Request/Result 为 v24/v36，Artifact/Run Outcome 为 v38/v35；Simulator v10 的 accounting 算术不变，但缺失执行区间不得产生任何后续账本事实。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation 与 EOD valuation 已守恒；R4.51 允许 exact Funding/Mark 在 frozen halt 内继续进入账本/风险观察，R4.52–R4.65 只增强这些状态事实的来源、采集、准入、取消、observation、协调、ack retry 与 operational outbox 闭包，不改变经济执行。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
5. Reservation/Attempt Cancellation v1、transport-neutral coordinator、进程内 no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission 与 monotonic renewal binding 已冻结；SQLite reference path 可测三段 latency，重复 ack/outbox 保留首次时间，authority cancel 删除无权 diagnostic 文件但保留 operational outbox。Control Plane poll 返回与 local commit 之间仍无跨存储事务；重启仍需 exact latest-generation invocation rehydration。Attempt namespace discovery/recovery index owner、outbox retention/GC、remote outbox/store、跨进程实现究竟用 push、poll、lease-renew response、IPC 或网络、停止确认 SLA、worker 不响应时的进程 watchdog，以及 provider build/capability receipt 是否需要外部签名，仍需结合部署模型决定，不能让 Replay 动态查询 registry 代替。
