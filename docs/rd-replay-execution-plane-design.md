...
**当前状态：完成第八十九子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.89 pre-integration evidence/admission/ordering/Wire candidate trace/cursor/cut/view/observation 不改变已发布结果的经济投影；R4.85–R4.89 更机器禁止这些对象产生 execution effect、追溯执行、Signal/Order 或 decision authority。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.89 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、projection、parallel candidate Wire v2、typed materialization、legacy shared-schedule parity、非经济 gate/reducer、dual-clock cursor、closed-world cut、Cut-bound PIT payload view 与 observation field whitelist，明确 source rank 只负责确定性、availability 决定最早可见且迟到事实不得追溯执行、as-of view 不得少报已可见事实、未来 payload 与未闭合 bar 字段不得物化，legacy SourceEvent 不原地修改；尚未纳入 Mark，也未决定 Wire v2 何时绑定 Request/Reservation。Legacy payload/EventKey/cross-source-order parity、Harness Context binding、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.89 已机器拒绝仅凭 sidecar/projection/wire/cursor/cut/view/observation 开放 Runner exact-trigger。
