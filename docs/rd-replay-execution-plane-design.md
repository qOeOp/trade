...
| execution authority | Reservation scheduled/immediate cancellation、active Attempt cancellation、terminal race、observation ack、ack registry/outbox persistence failure、restart recovery | receipt/observation/outbox self-hash、CAS/单目标；新 claim 在 effective boundary 阻断；active cancellation 精确命中 lease generation；directive 只接受未过期 exact lease；outbox 必须先于 ack；terminal 后无新 checkpoint/Result authority；三段 latency 非负守序 | 生效前 active claim 的幂等重送不变；Reservation-only cancellation 不改变 active Attempt；Attempt-only cancellation 不暗中禁止 retry；重复 observation/outbox 保留首次时间；recovery 不增加 poll/Engine execution；authority cancel 清除无权 diagnostic 文件但保留 outbox，普通 cooperative resume 不变 | Control Plane claim/renew/finalize/Receipt fence + injected coordinator + local durable outbox + SQLite directive → Runner Run Outcome v35 Observation → Control Plane registry/latency projection；remote outbox/transport/polling/SLA 未认证 |
...
5. Reservation/Attempt Cancellation v1、transport-neutral coordinator、进程内 no-replay ack retry 与 local durable outbox recovery 已冻结；SQLite reference path 可测三段 latency，重复 ack/outbox 保留首次时间，authority cancel 删除无权 diagnostic 文件但保留 operational outbox。outbox retention/GC、remote outbox/store、跨进程实现究竟用 push、poll、lease-renew response、IPC 或网络、停止确认 SLA、worker 不响应时的进程 watchdog，以及 provider build/capability receipt 是否需要外部签名，仍需结合部署模型决定，不能让 Replay 动态查询 registry 代替。
