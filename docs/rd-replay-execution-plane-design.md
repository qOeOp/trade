...
| 已正式实现并测试 | 单个 pre-entry Limit GTC/IOC 或 Stop-market GTC executable lane；fixed/scheduled contract-owned GTC Cancel | Limit 可推导 Fill/expiry/active/cancelled，Stop 可推导 triggered-and-filled/active/cancelled；Stop range 触发且同 bar 触达保护价时 typed-fail、无 Result。R4.77–R4.100 resolver、archive/provider、Control Plane sidecar、cross-source ordering、projection/materialization/parity、非经济 candidate trace、dual-clock cursor、visibility cut、PIT payload view、decision observation projection、schedule-entry/as-of binding、closed-world Binding Set、portable Observation Bundle、Bundle/Derivation Admission、derivation attestation、Harness Context identity binding、formal market/empty-supplemental snapshots、独立 initial-signal non-empty supplemental materialization、非执行型 Worker Input Assembly 与 supplemental-independent Market materialization 仅形成 pre-worker evidence seam，不是 Runner capability。amend、FOK/GTD、真实 queue/partial、多订单 allocation、multi-entry/reversal 尚未设计 |
