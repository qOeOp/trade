...
| --- | --- | --- | --- | --- |
| clock/visibility | close signal、next-open、funding boundary；effective/availability 双时钟、500ms 迟到 status、边界前/等于/首事件前 cut/view | 无事件可在 availability 前消费；迟到事实不得追溯产生 execution effect；as-of cut 必须是完整 Cursor 前缀；future payload 不得物化 | 全部时间平移不改相对结果；reorder+rehash 不能提前揭示，omit+rehash 不能隐藏已可见事实，payload substitution+rehash 不能脱离 Wire | availability 全 immediate 时 visibility/effective order 一致；step 与 replay chunking digest 尚待统一经济消费者 |
| cross-source ordering/projection/wire | status/funding/aggregate-trade/OHLCV 同刻与错峰 fixture；Admission-bound 一对一 projection；typed Wire v2 | 各源 native order、半开 window、content/event/attestation/admission/projection/wire/payload/trace/cursor/cut/view hash；同刻跨源必须生成 ambiguity group；各层不得丢 native identity、payload lineage、availability、source rank 或 ordinal | 相同输入复读 hash 不变；无跨源同刻时仅按 declared timestamp exact；Admission/projection/payload/envelope/order/visibility drift 与 migration overclaim fail-closed；证明旧 key 映射会反转同刻 source rank | materializer、受限 legacy shared-schedule parity、非经济 candidate reducer/dual-clock cursor/closed-world cut/PIT payload view 已认证；legacy payload/EventKey/cross-source-order parity、Harness、Runner 与经济 EventKey 尚未消费 |
...
**当前状态：完成第八十八子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.88 pre-integration evidence/admission/ordering/Wire candidate trace/cursor/cut/view 不改变已发布结果的经济投影；R4.85–R4.88 更机器禁止 candidate trace/cursor/cut/view 产生 execution effect、追溯执行或 decision authority。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.88 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、projection、parallel candidate Wire v2、typed materialization、legacy shared-schedule parity、非经济 gate/reducer、dual-clock cursor、closed-world cut 与 Cut-bound PIT payload view，明确 source rank 只负责确定性、availability 决定最早可见且迟到事实不得追溯执行、as-of view 不得少报已可见事实、未来 payload 不得物化，legacy SourceEvent 不原地修改；尚未纳入 Mark，也未决定 Wire v2 何时绑定 Request/Reservation。Legacy payload/EventKey/cross-source-order parity、Harness input projection、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.88 已机器拒绝仅凭 sidecar/projection/wire/cursor/cut/view 开放 Runner exact-trigger。
