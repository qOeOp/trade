...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification/termination、Reservation/Attempt cancellation/fencing、durable Artifact/outbox、checkpoint resume、Dataset Manifest v11、PIT supplemental/status/provenance、closed-candle/next-open、simple bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market；GTC 可由冻结 closed-bar Cancel 终止，Stop range 触发后路径不可证明时 typed-fail 且无 Result。R4.77–R4.134 已锁定四源 pre-worker evidence 链及 Worker authority cutover。R4.128 持久化 `launch_decision=denied` Gate；R4.129 冻结 Frame v2；R4.130 构建 authority-stdio artifact；R4.131 签发 Authority Transport v3；R4.132 以晚于旧 Intent 的 fresh Control Plane current-Lease read 签发 Authority Command v2；R4.133 再以晚于新 Command 的 fresh read 签发 Authority Process Launch Intent v2；R4.134 从 exact committed Transport/Command/Intent 物化七字段 Authority Capsule。当前旧 Command=1、旧 Intent=1、readiness gate=1、build contract=1、activated artifact=1、Authority Transport v3=1、新 Command v2=1、新 Intent v2=1、authority capsule=1；spawn revalidation、process/frame/Response 仍为零，不能回写或复用旧 authority。正式结果继续写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45；真实 queue/depth partial、FOK/GTD、runtime Cancel/amend、多 pending order、Runner exact source、remote transport、停牌结算、cross/shared portfolio、tick/L2、generic matching 与 step/fast parity 仍缺，因此不升到 M3。
...
**当前状态：完成第一百三十四子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat 与三种 pending-entry 零成交终态。R4.77–R4.134 pre-worker evidence 不改变经济投影；R4.112–R4.134 只增加 Lease lineage、持久化、claim、process probe、blocked/authority transport contracts、v10 decoder/stdio/authority-stdio capability、registry/clock provenance、旧 command/intent、Authority Command/Intent v2、Capsule v1、launch-denied Gate 与 authority-frame build contract。Activated artifact、Authority Transport v3、新 Command/Intent 与 capsule 已构建，但无 process/frame，未派发 Worker Request、未接受 DecisionOutput，也未执行策略或重算账本。partial PnL/Fee、Funding、cash/equity、Margin、EOD/Cancel valuation 与零成交 cash conservation 已守恒。现有 lane 仍是单 settlement asset、单 isolated Position；borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.134 已冻结四源 pre-integration evidence 到不可执行 v10 Request/零实例 Response/Transport Contracts/Attempt Envelope/Lease authority，并持久化 receipt、bundle、claim、process probe、blocked Gate、v10 Decoder/Stdio/Activated Stdio Capability、registry/clock provenance、唯一旧 Command/Intent、launch-denied Gate、Authority Frame Build Contract、Authority Transport v3、Authority Command/Intent v2 与 Authority Capsule v1。R4.128 证明旧 artifact/frame 未消费 authority，R4.129 冻结 Frame v2，R4.130 构建 activated artifact，R4.131 冻结新 Transport，R4.132–R4.133 依次以 fresh current-Lease read 重签 Command/Intent，R4.134 才从 committed lineage 物化 exact 七字段 capsule，防止旧签发时刻、自哈希 payload、caller field 或 registry metadata 被迁移冒充。仍必须完成 spawn-boundary current-state revalidation 后才可 spawn；长期 artifact taxonomy 不能由当前 bridge 代替。尚未实现的是 spawn-boundary receipt、Attempt-bound process receipt、有效 frame/write/decode/dispatch、Response read/admission、claim/launch crash 人工恢复、kernel-start/remote process attestation、Response reproducibility pair、Schedule admission/Harness Receipt 与 transport certification。Runner local CAS 不是 OS-enforced immutability 或 remote durability；local process clock不是 signed TSA/NTP/remote trusted time，旧 Command=1、旧 Intent=1、gate=1、build contract=1、activated artifact=1、Authority Transport v3=1、新 Command v2=1、新 Intent v2=1、capsule=1，但 process/frame=0，R4.117/R4.119 与旧 v9 probe 保持不可改写。R4.85–R4.134 已机器拒绝仅凭 schema、claim、clock evidence、旧 process start、artifact build、negative probe、zero-instance contract、未消费 command、未 fresh revalidate spawn 或非可执行 intent 开放 Runner exact-trigger。
