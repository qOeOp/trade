...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 的 entry 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market。Limit 以 OHLC open/strict-cross、PIT capacity attestation 与 queue limitation 解析；Stop open gap 取 observed open，range trigger 取 trigger reference 后施加不利滑点。GTC 到数据边界仍未成交/触发则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`；Cancel 可固定执行或由 Harness `pending_entry` 相位重算。Stop range 触发 bar 若同时触达保护价，OHLC 无法证明触发后的路径，必须产生 self-hashed typed ambiguity 且无 Result。R4.77–R4.83 已冻结 exact-trade primitive、离线 archive/provider、Control Plane sidecar、cross-source ordering、lossless projection 与 parallel candidate SourceEvent Wire v2；R4.84 已补 Projection-bound typed payload materializer，并用真实 legacy builder 认证 Instrument Status、Funding 与 OHLCV 的共有事件调度 correspondence。该 parity 不覆盖 payload、EventKey、跨源同刻顺序或 Wire-only Aggregate Trade，Wire v2 仍固定 `runner_compatibility=not_bound`；外部 completeness、insurance/ADL、Fill/queue/cost 均未被证明。IOC 仍只属于 Limit earliest-open。正式结果写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、venue 历史 archive 外部穷尽审计、Runner exact source、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
**当前状态：完成第八十四子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链；R4.77–R4.81 冻结 aggregate-trade resolver/archive/provider/admission、跨源 Ordering Attestation 与 Control Plane sidecar；R4.82 完成 Admission-bound 无损 projection；R4.83 冻结不改写 legacy hash 的 parallel Wire v2 contract；R4.84 实现 Projection-bound payload materializer 并认证 legacy 共有事件调度 correspondence。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺 legacy payload/EventKey/cross-source-order parity、ambiguity execution gate、外部 complete-history certification、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
**当前状态：完成第八十四子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.84 pre-integration evidence/admission/ordering/projection/Wire v2 materialization/parity 不改变已发布结果的经济投影。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.84 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、hash-only lossless projection、parallel candidate Wire v2、Projection-bound typed materialization 与 legacy shared-schedule parity，明确 source rank 只负责确定性而不证明 venue 顺序，且 legacy SourceEvent 不原地修改；尚未纳入 Mark，也未决定 Wire v2 何时绑定 Request/Reservation。Legacy payload/EventKey/cross-source-order parity、ambiguity execution gate 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留，Runner exact-trigger 不能仅凭 sidecar/projection/wire schema 开放。
