...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.68 已贯通 status imported archive/provider 与执行 authority；R4.79 建立 aggregate-trade certification/termination 和 Reservation-bound admission；R4.80 冻结跨源 envelope/key/ambiguity attestation；R4.81 再由 Control Plane 将真实 Attestation、aggregate admission、Reservation authority 与四源 hash 锁为不可变 sidecar；R4.82 认证 hash-only projection 的无损 lineage；R4.83 选择并冻结 parallel candidate Wire v2 schema；R4.84 补齐 Projection-bound typed materialization 与受限 legacy shared-schedule parity；R4.85–R4.92 增加只放行非经济 schedule trace 的 gate/reducer、availability-aware dual-clock cursor、closed-world as-of cut、Cut-bound PIT payload view、decision observation field projection、selected schedule-entry/as-of binding、完整 Binding Set 与 portable Observation Bundle。历史 archive 仍只能显式导入，两类 admission、projection、Wire v2、candidate trace、cursor/cut/view/observation/binding/set/bundle 都不进入 Request/Runner；新 Binding/Set/Bundle 也不替代 Request-aware Schedule validator 或 Harness Context。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、legacy payload/EventKey/cross-source-order parity、Harness Context binding、economic Wire consumer、worker supervisor/pool identity、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第九十二子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.92 pre-integration evidence/admission/ordering/Wire candidate trace/cursor/cut/view/observation/binding/set/bundle 不改变已发布结果的经济投影；R4.85–R4.92 更机器禁止这些对象产生 execution effect、追溯执行、Signal/Order 或 decision authority。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.92 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、projection、parallel candidate Wire v2、typed materialization、legacy shared-schedule parity、非经济 gate/reducer、dual-clock cursor、closed-world cut、Cut-bound PIT payload view、observation field whitelist、frozen schedule-entry/as-of binding、完整 Binding Set 与 portable Observation Bundle，明确 source rank 只负责确定性、availability 决定最早可见且迟到事实不得追溯执行、as-of view 不得少报已可见事实、未来 payload/未闭合 bar 字段不得物化、selected decision time 不得读取错位 prefix、Schedule member 不得缺失/重复/乱序/混装、portable payload 不得偏离 binding 且仍需 parent lineage，legacy SourceEvent 不原地修改；尚未纳入 Mark，也未决定 Wire v2 何时绑定 Request/Reservation。Request-aware Schedule authority、Harness Context binding、legacy payload/EventKey/cross-source-order parity、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.92 已机器拒绝仅凭 sidecar/projection/wire/cursor/cut/view/observation/binding/set/bundle 开放 Runner exact-trigger。
