...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。新 owner 已实现 Control Plane identity binding、manifest/ref/content-hash 校验、UTC/lifecycle/PIT 声明、closed-candle/next-open、简单 bracket、EventKey 全序、average-cost Position、Cash Ledger、Equity v1 与 Journal v4。Control Plane 可签发带准入截止时间的 Trial Reservation、Replay Attempt Lease、Checkpoint Receipt 与 Resume Authorization；State Store 保证 Reservation 半开有效期、active-attempt 唯一、heartbeat generation fencing、receipt offset 单调、expiry takeover、terminal immutable，并只把 `cancelled/expired` source Attempt 的最新 receipt 绑定到同 authority 下更晚的 active target Attempt。Runner 只通过 Attempt-scoped Artifact Store port 发布与读取证据；certified local adapter 在完整 source-event 边界按 `rd-replay-local-fsync-link-cas-v1` 耐久发布 payload/commit/member/Manifest，恢复校验 authorization、source commit、storage policy、namespace、target Attempt/worker 与 lease generation floor，结果与 clean run byte-semantically 相同。Capability v1 已冻结 remote conditional-put 目标合同，但 remote adapter 未认证且在 engine 前显式拒绝。Dataset Manifest v7 已认证连续多规则 epoch schedule、supplemental signal-time revision view 与 Contract-derived Requirement Set；Decision Input Snapshot v1 固化实际可见输入。required lane 现在只能按 `harness_hash` 从 certified registry capability 解析 immutable UTF-8 source-set bundle 与 reviewed static entrypoint；Bundle、Receipt、Result、Fingerprint、Artifact、checkpoint 全链绑定，未知 bundle/source tamper/loader drift/direct bypass 均拒绝。该闭包证明“登记的 bundle hash + reviewed entrypoint mapping 被执行并复现授权 Order”，尚无编译产物/机器码 attestation 与进程级隔离，也未证明通用 feature DAG 重算。自动历史规则采集、动态多决策 feature join、会改变 accounting 的 instrument-spec epoch、部分强平、cross/shared portfolio、真实 tick/L2、generic matching 与 step/fast parity 尚未形成，因此成熟度仍为 M2。
...
flowchart LR
  ...
  subgraph RP["Replay Execution Plane"]
    ...
    DH["Decision Harness\nfrozen snapshot -> authorized Order + receipt"]
...
| 已正式实现并测试 | supplemental temporal provenance、Decision Input Snapshot、immutable Source Bundle、Registry resolution 与实际 Harness invocation Receipt | 已绑定 Control Plane/Request/Result/Fingerprint/Artifact/checkpoint；Registry 只解析 reviewed static entrypoint，尚不证明构建产物字节、运行时依赖或进程隔离 |
...
5. **Harness 构建与隔离证明仍缺失**：required lane 已把 Request `harness_hash` 收紧为完整 UTF-8 Source Bundle hash，并经固定 Registry/Loader Policy 解析 reviewed static entrypoint；但 source-to-build attestation、依赖闭包、运行时字节与隔离 worker 尚未认证，动态策略每个 decision boundary 的 join 与完整 feature DAG trace 也未形成同一闭包。
...
当前 certified wire id 为 Control Plane `trade-flow.rd-experiment-contract.v3`、`trade.rd-trial-reservation-snapshot.v5`、`trade.rd-replay-attempt-lease.v1`、`trade.rd-replay-checkpoint-receipt.v2`、`trade.rd-replay-resume-authorization-snapshot.v1`，以及 Replay `trade.rd-replay-execution-request.v14`、`trade.rd-replay-dataset-manifest.v7`、`trade.rd-replay-supplemental-requirement-set.v1`、`trade.rd-replay-decision-input-snapshot.v1`、`trade.rd-replay-decision-harness-source-bundle.v1`、`trade.rd-replay-decision-harness-registry-capability.v1`、`trade.rd-replay-decision-harness-capability.v2`、`trade.rd-replay-decision-harness-receipt.v2`、`trade.rd-replay-result.v21`、`trade.rd-replay-artifact-manifest.v23`、`trade.rd-replay-artifact-store-capability.v1`、`trade.rd-replay-engine-checkpoint.v6`、`trade.rd-replay-diagnostic-checkpoint-commit.v2`、`trade.rd-replay-run-outcome.v18`；Harness Registry/Loader Policy 为 `rd-replay-decision-harness-registry-v1` / `rd-replay-registered-entrypoint-loader-v1`，Local Storage/Remote Target/Simulator/Numeric/Journal/Equity/Margin Policy 分别为 `rd-replay-local-fsync-link-cas-v1`、`rd-replay-object-conditional-put-cas-v1`、`rd-replay-simulator-v7`、`rd-replay-number-v3`、`rd-replay-journal-v4`、`rd-replay-equity-v1`、`rd-replay-isolated-margin-v7`。Experiment Contract v3 的 `replay_execution_input` 冻结 Requirement Set schema/hash；Control Plane 签发 Reservation 时复核该 hash，Developer 不能把 required 降级为 `none`。Attempt、checkpoint、authorization 与 storage 是运行 envelope；Decision Input Snapshot、source bundle 与 Harness Receipt 进入 Result/Fingerprint/Artifact/checkpoint binding。Artifact Manifest 另持久记录 storage policy 与 producer Attempt/lease hash，Control Plane 只接受当前 fencing generation 的 terminal finalize。
...
```json
{
  "schema_version": "trade.rd-replay-execution-request.v14",
  "run_id": "...",
  "idempotency_key": "...",
  "identity": {
    "experiment_id": "...",
...
当前受限实现已完成 reservation + attempt + receipt + resume authority 闭包：Control Plane 只从 `status=reserved` Trial 签发 v5 reservation，并在写 Attempt 前强制 `issued_at <= claimed_at < expires_at`；claim 还校验权威 Trial、两份 schedule、supplemental-facts 与 Requirement Set binding，active-attempt 唯一索引阻止双 worker；renew 必须在旧 lease 到期前推进 generation。Runner 通过 Artifact Store port 在 source-event 完整边界先按认证 storage policy 写由 `attempt lease generation + next_source_offset + checkpoint hash` 定名的 immutable payload/commit，再把 descriptor 交给外部控制回调。Control Plane 只在 descriptor producer 与当前 lease 精确一致、storage policy 受支持、`heartbeat_at <= recorded_at < lease_expires_at` 且 offset 前进时追加 Receipt v2。Resume Authorization 只接受 source 最新 receipt 并绑定 later active target Attempt；Runner 不接受裸 locator。cooperative cancel 返回 Run Outcome v18，不含 Result/Artifact；未认证 store 返回 `artifact-store-rejected/unsupported_contract`，只有 completed 可携带 Result hash、manifest ref/hash 与 terminal completeness hash。

### 5.2 目标 `ReplayExecutionResult`

```json
{
  "schema_version": "trade.rd-replay-result.v21",
  "result_id": "...",
  "run_id": "...",
  "attempt_id": "...",
  "idempotency_key": "...",
  ...
  "decision_input_snapshot": {"schema_version": "trade.rd-replay-decision-input-snapshot.v1", "snapshot_hash": "sha256", "selected_records": []},
  "decision_harness_bundle": {"schema_version": "trade.rd-replay-decision-harness-source-bundle.v1", "bundle_ref": "...", "bundle_hash": "sha256", "entrypoint": {}, "files": []},
...
R4.29 消除 caller 直接注入“自称匹配 hash 的 callback”：Request v14 的 required-lane `harness_hash` 必须等于 Source Bundle v1 hash。Bundle 以 canonical `utf8_source_set` 固化排序后的 repo-relative path、逐文件原始 UTF-8 内容/SHA-256 与唯一 `file_path + export_name` entrypoint；`bundle_ref` 只是 locator，不进入 bundle identity。Runner 只接受 Registry Capability v1，按 bundle hash 解析 process-lifetime immutable、reviewed static registration；未知 hash、文件内容/hash 漂移、entrypoint 缺失、registry/loader policy 漂移或 Order 不等均在 Engine 前 typed reject。Capability v2 与 Receipt v2 绑定 bundle ref/hash、Registry/Loader Policy、Snapshot、Order、trace 和自哈希。Result v21/Fingerprint/Artifact v23 持久化完整 Bundle，Engine Checkpoint v6 绑定 Snapshot/Bundle/Receipt/Loader Policy；幂等重读从 Artifact 恢复，不访问 registry、不执行 entrypoint。

这一闭包证明的是“完整源文件集合已有稳定内容身份，且 reviewed static registry 将该 hash 解析到一次实际执行的 entrypoint”；它不等同于编译产物或机器码 attestation，也不把同进程注册表当作未信任代码安全沙箱。Result 因此将旧 `decision-harness-source-registry-uncertified` 收窄为 `decision-harness-build-attestation-uncertified` info limitation。任意 feature DAG 重算、动态多 decision join、构建 provenance/SBOM/signature、进程隔离和资源配额仍未认证。venue-risk 与 instrument-spec epochs 继续以连续半开 schedule 覆盖完整窗口；pre/post lifecycle、future funding、hash drift、错位 grid、schedule gap/overlap、supplemental payload/sequence/revision/hash 漂移直接失败；整周期缺 bar、`current_snapshot_only`、`survivor_only` 仍输出 `resolution_limited`。不得把当前子集表述为任意 Candidate 可执行闭包。
...
当前 Result v21 认证基础 PnL/cost、observed margin 与独立 `total_liquidation_fees`，并持久化 Decision Harness Source Bundle、Decision Input Snapshot 与 Harness Receipt；wallet/collateral/settled cash 是 Trial Balance 事实。exact breach Result 含一次 simulated liquidation execution，但尚未宣称通用 `liquidation_count` metric；OHLCV breach 仍无正式 Result metrics，只在 typed failure 携带首次 snapshot + Observation。
...
├── decision-harness-bundle.json   # 完整 source-set；mode=none 时为 JSON null
├── decision-input-snapshot.json   # signal-time 实际可见输入；自哈希
├── decision-harness-receipt.json  # mode=none 时为 JSON null
...
├── engine checkpoint v6            # 非权威可恢复 payload；绑定 snapshot/bundle/receipt/loader
└── diagnostic checkpoint commit v2 # immutable versioned marker + storage policy；登记 Receipt 后方可授权恢复
```

当前 Artifact v23 通过 Artifact Store port 在 `logical idempotency key / attempt_id` Attempt namespace 中提交并持久绑定 storage policy；required role 包含完整 Decision Harness Bundle、Decision Input Snapshot 与 Harness Receipt，Bundle/Receipt 即使为 `null` 也必须存在，防止“未执行”和“Artifact 丢失”混淆。Runner 不再直接调用文件系统。Capability v1 固定 `namespace_scope=attempt`、`collision_identity=sha256_full_content`、`read_after_write=strong`、`commit_marker=manifest_last`。certified local adapter 的 `rd-replay-local-fsync-link-cas-v1` 创建并 `fsync` 同目录临时文件，以 hard-link create-if-absent 发布，删除临时文件后 `fsync` 目录；同 ref 同 hash 幂等、异 hash collision。全部 member 先不可变发布，Manifest 最后发布，旧 worker 不能覆盖已引用成员。remote target `rd-replay-object-conditional-put-cas-v1` 要求 `If-None-Match: *`、完整内容 SHA-256、strong read-after-write、Manifest-last 与 abort/expiry incomplete upload；这是准入合同而非已实现 adapter，当前注入会在 namespace open 和 engine 前 typed reject。Engine Checkpoint v6 只在完整 source-event 后产生并绑定 decision snapshot/bundle/receipt/loader policy；Diagnostic Commit v2 与 Receipt v2 绑定 storage policy。恢复只接受最新 receipt 派生 Authorization，并通过 port 验证 namespace/ref/hash/source prefix；listing、未登记对象和 KG 都不是 authority。
...
- `supplemental_facts_hash`（完整 revision stream）、`supplemental_requirement_set_hash`（Contract-derived 闭世界需求）、`decision_input_snapshot_hash`、nullable `decision_harness_bundle_hash/receipt_hash` 与 Registry/Loader Policy version
...
- `engine_version` 标识实现发布；required lane 的 `harness_hash` 是完整 Source Bundle identity；Registry/Loader Policy 与 `Decision Harness Receipt` 证明该 bundle 通过 reviewed static entrypoint 被解析并调用，但不证明 source-to-build、依赖闭包、运行时字节或隔离边界；`simulator_policy_version` 标识经济/撮合语义。各者不能互相替代。
...
| decision harness | registry 缺失、未知 bundle、source/file hash 或 loader policy 漂移、Snapshot/Receipt 篡改、派生 Order 漂移、direct Engine bypass | required lane 恰好解析并执行一次；Bundle/Receipt 绑定 Snapshot/Order/trace；幂等重读执行零次 | 追加未来 revision 不改变 Snapshot hash；相同 Snapshot/Bundle 必得相同 Receipt | Runner preflight 与 Engine admission；Result/Fingerprint/Artifact/checkpoint 的 bundle/registry/loader hash 一致 |
...
- 冻结版本化 authority/evidence schema：Trial Reservation v5、Attempt Lease v1、Checkpoint Receipt v2、Resume Authorization v1、Request v14、Dataset Manifest v7、Supplemental Requirement Set v1、Decision Input Snapshot v1、Decision Harness Source Bundle v1、Registry Capability v1、Harness Capability/Receipt v2、Result v21、Artifact v23、Artifact Store Capability v1、Engine Checkpoint v6、Diagnostic Commit v2、Run Outcome v18；Local Storage/Remote Target/Registry/Loader/Simulator/Numeric/Journal/Equity/Margin Policy 分别为 local-v1/object-v1/registry-v1/loader-v1/v7/v3/v4/v1/v7。
...
**当前状态：完成 Trial Reservation expiry、Attempt fencing、Checkpoint Receipt、Resume Authorization、运行边界控制、Artifact Store port、本地耐久提交、硬崩溃后最新已登记进度恢复、主数据准入、多规则 epoch、supplemental signal-time 闭世界准入，以及 Source Bundle/Registry 约束下的单决策 Harness admission。** Control Plane 已实现 Reservation v5 半开 claim 窗口、schedule/supplemental-stream/Requirement-Set binding、claim/renew/expire/finalize CAS、有效 lease 下 append-only Receipt v2 和 latest-receipt→target Attempt 授权；Runner 在 Engine 前复核 claim window 与全部 binding，且不以 Reservation 到期撤销已合法 claim 的 Attempt。完整 revision stream、Decision Harness Bundle、Decision Input Snapshot 与 nullable Harness Receipt 作为 Artifact member 耐久提交；required lane 只能按 Request bundle hash 经固定 Registry/Loader Policy 解析 reviewed static entrypoint，Order 不等即拒绝，Engine 二次校验并将 bundle/snapshot/receipt/loader binding 纳入 Result/Fingerprint/checkpoint。空证据绕过、缺失、陈旧、窗口外、未声明、重叠 scope、registry 缺失、未知或篡改 bundle、loader/Order/Snapshot 漂移与 direct bypass 均有负例；future revision 不改变过去 Snapshot 的 metamorphic invariant 已锁定。local parity、namespace confinement、stale generation/receipt、receipt idempotency、expiry takeover、CAS collision、authorization/tamper rejection、定向清理和 clean/resume parity 已实现。远端 adapter/provider fault certification、Harness source-to-build attestation 与进程隔离、历史规则采集、会改变 accounting 的 spec epoch 与动态多决策 supplemental join 仍未完成。
...
**当前状态：完成第二十九子集，R3 未完成。** Simulator v7 的 matching 算法未改变；本子集只收紧 Harness source identity 与 loader admission，不把 supplemental record、source file 或 feature trace 混入市场 SourceEvent。halt/resume、external command、多订单 matching、limit、amend/TIF、multi-entry/reversal、部分强平和真实 partial liquidity 尚未接入。
...
**当前状态：完成第二十九子集，未完成统一组合账本。** Request/Result 为 v14/v21，Artifact/Run Outcome 为 v23/v18；Reservation v5 与 Fingerprint 绑定 venue-risk/instrument-spec schedule、supplemental-facts、Requirement Set、Decision Harness Bundle、Decision Input Snapshot、Harness Receipt 与 Registry/Loader Policy，Margin v7 按事件时间解析风险 epoch，Journal v4 算法不变。flat/open/liquidation golden digest 分别为 `c6834df3a9b3e1761da5800a1cbdfec2e343ff2746c0fef7f944183e2877558d`、`44fcc2c1e0c4448957885451f9de43e41c30e4b53f57b35f5bd4a3e14ed3768b`、`3a8472daf206ecfc18a1f7ce0a590fbab31afe2aa54788cb34c612d252ab5363`。会改变 accounting 的 spec epoch、partial liquidation、bankruptcy/insurance/ADL、动态 collateral、borrow、cross/shared portfolio 与真实 execution reconstruction 未开始。
...
4. venue risk/instrument spec 历史快照的采集、签名、保留与纠错 owner；多 epoch 切换已按外部不可变 schedule 认证，但自动采集与会改变 accounting 的 spec epoch 仍未设计，不能声称自动复原 Binance 历史规则。
