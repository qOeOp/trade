...
当前 certified wire id 为 Control Plane `trade-flow.rd-experiment-contract.v3`、Trial Reservation v8、Provider Certification v1、Provider Certification Termination v1、Attempt Lease v1、Checkpoint Receipt v2、Resume Authorization v1，以及 Replay Request v24、Dataset Manifest v10、Instrument Status Snapshot v1/Provenance v2、Supplemental Requirement Set v1、Decision Input/Market Snapshot v1、Decision State Snapshot v3、Partial Reduce Intent v1、Reduce-only Exit Intent v1、Protective Stop Replace Intent v1、Decision Schedule v5、Decision Harness Context v6、Source Bundle v1、Build Attestation v2、Worker Protocol v7、Registry Capability v7、Harness Capability/Receipt v9、Decision Boundary v7、Decision Evidence Timeline v8、OHLCV Resolution Evidence v3、Data Gap Failure Evidence v1、Result v36、Artifact v38、Artifact Store Capability v1、Engine Checkpoint v17、Diagnostic Commit v2、Run Outcome v34。Termination 是 Control Plane admission fact，不是 Replay Request/Result wire 字段。Simulator 为 v10；Storage/Numeric/Journal/Equity/Margin Policy 不变。

Trial Reservation v8 冻结授权准入窗口 `[issued_at, expires_at)`、risk/spec/status 三份 schedule hash、status provenance/provider capability/provider certification hash、完整 supplemental revision stream hash 与 Requirement Set hash，并内嵌认证快照。发放时 Control Plane 从注册表按 certification hash 读取，不接受调用方内嵌对象；认证须处于 `[certified_at, min(valid_until, termination.effective_at))` 且 capability 与 binding 相等。Termination v1 只裁定新签发，不进入 Reservation：cutover 前已签发的 Reservation 即使尚未 claim，也不会隐式失效；若事故需要停止它，必须由独立的 Trial/Reservation/Attempt cancellation authority 显式表达。Reservation TTL 仍只控制新 Attempt claim；已合法 claim 的运行权继续由 Attempt lease/generation fencing 决定。Replay 不查询注册表、不刷新、延长或解析 termination，只复核 Reservation、Request 与 Dataset 三方闭包。
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation、Attempt、Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.57 已贯通 imported archive/provider、registry admission、source-batch audit、current-snapshot acquisition receipt 与非追溯 rotation/revocation；历史 archive 仍只能显式导入。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、既有 Reservation 紧急吊销、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第五十五子集，未完成统一组合账本。** Request/Result 为 v24/v36，Artifact/Run Outcome 为 v38/v34；Simulator v10 的 accounting 算术不变，但缺失执行区间不得产生任何后续账本事实。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation 与 EOD valuation 已守恒；R4.51 允许 exact Funding/Mark 在 frozen halt 内继续进入账本/风险观察，R4.52–R4.57 只增强这些状态事实的来源、采集与准入闭包，不改变经济执行。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
5. Termination v1 已冻结非追溯 rotation/revocation 与显式 hash 选择；若安全事故必须连同 cutover 前已签发但尚未 claim 的 Reservation 或 active Attempt 一并停止，应新增哪一层 cancellation authority，以及 provider build/capability receipt 是否需要外部签名，仍需结合部署模型决定，不能让 Replay 动态查询 registry 代替。
