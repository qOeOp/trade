...
| instrument status | complete trading/halted/resumed、same-time delisting、halted Funding/Mark、resume gap、acquisition receipt/raw payload、immutable archive/provider、archive/current/periodic provenance | attempt chronology/hash/payload bytes/CAS 自洽；current snapshot 禁止生成 historical batch；archive sequence/transition/anchor/finality/content/CAS 自洽；schedule 连续/覆盖/hash-bound；decision/executable/bar 不落 halt；缺 bar 只有全程 halt 才可跨越；halted breach 无 Result/Fill | 429→success retry、invalid body non-retry、receipt idempotence、payload tamper、missing/mismatched receipt、相同 archive/provider 重跑；相同 epochs 更换 source/normalization 必改变 provenance hash；cancel/resume 不重放 status/Funding | Collector + Store receipt/payload/source-batch gate + Provider capability/evidence + Replay Contract/Adapter/Reservation/Runner/Fingerprint；外部 venue authenticity/completeness 未认证 |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification、Reservation、Attempt、Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.56 已贯通 imported archive/provider、registry admission、source-batch audit 与 current-snapshot acquisition receipt；历史 archive 仍只能显式导入。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、认证撤销/轮换、halt settlement 与动态 supplemental join 仍未完成。
