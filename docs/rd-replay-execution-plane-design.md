...
当前受限实现已完成 reservation + cancellation + attempt + receipt + resume authority 闭包：Control Plane 只从 `status=reserved` Trial 签发 v8 Reservation，并在写 Attempt 前强制 `issued_at <= claimed_at < expires_at` 与未命中 effective Reservation Cancellation；claim 还校验权威 Trial、risk/spec/status schedule、status provenance、supplemental-facts 与 execution-spec binding。active-attempt 唯一索引阻止双 worker；renew 必须在旧 lease 到期前推进 generation。Attempt Cancellation 以当前 generation 原子终止 authority，旧 worker 后续写入拒绝。Runner 通过 Artifact Store port 在 source-event 完整边界提交 immutable checkpoint，再交给外部控制回调登记 Receipt；生产级 cancellation delivery adapter 尚未认证。Resume Authorization 只接受 source 最新 receipt；Runner 不接受裸 locator。cooperative cancel 返回 Run Outcome v34，不含 Result/Artifact；未认证 store、execution-relevant data gap 或 halted margin breach返回 typed failure，只有 completed 可携带权威 Result/Manifest/terminal completeness hash。
...
| execution authority | Reservation scheduled/immediate cancellation、active Attempt cancellation、terminal race | receipt self-hash/CAS/单目标；新 claim 在 effective boundary 阻断；active cancellation 精确命中 lease generation；terminal 后无新 checkpoint/Result authority | 生效前 active claim 的幂等重送不变；Reservation-only cancellation 不改变 active Attempt；Attempt-only cancellation 不暗中禁止 retry | Control Plane claim/renew/finalize/Receipt fence + Runner cooperative cancel；production delivery latency 未认证 |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation、Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.58 已贯通 imported archive/provider、registry admission、source-batch audit、current-snapshot acquisition receipt、非追溯 rotation/revocation 与显式 emergency cancellation authority；历史 archive 仍只能显式导入。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、production cancellation delivery/ack SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第五十六子集，未完成统一组合账本。** Request/Result 为 v24/v36，Artifact/Run Outcome 为 v38/v34；Simulator v10 的 accounting 算术不变，但缺失执行区间不得产生任何后续账本事实。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation 与 EOD valuation 已守恒；R4.51 允许 exact Funding/Mark 在 frozen halt 内继续进入账本/风险观察，R4.52–R4.58 只增强这些状态事实的来源、采集、准入与取消闭包，不改变经济执行。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
5. Reservation/Attempt Cancellation v1 已冻结 authority/storage fence；跨进程 coordinator 用 push、poll 还是 lease-renew response 投递，停止确认 SLA、worker 不响应时的 orphan artifact GC，以及 provider build/capability receipt 是否需要外部签名，仍需结合部署模型决定，不能让 Replay 动态查询 registry 代替。
