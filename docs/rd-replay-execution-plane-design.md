...
| 已正式实现并测试 | 单个 pre-entry Limit GTC/IOC 或 Stop-market GTC executable lane；fixed/scheduled contract-owned GTC Cancel | Limit 可推导 Fill/expiry/active/cancelled，Stop 可推导 triggered-and-filled/active/cancelled；Stop range 触发且同 bar 触达保护价时 typed-fail、无 Result。R4.77–R4.103 resolver、archive/provider、Control Plane sidecar、cross-source ordering、closed-world observation/derivation、Harness Context binding、formal Supplemental/Market/State input 与 Worker Input Assembly v1/v2/v3 仅形成 pre-worker evidence seam，不是 Runner capability；v3 tuple 完整不等于 Worker、Harness 或 runtime provenance 已认证。amend、FOK/GTD、真实 queue/partial、多订单 allocation、multi-entry/reversal 尚未设计 |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt、Resume Authorization、Bundle Admission 与 Derivation Admission；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53–R4.95 已贯通 status archive/provider、执行 authority、四源 ordering/Wire、双时钟 closed-world observation 与 Bundle/derivation governance；R4.96–R4.101 由 Data Adapter 物化静态 Supplemental/Market inputs 与 Assembly v2；R4.102–R4.103 由 Engine 物化 position-open State 并按 Schedule 完成 Assembly v3。上述 pre-worker 对象不进入 Runner，也不替代 raw dataset validation、完整 upstream lineage replay、Worker Request 或 Harness Receipt。OS sandbox、signed build provenance、venue 历史 archive 外部穷尽审计、legacy payload/EventKey/cross-source-order parity、滚动 supplemental、source bundle/build/invocation identity、economic Wire consumer、worker supervisor/pool identity、remote transport/SLA、halt settlement 与动态 supplemental requirement/window 仍未完成。
...
**当前状态：完成第一百零三子集，R3 未完成。** R4.43–R4.76 已认证 partial terminal owner、simple-bracket economics/oracle、data/status/execution authority 与单 pending-order Limit/Stop/Cancel 经济链；R4.77–R4.95 锁定 aggregate-trade、跨源 ordering/Wire、双时钟 PIT observation、portable Bundle 与 derivation governance；R4.96–R4.101 生成正式 Supplemental/Market inputs 与 Assembly v2；R4.102–R4.103 生成 Engine-owned position-open State materialization，并以 v3 完成所有 phase-required input tuple。v3 仍不重算经济状态、不证明缺少外部 parents 时的 upstream provenance，也不创建 Worker Request。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、continuity fence 与 clean/resume parity 已锁定。仍缺滚动 supplemental、source bundle/build/invocation binding、Worker Request/economic Wire consumer admission、legacy parity、外部 complete-history certification、Runner exact-trigger、runtime Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
**当前状态：完成第一百零三子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.103 pre-integration/pre-worker evidence 不改变已发布结果的经济投影；R4.102/v3 只引用正式 State Snapshot v3，不重算 Position/Cash/Fee/Funding/PnL。partial realized PnL/Fee、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position；borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.103 已冻结四源 pre-integration EventKey/ambiguity、Control Plane admission、Wire v2、双时钟 closed-world observation、Schedule binding、portable Bundle、full-chain Derivation、Harness Context identity、正式 Supplemental/Market/State input materialization 与 Worker Input Assembly v1/v2/v3。source rank 只提供确定性顺序；`availability_at` 决定最早可见，迟到事实不得追溯执行；未来 payload、未闭合 bar 字段、错位 prefix、缺失/重复/乱序 Schedule member 与 mixed parent root 均被拒绝。R4.103 只按 Schedule 将 R4.101 v2 与每个 position-open R4.102 parent 组合成完整但不可执行的 tuple；embedded parent schema/hash 与 cross-binding 可自校验，更上游 lineage、完整 SourceEvent prefix 仍需外部 parents。全部对象均不创建 Worker Request、不调用 Harness，Mark 仍未纳入 parallel candidate Wire。滚动 supplemental、source bundle/build/invocation identity、legacy payload/EventKey/cross-source-order parity、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.103 已机器拒绝仅凭 pre-worker evidence 开放 Runner exact-trigger。
