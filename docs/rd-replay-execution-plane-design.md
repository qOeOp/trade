...
当前成熟度判断：**M2 / 5，已认证的受限纵切**。Control Plane authority、provider certification registry/termination、Reservation/Attempt cancellation/fencing、local durable Artifact/cancellation outbox、checkpoint resume、Dataset Manifest v11、supplemental PIT Snapshot、PIT instrument status/provenance、closed-candle/next-open、简单 bracket、EventKey、average-cost Position、Cash Ledger、Equity v3、Journal v5 与 isolated Margin v7 已贯通。Request v30 / Schedule v7 的 entry 可冻结 next-open market、单个 pre-entry GTC/IOC Limit 或 GTC Stop-market。Limit 以 OHLC open/strict-cross、PIT capacity attestation 与 queue limitation 解析；Stop open gap 取 observed open，range trigger 取 trigger reference 后施加不利滑点。GTC 到数据边界仍未成交/触发则产生 `unfilled_at_data_end`，或由 Experiment Contract 预冻结的 closed-bar Cancel 产生 `cancelled_unfilled`；Cancel 可固定执行或由 Harness `pending_entry` 相位重算。Stop range 触发 bar 若同时触达保护价，OHLC 无法证明触发后的路径，必须产生 self-hashed typed ambiguity 且无 Result。R4.77–R4.78 冻结 exact-trade primitive、Market Data 离线归档与 provider；R4.79 已补 Control Plane 独立 certification/termination 和 Reservation-bound Evidence Admission，但其 scope 仍是 pre-integration sidecar，尚未进入 Runner，且外部 completeness、insurance/ADL、跨源顺序、Fill/queue/cost 均不由此证明。IOC 仍只属于 Limit earliest-open。正式结果写入 Checkpoint v22、Result v43/Fingerprint 与 Artifact v45。position-open 仍只允许互斥的一次 stop tighten 或 fixed partial，partial 后可追加一次末位 full exit。该纵切仍不支持真实 queue/depth partial、FOK/GTD、未冻结运行时 Cancel/amend、多 pending order、venue 历史 archive 外部穷尽审计、Runner exact source、remote transport、OS sandbox、停牌结算、cross/shared portfolio、tick/L2、generic matching 或 step/fast parity，因此不升到 M3。
...
**当前状态：完成第七十九子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链；R4.77–R4.79 冻结 aggregate-trade coverage/ordered resolver、离线 archive/provider 与 Control Plane sidecar admission。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺外部 complete-history certification、跨 source EventKey merge、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
**当前状态：完成第七十九子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.79 exact pre-integration evidence/admission 不改变已发布结果的经济投影。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
2. R4.71 已冻结 capacity attestation 的内部 schema、PIT 因果与跨合同哈希闭包，R4.72 已冻结数据边界 active/unfilled，R4.73 已冻结 IOC first-open expired/unfilled，R4.74–R4.75 已冻结 fixed/scheduled contract GTC Cancel，R4.76 已冻结 OHLCV Stop-market GTC、Cancel v2 与 same-bar path ambiguity，R4.77–R4.79 已冻结 aggregate-trade event/coverage、ordered resolver、离线 archive/provider、Control Plane certification 与 Reservation-bound sidecar admission；production capacity source/校准算法、aggregate-trade 外部 complete-history acquisition/authenticity/completeness 认证仍未决。两类 attestation 都不是 maker queue/depth 证明；真实 queue/depth/partial model，以及未冻结运行时 Cancel、IOC partial remainder与多订单何时进入 executable capability，须由真实数据能力和独立订单终态协议驱动。
...
8. R4.77–R4.79 已冻结 aggTrades 内部连续 id 排序、imported raw-byte/archive/provider hash closure 及 Trial-sidecar admission；但 admission 不提供 aggTrades 与 mark、funding、instrument status、bar open/close 的跨 source EventKey phase/sequence。缺 exchange sequence、外部 completeness 或跨源时钟关系时必须保留 limitation，不能用 stable id 或 Control Plane admission 猜时序。
