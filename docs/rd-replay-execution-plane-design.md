...
**当前状态：完成第九十三子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.93 pre-integration evidence/admission/ordering/Wire candidate trace/cursor/cut/view/observation/binding/set/bundle/admission 不改变已发布结果的经济投影；R4.85–R4.93 更机器禁止这些对象产生 execution effect、追溯执行、Signal/Order 或 decision authority。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.93 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、projection、parallel candidate Wire v2、typed materialization、legacy shared-schedule parity、非经济 gate/reducer、dual-clock cursor、closed-world cut、Cut-bound PIT payload view、observation field whitelist、frozen schedule-entry/as-of binding、完整 Binding Set、portable Observation Bundle 与 Request/Reservation-aware Bundle Admission，明确 source rank 只负责确定性、availability 决定最早可见且迟到事实不得追溯执行、as-of view 不得少报已可见事实、未来 payload/未闭合 bar 字段不得物化、selected decision time 不得读取错位 prefix、Schedule member 不得缺失/重复/乱序/混装、portable payload 不得偏离 binding 且仍需 parent lineage，legacy SourceEvent 不原地修改。R4.93 只将 Wire identity、Schedule、Bundle 与治理 authority 绑定，不认证 Projection derivation；尚未纳入 Mark。Harness Context binding、legacy payload/EventKey/cross-source-order parity、完整 parent derivation certification、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.93 已机器拒绝仅凭 sidecar/projection/wire/cursor/cut/view/observation/binding/set/bundle/admission 开放 Runner exact-trigger。
