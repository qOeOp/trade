...
- 冻结版本化 authority/evidence schema：Trial Reservation v9、Provider Certification/Termination v1、Reservation/Attempt Cancellation v1、Attempt Cancellation Observation v1、Attempt Lease v1、Checkpoint Receipt v2、Resume Authorization v1、Cross-source Ordering Admission v1、Decision Observation Bundle Admission v1、Decision Observation Bundle Derivation Attestation/Admission v1、Request v30、Dataset Manifest v11、Liquidity Capacity Attestation v1、Instrument Status Snapshot v1/Provenance v2、Supplemental Requirement Set v1、Decision Input/Market Snapshot v1、Decision State Snapshot v3、Entry Cancel Intent v1/v2、Partial Reduce/Reduce-only Exit/Protective Stop Replace Intent v1、Decision Schedule v7、Harness Context v7、Source Bundle v1、Build Attestation v2/Worker Protocol v9、Registry Capability v9、Harness Capability/Receipt v11、Boundary v8、Timeline v10、Pending Order Resolution v2、OHLCV Resolution Evidence v3、Stop Entry Same-bar Path Ambiguity v1、Aggregate Trade Event/Coverage Attestation/Exact Trade Stop Resolution v1、Data Gap Failure Evidence v1、Result v43、Artifact v45、Artifact Store Capability v1、Engine Checkpoint v22、Diagnostic Commit v2、Run Outcome v35、Cancellation Coordination Result v1；Simulator v16，Storage/Numeric/Journal/Equity/Margin Policy 不变。Aggregate-trade 三合同、Bundle Admission、Derivation Attestation 与 Derivation Admission 仍是 pre-integration authority/evidence schema，不属于 Request/Result wire。
...
**当前状态：完成第九十五子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50–R4.68 完成 data/status/provider/execution-authority 受限闭包；R4.69–R4.76 完成单 pending-order resolver、pre-entry GTC/IOC Limit 与 GTC Stop-market 经济链；R4.77–R4.81 冻结 aggregate-trade resolver/archive/provider/admission、跨源 Ordering Attestation 与 Control Plane sidecar；R4.82–R4.84 完成 projection、parallel Wire v2、typed materialization 与受限 legacy parity；R4.85–R4.92 以 pre-execution gate/candidate reducer、dual-clock cursor、visibility cut、PIT payload view、observation field policy、frozen decision-time binding、closed-world Binding Set 与 portable Observation Bundle 锁定 ambiguity 不得升级成经济 exact-trigger、迟到事实不得提前可见或追溯执行、已可见事实不得少报、未来 payload/未闭合 bar 字段不得泄漏、schedule boundary 不得读取错位 as-of prefix、完整 Schedule bindings 不得缺失/重复/乱序/混装、携带 payload 不得偏离 binding；R4.93 锁定 Bundle 治理 identity，R4.94 重放完整 derivation 并拒绝 mixed roots，R4.95 将 Attestation 与 admitted Bundle 锁成新 Control Plane receipt。terminal preemption、capacity causality、zero-execution accounting、Fill/trigger/Cancel EventKey race、Limit touch/Stop path ambiguity、Harness evaluated/not-reached、status/provenance/continuity fence、artifact 复读与 clean/resume parity 已锁定。仍缺 Harness Context binding、legacy payload/EventKey/cross-source-order parity、economic Wire consumer admission、外部 complete-history certification、Runner exact-trigger、未冻结运行时 Cancel、IOC partial remainder、FOK/GTD、amend、真实 liquidity partial、多订单 allocation、add/reversal、partial liquidation、halt settlement、generic multi-order resolver 与 step/fast parity。
...
**当前状态：完成第九十五子集，未完成统一组合账本。** Request/Result 为 v30/v43，Artifact/Run Outcome 为 v45/v35；Simulator v16、Equity v3、Journal v5 已区分 filled、open、flat、GTC active-never-opened、IOC expired-never-opened 与 GTC cancelled-never-opened。scheduled Cancel、Stop ambiguity 与 R4.77–R4.95 pre-integration evidence/admission/ordering/Wire candidate trace/cursor/cut/view/observation/binding/set/bundle/admission/derivation/derivation-admission 不改变已发布结果的经济投影；R4.85–R4.95 更机器禁止这些对象产生 execution effect、追溯执行、Signal/Order 或 decision authority。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation、EOD/Cancel valuation、受限 Limit/Stop entry 与零成交 cash conservation 已守恒。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
8. R4.80–R4.95 已冻结 Instrument Status、Funding、Aggregate Trade、OHLCV 的 pre-integration EventKey/ambiguity attestation、Reservation-bound Control Plane admission、projection、parallel candidate Wire v2、typed materialization、legacy shared-schedule parity、非经济 gate/reducer、dual-clock cursor、closed-world cut、Cut-bound PIT payload view、observation field whitelist、frozen schedule-entry/as-of binding、完整 Binding Set、portable Observation Bundle、Request/Reservation-aware Bundle Admission、full-chain Derivation Attestation 与 Derivation Admission，明确 source rank 只负责确定性、availability 决定最早可见且迟到事实不得追溯执行、as-of view 不得少报已可见事实、未来 payload/未闭合 bar 字段不得物化、selected decision time 不得读取错位 prefix、Schedule member 不得缺失/重复/乱序/混装、portable payload 不得偏离 binding 且仍需 parent lineage，legacy SourceEvent 不原地修改。R4.94 在 Replay 内重放完整 parents 并拒绝 mixed root；R4.95 只由 Control Plane 验证 Attestation schema/hash 与 admitted Bundle binding，不重复执行父链，也不把接纳升级为执行权威；尚未纳入 Mark。Harness Context binding、legacy payload/EventKey/cross-source-order parity、经济 consumer admission 与旧 Result/Checkpoint 到新 epoch 的升级门仍未设计。缺 exchange global sequence、bar/trade 原始 source-link、外部 completeness 或跨源时钟关系时 limitation 必须保留；R4.85–R4.95 已机器拒绝仅凭 sidecar/projection/wire/cursor/cut/view/observation/binding/set/bundle/admission/derivation/derivation-admission 开放 Runner exact-trigger。
