...
| execution authority | Reservation scheduled/immediate cancellation、active Attempt cancellation、lease renewal、terminal race、observation ack、pre-terminal outbox、restart recovery-first admission、local namespace discovery/startup job | receipt/observation/outbox self-hash、CAS/单目标；active cancellation 精确命中未过期 current lease；outbox constructor 冻结 Attempt identity/generation floor，v2 persist/record 绑定 exact current lease 与 derived namespace；startup job 要求 existing authority DB/store 并先全量验证 discovery set | 生效前 active claim 幂等重送不变；同 Attempt generation 单调推进后仍可取消；already-registered 不重投；旧 generation、rollback、worker/Attempt 漂移、misplaced namespace、symlink、tamper、wrong DB 均 fail-before-delivery/poll/Engine；输出不含本机路径 | Control Plane claim/renew/finalize/Receipt fence + injected coordinator + Attempt-scoped local outbox + SQLite directive → current-generation pre-terminal Outbox/Observation → `research.replay-cancellation-recovery` local discovery + pending/already inspection → no-replay ack；deployment admission hook、remote discovery/transport/polling/SLA 未认证 |
...
**当前状态：完成 authority/storage/data/Harness 的受限闭包。** Control Plane 单写 Provider Certification/Termination、Reservation/Cancellation、Attempt/Cancellation/Observation、Checkpoint Receipt 与 Resume Authorization；Request/Execution Spec 冻结 Decision Schedule、status schedule/provenance 与 provider certification。R4.53—R4.67 已贯通 imported archive/provider、registry admission、source-batch audit、current-snapshot acquisition receipt、非追溯 rotation/revocation、emergency cancellation authority、transport-neutral coordinator、SQLite reference observation/ack/latency、no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission、renewal-aware binding、local namespace discovery 与可执行 startup recovery job；历史 archive 仍只能显式导入。OS sandbox、signed build provenance、remote adapter/provider fault certification、venue 历史 archive 自动采集/签名/外部穷尽审计、deployment admission hook、remote outbox discovery/transport/latency SLA、halt settlement 与动态 supplemental join 仍未完成。
...
**当前状态：完成第六十五子集，R3 未完成。** R4.43 已认证 partial 后 terminal owner 闭包；R4.44–R4.49 冻结 simple-bracket resolution/economics 与独立跨语言 oracle；R4.50 阻断 missing-grid 被误当 observed gap；R4.51 将 complete halt gap 与普通缺口分离；R4.52 阻断 current/periodic snapshot 被包装成 complete history；R4.53 增加 imported archive/provider 内容闭包；R4.57–R4.67 补齐 provider certification 未来 admission、显式 cancellation authority、observation/ack、transport-neutral coordinator、no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission、renewal-aware binding、local discovery 与 startup job。terminal preemption、long/short lane symmetry、economic-policy tamper rejection、status/provenance/continuity fence 与 clean/resume parity 已锁定。仍缺外部 venue completeness、multiple partial、真实 historical liquidity partial、通用 cancel/amend、limit/TIF、add/reversal、partial liquidation、halt settlement、generic OHLC resolver 与 step/fast parity。
...
**当前状态：完成第六十五子集，未完成统一组合账本。** Request/Result 为 v24/v36，Artifact/Run Outcome 为 v38/v35；Simulator v10 的 accounting 算术不变，但缺失执行区间不得产生任何后续账本事实。partial fee/realized PnL、Funding、cash/equity bridge、Margin、post-partial liquidation 与 EOD valuation 已守恒；R4.51 允许 exact Funding/Mark 在 frozen halt 内继续进入账本/风险观察，R4.52–R4.67 只增强这些状态事实的来源、采集、准入、取消、observation、协调、ack retry 与 operational outbox/discovery/startup 闭包，不改变经济执行。现有 certified lane 仍是单 settlement asset、单 isolated Position。borrow、cross/shared portfolio、halt settlement、partial liquidation、bankruptcy/insurance/ADL 与多资产组合尚未开始。
...
5. Reservation/Attempt Cancellation v1、transport-neutral coordinator、进程内 no-replay ack retry、local durable outbox、pre-terminal handoff、recovery-first admission、monotonic renewal binding、local namespace discovery 与 Control Plane startup job 已冻结；Outbox v2 可独立 rehydrate cancellation-generation lease，SQLite reference path 可裁决 pending/already 并测三段 latency，authority cancel 删除无权 diagnostic 文件但保留 operational outbox。尚未决定由哪个 deployment worker supervisor/automation hook 在 fresh claim 前强制调用该 job；Control Plane poll 返回与 local commit 之间仍无跨存储事务。remote discovery/index、outbox retention/GC/quarantine、remote store、push/poll/lease-renew response/IPC/网络、startup/停止确认 SLA、worker watchdog 与外部签名仍需结合部署模型决定，不能让 Replay 或 J04 research loop 擅自固定。
