...
| J04 supervisor、Replay、forward、review、durable RD state | bounded autonomy refill、formal Replay、Replay→Reviewer、accepted Decision→candidate Draft→certified source package 常驻链已闭合；真实 Manifest provider、Forward 自动接线、长时 J04-J05-J07 soak 尚未闭合 |
...
| D5.9 | Registry / Forward / Governance 沿正式入口接纳 | active-partial：Registry owner-fact compiler、fenced resident queue、source/replay provenance、candidate manifest、隔离 source adoption 与标准 no-live package 已完成；Forward / Governance 待完成，始终 no hot-load / automatic promotion |
