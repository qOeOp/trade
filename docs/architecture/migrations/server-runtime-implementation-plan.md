...
| J04 supervisor、Replay、forward、review、durable RD state | bounded autonomy refill、formal Replay、Replay→Reviewer、accepted Decision→candidate Draft→certified source→Forward source admission→post-freeze OHLCV demand 常驻链已闭合；真实 Manifest provider、Forward Session/Result、Governance、长时 J04-J05-J07 soak 尚未闭合 |
...
| D5.9 | Registry / Forward / Governance 沿正式入口接纳 | active-partial：Registry owner-fact compiler、fenced resident queue、source/replay provenance、candidate manifest、隔离 source adoption、标准 no-live package、certified-source admission 与 immutable Observation Program/OHLCV demand续租已完成；Forward Session/Result 与 Governance 待完成，始终 no hot-load / automatic promotion |
| D5.10 | mechanical Replay、Agent-assisted evaluation 和 Forward evidence 分权 | active-partial：formal Result 只授予 classified Reviewer input；Reviewer 接受只授予 Registry candidate source；Ops certification只授予 exact source binding；Research Program只请求严格 post-freeze OHLCV，仍无 Session/Replay/deployment/trading/promotion authority；真实 Forward Result 尚待常驻生成 |
