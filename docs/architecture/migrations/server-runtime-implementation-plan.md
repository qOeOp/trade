...
| D5.9 | Registry / Forward / Governance 沿正式入口接纳 | active-partial：Registry owner-fact compiler、fenced resident queue、source/replay provenance、candidate manifest、隔离 source adoption、标准 no-live package、certified-source admission、Observation Program/OHLCV demand续租、gapless candle segment 与 hash-verified OHLCV-only Dataset Candidate 已完成；补充组件/正式 Dataset Manifest、Forward Session/Result 与 Governance 待完成，始终 no hot-load / automatic promotion |
