...
| J04 supervisor、Replay、forward、review、durable RD state | bounded autonomy refill、formal Replay、Replay→Reviewer、accepted Decision→candidate Draft→certified source→Forward source admission→post-freeze OHLCV demand→gapless owner-proven candle segment 常驻链已闭合；正式 Dataset Manifest/status-spec-risk provider、Forward Session/Result、Governance、长时 J04-J05-J07 soak 尚未闭合 |
...
| D2.10 | 将 funding exact-event window 纳入正式数据产品 | active-partial：v1 保持封闭、v2 demand/fact、固定 Binance USDM endpoint resident、raw page append-only archive、连续 cursor/终页耗尽 coverage audit、decimal-string event、冲突不自动选取、container profile/readiness 已完成；Forward Candidate 独立 demand/fact binding 与 formal Manifest 消费待完成 |
...
| D5.9 | Registry / Forward / Governance 沿正式入口接纳 | active-partial：Registry owner-fact compiler、fenced resident queue、source/replay provenance、candidate manifest、隔离 source adoption、标准 no-live package、certified-source admission、Observation Program/OHLCV demand续租、gapless candle segment、hash-verified OHLCV-only Dataset Candidate、Research append-only resident admission 与历史 Request/Manifest 驱动的 typed readiness blockers 已完成；候选按观测跨度节流且超出内存边界只报告 pending，不截断。formal funding 数据产品已具备但 Candidate demand/fact 尚未绑定；条件 mark/supplemental、fresh status/spec/risk、新 Forward decision、正式 Dataset Manifest/Session/Result 与 Governance 待完成，始终 no hot-load / automatic promotion |
