...
| J04 supervisor、Replay、forward、review、durable RD state | bounded autonomy refill、formal Replay、Replay→Reviewer、accepted Decision→candidate Draft→certified source→Forward source admission→post-freeze OHLCV demand→gapless owner-proven candle segment 常驻链已闭合；public current status/spec 的 resident、provider 与 bounded Forward binding 已闭合，venue risk、正式 Dataset Manifest、Forward Session/Result、Governance、长时 J04-J05-J07 soak 尚未闭合 |
...
S4/S5 已形成首个本地闭环：J04 路由先读 `plan_next`，ready/terminal 不调用模型；active empty/unready queue 才调用固定 Model Gateway，domain assessment 通过后以 `updated_at` CAS 原子入队，再委托既有 supervisor。模型失败不写 state/Trial，identical replay 幂等，stale/conflict fail closed；真实 provider probe 已通过。formal Replay 已有 durable single-slot queue、generation fencing、同 worker restart resume 与容器前台 worker；Agent overlay 以独立 fenced queue 把 classified Result 唤醒为 Reviewer Agent Run，再由无网络 Registry resident 将 `accept_for_draft` owner facts 编译成 source/replay-evidence-bound candidate Draft。Registry 只写独立 release-candidate volume并发布 immutable manifest；统一 resident adopter随后在 frozen revision 隔离 worktree运行 lint/quality/Replay audit并生成 no-live source package，全程不修改当前 checkout或运行镜像。source bridge 跨平面重验 certification 后由 Research owner写 immutable Forward source admission；后继无网络 resident 再闭合 ready Draft、historical Request、symbol/timeframe 与 strict post-freeze boundary，向现有 Market Data owner 建立并续租 OHLCV demand；独立 segment resident 只消费 owner-selected plan，对 closed-candle 增量窗口做零 gap audit 与 immutable export并写入 gapless chain。Public current instrument resident 同步轮询 owner-selected symbol；独立 provider 把 exact receipt/raw bytes 正规化为 status/spec/accounting，Research 仅接受已登记 capability certification 和 freeze→watermark 的 20 分钟上界快照序列，明确不证明 inter-sample history。该链仍不冒充正式 Dataset Manifest 或 Replay authority。真实 mid-Replay kill、venue risk、certified Manifest provider、Forward Session/Result、Governance、J04/J05/J07 长时 soak 与 Linux 采用仍待闭合。
...
| S5 research autonomy | 已实现 J04 bounded refill + CAS queue + existing supervisor delegation，并把 formal Replay 接入 durable resident queue；J05/J07 保持独立 cadence | 本地 stopped/blocked/no-write/duplicate/stale、Replay fencing/restart 门已过；真实 Manifest provider、mid-Replay kill 与长时 soak 待闭合 |
...
| D2.10 | 将 funding exact-event window 纳入正式数据产品 | complete at product/binding seam：v1 保持封闭、v2 demand/fact、固定 Binance USDM endpoint resident、raw page append-only archive、连续 cursor/终页耗尽 coverage audit、decimal-string event、冲突不自动选取、container profile/readiness、Replay-ready immutable slice 与 Forward Candidate 独立 demand/fact/binding 已完成；formal Manifest 消费待 D5 闭合 |
