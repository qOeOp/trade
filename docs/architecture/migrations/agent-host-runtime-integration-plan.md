...
| P5 R&D Agent 纵切 | active-partial | Planner → Developer → Replay → Reviewer → Registry → source adoption | Registration-bound formal Replay、Reviewer resident、candidate-only Registry 与 frozen-revision source adopter 已接通并有本地 fencing/recovery/no-hot-load fixture；真实 Manifest 输入、真实模型二次修改、Forward 与 Linux 采用待闭环 |
