...
| `modules/orchestration-ops/agent-host-openclaw/` | OpenClaw Host adapter | Agent Run → pinned OpenClaw Gateway agent/session、typed terminal artifact、transport verification 与 ops registry lifecycle | Program cadence、领域 owner、OpenClaw state、credentials、raw transcript、promotion/deploy/trade authority |
| `modules/orchestration-ops/agent-artifact-store/` | Agent artifact byte owner | 固定 durable/tmp 根下的 immutable content-addressed text、hash/bytes/secret 重验与 Host materialization | 领域语义、GC、workspace execution、merge/release/deploy 或交易 authority |
...
| `modules/research-strategy-development/research-control-plane/agent-run-orchestrator/` | provider-neutral role orchestrator | 冻结 role context、生成 Agent Run request、校验终态 artifact 并委托既有 owner admission | Host loop、workspace、Replay、strategy state bypass、promotion/live |
