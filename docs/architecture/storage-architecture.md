...
多环境 DB、测试隔离、sidecar 生命周期与工作区副作用闸仍处于 [Database Environment and Workspace Hygiene Migration](./migrations/database-environment-and-workspace-hygiene-plan.md) 提案阶段。当前 local data plane 不因该提案自动迁移，环境名也不产生真实交易权限。
...
| `research_state_store` | implemented | `research-strategy-development/research-control-plane/state-store` | `data/rd_state.db` | RD program memory + Control Plane facts；status/aggregate-trade provider certification；Reservation-bound exact-evidence/cross-source-ordering admission；Draft Strategy registry |
...
1. 先以对应 `docs/architecture/storage-schema/*.sql` 为 migration 起点。
