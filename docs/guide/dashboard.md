...
Surviving immutable evidence can deterministically reconstruct the former `1/1/1` facts, but the current state is
`RECOVERABLE_BY_RECONSTRUCTION_NOT_RESTORED`, not restored custody; whole-volume rollback is forbidden because
Windmill and R&D share the volume. The item-1406 Product Edge admission/receipt/outbox remains byte-identical, while
the default database, runtime/Web S1-to-S2, provider canary, full gates, candidate commit, and delivery remain
...
| Qualification protected‑feedback frontier     | `RECOVERABLE_BY_RECONSTRUCTION_NOT_RESTORED / NOT_ADMITTED` | A destructive test dropped and recreated the default persistent projection, head, and outbox tables empty. There is no trusted backup/PITR/archive. Surviving exact evidence may allow a separately authorized canonical Owner reconstruction, but until that occurs and direct Owner readback plus default‑Web revalidation pass, the Dashboard renders the Owner store unavailable, never `GENESIS_EMPTY`; hides Copy frontier and every positive S1/S2 action; shows the affected store/table set, last trusted cut, current `0/0/0` readback, recovery classification, and immutable incident evidence; and offers only Open incident evidence and Copy locator. There is no Restore, Reconstruct, Clear incident, successor, or retry control |
