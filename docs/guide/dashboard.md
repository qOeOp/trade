...
| Sealed S1 basis; Qualification and terminal Research receipt absent      | Disabled                              | Resolve same request identity enabled                                              | Disabled                                                                   | `SEALED_BASIS_PENDING_QUALIFICATION`; show basis receipt/head/outbox and missing next receipt; no duplicate basis or fresh‑authority gate |
...
| `AuthorizationSuccessorReadiness`                                         | read‑only prior authorization identity/scope/sequence, admission/current generations, `successor_distance=0\|1`, terminal expiry/revocation state, canonical direct‑successor operation availability, successor receipt/identity when present, and exact missing/invalid stop. Distance greater than one is `DIRECT_SUCCESSOR_REQUIRED` and unavailable. It never walks a chain, constructs scope, chooses a replacement, signs, renews, revokes, or calls a transport‑token control. Until one immediate Owner‑issued equivalent successor exists, `Current authority` stays unavailable while the admission snapshot remains visible                                                                                                                                                                                                                               |
...
| `S1StageCustodyPanel`                                                     | read‑only `SEALED_BASIS_PENDING_QUALIFICATION` geometry binding exact request and original admission, basis receipt/identity, basis head/outbox, commit cut, missing Qualification/terminal Research receipt, and next action. It renders only after canonical basis‑stage verification and permits same‑identity Resolve/completion from historical custody; Submit/successor are absent, duplicate basis/head/outbox is unavailable, and changed request/admission is conflict                                                                                                                                                                                                                                                                                                                                                                                     |
...
stage resolver canonically verifies their exact request, original admission, digests, and commit cut. When the
Qualification and terminal Research receipts are still absent, it returns sealed
`SEALED_BASIS_PENDING_QUALIFICATION` custody and the second transaction uses `Historical` completion semantics.
Ordinary row existence cannot construct that custody; duplicate basis/head/outbox writes are forbidden, and a
changed request or admission conflicts. Dashboard/BFF may display this sealed projection but cannot mint it or
substitute current authority for its historical completion.
