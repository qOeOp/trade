...
enables **Run bounded Agent + sandbox**; invoking it starts the existing claim exactly once and never creates a
successor, a replacement claim, or a second provider invocation. Once start commits, the Run control disappears and
every replay renders `OUTCOME_UNKNOWN` with
...
| Versioned scripts and App dependencies                | App and MCP must call the same `research_goal_v2` Product Edge operation; the observed `research_goal_v1` MCP profile mismatch is `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`. Windmill records dependency‑build jobs separately                                                | `TARGET_DRAFT/KEEP_SEMANTICS`: one versioned operation registry entry, content digest, typed BFF gateway, and explicit dependency state; a channel‑version mismatch renders the operation unavailable |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim, admission and attempt identities; committed time; digest; `CLAIMED_NEW / ALREADY_CLAIMED / unavailable`; current `CLAIMED / INVOCATION_STARTED`; and exact Owner‑projected next action. `ALREADY_CLAIMED` links the original receipt. Only recovered `CLAIMED + RUN_BOUNDED_EXECUTION_AGENT` may enter the atomic start gate for that same claim; every other state suppresses the Run control                                                                                                                                                                                                                                                                                                                                                          |
...
| `ProviderInvocationStateCard`                                             | Two fixed geometries. Resumable `CLAIMED` shows the same request/attempt/claim, no started time, next action `RUN_BOUNDED_EXECUTION_AGENT`, and buttons in order: Run bounded Agent + sandbox, Resolve same attempt, Copy claim, Open operational run. `INVOCATION_STARTED / OUTCOME_UNKNOWN` uses fixed red geometry with `MANUALLY_RECONCILE_PROVIDER_INVOCATION` and buttons Resolve same attempt, Copy claim, Open operational run; Run and successor controls are absent. It never creates a new claim, retries the provider, marks success, dismisses the stop, or infers an outcome                                                                                                                                                                                  |
