...
The evidence cut is 2026-08-22 at observed Dashboard checkout base
`ae2939736ed3fcecf606d9bc8006027250c7a0c5`, tree `ad4242df56292bcce765c781da36459241fe4760`.
The latest exact R&D candidate `a05d76ea18e2b35d7e55d74357fbc30b971ec1a2`, tree
`eb25b1a8325c4711ebd8d2cd012b3a87f70741c6`, completed static Workbench 5/5 and PostgreSQL 20/20 evidence but
stopped before candidate service replacement, Windmill execution, or provider invocation. Its explicit bootstrap
durably committed one Operator Authorization genesis receipt, then Product Edge failed because its canonical
`SELECT ... FOR SHARE` read requires `UPDATE` privilege that the unique-writer boundary correctly denies. Product
Edge binding/head/outbox remained `0/0/0`. The genesis receipt is historical Owner evidence, not proof that action
admission is ready; the runtime capability therefore remains `OBSERVED_CANDIDATE_NOT_CURRENT / NOT_ADMITTED`.
...
| Product Edge action authorization             | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Authority review rejected the earlier self‑proving environment/default‑policy path. Isolated PostgreSQL tests now prove complete Issuer/Product Edge history verification, lock order, cutover/revocation replay and the one‑use invocation claim/start state. Workbench 5/5 proves `claim -> start -> provider`: claim response loss can recover `CLAIMED` and continue to start, but replay after committed `INVOCATION_STARTED` returns `OUTCOME_UNKNOWN`, shows claim identity/digest plus `MANUALLY_RECONCILE_PROVIDER_INVOCATION`, and never calls the provider again. The latest dynamic attempt additionally proves that an OA genesis receipt may exist while Product Edge binding/head/outbox are absent: bootstrap stopped at the cross‑Owner row‑lock privilege boundary before service replacement, Windmill execution, or provider invocation. That partial state renders the sealed OA receipt for audit but keeps admission `unavailable` and every mutating control disabled. Actual‑provider‑call, provider reconciliation, full runtime and final review remain `NOT_ADMITTED` |
...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required counts; `P=ArtifactTable`; `Q=ArtifactOutcomeProjectionGate + ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel + ProviderInvocationStateCard + LegacyTerminalQuarantinePanel`; `T=BuildAndSecurityEvidence` with deterministic double‑build and sandbox policy | Open Artifact, Resolve same attempt, Copy provider claim, Open operational run, Ask Agent to revise, Start exploratory replay                                                                                               | Existing S2 is `CURRENT/PARTIAL`; result‑authority precedence, binding, no‑Artifact closure and invocation state are `OBSERVED_CANDIDATE_NOT_CURRENT`. The gate first selects sealed R&D `SUCCESS` or `FAILED_NO_ARTIFACT`; only without either terminal may `INVOCATION_STARTED` project `OUTCOME_UNKNOWN`. Unknown hides review/successor actions and exposes only claim copy, operational evidence, same‑attempt resolve and manual reconciliation. Legacy terminal renders historical receipt only. There is no provider retry button; `ACTUAL_PROVIDER_CALL_AT_MOST_ONCE` remains `NOT_ADMITTED` |
...
| `ProviderInvocationStateCard`                                             | claim identity/digest, state digest, started time, `STARTED_NEW / OUTCOME_UNKNOWN`, operational‑run link and exact next action. It is the active outcome panel only when `ArtifactOutcomeProjectionGate` finds no sealed R&D terminal. `OUTCOME_UNKNOWN` uses the fixed red geometry and `MANUALLY_RECONCILE_PROVIDER_INVOCATION`; buttons are Copy claim, Open operational run and Resolve same attempt. There is no retry‑provider, mark‑success or dismiss control, and no inferred provider outcome                                                                                                                                                                                                                                                   |
...
The observed candidate additionally proves that Operator Authorization custody and Product Edge readiness are
not one service state. OA genesis may commit while Product Edge binding/head/outbox remain absent because the
cross-Owner admission read cannot acquire its required serialization boundary. Until the canonical Owners admit
and dynamically verify a lock protocol that preserves unique writers and least privilege, the gateway projects
that combination as `unavailable`, returns the exact stop predicate, and issues zero business/provider effect.
Dashboard and BFF code must not grant cross-Owner `UPDATE`, replace the lock with an unlocked read, or infer
readiness from the OA receipt. The repair protocol itself remains backend `NOT_ADMITTED`; the UI contract is only
the fail-closed partial-state projection.
