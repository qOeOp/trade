...
| H1 fresh invocation admission                 | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Exact‑head consumer review found the candidate App/MCP projection fail‑closed, but authority review rejected `ca5154ed9f`: the provider claim persisted the historical request admission and attempt without a distinct sealed receipt for the directly resolved current authorization frontier, policy‑equivalent binding, manifest, and final locked write cut. Expiry could also cross between validation and the independently sampled claim commit time. `CLAIMED` alone must therefore render unavailable and cannot enable Run; the target below inserts a separate `InvocationAdmissionReceipt` before claim/start. Dynamic PostgreSQL, Windmill, provider, and real‑consumer evidence remains unavailable                                                             |
...
was admitted at the original cut. It never supplies the current gate. Before a first provider claim, Product Edge
must atomically re-read the complete current deployment and authorization histories and persist a distinct sealed
...
or transient resolver output. Claim disposition is
...
invocation. Missing, stale, malformed, or mismatched invocation admission renders unavailable even when a claim row
exists. Once start commits, the Run control disappears and every replay renders `OUTCOME_UNKNOWN` with
...
creates no current Research View, provider action, successor action, or TrialFamily repair action.
...
| Versioned scripts and App dependencies                | App and MCP must call the same `research_goal_v2` Product Edge operation; the observed `research_goal_v1` MCP profile mismatch is `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`. The exact H1 candidate sync inventory adds `consumer_projection_v1` as an empty‑input canonical‑stamp validator imported by the App, not as an App/MCP business operation; it remains candidate‑only. Windmill records dependency‑build jobs separately | `TARGET_DRAFT/KEEP_SEMANTICS`: one versioned operation registry entry, content digest, typed BFF gateway, and explicit dependency state; compile projection verification into the typed gateway/library with no catalog item, route, action, or user‑run record. A channel‑version mismatch renders the operation unavailable |
...
| Access `/settings/access`               | Principal/session/token/revoked counts plus binding `ACTIVE/SUPERSEDED/zero‑active` and authorization available/expired/revoked/unavailable counts; `P=LocalPrincipalCard`; `Q=AuthorizationLineagePanel` with fixed `Current authority / Admission snapshot` tabs and separate `Operator Authorization / Product Edge readiness` rows; `T=CapabilityManifest + CredentialAudit` | Re‑authenticate local session, Issue narrow transport token, Revoke token, Copy once | Transport credential controls never mint, renew, or revoke Operator Authorization. `Current authority` alone feeds action state; `Admission snapshot` is immutable audit readback and may remain available after cutover/revocation. A sealed OA genesis with missing PE binding/head/outbox is audit evidence but keeps the current gate unavailable. Both tabs expose exact binding/head, issuer/audience/scope, expiry/revocation frontier, manifest digest, source cut, and stop predicate; secret/token values remain one‑time and never logged |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim, invocation‑admission receipt identity, historical request‑admission lineage, and attempt identity; committed time; digest; `CLAIMED_NEW / ALREADY_CLAIMED / unavailable`; current `CLAIMED / INVOCATION_STARTED`; and exact Owner‑projected next action. `ALREADY_CLAIMED` links the original receipt. Only recovered `CLAIMED + RUN_BOUNDED_EXECUTION_AGENT` with direct equality to the sealed invocation‑admission receipt may enter the atomic start gate for that same claim; every other state suppresses the Run control                                                                                                                                                                                                                         |
...
| `ProviderInvocationStateCard`                                             | Two fixed geometries. Resumable `CLAIMED` first shows the exact sealed invocation‑admission receipt, then the same request/attempt/claim, no started time, next action `RUN_BOUNDED_EXECUTION_AGENT`, and buttons in order: Run bounded Agent + sandbox, Resolve same attempt, Copy claim, Open operational run. Missing or mismatched invocation admission keeps the same geometry in unavailable state and removes Run. `INVOCATION_STARTED / OUTCOME_UNKNOWN` uses fixed red geometry with `MANUALLY_RECONCILE_PROVIDER_INVOCATION` and buttons Resolve same attempt, Copy claim, Open operational run; Run and successor controls are absent. It never creates a new claim, retries the provider, marks success, dismisses the stop, or infers an outcome               |
| `LegacyTerminalQuarantinePanel`                                           | verified historical terminal receipt identity, legacy custody generation, observed time and quarantine reason. It exposes Open/Copy historical receipt only; no current Research View, Artifact promotion, provider invocation, successor, TrialFamily repair, or dismiss action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
