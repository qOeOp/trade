...
| Product Edge action authorization             | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Authority review found the candidate constructing `trusted_context_v2` and Artifact context from environment/default policy strings, then validating against that same policy object. This self‑proving path cannot enable a Dashboard action. Every mutating control remains `permission_unavailable` until Product Edge resolves a canonical unique `ACTIVE` deployment‑history head, non‑self‑assertable Operator Authorization, current Time Evidence and revocation frontier, plus the exact content‑addressed Agent Operation Manifest from independent authority stores                                                                                                                    |
...
| Access `/settings/access`               | Principal/session/token/revoked counts plus binding `ACTIVE/SUPERSEDED/zero‑active` and authorization available/expired/revoked/unavailable counts; `P=LocalPrincipalCard`; `Q=AuthorizationLineagePanel`; `T=CapabilityManifest + CredentialAudit` | Re‑authenticate local session, Issue narrow transport token, Revoke token, Copy once | Transport credential controls never mint, renew, or revoke Operator Authorization. The panel is read‑only and exposes exact binding/head, issuer/audience/scope, expiry/revocation frontier, manifest digest, source cut, and stop predicate; secret/token values remain one‑time and never logged |
