...
- Detail: heading is Selected service or Service details, compact service label, and Ready/Offline badge. Four ordered
  ...
  Recent activity: Run link, Started, with the business-facing run state or No activity in the title. Supported work: role and
...
- Action order/admission: header info then Refresh; table identity navigation; D Recent activity link only with exact run
  identity; exact-route footer Back to services. Filters, sorting and pagination are local. All remote
