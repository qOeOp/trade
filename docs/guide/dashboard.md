...
projection, and complete local lineage before family formation. Candidate
`0d719df5661f8b4ebe52de2d1ceb4e68cfa235ed`, tree `8edf8501cb9b7a813573868c2f9de331fceffec3`,
dynamically proved the default-Web chain
...
plus restart/cache-loss recovery.
Fresh authority review rejected it because prerequisite authority could be written before complete V2 validation
and lineage SQL could filter on unverified JSON before canonical decode. The same Task is correcting both findings;
consumer review and current delivery remain pending, so the whole chain is still candidate-only design evidence.
...
| --------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1 V2 TrialFamily -> S2 Artifact binding      | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Rejected candidate `0d719df566` dynamically proved `basis receipt -> Qualification GENESIS_EMPTY receipt -> S1 ACCEPTED/family census -> S2 SUCCESS/time-bound binding receipt -> REVIEW_ARTIFACT`; restart plus cache loss recovered the same request, attempt, basis, Qualification frontier, family, Artifact and binding identities. The successor must close validation‑before‑prerequisite‑write and selector‑free canonical lineage verification before fresh reviews                                                                                         |
...
| TrialFamily lineage/feedback authority        | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Architecture now admits an R&D basis/genesis Owner fact and a Qualification opaque frontier fact. The App no longer accepts predecessor/feedback/independence authority fields; it renders sealed basis and Qualification receipts. Positive S1 must validate the complete V2 request before either prerequisite write, then under `scope -> request` locks enumerate every lineage receipt without a raw selector, canonical‑verify every row, and only then filter/form family. Corrupt or unavailable history returns `SUBMITTED_OR_UNKNOWN`, never false genesis |
...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required counts; `P=ArtifactTable`; `Q=ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel`; `T=BuildAndSecurityEvidence` with deterministic double‑build and sandbox policy | Open Artifact, Resolve same attempt, Ask Agent to revise, Start exploratory replay                                                                                                                                          | Existing S2 is `CURRENT/PARTIAL`; Artifact‑family binding and canonical no‑Artifact receipt closure are `OBSERVED_CANDIDATE_NOT_CURRENT`. Last two actions require admitted review actions; `SUCCESS` without complete binding readback renders unavailable                                                                                                                                                                           |
...
| Protected feedback `/qualification/protected-feedback` | Current/genesis‑empty/unknown/corrupt counts; `P=QualificationFrontierTable`; `Q=QualificationFrontierReceiptPanel + IndependenceBasisLink`; `T=OpaqueFrontierTimeline`                                                   | Refresh, Resolve current by exact basis, Open R&D basis receipt, Copy opaque frontier reference                                                                   | `OBSERVED_CANDIDATE_NOT_CURRENT`; the default Workbench consumes a sealed `GENESIS_EMPTY` receipt. Page exposes identity/cut/digest/state only, never protected content. Candidate Intake, protected attempts, eligibility, holdout, and cross‑family ancestry remain `NOT_ADMITTED`             |
...
| Audit `/operations/audit`               | Execute/update/create/delete/success/failure counts; `P=AuditTable`; `Q=AuditCorrelationCard`; `T=OperationTimeline`                                                 | Filter, Open correlation, Copy audit locator                                        | Real Windmill use; append‑only control‑plane audit, not Owner business truth                                                                                                                     |
...
| Access `/settings/access`               | Principal/session/token/revoked counts; `P=LocalPrincipalCard`; `Q=CapabilityManifest`; `T=CredentialAudit`                                                          | Issue narrow token, Revoke token, Copy once                                         | Action‑time confirmation; values are never recoverable or logged                                                                                                                                 |
