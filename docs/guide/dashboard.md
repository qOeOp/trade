...
| H1 fresh invocation admission                 | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Fresh authority and consumer reviews rejected `fdbc5aa6`: although Product Edge now seals a distinct invocation admission, the Windmill exact‑key verifiers omit its identity/digest fields and downgrade a real claim to `SUBMITTED_OR_UNKNOWN`; start/recovery reruns current R&D `prepare` freshness after claim; verified legacy quarantine has no reachable consumer projection; and Operator Authorization cannot canonically issue a policy‑equivalent successor after historical expiry. The fixed target below preserves the exact wire receipt, starts only from sealed historical attempt custody, projects legacy‑only evidence, and renders successor issuance unavailable without inventing authority. Dynamic PostgreSQL, Windmill, provider, and real‑consumer evidence remains unavailable |
...
discriminant, request/attempt identity, terminal receipt, custody generation, and quarantine reason must survive the
shared consumer projector as one strict legacy‑only branch. If that branch is missing or malformed, the fixed legacy
slot renders `Unavailable` with same‑attempt Resolve; it must not silently collapse into an untyped generic unknown.
...
| Versioned scripts and App dependencies                | App and MCP must call the same `research_goal_v2` Product Edge operation; the observed `research_goal_v1` MCP profile mismatch is `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`. H1 sync adds candidate‑only `consumer_projection_v1` as an empty‑input canonical‑stamp validator imported by the App, not a business operation. Review of `fdbc5aa6` proves the Windmill operation adapter and shared projector can both retain an old exact‑key claim shape while Rust adds sealed‑receipt fields, downgrading a real claim. Windmill records dependency‑build jobs separately | `TARGET_DRAFT/KEEP_SEMANTICS`: one versioned operation registry entry, content digest, typed BFF gateway, and explicit dependency state. Generate one exact Rust‑to‑gateway parser consumed by adapter/projector; schema drift keeps fixed unavailable geometry. Projection verification compiles into the typed library with no catalog item, route, action, or user‑run record |
...
A1 CLAIMED response-loss recovery + exact next action RUN_BOUNDED_EXECUTION_AGENT;
   ...
   schema mismatch is unavailable; fresh Research read is never a post-claim start gate
...
L  LEGACY_TERMINAL_QUARANTINED -> historical receipt/custody generation/quarantine reason;
...
R  Result: allowlisted/redacted bounded JSON/tree view with Copy field, Copy JSON,
...
| Sealed `CLAIMED`; fresh Research read is stale or unavailable            | Run bounded Agent + sandbox enabled   | Resolve same attempt identity enabled                                              | Disabled                                                                   | Historical claim/attempt custody is the gate; fresh Research state is labelled non‑gating              |
| Verified legacy terminal                                                 | Disabled                              | Resolve same attempt identity enabled                                              | Disabled                                                                   | Fixed legacy‑only panel with historical receipt/quarantine reason; no Artifact/provider/successor      |
...
R  Successor readiness: prior identity/scope/sequence -> Owner operation availability ->
   successor receipt/identity or exact unavailable reason; no editable value or selector
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                        | cross‑bind the current next action to the unique `ACTIVE` shell binding/history head, independent Operator Authorization, Time Evidence/revocation frontier, Product Edge binding/head/outbox readiness, and exact content‑addressed operation manifest. The gate alone controls button enablement. The panel has fixed `Current authority` then `Admission snapshot` tabs; each keeps `Operator Authorization` and `Product Edge readiness` as separate rows with identity, cut, state and stop predicate. A sealed OA receipt with absent PE custody remains visible but unavailable and never feeds the gate. The snapshot is historical. Neither component constructs, signs, refreshes, repairs, or infers authority from credential or local policy                                       |
| `AuthorizationSuccessorReadiness`                                         | read‑only prior authorization identity/scope/sequence, terminal expiry/revocation state, canonical successor‑operation availability, successor receipt/identity when present, and exact missing/invalid stop. It never constructs scope, chooses a replacement, signs, renews, revokes, or calls a transport‑token control. Until an Owner‑issued equivalent successor exists, `Current authority` stays unavailable while the admission snapshot remains visible                                                                                                                                                                                                                                                                                                                               |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim plus exact public wire fields `invocation_admission_receipt_identity` and `invocation_admission_receipt_digest`, historical request‑admission lineage, attempt identity, committed time, claim digest, `CLAIMED_NEW / ALREADY_CLAIMED / unavailable`, current `CLAIMED / INVOCATION_STARTED`, and Owner‑projected next action. One versioned exact parser is shared by operation adapter and consumer projector. Missing/extra/tampered fields keep A0/A1 unavailable. Only recovered `CLAIMED + RUN_BOUNDED_EXECUTION_AGENT` with direct sealed‑receipt equality may enter the atomic start gate                                                                                                                                                                            |
...
duplicate-sequence requests conflict. `AuthorizationSuccessorReadiness` may resolve and display that Owner fact, but
the Dashboard BFF never invokes the issuance operation. Until canonical Owner API plus PostgreSQL evidence exists,
expired-history recovery is `NOT_ADMITTED` and every dependent Product Edge claim remains unavailable.
