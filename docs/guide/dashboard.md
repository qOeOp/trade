...
| S2 action‑time Research freshness             | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Consumer v6 rejected `4c28bd583f`: Workbench caches an Owner `AVAILABLE` projection and validates only its interval shape, so a page left open across `valid_through` still enables S2 Run. The current chain commits a new Product Edge admission before R&D `prepare` discovers stale custody. TARGET makes the click a read‑only `PREFLIGHTING` transition, obtains same‑identity Owner freshness at an Owner cut, and permits the first Product Edge/S2 write only inside the locked fresh path. Stale/unavailable returns `STALE` or `REVALIDATION_REQUIRED`, exposes only Resolve/Open evidence, and creates zero Product Edge admission, R&D attempt, invocation admission, or claim. Static 16/16 projector/control tests missed natural expiry; live Windmill/default‑Web and dynamic PostgreSQL remain unavailable                                                                                                                                                                                                                                                   |
...
| H0 exact‑head correction set                  | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Review rejected `c224927c54`: R&D retained physical Qualification authority; final write‑edge freshness could expire after its checked cut; in‑flight admissions could lose predecessor continuity after a policy‑equivalent cutover; provider claim response loss lacked the exact resumable Run action; unavailable S2 authority could render input rejection; and MCP still selected `research_goal_v1`. The target contracts below correct all six, but no corrected candidate, default App run, MCP parity, provider invocation, or product acceptance exists yet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| H1 fresh invocation admission                 | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Fresh exact‑head reviews rejected `6bd9f627`: the candidate added OA successor issuance, sealed start custody, claim receipt fields and a reachable legacy branch, but Product Edge accepts an arbitrary policy‑equivalent successor chain where only the original binding or one immediate successor is legal; Rust omits optional family keys for real claim/non‑success wires while both Windmill exact‑key verifiers require explicit `null`; App/MCP still runs freshness‑sensitive `prepare` before an existing claim and Owner terminalization/readback still loses durable receipts after Research expiry; and the legacy projector accepts only two of four Rust‑verified dispositions. The fixed target below freezes direct‑successor distance, discriminated absent/present keys, a prepare‑free claimed start, stale‑safe terminal custody, and all four read‑only legacy dispositions. Dynamic PostgreSQL, Windmill, provider, and real‑consumer evidence remains unavailable                                                                                    |
...
not an effect authorized by cached `AVAILABLE`. Its first transition is read‑only `PREFLIGHTING`: the BFF rereads
the same R&D identity using Owner-controlled time before requesting any Product Edge admission. The control is
disabled with a spinner while that call is pending. Only an Owner-cut `AVAILABLE` result may enter the locked
admission/write path; `STALE`, `UNAVAILABLE`, timeout, malformed output, or page cancellation returns a fixed
non-positive state with zero Product Edge admission, R&D attempt, invocation admission, or claim. The browser may
conservatively replace an aged cached action with `REVALIDATION_REQUIRED`, but only the Owner response may label it
`STALE` or restore `Check & Run`.
...
| Versioned scripts and App dependencies                | App and MCP must call the same `research_goal_v2` Product Edge operation; the observed `research_goal_v1` MCP profile mismatch is `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`. H1 sync adds candidate‑only `consumer_projection_v1` as an empty‑input canonical‑stamp validator imported by the App, not a business operation. Exact‑head review of `6bd9f627` proves both Windmill verifiers still consume hand‑authored `null` family keys that Rust omits for claim/non‑success wires; the Run adapter also calls `prepare` before an already recovered claim. Windmill records dependency‑build jobs separately | `TARGET_DRAFT/KEEP_SEMANTICS`: one versioned operation registry entry, content digest, typed BFF gateway, and explicit dependency state. Generate one resolution‑discriminated Rust‑to‑gateway parser consumed by adapter/projector, feed Rust serialization fixtures directly into both, and route recovered `CLAIMED` straight to start without `prepare`; schema drift keeps fixed unavailable geometry. Projection verification compiles into the typed library with no catalog item, route, action, or user‑run record |
...
B  sealed basis stage -> basis receipt/identity -> basis head/outbox -> commit cut ->
   ...
   target Resolve consumes sealed complete typed request meaning and enters Historical completion;
...
T  WorkerTable: Worker | Group/tags | Started | Jobs | Last run | Occupancy | Memory | Version | Status
D  [Lease] WorkerLeaseCard: identity, host, lease/readiness, heartbeat age, limits, last run
   [Heartbeat] WorkerHeartbeatHistory / explicit unavailable reason [Open last run]
```
...
the last-observed time. On narrow screens `T` becomes a worker-card list and selection opens the same `D` as a
full-screen drawer.
...
Workers omit `P/Q`, retain their full-width `T`, and open `D` as a full-screen overlay. Filters move into a
route-local drawer while the primary table/card state remains visible.
...
| Cached `AVAILABLE`; S2 action selected, Owner preflight pending          | `Check & Run` enters `PREFLIGHTING`; no mutation | Cancel or Refresh Owner state                                                      | Disabled                                                                   | Primary immediately disables with spinner. Same‑identity Owner reread precedes Product Edge admission; only locked Owner‑cut `AVAILABLE` continues. Timeout/cancel/malformed remains `REVALIDATION_REQUIRED` with zero business writes                                              |
| S2 action‑time Owner preflight returns stale or unavailable              | Disabled                                         | Resolve same request identity enabled                                              | Open/Copy historical evidence                                              | Owner `STALE` may be shown only from the readback; unavailable stays `REVALIDATION_REQUIRED`. No Product Edge admission, R&D attempt, invocation admission, claim, provider call, successor, or review action                                                                       |
...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required counts; `P=ArtifactTable`; `Q=ArtifactOutcomeProjectionGate + ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel + InvocationAdmissionReceipt + ProviderInvocationStateCard + LegacyTerminalQuarantinePanel`; `T=BuildAndSecurityEvidence` with deterministic double‑build and sandbox policy | Open Artifact, Resolve same attempt, Copy provider claim, Open operational run, Ask Agent to revise, Start exploratory replay                                                                                                                                             | Existing S2 is `CURRENT/PARTIAL`; result‑authority precedence, binding, no‑Artifact closure and invocation state are `OBSERVED_CANDIDATE_NOT_CURRENT`. The gate first selects sealed R&D terminal receipts; only without one may `INVOCATION_STARTED` project `OUTCOME_UNKNOWN`. Rejected `6bd9f627` requires `null` family keys that Rust omits, sends recovered claims through `prepare`, loses terminal custody after Research expiry, and projects only two legacy dispositions. TARGET uses resolution‑specific absent/present keys, direct claimed start, stale‑safe terminal receipts, and all four read‑only legacy dispositions while preserving fixed unavailable geometry on mismatch. Unknown exposes only bounded evidence and same‑attempt resolution. There is no provider retry button; `ACTUAL_PROVIDER_CALL_AT_MOST_ONCE` remains `NOT_ADMITTED` |
...
| Tab and route                           | Fixed `S / P / Q / T` contents                                                                                                                                                                                                                                                                                                                                                                                     | Buttons in order                                                                     | Default evidence state                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Runs `/operations`                      | Queued/running/unknown/completed/dependency counts; `T=RunTable` with status/date/path/trigger/principal/tag/duration; `D=RunSummaryCard` after row selection; `P/Q` omitted                                                                                                                                                                                                                                       | Refresh, Filter, Open run, Resolve Owner outcome, Delete disposable completed cache  | Real Windmill use; operational only and deletion never changes business truth                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Run Detail `/operations/runs/:runId`    | Semantic/operational/timing summaries; `P=RunMetadataAndInputs`; `Q=OwnerViewCard`; `T=RunResultView` followed by fixed nested `Logs/Metrics/Traces/Assets` tabs                                                                                                                                                                                                                                                   | Copy locator, Refresh, Resolve same identity, Download bounded result/log            | Exact fixed skeleton above; no generic rerun/edit/share                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Workers `/operations/workers`           | Group/online/busy/unavailable counts; `T=WorkerTable`; `D=WorkerLeaseCard + WorkerHeartbeatHistory` after row selection; `P/Q` omitted                                                                                                                                                                                                                                                                             | Refresh, Open last run                                                               | Real `rd-product-edge` worker; no create/edit/restart/cache‑clean/REPL controls                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                                   | cross‑bind the current next action to the unique `ACTIVE` shell binding/history head, independent Operator Authorization, Time Evidence/revocation frontier, operation‑specific Owner freshness, Product Edge binding/head/outbox readiness, and exact content‑addressed operation manifest. Its fixed states are `IDLE / PREFLIGHTING / ADMITTED / REVALIDATION_REQUIRED / STALE / UNAVAILABLE`; the gate alone controls button enablement and never treats cached `AVAILABLE` as action authority. The panel has fixed `Current authority` then `Admission snapshot` tabs; each keeps `Operator Authorization` and `Product Edge readiness` as separate rows with identity, cut, state and stop predicate. A sealed OA receipt with absent PE custody remains visible but unavailable and never feeds the gate. The snapshot is historical. Neither component constructs, signs, repairs, or infers authority from credential or local policy |
