...
| Product Edge action authorization             | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Authority review rejected the earlier self‑proving environment/default‑policy path. The replacement candidate now has isolated PostgreSQL evidence that complete Issuer and Product Edge histories fail closed when issuance/head/frontier/outbox, binding/head/supersession, manifest, admission or invocation‑claim custody is mutated and return byte‑identical readback after restoration. It also proves that historical admission readback survives binding cutover/revocation while every first mutation re‑reads current binding/authorization and a revoked late claim or new admission is unavailable. This is candidate evidence only; every mutating control remains `permission_unavailable` until the real Workbench/runtime path and final review close |
...
re-read the complete current deployment and authorization histories before writing one invocation claim; cutover,
expiry, revocation, or any malformed/missing/extra history row returns `unavailable` with zero new effect. Exact
replay of an already committed claim renders `ALREADY_CLAIMED` and its original receipt, not another action or a
second provider call. The fixed presentation order is `Current authority`, `Admission snapshot`, `Invocation claim`.
...
| Audit `/operations/audit`               | Execute/update/create/delete/success/failure counts; `P=AuditTable`; `Q=AuditCorrelationCard + InvocationClaimReceipt`; `T=OperationTimeline`                                                                                                                                                                | Filter, Open correlation, Copy audit locator                                         | Real Windmill audit remains append‑only control‑plane evidence, not Owner business truth. A separately styled Product Edge claim receipt shows `CLAIMED_NEW / ALREADY_CLAIMED / unavailable`; historical admission resolution never implies a new effect                                                                                                                                                                      |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim identity, admission identity, attempt identity, committed time, digest and `CLAIMED_NEW / ALREADY_CLAIMED / unavailable` disposition. `ALREADY_CLAIMED` links the original receipt and disables invocation; it never renders as a retry or a second operational job                                                                                                                                                                                                                                                                                            |
