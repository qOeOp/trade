...
A 2026-08-23 candidate-lock check adds one narrower Windmill boundary.
...
| `workers/+page.svelte`                                                                                      | Group selection, worker table, occupancy/readiness, selected‑worker detail and last‑run navigation                                 | Worker config import/edit, tag administration, HTTP-agent creation, REPL, autoscaling and cache/restart controls |
...
| `audit_logs/+page.svelte` plus `AuditLogsFilters/Table/Timeline/Details`                                    | Filters, append‑only table, selected‑event timeline/detail and mobile drawer                                                       | Mutation, replay, dismiss and business‑truth projection from CE-redacted resource fields                         |
...
| Queued dependency cancellation    | `POST /api/operations/runs/{run_id}/cancel-dependency` -> `OperationalCancellationReceipt`            | Dispatcher accepts only `kind=dependency`, `state=queued`, empty domain effects and no worker claim. Compare‑and‑set changes that exact operational run to `cancelled`; an already claimed, terminal, unknown, mismatched or effect‑capable run fails closed. The receipt records run, prior state, principal, time and transition. It cannot cancel a domain request, provider/build/replay effect or Owner operation, and no batch endpoint exists                                                                                                                              |
...
| Workers and selected lease        | `GET /api/operations/workers` and `/workers/{worker_id}` -> `WorkerPage` or `WorkerLeaseEnvelope`     | `WorkerLeaseStore` is written only by worker registration/heartbeat/claim/release. UI reads identity/group/tags/version/start/limits/occupancy/last run/last observed plus the registered kind/tag/runtime/isolation compatibility set. Lease expiry yields `unavailable`; a live but non‑matching worker is `online / incompatible` for the selected run, never a UI‑authored `dead` or Ready state                                                                                                                                                                              |
...
last heartbeat. Readiness additionally requires a canonical compatibility match for the selected run's kind, tag,
runtime and required isolation. Client time, a missing row, process/container health, or a service-log message can
...
| Run Detail `/operations/runs/:runId`    | Semantic/operational/timing summaries; `P=RunMetadataAndInputs`; `Q=OwnerViewCard`; `T=RunResultView` followed by fixed nested `Logs/Metrics/Traces/Assets` tabs                                                                                                                                                                                                                                                   | Copy locator, Refresh, conditionally Cancel queued dependency, Resolve same identity, Download bounded result/log | Exact fixed skeleton above; Cancel occupies the third slot only for a queued, unclaimed, zero‑domain‑effect dependency run and is otherwise absent. No batch cancel or generic rerun/edit/share                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Workers `/operations/workers`           | Group/online/busy/unavailable counts; `T=WorkerTable`; `D=WorkerLeaseCard + WorkerCompatibilityMatrix + WorkerHeartbeatHistory` after row selection; `P/Q` omitted                                                                                                                                                                                                                                                 | Refresh, Open last run                                                                                            | Real `rd-product-edge` worker; liveness and selected‑run compatibility are separate. No create/edit/restart/cache‑clean/REPL controls                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- Every effect‑capable button is wrapped by `ActionAdmissionGate`. `Check & Run` is a composite control whose first
  click is a read-only preflight, not an admitted mutation; only its internal dispatch transition may cross into
  `ADMITTING`. Dispatch requires the current `NextLegalActionBar` operation and an `admitted` envelope for the same
  principal, scope, Owner, operation, schema, exact effect set, binding head, authorization and manifest digest.
  Expiry, revocation, head change, zero/dual `ACTIVE` bindings, manifest mismatch, resolver unavailability or
  preflight failure disables dispatch without preserving the previous positive state.
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                                          | cross‑bind the current next action to the unique `ACTIVE` shell binding/history head, independent Operator Authorization, Time Evidence/revocation frontier, operation‑specific Owner freshness, Product Edge binding/head/outbox readiness, exact operation/schema/effect set and content‑addressed manifest. Fixed gate states are `IDLE / PREFLIGHTING / ADMITTING / ADMITTED / REVALIDATION_REQUIRED / STALE / UNAVAILABLE`. `PREFLIGHTING` is cancellable and zero‑write; `ADMITTING` begins only after dispatch and is not cancellable; a post‑dispatch unknown exits the gate into the domain unknown state and same‑identity resolution. `ADMITTED` uses informational blue, never terminal green. The panel has fixed `Current authority` then `Admission snapshot` tabs, each with separate Operator Authorization and Product Edge rows. Historical snapshots never feed the current gate. Neither component constructs, signs, repairs or infers authority from credentials/local policy |
...
| `DependencyCancellationPanel`                                                               | fixed operational‑only confirmation for one queued dependency run: run/kind/path, queued‑since, required executor compatibility, explicit empty domain‑effect set, no‑claim proof and cancellation‑receipt slot. The sole effect button is `Cancel queued dependency`; pending reads `Cancelling…`, identity conflict/claim/terminal/unknown removes the button, and no batch, retry or domain cancellation exists                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `WorkerGroupTabs`, `WorkerLeaseCard`, `WorkerCompatibilityMatrix`, `WorkerHeartbeatHistory` | group, lease liveness, last observed/run, memory/version, then a fixed kind/tag/runtime/isolation compatibility matrix for the selected run. `online / incompatible` and isolation unavailable are explicit non‑ready states; no administration action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
