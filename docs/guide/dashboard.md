...
The latest exact uncommitted R&D candidate `a05d76ea18e2b35d7e55d74357fbc30b971ec1a2`, tree
`eb25b1a8325c4711ebd8d2cd012b3a87f70741c6`, now has fresh OA/PE static, ACL, corruption, migration-idempotency,
and PostgreSQL row-lock evidence. Exact image rebuild and bootstrap replay preserved both OA and Product Edge
genesis receipts byte-for-byte. Authenticated default-Web S1 job `01a0258e-773e-d80b-e464-6b4cd7a20c7e` then
rendered `REJECTED_NO_WRITE / CORRECT_INPUT_AND_CREATE_SUCCESSOR_REQUEST / missing Owner receipt`, with S2
disabled. Durable readback instead showed one committed Product Edge admission plus one admission outbox, zero R&D
receipt/Intent/family, and zero provider claim. The UI therefore misclassified an admitted-but-downstream-unavailable
handoff as an input rejection. The architecture Stop is a missing Product Edge-owned sealed downstream resolver:
R&D cannot call the OA API directly, cannot be granted OA authority, and must retain the OA-to-PE lock cut inside its
physical mutation transaction. S2, provider, full gates, commit, and delivery remain
...
| Product Edge action authorization             | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Fresh OA/PE static and PostgreSQL evidence now proves exact row locks, least‑privilege ACL, migration idempotency, corruption rejection and one external OA resolver path. Image/genesis replay preserved both Owner receipts. In the authenticated default Web, Product Edge then committed exactly one admission and outbox, but R&D wrote zero receipt/Intent/family because its transaction could not execute the OA resolver; provider claim/call remained zero. The observed Raw App incorrectly rendered `REJECTED_NO_WRITE` plus successor. The Dashboard must instead show committed PE admission, unavailable downstream R&D custody, overall `SUBMITTED_OR_UNKNOWN`, disabled S2, and same‑identity Resolve only. A Product Edge‑owned sealed downstream resolver that retains OA‑to‑PE locks in R&D's physical transaction is the missing backend contract. S2, provider, full runtime, commit, delivery and final review remain `NOT_ADMITTED` |
