...
| H1 fresh invocation admission                | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Exact‑head consumer review 确认候选 App/MCP projection fail‑closed，但 authority review 拒绝 `ca5154ed9f`：provider claim 只持久化 historical request admission 与 attempt，没有为直接解析的 current authorization frontier、policy‑equivalent binding、manifest 和 final locked write cut 建立独立 sealed receipt；expiry 也可能落在 validation 与独立采样的 claim commit time 之间。因此仅有 `CLAIMED` 必须渲染 unavailable，不能启用 Run；下文 TARGET 在 claim/start 之前加入独立 `InvocationAdmissionReceipt`。Dynamic PostgreSQL、Windmill、provider 与真实消费者证据仍 unavailable                                                                                                                   |
...
参与 current gate。第一次 provider claim 前，Product Edge 必须原子重读完整 current deployment 与
authorization history，并持久化一张独立 sealed invocation-admission receipt。该 receipt 绑定直接解析的 current
...
引用 sealed invocation admission，不能用 original request admission 或 transient resolver output 代替。Claim disposition 为
...
claim 一次，绝不创建 successor、replacement claim 或第二次 provider invocation。即使 claim row 存在，缺失、
stale、malformed 或 mismatch 的 invocation admission 仍渲染 unavailable。start 一旦提交，Run control 消失，任何 replay 都渲染 `OUTCOME_UNKNOWN` 与
...
| Versioned script 与 App dependency                   | App 与 MCP 必须调用同一个 `research_goal_v2` Product Edge operation；已观察到的 `research_goal_v1` MCP profile mismatch 是 `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`。准确 H1 candidate sync inventory 新增 `consumer_projection_v1`，它是被 App import 的 empty‑input canonical‑stamp validator，不是 App/MCP business operation，且仍只属于候选。Windmill 单独记录 dependency‑build job | `TARGET_DRAFT/KEEP_SEMANTICS`：一个 versioned operation registry entry、content digest、类型化 BFF gateway 与显式 dependency state；projection verification 编译进 typed gateway/library，不产生 catalog item、route、action 或用户 run record。Channel‑version mismatch 时 operation 渲染 unavailable |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim、invocation‑admission receipt identity、historical request‑admission lineage 与 attempt identity、committed time、digest、`CLAIMED_NEW / ALREADY_CLAIMED / unavailable`、current `CLAIMED / INVOCATION_STARTED`，以及精确 Owner‑projected next action。`ALREADY_CLAIMED` 链接原 receipt；只有恢复的 `CLAIMED + RUN_BOUNDED_EXECUTION_AGENT` 与 sealed invocation‑admission receipt 直接一致时，才可让同一 claim 进入 atomic start gate；其他状态都隐藏 Run control                                                                                                                                                                                                                           |
...
| `ProviderInvocationStateCard`                                             | 两种固定 geometry。可恢复 `CLAIMED` 先显示准确 sealed invocation‑admission receipt，再显示同一 request/attempt/claim、无 started time、next action `RUN_BOUNDED_EXECUTION_AGENT`，按钮依次为 Run bounded Agent + sandbox、Resolve same attempt、Copy claim、Open operational run。Missing 或 mismatch invocation admission 保持同一 geometry 但进入 unavailable，并移除 Run。`INVOCATION_STARTED / OUTCOME_UNKNOWN` 使用固定红色 geometry 与 `MANUALLY_RECONCILE_PROVIDER_INVOCATION`，按钮依次为 Resolve same attempt、Copy claim、Open operational run；不显示 Run 与 successor。绝不创建新 claim、重试 provider、标记 success、dismiss stop 或推断 outcome                                                   |
