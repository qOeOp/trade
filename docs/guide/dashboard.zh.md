...
| S2 action‑time Research freshness            | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Consumer v6 拒绝 `4c28bd583f`：Workbench 缓存 Owner `AVAILABLE` projection 且只校验 interval shape，页面跨过 `valid_through` 后仍启用 S2 Run；当前链路先提交新的 Product Edge admission，之后 R&D `prepare` 才发现 stale custody。TARGET 让点击先进入 read‑only `PREFLIGHTING`，在 Owner cut 下取得 same‑identity freshness，只允许 locked fresh path 执行首个 Product Edge/S2 写入。Stale/unavailable 返回 `STALE` 或 `REVALIDATION_REQUIRED`，只暴露 Resolve/Open evidence，且 Product Edge admission、R&D attempt、invocation admission 与 claim 全部零写入。Static 16/16 projector/control test 未覆盖自然 expiry；live Windmill/default‑Web 与 dynamic PostgreSQL 仍 unavailable                                                                                                                                                                                                                   |
...
| H0 exact‑head correction set                 | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Review 拒绝 `c224927c54`：R&D 仍保有 Qualification physical authority；final write‑edge freshness 可能在 checked cut 后过期；in‑flight admission 在 policy‑equivalent cutover 后可能失去 predecessor continuity；provider claim response loss 缺少精确 resumable Run action；S2 authority unavailable 可能被误渲染为 input rejection；MCP 仍选择 `research_goal_v1`。下文 TARGET 合同修正全部六项，但 corrected candidate、默认 App run、MCP parity、provider invocation 与 product acceptance 均尚不存在                                                                                                                                                                                                                                                                                                                                                                                               |
| H1 fresh invocation admission                | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Fresh exact‑head review 拒绝 `6bd9f627`：candidate 虽新增 OA successor issuance、sealed start custody、claim receipt field 与可达 legacy branch，但 Product Edge 在只允许 original binding 或一个 immediate successor 时却接受任意 policy‑equivalent successor chain；Rust 在真实 claim/non‑success wire 中省略 optional family key，而两个 Windmill exact‑key verifier 都要求显式 `null`；App/MCP 对既有 claim 仍先执行 freshness‑sensitive `prepare`，Owner terminalization/readback 也会在 Research expiry 后丢失 durable receipt；legacy projector 只接受 Rust 已验证四种 disposition 中的两种。下文固定 TARGET 明确 direct‑successor distance、按 resolution 区分的 absent/present key、prepare‑free claimed start、stale‑safe terminal custody 与四种 read‑only legacy disposition。Dynamic PostgreSQL、Windmill、provider 与真实消费者证据仍 unavailable                                         |
...
`AVAILABLE` 直接授权 effect。第一次 transition 是 read‑only `PREFLIGHTING`：BFF 在请求任何 Product Edge
admission 前，使用 Owner-controlled time 重读同一 R&D identity；pending 时 control disabled 并显示 spinner。
只有 Owner-cut `AVAILABLE` 可进入 locked admission/write path；`STALE`、`UNAVAILABLE`、timeout、malformed output
或 page cancellation 都进入固定 non-positive state，且 Product Edge admission、R&D attempt、invocation admission
与 claim 全部零写入。Browser 可以保守地把 aging cached action 改为 `REVALIDATION_REQUIRED`，但只有 Owner
response 可以标记 `STALE` 或重新启用 `Check & Run`。
...
| Versioned script 与 App dependency                   | App 与 MCP 必须调用同一个 `research_goal_v2` Product Edge operation；已观察到的 `research_goal_v1` MCP profile mismatch 是 `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`。H1 sync 新增 candidate‑only `consumer_projection_v1`，它是被 App import 的 empty‑input canonical‑stamp validator，不是 business operation。`6bd9f627` exact‑head review 证明两个 Windmill verifier 仍消费人工填写的 `null` family key，而 Rust 在 claim/non‑success wire 中会省略它们；Run adapter 对已恢复 claim 也仍先调用 `prepare`。Windmill 单独记录 dependency‑build job | `TARGET_DRAFT/KEEP_SEMANTICS`：一个 versioned operation registry entry、content digest、类型化 BFF gateway 与显式 dependency state。生成一份供 adapter/projector 共用、按 resolution 分支的 Rust‑to‑gateway parser，把 Rust serialization fixture 直接输入两者，并让 recovered `CLAIMED` 不经 `prepare` 直接进入 start；schema drift 保持固定 unavailable geometry。Projection verification 编译进 typed library，不产生 catalog item、route、action 或用户 run record |
...
| Run Detail / completed job 加 result API   | 成功 replay 显示 received/started time、duration、worker、run ID、5 MB peak、script hash/language、App trigger、exact input、JSON result 与 Owner receipt | `/operations/runs/:runId`；header action 为 Back to Runs、Copy locator、domain‑aware Resolve。排除 `Run again`、Share、Edit 与任意 script link                                 | `RunDetailProjection`；schema allowlist 限定的 immutable input 与 bounded result projection、显式 withheld/redacted disclosure、worker/timing/resource metadata 与 Owner receipt reference；无 raw payload fallback 或业务 custody |
...
worker card list，选择后把同一 `D` 显示为 full-screen drawer。
...
`T`，并把 `D` 作为 full-screen overlay 打开。Filter 收进 route-local drawer，primary table/card 与当前状态始终可见。
...
| Cached `AVAILABLE`；选择 S2 action，Owner preflight pending              | `Check & Run` 进入 `PREFLIGHTING`；无 mutation | Cancel 或 Refresh Owner state                                | Disabled                                        | Primary 立即 disabled 并显示 spinner。Same‑identity Owner reread 先于 Product Edge admission；只有 locked Owner‑cut `AVAILABLE` 继续。Timeout/cancel/malformed 保持 `REVALIDATION_REQUIRED`，且 business write 为零                                 |
| S2 action‑time Owner preflight 返回 stale 或 unavailable                 | Disabled                                       | Resolve same request identity enabled                        | Open/Copy historical evidence                   | 只有 Owner readback 可显示 `STALE`；unavailable 保持 `REVALIDATION_REQUIRED`。没有 Product Edge admission、R&D attempt、invocation admission、claim、provider call、successor 或 review action                                                      |
...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required 计数；`P=ArtifactTable`；`Q=ArtifactOutcomeProjectionGate + ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel + InvocationAdmissionReceipt + ProviderInvocationStateCard + LegacyTerminalQuarantinePanel`；`T=BuildAndSecurityEvidence`，含 deterministic double‑build 与 sandbox policy | Open Artifact、Resolve same attempt、Copy provider claim、Open operational run、Ask Agent to revise、Start exploratory replay                                                                                                              | 既有 S2 是 `CURRENT/PARTIAL`；result‑authority precedence、binding、no‑Artifact closure 与 invocation state 为 `OBSERVED_CANDIDATE_NOT_CURRENT`。gate 先选择 sealed R&D terminal receipt；只有不存在 terminal 时 `INVOCATION_STARTED` 才能投影 `OUTCOME_UNKNOWN`。被拒绝的 `6bd9f627` 要求 Rust 会省略的 `null` family key，让 recovered claim 经过 `prepare`，在 Research expiry 后丢失 terminal custody，并只投影两种 legacy disposition。TARGET 使用按 resolution 区分的 absent/present key、direct claimed start、stale‑safe terminal receipt 与四种 read‑only legacy disposition，同时在 mismatch 时保留固定 unavailable geometry。Unknown 只暴露有界 evidence 与 same‑attempt resolve。无 provider retry button；`ACTUAL_PROVIDER_CALL_AT_MOST_ONCE` 仍为 `NOT_ADMITTED` |
...
| Tab 与 route                            | 固定 `S / P / Q / T` 内容                                                                                                                                                                                                                                                                                                                                                                                    | Button 顺序                                                                          | 默认证据状态                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runs `/operations`                      | Queued/running/unknown/completed/dependency 计数；`T=RunTable`，含 status/date/path/trigger/principal/tag/duration；row selection 后 `D=RunSummaryCard`；省略 `P/Q`                                                                                                                                                                                                                                          | Refresh、Filter、Open run、Resolve Owner outcome、Delete disposable completed cache  | Windmill 真实使用；仅 operational，删除不改变业务 truth                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Run Detail `/operations/runs/:runId`    | Semantic/operational/timing summary；`P=RunMetadataAndInputs`；`Q=OwnerViewCard`；`T=RunResultView`，后接固定嵌套 `Logs/Metrics/Traces/Assets` tabs                                                                                                                                                                                                                                                          | Copy locator、Refresh、Resolve same identity、Download bounded result/log            | 使用上方精确固定 skeleton；无通用 rerun/edit/share                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Workers `/operations/workers`           | Group/online/busy/unavailable 计数；`T=WorkerTable`；row selection 后 `D=WorkerLeaseCard + WorkerHeartbeatHistory`；省略 `P/Q`                                                                                                                                                                                                                                                                               | Refresh、Open last run                                                               | 真实 `rd-product-edge` worker；无 create/edit/restart/cache‑clean/REPL control                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- 每个修改型 button 都由 `ActionAdmissionGate` 包裹。Enabled 同时要求当前 `NextLegalActionBar` operation 与
  `admitted` envelope 在 principal、scope、Owner、operation、schema、binding head、authorization 和 manifest
  digest 上完全一致。Expiry、revocation、head change、zero/dual `ACTIVE` binding、manifest mismatch 或 resolver
  unavailable 都必须 disable control，且不能保留之前的绿色状态。
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                                   | 把 current next action 与唯一 `ACTIVE` Shell binding/history head、独立 Operator Authorization、Time Evidence/revocation frontier、operation‑specific Owner freshness、Product Edge binding/head/outbox readiness 和准确 content‑addressed operation manifest 交叉绑定。固定 state 为 `IDLE / PREFLIGHTING / ADMITTED / REVALIDATION_REQUIRED / STALE / UNAVAILABLE`；Gate 单独决定 button enablement，且绝不把 cached `AVAILABLE` 当作 action authority。Panel 内固定先 `Current authority`、后 `Admission snapshot` 两个 tab；每个 tab 都把 `Operator Authorization` 与 `Product Edge readiness` 保持为独立 row，并显示 identity、cut、state 与 stop predicate。sealed OA receipt 加缺失 PE custody 仍可见但 unavailable，绝不参与 gate；snapshot 是 historical。两个组件都不能从 credential 或 local policy 构造、签名、修复或推断 authority |
