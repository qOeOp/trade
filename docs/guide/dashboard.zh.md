...
2026-08-23 的 candidate lock 核对新增了一条更窄的 Windmill 边界。对三个 Product Edge script 与 Raw App
...
| `workers/+page.svelte`                                                                                    | group selection、worker table、occupancy/readiness、selected‑worker detail 与 last‑run navigation                                 | worker config import/edit、tag administration、HTTP-agent creation、REPL、autoscaling 与 cache/restart control |
...
| `audit_logs/+page.svelte` 加 `AuditLogsFilters/Table/Timeline/Details`                                    | filter、append‑only table、selected‑event timeline/detail 与 mobile drawer                                                        | mutation、replay、dismiss，以及从 CE 已脱敏 resource field 投影业务事实                                        |
...
| Queued dependency cancellation    | `POST /api/operations/runs/{run_id}/cancel-dependency` -> `OperationalCancellationReceipt`           | Dispatcher 只接受 `kind=dependency`、`state=queued`、空 domain effect 且没有 worker claim 的 run。Compare‑and‑set 只把该精确 operational run 改为 `cancelled`；已 claim、terminal、unknown、identity mismatch 或 effect‑capable run 一律 fail close。Receipt 记录 run、prior state、principal、time 与 transition；不能取消 domain request、provider/build/replay effect 或 Owner operation，且不存在 batch endpoint                                                                                                         |
...
| Workers 与 selected lease         | `GET /api/operations/workers` 与 `/workers/{worker_id}` -> `WorkerPage` 或 `WorkerLeaseEnvelope`     | 只有 worker registration/heartbeat/claim/release 可以写 `WorkerLeaseStore`。UI 读取 identity/group/tag/version/start/limit/occupancy/last run/last observed，加已注册的 kind/tag/runtime/isolation compatibility set；lease expiry 产生 `unavailable`；live 但不匹配的 worker 对 selected run 是 `online / incompatible`，绝不能由 UI 写 `dead` 或 Ready state                                                                                                                                                               |
...
`unknown`。Worker liveness 只从 stored lease deadline 与 last heartbeat 计算；readiness 还必须对 selected
run 的 kind、tag、runtime 与 required isolation 做 canonical compatibility match。Client time、missing row、
process/container health 或 service-log message 既不能提升 liveness，也不能伪造 compatibility。
...
| Run Detail `/operations/runs/:runId`    | Semantic/operational/timing summary；`P=RunMetadataAndInputs`；`Q=OwnerViewCard`；`T=RunResultView`，后接固定嵌套 `Logs/Metrics/Traces/Assets` tabs                                                                                                                                                                                                                                                          | Copy locator、Refresh、条件成立时 Cancel queued dependency、Resolve same identity、Download bounded result/log | 使用上方精确固定 skeleton；Cancel 只在 queued、unclaimed、zero‑domain‑effect dependency run 中占据第三个按钮位，其余状态该 slot 缺席。无 batch cancel 或通用 rerun/edit/share                                                                                                                                                                                                                                                                                                                                                                                              |
| Workers `/operations/workers`           | Group/online/busy/unavailable 计数；`T=WorkerTable`；row selection 后 `D=WorkerLeaseCard + WorkerCompatibilityMatrix + WorkerHeartbeatHistory`；省略 `P/Q`                                                                                                                                                                                                                                                   | Refresh、Open last run                                                                                         | 真实 `rd-product-edge` worker；liveness 与 selected‑run compatibility 分开；无 create/edit/restart/cache‑clean/REPL control                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- 每个修改型 button 都由 `ActionAdmissionGate` 包裹。S2 的 `Check & Run` 是 composite control：第一段只执行
  可取消、零写的 same-request preflight；dispatch 后才进入不可取消的 `ADMITTING`。Enabled 同时要求当前
  `NextLegalActionBar` operation 与 `admitted` envelope 在 principal、scope、Owner、operation、schema、exact
  effect set、binding head、authorization 和 manifest digest 上完全一致。Expiry、revocation、head change、
  zero/dual `ACTIVE` binding、manifest mismatch 或 resolver unavailable 都必须 disable control，且不能保留之前
  的绿色状态。
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                                          | 把 current next action 与唯一 `ACTIVE` Shell binding/history head、独立 Operator Authorization、Time Evidence/revocation frontier、operation‑specific Owner freshness、Product Edge binding/head/outbox readiness、exact operation/schema/effect set 和 content‑addressed manifest 交叉绑定。固定 state 为 `IDLE / PREFLIGHTING / ADMITTING / ADMITTED / REVALIDATION_REQUIRED / STALE / UNAVAILABLE`。`PREFLIGHTING` 可取消且零写；`ADMITTING` 只在 dispatch 后开始且不可取消；dispatch 后 unknown 离开 gate，进入 domain unknown state 与 same‑identity resolution。`ADMITTED` 使用信息蓝，绝不使用 terminal green。Panel 内固定先 `Current authority`、后 `Admission snapshot` 两个 tab；每个 tab 都把 Operator Authorization 与 Product Edge row 分开。Historical snapshot 绝不参与 current gate。两个组件都不能从 credential/local policy 构造、签名、修复或推断 authority |
...
| `DependencyCancellationPanel`                                                               | 只针对一个 queued dependency run 的固定 operational confirmation：run/kind/path、queued‑since、required executor compatibility、显式空 domain‑effect set、no‑claim proof 与 cancellation‑receipt slot。唯一 effect button 为 `Cancel queued dependency`；pending 显示 `Cancelling…`；identity conflict、claim、terminal 或 unknown 会移除按钮，且不存在 batch、retry 或 domain cancellation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `WorkerGroupTabs`, `WorkerLeaseCard`, `WorkerCompatibilityMatrix`, `WorkerHeartbeatHistory` | group、lease liveness、last observed/run、memory/version，后接针对 selected run 的固定 kind/tag/runtime/isolation compatibility matrix。`online / incompatible` 与 isolation unavailable 是显式 non‑ready state；无管理 action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
