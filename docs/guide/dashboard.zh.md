...
| H1 fresh invocation admission                | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Fresh authority 与 consumer review 拒绝 `fdbc5aa6`：虽然 Product Edge 已密封独立 invocation admission，但 Windmill exact‑key verifier 遗漏其 identity/digest 字段，把真实 claim 降级为 `SUBMITTED_OR_UNKNOWN`；start/recovery 在 claim 后重新执行 current R&D `prepare` freshness；verified legacy quarantine 没有可达 consumer projection；Operator Authorization 也无法在历史 expiry 后 canonical 签发 policy‑equivalent successor。下文固定 TARGET 保留准确 wire receipt、只从 sealed historical attempt custody 启动、投影 legacy‑only evidence，并在不伪造 authority 的前提下把 successor issuance 渲染 unavailable。Dynamic PostgreSQL、Windmill、provider 与真实消费者证据仍 unavailable            |
...
generated/exact parser，并在投影 claim 前绑定两个值。Missing、extra、schema-mismatched 或 tampered wire field
...
`MANUALLY_RECONCILE_PROVIDER_INVOCATION`，绝不再次调用 provider 或伪造 provider outcome。固定展示顺序为
...
request/attempt identity、terminal receipt、custody generation 与 quarantine reason 必须作为一个 strict
legacy‑only branch 穿过共享 consumer projector。该 branch 缺失或 malformed 时，固定 legacy slot 显示
`Unavailable` 并只允许 same‑attempt Resolve，绝不能静默折叠成 untyped generic unknown。
...
| Versioned script 与 App dependency                   | App 与 MCP 必须调用同一个 `research_goal_v2` Product Edge operation；已观察到的 `research_goal_v1` MCP profile mismatch 是 `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`。H1 sync 新增 candidate‑only `consumer_projection_v1`，它是被 App import 的 empty‑input canonical‑stamp validator，不是 business operation。`fdbc5aa6` review 证明 Windmill operation adapter 与共享 projector 会同时保留旧 exact‑key claim shape，而 Rust 已新增 sealed‑receipt field，从而降级真实 claim。Windmill 单独记录 dependency‑build job | `TARGET_DRAFT/KEEP_SEMANTICS`：一个 versioned operation registry entry、content digest、类型化 BFF gateway 与显式 dependency state。生成一份供 adapter/projector 共用的 exact Rust‑to‑gateway parser；schema drift 保持固定 unavailable geometry。Projection verification 编译进 typed library，不产生 catalog item、route、action 或用户 run record |
...
| Sealed `CLAIMED`；fresh Research read stale/unavailable                  | Run bounded Agent + sandbox enabled   | Resolve same attempt identity enabled                        | Disabled                                        | Historical claim/attempt custody 是 gate；fresh Research state 明确标为 non‑gating               |
| Verified legacy terminal                                                 | Disabled                              | Resolve same attempt identity enabled                        | Disabled                                        | 固定 legacy‑only panel 显示 historical receipt/quarantine reason；无 Artifact/provider/successor |
...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required 计数；`P=ArtifactTable`；`Q=ArtifactOutcomeProjectionGate + ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel + InvocationAdmissionReceipt + ProviderInvocationStateCard + LegacyTerminalQuarantinePanel`；`T=BuildAndSecurityEvidence`，含 deterministic double‑build 与 sandbox policy | Open Artifact、Resolve same attempt、Copy provider claim、Open operational run、Ask Agent to revise、Start exploratory replay                                                                                                              | 既有 S2 是 `CURRENT/PARTIAL`；result‑authority precedence、binding、no‑Artifact closure 与 invocation state 为 `OBSERVED_CANDIDATE_NOT_CURRENT`。gate 先选择 sealed R&D `SUCCESS` 或 `FAILED_NO_ARTIFACT`；只有两者都不存在时 `INVOCATION_STARTED` 才能投影 `OUTCOME_UNKNOWN`。被拒绝的 `fdbc5aa6` 因 Windmill verifier 遗漏 invocation‑admission receipt identity/digest 而降级真实 Rust claim，在 sealed start 前重跑 Research freshness，并丢失 Owner legacy discriminant。TARGET 在 wire mismatch 时保留 A0/A1 或 legacy geometry 为 unavailable，claim 后把 fresh Research 标为 non‑gating，并渲染 strict legacy‑only branch。Unknown 只暴露有界 evidence 与 same‑attempt resolve。无 provider retry button；`ACTUAL_PROVIDER_CALL_AT_MOST_ONCE` 仍为 `NOT_ADMITTED` |
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                        | 把 current next action 与唯一 `ACTIVE` Shell binding/history head、独立 Operator Authorization、Time Evidence/revocation frontier、Product Edge binding/head/outbox readiness 和准确 content‑addressed operation manifest 交叉绑定。Gate 单独决定 button enablement。Panel 内固定先 `Current authority`、后 `Admission snapshot` 两个 tab；每个 tab 都把 `Operator Authorization` 与 `Product Edge readiness` 保持为独立 row，并显示 identity、cut、state 与 stop predicate。sealed OA receipt 加缺失的 PE custody 仍可见但 unavailable，绝不参与 gate；snapshot 是 historical。两个组件都不能从 credential 或 local policy 构造、签名、刷新、修复或推断 authority                                              |
| `AuthorizationSuccessorReadiness`                                         | 只读显示 prior authorization identity/scope/sequence、terminal expiry/revocation state、canonical successor‑operation availability、存在时的 successor receipt/identity，以及准确 missing/invalid stop。它绝不构造 scope、选择 replacement、签名、续期、撤销或调用 transport‑token control。Owner‑issued equivalent successor 存在前，`Current authority` 保持 unavailable，而 admission snapshot 仍可见                                                                                                                                                                                                                                                                                                        |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim 加准确 public wire field `invocation_admission_receipt_identity` 与 `invocation_admission_receipt_digest`、historical request‑admission lineage、attempt identity、committed time、claim digest、`CLAIMED_NEW / ALREADY_CLAIMED / unavailable`、current `CLAIMED / INVOCATION_STARTED` 与 Owner‑projected next action。Operation adapter 与 consumer projector 共享一个 versioned exact parser。Missing/extra/tampered field 让 A0/A1 保持 unavailable。只有恢复的 `CLAIMED + RUN_BOUNDED_EXECUTION_AGENT` 与 sealed receipt 直接一致时，才可进入 atomic start gate                                                                                                                          |
...
| `ProviderInvocationStateCard`                                             | 两种固定 geometry。可恢复 `CLAIMED` 先显示准确 sealed invocation‑admission receipt，再显示同一 request/attempt/claim、无 started time、next action `RUN_BOUNDED_EXECUTION_AGENT`，按钮依次为 Run bounded Agent + sandbox、Resolve same attempt、Copy claim、Open operational run。Missing/mismatch invocation admission 保持同一 geometry unavailable 并移除 Run。Claim 后 stale/unavailable fresh Research read 标为 non‑gating；start 消费 sealed historical Intent/attempt custody，绝不重跑 prepare。`INVOCATION_STARTED / OUTCOME_UNKNOWN` 使用固定红色 geometry 与 manual reconciliation，且无 Run/successor。绝不创建 claim、重试 provider、标记 success、dismiss stop 或推断 outcome                    |
| `LegacyTerminalQuarantinePanel`                                           | Owner discriminant、request/attempt identity、verified historical terminal receipt identity、legacy custody generation、observed time 与 quarantine reason 的 strict legacy‑only projection。只依次暴露 Resolve same attempt、Open/Copy historical receipt；没有 current Research View、Artifact promotion、provider invocation、successor、TrialFamily repair 或 dismiss action。Missing/malformed projection 保留同一固定 geometry 并显示 unavailable，绝不折叠成 generic unknown                                                                                                                                                                                                                             |
...
request conflict。`AuthorizationSuccessorReadiness` 可以解析并显示该 Owner fact，但 Dashboard BFF 绝不调用
issuance operation。Canonical Owner API 与 PostgreSQL evidence 存在前，expired-history recovery 是
