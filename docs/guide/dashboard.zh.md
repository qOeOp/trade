...
`d623e0c3274b9446bb3505f94de31cefd28aba7e`、tree `e70053ec6859670b0d930ac067ddad32df0eec7f`。
最新精确未提交 R&D candidate `a05d76ea18e2b35d7e55d74357fbc30b971ec1a2`、tree
`eb25b1a8325c4711ebd8d2cd012b3a87f70741c6`，现已有 fresh OA/PE static、ACL、corruption、migration
幂等与 PostgreSQL row-lock 证据。精确 image 重建和 bootstrap replay 逐字节保留了 OA 与 Product Edge
两张 genesis receipt。随后认证默认 Web S1 job `01a0258e-773e-d80b-e464-6b4cd7a20c7e` 渲染
`REJECTED_NO_WRITE / CORRECT_INPUT_AND_CREATE_SUCCESSOR_REQUEST / missing Owner receipt`，并禁用 S2；但
durable readback 显示 Product Edge admission 与 admission outbox 各一张、R&D receipt/Intent/family 为零、provider
claim 为零。因此 UI 把"已 admission 但 downstream unavailable"的 handoff 错标为输入拒绝。Architecture Stop
是缺少 Product Edge-owned sealed downstream resolver：R&D 既不能直接调用 OA API，也不能被授予 OA 权限，并且
必须在自己的物理 mutation transaction 内保留 OA-to-PE lock cut。S2、provider、完整 gates、commit 与交付仍为
...
| Product Edge action authorization            | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Fresh OA/PE static 与 PostgreSQL 证据现已证明 exact row lock、最小权限 ACL、migration 幂等、corruption rejection 和唯一外部 OA resolver 路径；image/genesis replay 保留了两张 Owner receipt。认证默认 Web 中 Product Edge 随后准确提交一张 admission 与 outbox，但 R&D transaction 因不能执行 OA resolver 而写入零 receipt/Intent/family，provider claim/call 也为零。被观察 Raw App 错误渲染 `REJECTED_NO_WRITE` 与 successor；Dashboard 必须改为显示 committed PE admission、unavailable downstream R&D custody、整体 `SUBMITTED_OR_UNKNOWN`、disabled S2 和唯一 same‑identity Resolve。缺失的后台合同是在 R&D 物理 transaction 内保持 OA‑to‑PE lock 的 Product Edge‑owned sealed downstream resolver。S2、provider、完整 runtime、commit、交付与最终 review 仍为 `NOT_ADMITTED` |
