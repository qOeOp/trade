...
本设计证据切面为 2026-08-22、已观察 Dashboard checkout base
`ae2939736ed3fcecf606d9bc8006027250c7a0c5`、tree `ad4242df56292bcce765c781da36459241fe4760`。
最新精确 R&D candidate `a05d76ea18e2b35d7e55d74357fbc30b971ec1a2`、tree
`eb25b1a8325c4711ebd8d2cd012b3a87f70741c6`，完成了 static Workbench 5/5 与 PostgreSQL 20/20
证据，但在替换 candidate service、执行 Windmill 或调用 provider 前停止。它的显式 bootstrap 持久化了
一张 Operator Authorization genesis receipt，随后 Product Edge 失败，因为其 canonical
`SELECT ... FOR SHARE` 读取需要 `UPDATE` 权限，而唯一写者边界正确拒绝该权限。Product Edge
binding/head/outbox 保持 `0/0/0`。genesis receipt 是历史 Owner 证据，不证明 action admission 已就绪；
runtime 能力因此仍为 `OBSERVED_CANDIDATE_NOT_CURRENT / NOT_ADMITTED`。
...
| Product Edge action authorization            | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Authority review 已拒绝较早的 environment/default‑policy 自证路径。隔离 PostgreSQL 测试现已证明完整 Issuer/Product Edge history verification、锁序、cutover/revocation replay 与 one‑use invocation claim/start state。Workbench 5/5 证明 `claim -> start -> provider`：claim 响应丢失可恢复 `CLAIMED` 并继续 start；但 `INVOCATION_STARTED` 提交后的 replay 固定返回 `OUTCOME_UNKNOWN`，显示 claim identity/digest 与 `MANUALLY_RECONCILE_PROVIDER_INVOCATION`，且绝不再次调用 provider。最新动态尝试还证明 OA genesis receipt 可以存在而 Product Edge binding/head/outbox 缺失：bootstrap 在跨 Owner 行锁权限边界停止，未替换服务、执行 Windmill 或调用 provider。该 partial state 只为 audit 渲染 sealed OA receipt，admission 保持 `unavailable`，所有修改型控件禁用。actual‑provider‑call、provider reconciliation、完整 runtime 与最终 review 仍为 `NOT_ADMITTED` |
| Qualification protected‑feedback frontier    | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Missing Qualification head 不能证明 `GENESIS_EMPTY`。Qualification 必须先穷尽枚举并 canonical verify 每张历史 projection/outbox，再按 exact basis filter 或发布 empty frontier。Missing、malformed、ambiguous 或 unverifiable history 必须渲染 `unknown/unavailable`，只暴露 exact‑basis Resolve，且不得写 positive projection/head/outbox                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
参与 current gate。第一次 provider invocation 必须在写入 immutable invocation claim 前，原子重读完整
current deployment 与 authorization history；cutover、expiry、revocation 或任何 malformed/missing/extra
history row 都返回 `unavailable` 且零新增 effect。

Claim receipt 与 invocation state 是两个独立 Product Edge fact。Claim disposition 为
`CLAIMED_NEW | ALREADY_CLAIMED`；state 为 `CLAIMED | INVOCATION_STARTED`；start disposition 为
`STARTED_NEW | OUTCOME_UNKNOWN`。Claim 响应丢失可以恢复同一 `CLAIMED` receipt，再通过唯一原子 start
transition；start 一旦提交，任何 replay 都渲染 `OUTCOME_UNKNOWN` 与
`MANUALLY_RECONCILE_PROVIDER_INVOCATION`，绝不再次调用 provider 或伪造 provider outcome。固定展示顺序为
`Current authority`、`Admission snapshot`、`Invocation claim`、`Invocation state`。
...
Research View、provider action、successor action 或 TrialFamily repair action。
...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required 计数；`P=ArtifactTable`；`Q=ArtifactOutcomeProjectionGate + ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel + ProviderInvocationStateCard + LegacyTerminalQuarantinePanel`；`T=BuildAndSecurityEvidence`，含 deterministic double‑build 与 sandbox policy | Open Artifact、Resolve same attempt、Copy provider claim、Open operational run、Ask Agent to revise、Start exploratory replay                                                           | 既有 S2 是 `CURRENT/PARTIAL`；result‑authority precedence、binding、no‑Artifact closure 与 invocation state 为 `OBSERVED_CANDIDATE_NOT_CURRENT`。gate 先选择 sealed R&D `SUCCESS` 或 `FAILED_NO_ARTIFACT`；只有两者都不存在时 `INVOCATION_STARTED` 才能投影 `OUTCOME_UNKNOWN`。Unknown 隐藏 review/successor action，只暴露 claim copy、operational evidence、same‑attempt resolve 与 manual reconciliation。Legacy terminal 只渲染历史 receipt。无 provider retry button；`ACTUAL_PROVIDER_CALL_AT_MOST_ONCE` 仍为 `NOT_ADMITTED` |
...
| Protected feedback `/qualification/protected-feedback` | Current/genesis‑empty/unknown/corrupt 计数；`P=QualificationFrontierTable`；`Q=QualificationFrontierReceiptPanel + IndependenceBasisLink`；`T=OpaqueFrontierTimeline`                                      | Refresh、按 exact basis Resolve current、Open R&D basis receipt、Copy opaque frontier reference                                           | `OBSERVED_CANDIDATE_NOT_CURRENT`；隔离 canonical‑history Workbench flow 消费了 sealed `GENESIS_EMPTY` receipt，但 missing head 本身永远不充分。历史 projection/outbox 穷尽验证成功前，页面渲染 `unknown/unavailable`、隐藏 Copy frontier，并且只暴露 exact‑basis Resolve。Identity/cut/digest/state 保持可见；protected content、candidate Intake、protected attempt、eligibility、holdout 与 cross‑family ancestry 仍为 `NOT_ADMITTED` |
...
| Audit `/operations/audit`               | Execute/update/create/delete/success/failure 计数；`P=AuditTable`；`Q=AuditCorrelationCard + InvocationClaimReceipt + ProviderInvocationStateCard`；`T=OperationTimeline`                                                                                                                                                                                                  | Filter、Open correlation、Copy audit locator、Copy provider claim                    | Windmill audit 仍是 append‑only control‑plane evidence，不是 Owner business truth。Product Edge 单独显示 claim disposition、`CLAIMED / INVOCATION_STARTED`、start disposition 与 state digest。`OUTCOME_UNKNOWN` 是持久 manual‑reconciliation stop；历史 admission 或 claim resolve 绝不意味着新 effect 或 provider retry                                                                                                                                                              |
...
| Access `/settings/access`               | Principal/session/token/revoked 计数，以及 binding `ACTIVE/SUPERSEDED/zero‑active` 与 authorization available/expired/revoked/unavailable 计数；`P=LocalPrincipalCard`；`Q=AuthorizationLineagePanel`，内部固定 `Current authority / Admission snapshot` tabs，并分开显示 `Operator Authorization / Product Edge readiness` rows；`T=CapabilityManifest + CredentialAudit` | Re‑authenticate local session、Issue narrow transport token、Revoke token、Copy once | Transport credential 控件绝不能签发、续期或撤销 Operator Authorization。只有 `Current authority` 驱动 action state；`Admission snapshot` 是 immutable audit readback，在 cutover/revocation 后仍可用。sealed OA genesis 加缺失的 PE binding/head/outbox 只是 audit evidence，current gate 保持 unavailable。两者都显示准确 binding/head、issuer/audience/scope、expiry/revocation frontier、manifest digest、source cut 与 stop predicate；secret/token value 仍只显示一次且不记录日志 |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim identity、admission identity、attempt identity、committed time、digest、`CLAIMED_NEW / ALREADY_CLAIMED / unavailable` disposition 与 current `CLAIMED / INVOCATION_STARTED` state。`ALREADY_CLAIMED` 链接原 receipt；只有恢复的 `CLAIMED` 才可进入独立原子 start gate                                                                                                                                                                                                                                                                                                                                                                           |
...
| `ProviderInvocationStateCard`                                             | claim identity/digest、state digest、started time、`STARTED_NEW / OUTCOME_UNKNOWN`、operational‑run link 与 exact next action。只有 `ArtifactOutcomeProjectionGate` 找不到 sealed R&D terminal 时，它才是 active outcome panel。`OUTCOME_UNKNOWN` 使用固定红色 geometry 与 `MANUALLY_RECONCILE_PROVIDER_INVOCATION`；按钮为 Copy claim、Open operational run、Resolve same attempt。没有 retry‑provider、mark‑success 或 dismiss control，也不推断 provider outcome                                                                                                                                                                                                |
| `LegacyTerminalQuarantinePanel`                                           | 已验证 historical terminal receipt identity、legacy custody generation、observed time 与 quarantine reason。只暴露 Open/Copy historical receipt；没有 current Research View、Artifact promotion、provider invocation、successor、TrialFamily repair 或 dismiss action                                                                                                                                                                                                                                                                                                                                                                                              |
...
被观察 candidate 还证明 Operator Authorization custody 与 Product Edge readiness 不是同一个 service state。
OA genesis 可以提交，而 Product Edge binding/head/outbox 因跨 Owner admission read 无法取得所需串行化边界而
保持缺失。在 canonical Owners 准入并动态验证同时保留唯一写者与最小权限的 lock protocol 前，gateway 把该组合
投影为 `unavailable`，返回精确 stop predicate，并产生零 business/provider effect。Dashboard 与 BFF 不能授予
跨 Owner `UPDATE`、用 unlocked read 替代锁，也不能从 OA receipt 推断 readiness。repair protocol 本身仍是后台
`NOT_ADMITTED`；UI contract 只包含 fail-closed partial-state projection。
