...
| Sealed S1 basis；Qualification 与 terminal Research receipt 缺失         | Disabled                              | Resolve same request identity enabled                        | Disabled                                        | `SEALED_BASIS_PENDING_QUALIFICATION`；显示 basis receipt/head/outbox 与缺失的 next receipt；无 duplicate basis 或 fresh‑authority gate |
...
| `AuthorizationSuccessorReadiness`                                         | 只读显示 prior authorization identity/scope/sequence、admission/current generation、`successor_distance=0\|1`、terminal expiry/revocation state、canonical direct‑successor operation availability、存在时的 successor receipt/identity，以及准确 missing/invalid stop。Distance 大于一时显示`DIRECT_SUCCESSOR_REQUIRED`并 unavailable。它绝不遍历 chain、构造 scope、选择 replacement、签名、续期、撤销或调用 transport‑token control。一个 immediate Owner‑issued equivalent successor 存在前，`Current authority` 保持 unavailable，而 admission snapshot 仍可见                                                                                                                                                                                          |
...
| `S1StageCustodyPanel`                                                     | 只读 `SEALED_BASIS_PENDING_QUALIFICATION` geometry，绑定准确 request 与 original admission、basis receipt/identity、basis head/outbox、commit cut、缺失的 Qualification/terminal Research receipt 及 next action。仅在 canonical basis‑stage verification 后渲染，并允许从 historical custody 做 same‑identity Resolve/完成；Submit/successor 缺席，duplicate basis/head/outbox 为 unavailable，changed request/admission 为 conflict                                                                                                                                                                                                                                                                                                                        |
...
canonical verify 其准确 request、original admission、digest 与 commit cut。Qualification 和 terminal Research
receipt 仍缺失时，它返回 sealed `SEALED_BASIS_PENDING_QUALIFICATION` custody，第二笔 transaction 使用
`Historical` completion semantics。普通 row 存在不能构造该 custody；duplicate basis/head/outbox write 被禁止，
changed request 或 admission 必须 conflict。Dashboard/BFF 只能显示 sealed projection，绝不能铸造它或用
current authority 替代 historical completion。
