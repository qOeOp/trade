...
| Product Edge action authorization            | `ACTIVE / OBSERVED_CANDIDATE_NOT_CURRENT`                 | Authority review 已拒绝较早的 environment/default‑policy 自证路径。替代 candidate 现有隔离 PostgreSQL 证据证明：issuance/head/frontier/outbox、binding/head/supersession、manifest、admission 或 invocation‑claim custody 任一突变时，完整 Issuer 与 Product Edge history 都会 fail closed，恢复后返回 byte‑identical readback；历史 admission 在 binding cutover/revocation 后仍可读，但每次 first mutation 都重新读取 current binding/authorization，revoked late claim 与 new admission 均 unavailable。该证据仍只属于 candidate；真实 Workbench/runtime 路径与最终 review 闭合前，所有修改型控件继续保持 `permission_unavailable` |
...
参与 current gate。第一次 provider invocation 必须在写入唯一 invocation claim 前，原子重读完整 current
deployment 与 authorization history；cutover、expiry、revocation 或任何 malformed/missing/extra history row
都返回 `unavailable` 且零新增 effect。已提交 claim 的精确 replay 渲染 `ALREADY_CLAIMED` 与原 receipt，不产生
另一个 action 或第二次 provider call。固定展示顺序为 `Current authority`、`Admission snapshot`、
`Invocation claim`。
...
| Audit `/operations/audit`               | Execute/update/create/delete/success/failure 计数；`P=AuditTable`；`Q=AuditCorrelationCard + InvocationClaimReceipt`；`T=OperationTimeline`                                                                                                                                                             | Filter、Open correlation、Copy audit locator                                         | Windmill audit 仍是 append‑only control‑plane evidence，不是 Owner business truth。单独样式的 Product Edge claim receipt 显示 `CLAIMED_NEW / ALREADY_CLAIMED / unavailable`；历史 admission resolve 绝不意味着新 effect                                                                                                                                                          |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim identity、admission identity、attempt identity、committed time、digest 与 `CLAIMED_NEW / ALREADY_CLAIMED / unavailable` disposition。`ALREADY_CLAIMED` 链接原 receipt 并禁用 invocation；绝不渲染为 retry 或第二个 operational job                                                                                                                                                                                                                                               |
