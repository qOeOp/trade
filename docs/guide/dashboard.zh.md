...
| `DependencyCancellationPanel`                                                            | 只针对一个 queued dependency run 的固定 operational confirmation：run/kind/path、queued‑since、required executor compatibility、current `OperationalActionEnvelope` identity/expiry、显式空 domain‑effect set、no‑claim proof 与 cancellation‑receipt slot。唯一 effect button 为 `Cancel queued dependency`；pending 显示 `Cancelling…`；missing/expired/revoked capability、identity/version conflict、claim、terminal 或 unknown 会移除按钮，且不存在 batch、retry 或 domain cancellation                                                                                                                                                                                                                                                                                                                                                                                    |
