...
| Artifacts `/rd/artifacts`        | Available/failed/unknown/review‑required 计数；`P=ArtifactTable`；`Q=ArtifactReviewPanel + ArtifactTrialFamilyBindingPanel + NoArtifactReceiptPanel + ProviderInvocationStateCard`；`T=BuildAndSecurityEvidence`，含 deterministic double‑build 与 sandbox policy | Open Artifact、Resolve same attempt、Copy provider claim、Open operational run、Ask Agent to revise、Start exploratory replay                                                           | 既有 S2 是 `CURRENT/PARTIAL`；binding、no‑Artifact closure 与 invocation state 是 `OBSERVED_CANDIDATE_NOT_CURRENT`。`OUTCOME_UNKNOWN` 隐藏 review/successor action，只暴露 claim copy、operational evidence、same‑attempt resolve 与固定 manual‑reconciliation stop；绝无 provider retry button。`ACTUAL_PROVIDER_CALL_AT_MOST_ONCE` 仍为 `NOT_ADMITTED`                                       |
...
| Access `/settings/access`               | Principal/session/token/revoked 计数，以及 binding `ACTIVE/SUPERSEDED/zero‑active` 与 authorization available/expired/revoked/unavailable 计数；`P=LocalPrincipalCard`；`Q=AuthorizationLineagePanel`，内部固定 `Current authority / Admission snapshot` tabs；`T=CapabilityManifest + CredentialAudit` | Re‑authenticate local session、Issue narrow transport token、Revoke token、Copy once | Transport credential 控件绝不能签发、续期或撤销 Operator Authorization。只有 `Current authority` 驱动 action state；`Admission snapshot` 是 immutable audit readback，在 cutover/revocation 后仍可用。两者都显示准确 binding/head、issuer/audience/scope、expiry/revocation frontier、manifest digest、source cut 与 stop predicate；secret/token value 仍只显示一次且不记录日志 |
...
| `ActionAdmissionGate`, `AuthorizationLineagePanel`                        | 把 current next action 与唯一 `ACTIVE` Shell binding/history head、独立 Operator Authorization、Time Evidence/revocation frontier 和准确 content‑addressed operation manifest 交叉绑定。Gate 单独决定 button enablement。Panel 内固定先 `Current authority`、后 `Admission snapshot` 两个 tab，使用相同 identity/scope/issuer/audience/expiry/frontier/manifest geometry；snapshot 明确标为 historical，绝不参与 gate。两个组件都不能从 credential 或 local policy 构造、签名、刷新或推断 authority |
...
| `ProviderInvocationStateCard`                                             | claim identity/digest、state digest、started time、`STARTED_NEW / OUTCOME_UNKNOWN`、operational‑run link 与 exact next action。`OUTCOME_UNKNOWN` 使用固定红色 geometry 与 `MANUALLY_RECONCILE_PROVIDER_INVOCATION`；按钮为 Copy claim、Open operational run、Resolve same attempt。没有 retry‑provider、mark‑success 或 dismiss control，也不推断 provider outcome                                                                                                                                  |
