...
**Run bounded Agent + sandbox**；点击后只启动既有 claim 一次，绝不创建 successor、replacement claim 或第二次
provider invocation。start 一旦提交，Run control 消失，任何 replay 都渲染 `OUTCOME_UNKNOWN` 与
...
| Versioned script 与 App dependency                   | App 与 MCP 必须调用同一个 `research_goal_v2` Product Edge operation；已观察到的 `research_goal_v1` MCP profile mismatch 是 `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`。Windmill 单独记录 dependency‑build job               | `TARGET_DRAFT/KEEP_SEMANTICS`：一个 versioned operation registry entry、content digest、类型化 BFF gateway 与显式 dependency state；channel version mismatch 时 operation 渲染 unavailable |
...
| `InvocationClaimReceipt`                                                  | Product Edge claim、admission 与 attempt identity、committed time、digest、`CLAIMED_NEW / ALREADY_CLAIMED / unavailable`、current `CLAIMED / INVOCATION_STARTED`，以及精确 Owner‑projected next action。`ALREADY_CLAIMED` 链接原 receipt；只有恢复的 `CLAIMED + RUN_BOUNDED_EXECUTION_AGENT` 才可让同一 claim 进入 atomic start gate，其他状态都隐藏 Run control                                                                                                                                                                                                                                                                                                                                                |
...
| `ProviderInvocationStateCard`                                             | 两种固定 geometry。可恢复 `CLAIMED` 显示同一 request/attempt/claim、无 started time、next action `RUN_BOUNDED_EXECUTION_AGENT`，按钮依次为 Run bounded Agent + sandbox、Resolve same attempt、Copy claim、Open operational run。`INVOCATION_STARTED / OUTCOME_UNKNOWN` 使用固定红色 geometry 与 `MANUALLY_RECONCILE_PROVIDER_INVOCATION`，按钮依次为 Resolve same attempt、Copy claim、Open operational run；不显示 Run 与 successor。绝不创建新 claim、重试 provider、标记 success、dismiss stop 或推断 outcome                                                                                                                                                                                                |
