...
| Product Edge action authorization            | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`            | Authority reviewer 发现 candidate 从 environment/default policy string 构造 `trusted_context_v2` 与 Artifact context，再与同一个 policy object 比较，形成自证路径。该路径绝不能启用 Dashboard action。Product Edge 从独立 authority store 解析 canonical 唯一 `ACTIVE` deployment‑history head、不可自我声明的 Operator Authorization、当前 Time Evidence 与 revocation frontier，以及准确 content‑addressed Agent Operation Manifest 前，所有修改型控件都保持 `permission_unavailable`                                                                                                                                   |
...
| Access `/settings/access`               | Principal/session/token/revoked 计数，以及 binding `ACTIVE/SUPERSEDED/zero‑active` 与 authorization available/expired/revoked/unavailable 计数；`P=LocalPrincipalCard`；`Q=AuthorizationLineagePanel`；`T=CapabilityManifest + CredentialAudit` | Re‑authenticate local session、Issue narrow transport token、Revoke token、Copy once | Transport credential 控件绝不能签发、续期或撤销 Operator Authorization。Panel 只读显示准确 binding/head、issuer/audience/scope、expiry/revocation frontier、manifest digest、source cut 与 stop predicate；secret/token value 仍只显示一次且不记录日志 |
