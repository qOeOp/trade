...
| Run Detail `/operations/runs/:runId`    | Semantic/operational/timing summary；`P=RunMetadataAndInputs + RunWorkerCompatibilityMatrix` 并绑定 `:runId`；`Q=OwnerViewCard`；`T=RunResultView`，后接固定嵌套 `Logs/Metrics/Traces/Assets` tabs                                                                                                                                                                                                           | Copy locator、Refresh、条件成立时 Cancel queued dependency、Resolve same identity、Download bounded result/log | 使用上方精确固定 skeleton；Cancel 只在 queued、unclaimed、zero‑domain‑effect dependency run 中占据第三个按钮位，其余状态该 slot 缺席。Worker readiness 只针对这个精确 run 推导。无 batch cancel 或通用 rerun/edit/share                                                                                                                                                                                                                                                                                                                                                    |
| Workers `/operations/workers`           | Group/online/busy/unavailable 计数；`T=WorkerTable`；row selection 后 `D=WorkerLeaseCard + WorkerCapabilityMatrix + WorkerHeartbeatHistory`；省略 `P/Q`                                                                                                                                                                                                                                                      | Refresh、Open last run                                                                                         | 真实 `rd-product-edge` worker；只展示 registered capability 与 lease liveness，不对未绑定 run 声明 readiness。无 create/edit/restart/cache‑clean/REPL control                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `WorkerGroupTabs`, `WorkerLeaseCard`, `WorkerCapabilityMatrix`, `WorkerHeartbeatHistory` | group、lease liveness、last observed/run、memory/version，后接该 worker 已注册的 kind/tag/runtime/isolation capability。这个只绑定 worker 的 surface 不声明 run readiness，也不暴露管理 action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
