...
### 2026-08-23 活跃 F1 review 回读

Hub turn `01a02e99-5bdb-7a83-a88b-576d59cfff4a` 在 observation cursor
`81158c84-76fe-42f6-b9ca-52a0e263d3f8:1664` 报告两项正由 real consumer 复现的 fresh HIGH。Scanner
存在绕过 Owner 构造 public terminal receipt 的路径；修复并验收前，terminal count、badge、receipt、Matcher
invocation 或 Proposal 只能在 exact Scanner Owner resolve 后出现。Governance 会在 Eligibility input invalid
时仍持久化 rejected receipt；TARGET 要求该 pre-admission branch 对 Governance 零写入，并且没有 lifecycle
row、decision card、Runtime handoff 或 successor action。两个 surface 都是
`ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`，不是 current product capability。该次观察中的
Market Data candidate 后来已被下文 exact merged readback 取代；短前缀 `0a002cce…` 仍只是
non-authoritative 历史观察，不重用为 candidate identity。
...
仍 unavailable。Scanner 与 Governance 现在还带有上文绑定的活跃 HIGH，其修复仍是 candidate-only，尚未进入
共享 product workspace。PR #330 Runtime foundation 是 `CURRENT/PARTIAL`，但唯一 state 是 non-authoritative
...
| Scanner public terminal projection           | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`             | F1 review 发现 public terminal receipt 存在 non‑Owner construction path。Exact Scanner Owner resolve 关闭 real‑consumer path 前，Dashboard terminal row、count、badge、receipt、Matcher invocation 与 Proposal evidence 全部 unavailable；复制或本地重建的 receipt shape 没有 row geometry                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Governance invalid Eligibility admission     | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`             | F1 review 发现 invalid Eligibility 仍可持久化 rejected receipt。TARGET 在 Governance admission 前拒绝它，Governance receipt/lifecycle/outbox 零写入，没有 Runtime handoff 或 successor action。Receipt‑backed `REJECTED_NO_WRITE` 是另一条 post‑admission Governance decision，不能代表 invalid input                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DRAWABLE_EXACT`                      | Operations Runs `/operations`、Run Detail `/operations/runs/:runId`；Market Data `/data` 与 `/data/pit-catalog`；全部四个 Runtime route                              | 本章固定 route slot、内部 field/column 顺序、尺寸或 responsive transformation、state geometry 与 button 顺序。Fail‑closed route 可以用固定 unavailable/not‑ready value 绘制；该状态不代表其 backend 或 Dashboard consumer available                              |
...
| Runs `/scanner/runs`                                   | Running/unknown/rejected/terminal 计数；`P=ScannerAttemptTable`；`Q=AttemptReceipt + ScannerPublicReceiptIntegrityPanel`；`T=MatcherInvocationEvidence`                                                    | Open run、Resolve same attempt                                                                                                            | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`：任何 public terminal row/count/badge/receipt 或 Matcher/Proposal evidence 前必须 exact Scanner Owner resolve。Caller‑constructed 或本地重建的 terminal shape 渲染 unavailable，保留 evidence locator，且不计入 terminal count                                                                                                                                                                                                       |
...
| Lifecycle `/strategy/lifecycle`      | Pending/accepted/rejected‑no‑write/unknown 计数；`P=LifecycleRequestTable`；`Q=GovernanceEligibilityAdmissionPanel + GovernanceDecisionCard`；`T=ContenderFrontier` | Eligibility valid 时：Submit lifecycle request、Resolve same request、Create successor；否则 Open Eligibility evidence、Copy locator | `ACTIVE / OBSERVED_VISIBLE_DEFECT_NOT_CURRENT`：invalid/unavailable Eligibility 是 pre‑admission 零写入状态，没有 lifecycle row、rejected receipt、Runtime handoff 或 successor action。Receipt‑backed `REJECTED_NO_WRITE` 是另一条 post‑admission Governance decision，且必须关联 Runtime `NO_APPLICATION_RECEIPT`；accepted/positive 仍 `NOT_ADMITTED` |
