...
- Proposal → Developer → formal Contract 链已闭合为 latest Proposal Admission → immutable Development Brief → Developer Contract Draft → `received_unvalidated` Receipt → immutable `valid|invalid` Validation Record → atomic Freeze Record。Freeze compiler v1 从 predeclared scalar-enumeration assignments 派生 Candidate identities/ordinals 和 Trial Group hash，并在一个事务注册 Contract、Group、Candidates 与 bootstrap lifecycle；Trial Group 仍为 `registered`，没有 Trial/Reservation/Replay authority。当前 `rd_experiment_contract` 的 legacy Proposal 外键由同事务 compatibility projection 承接，该投影不是新 authority，后续 schema migration 应移除。下一边界是从 frozen Contract 显式启动 Group、按预算预留 Trial 并签发 Trial Reservation；不得让 Developer 自造执行 identity。
