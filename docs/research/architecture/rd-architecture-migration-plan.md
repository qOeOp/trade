...
- Proposal → Developer → formal Contract → Replay Request 链现为 latest Proposal Admission → immutable Brief → Draft/Receipt → `valid|invalid` Validation → atomic Freeze → atomic Experiment Trial Plan → Replay Trial Reservation Admission v1 → Replay Request Registration v1。Request Registration 不接收 Request payload，只从 Admission 持久化的 Spec/Manifest/Snapshot 组装并登记 exact Request，过期/取消/状态漂移 fail closed，且不创建 Attempt。下一边界是让 Attempt admission 强制引用 registered Request identity/hash并停用 caller-supplied 任意 Request hash。legacy Proposal compatibility projection、raw Snapshot helper 与 ungated `claimReplayAttempt` 仍是待收口债务。
