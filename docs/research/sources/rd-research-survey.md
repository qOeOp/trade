...
策略宇宙、research pipeline、agent 入口纪律与 family backlog 语义统一维护在 `docs/research/strategy/rd-strategy-universe-design.md`。本文件回答“如何证明”；主设计文档回答“应该研究哪些 edge、机制、数据面和执行语义”。两者必须同时成立：分类不完整会让 RD 错过大类机会，验证不严格会让 RD 把噪音落成策略。
