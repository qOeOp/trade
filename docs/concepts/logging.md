...
- **Python and Vibe components**: Log directly through the Vibe Logger.
- **External `log` crate users**: Filtered by `stdout_level`/`fileout_level` in `LoggerConfig`.
- **External `tracing` crate users**: When enabled, output goes directly to stdout (separate from Vibe logging), filtered by the `RUST_LOG` environment variable.
- **Logging thread**: All Vibe log events are sent through an MPSC channel to a dedicated thread, ensuring the main thread isn't blocked by I/O operations.
...
The `VIBE_LOG` environment variable provides an alternative way to configure logging using a semicolon-separated spec string. This is useful for Rust-only binaries or when you want to override logging settings without modifying code.
...
:::note
For Rust-only binaries, setting `VIBE_LOG` enables lazy initialization of the logging subsystem on first use, without requiring explicit `init_logging()` calls.
:::
...
When using the `VIBE_LOG` environment variable, you can filter by Rust module paths in addition to component names. Keys containing `::` are treated as module path filters with prefix matching, while keys without `::` are component filters with exact matching.
...
The longest matching prefix takes precedence. In the example above, `vibe_okx::websocket::handler` would use the `Debug` level (longer prefix), while `vibe_okx::data` would use `Warn`.
...
:::note
Module path filtering is only available via the `VIBE_LOG` environment variable. The Python
...
If you ***aren't*** using an object which already initializes a `VibeKernel` (and logging) such as `BacktestEngine` or `LiveNode`,
...
directly to stdout. This is separate from the Vibe logging infrastructure - tracing output
uses a Vibe-aligned format with nanosecond timestamps.
...
**Differences from Vibe logging:**

- Tracing output goes directly to stdout, not through the Vibe logging thread.
- Tracing events are not written to Vibe log files.
...
the Vibe logger and are filtered by `stdout_level`/`fileout_level` in `LoggerConfig`.

:::tip
`RUST_LOG` only affects crates using `tracing`. For crates using `log`, configure verbosity through
`LoggerConfig` or the `VIBE_LOG` environment variable (e.g., `VIBE_LOG=stdout=Debug`).
:::
...
This issue is tracked in GitHub issue #3027 (source issue #3027).
