...
`OrderModifyRejected` 表示 `ModifyOrder` 命令被交易场所拒绝。`ExecutionEngine`
将该事件应用于订单、更新 `Cache`，并在 `MessageBus` 上发布。当交易场所拒绝修改请求时，
会触发此事件。
...
除[订单事件公共字段](index.md#common-order-event-fields)外，`OrderModifyRejected` 还携带：

| 字段     | Python 类型 | 必填/默认值 | 说明                 |
| -------- | ----------- | ----------- | -------------------- |
| `reason` | `str`       | 必填        | 订单更新被拒的原因。 |
