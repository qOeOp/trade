...
Vibe locally mimics the behavior of these order types (such as `STOP_LIMIT` or `TRAILING_STOP` orders)
while using only `MARKET` and `LIMIT` orders for actual execution on the venue.

When you create an emulated order, Vibe continuously tracks a specific type of market price (specified by the
...
For example, if you create an emulated `STOP_LIMIT` order, Vibe will monitor the market price until your `stop`
price is reached, and then automatically submits a `LIMIT` order to the venue.

To perform emulation, Vibe needs to know which **type of market price** it should monitor.
By default, it uses bid and ask prices (quotes), which is why you'll often see `emulation_trigger=TriggerType.DEFAULT`
in examples (this is equivalent to using `TriggerType.BID_ASK`). However, Vibe supports various other price types,
