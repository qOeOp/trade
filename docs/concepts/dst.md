...
Under the `simulation` Cargo feature on `vibe-common`, four `tokio` submodules are routed
...
These re-exports live in `vibe_common::live::dst`. DST-path call sites for `time`, `task`,
...
- **Wall-clock reads** go through `vibe_core::time::duration_since_unix_epoch`. Under
...
- **Monotonic reads** go through `vibe_common::live::dst::time::Instant`. The type resolves to
...
- **Network-local monotonic reads** go through `vibe_network::dst::time`. The crate sits
  below `vibe-common` in the dependency graph and exposes a local re-export module with the
  same semantics.
...
- Monotonic time reads route through the DST seam (either `vibe_common::live::dst::time` or
  `vibe_network::dst::time`), not `std::time::Instant::now` directly.
- Wall-clock time reads route through `vibe_core::time::duration_since_unix_epoch`.
...
- **Rule 6**: direct `tokio::net::TcpStream::connect` / `tokio::net::TcpListener::bind` reaches
  that bypass `vibe_network::net`. The seam re-exports `tokio::net` types under normal builds
...
- **Rule 7**: raw `tokio::{time,task,runtime,signal}` paths in production code on the madsim build
  path. Callers route these modules through `vibe_common::live::dst`; the facade definition,
...
The hook applies to 17 workspace crates: the 16 crates in the transitive closure of `vibe-live`
...
the `vibe-network` transport tests can explore link and reconnect orderings that the madsim
...
- An ignored reconnect soak sweeps Turmoil seeds until stopped, or until
  `VIBE_TURMOIL_SOAK_COUNT` seeds have run. Each seed runs the Tungstenite WebSocket backend
...
`VIBE_TURMOIL_SOAK_START` sets the first seed, which resumes a sweep where an earlier run
...
Remaining `AHashMap` / `AHashSet` sites in the original `vibe-live` closure are lookup‑only,
...
`vibe_core::time::nanos_since_unix_epoch()` and compares in `u64` nanos directly.
...
- `crates/execution/src/matching_engine/ids_generator.rs` uses
  `vibe_core::UUID4::new()` in the position and venue order id generators for the
...
`vibe_common::live::dst::signal` exposes routed `ctrl_c` and `terminate` re-exports. The
...
- Layer 1 (runtime swap) is implemented. `vibe_common::live::dst` exposes routed re-exports
...
The local `make cargo-test-sim` gate builds
`vibe-common`, `vibe-core`, `vibe-network`, `vibe-execution`, and `vibe-live`
...
so the explicit cfg branches and virtual time are both validated. `vibe-common` and
`vibe-execution` consume `vibe-model` types, so each also runs a second leg with
...
- All simulation-compatible `vibe-common` tests. This leg compiles with
  `vibe-core/simulation` propagated, so the explicit `wall_clock_now` cfg branch is selected
...
- The `vibe-live` startup reconciliation timeout regression. This runs under a madsim
...
- All of `vibe-network`, with transport-bound test modules gated out at the source. Includes
...
- All of `vibe-execution`. These are plain `#[rstest]` cases, so they compile and exercise the
...
- The cross-crate seam pinning tests in `vibe-core` (`wall_clock_now` virtual time).

Deterministic-scheduler coverage comes from the `#[madsim::test]` cases in `vibe-common`,
`vibe-core`, `vibe-network`, and `vibe-live`. The gate as a whole catches drift in the
