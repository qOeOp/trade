...
exchange. The `{symbol}.{venue}` pair must be unique inside a Vibe system.
...
Rust users work with the `vibe_model` instrument structs and `InstrumentAny`:
...
calculates initial and maintenance margin. Vibe uses one fee-rate convention across
