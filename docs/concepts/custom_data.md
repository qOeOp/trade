...
Vibe Trader supports custom data authored in Python and Rust, and moves
...
Instead of hardcoding every type into the main binary, Vibe resolves
...
For Rust types defined inside Vibe:
...
For Rust types compiled into Vibe, the inner payload is the concrete Rust
...
Built-in Vibe data types have schemas and encoders known statically to the
...
For those types, Vibe uses the Arrow C FFI interface to pass `RecordBatch`
...
Custom data is not only a persistence feature. It also participates in Vibe
runtime routing.
...
This architecture gives Vibe two important properties:
