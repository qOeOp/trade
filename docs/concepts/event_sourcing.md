...
- External I/O becomes replayable only when Vibe captures it as commands, raw reports, or
...
[`vibe-event-store` README](https://github.com/qOeOp/trade/blob/main/crates/event_store/README.md)
plus the [`vibe-event-store` crate](../../crates/event_store/) as the API reference.
:::

## Why event sourcing

The cache answers "what is true now". The event store answers "how did Vibe get here". It gives
...
The event store provides Vibe with a durable basis to:
...
with `vibe_event_store::ParquetReplayCatalog` to plan selected catalog files and
...
The sidecar supports one audit claim: when marker capture is enabled, Vibe observed data
...
- Prove venue send order before Vibe observed the message.
...
for the same key and timestamp, the sidecar can prove that Vibe observed two deliveries in a
...
Adapter network I/O remains outside bit-identical replay unless Vibe captures the relevant
