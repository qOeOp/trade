...
Vibe has a complete Rust implementation under the `crates/` directory.
...
The Vibe crates are internal workspace packages and are not published. Workspace members should
inherit them from the root `Cargo.toml`; an external experiment can pin the repository explicitly:
...
To track the latest development branch, point all Vibe dependencies at the
...
| `streaming`      | `vibe-backtest` | Catalog‑based data streaming via `BacktestNode`.              |
| `defi`           | `vibe-model`    | DeFi data types. Implies `high-precision`.                    |
...
The `vibe` CLI and Python wheels on Linux and Windows use
...
not manage orders. Implement the `DataActor` trait and use `vibe_actor!` to
...
[`BookImbalanceActor`](https://github.com/qOeOp/trade/tree/main/crates/trading/src/examples/actors/imbalance).

## Strategies

A strategy extends an actor with order management. Implement `DataActor` for
data handling and use `vibe_strategy!` to wire your `StrategyCore` field
...
[`EmaCross`](https://github.com/qOeOp/trade/tree/main/crates/trading/src/examples/strategies/ema_cross)
and
[`GridMarketMaker`](https://github.com/qOeOp/trade/tree/main/crates/trading/src/examples/strategies/grid_mm).
...
`vibe-backtest`. See the full working example:
