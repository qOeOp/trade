...
For modify or cancel preparation, Vibe emits the matching rejection only when the failure is
...
The `RiskEngine` is a component of every Vibe system, including backtest, sandbox, and live
...
Some venues can later reduce or invalidate a fill. Vibe records this as an
...
Vibe adapters emit `OrderFillVoided` where the venue publishes the void on a stream the adapter
