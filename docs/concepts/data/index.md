...
Initialize a catalog from a path‑like object, a URI, or the `VIBE_PATH` environment variable.

:::note[VIBE_PATH environment variable]
Set `VIBE_PATH` to the parent of the catalog directory. For example,
`VIBE_PATH=/home/user/trading_data` makes `ParquetDataCatalog.from_env()` use
...
The `vibe_persistence` crate provides two utilities:
