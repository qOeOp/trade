...
- `--strategy-calibration-suite`：固定跑 buy-and-hold / cash baseline、趋势基准、横截面强弱基准，可消费 dataset `indicator_report_path` 中的 exact funding events，并输出 data_panel、beta、fee/slippage 成本拆分、funding、换手、暴露、时间稳定性、负对照与数据广度归因；只暴露系统问题，不产生准入证据
