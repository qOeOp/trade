...
| 容器 | 内含 | 说明 |
| --- | --- | --- |
| `PLAN-POOL` | PLAN-CHAIN[] | 每条 chain 是一个交易机会的决策历史 |
| `STRATEGY-POOL` | STRATEGY-CHAIN[] | 策略资产，离线演化结果 |
...
**跨链 exposure 视图**：PLAN-POOL 层需聚合所有 `status` 为活跃态（`wait-condition / ready-execute / wait-until-fill / in-position`）的链，计算整体方向性暴露。对冲锁仓场景中，对冲链通过 `parent_plan_keys` 引用被对冲链，PLAN-POOL 视图可据此做净值计算，识别真正的净方向敞口。单个 plan 不感知其他链，跨链聚合由调用层（OBSERVE checklist 或 REVIEW）负责。
...
| **WHY** | thesis / edge_type / setup_ref / confluence / conviction | 中 | 人 + agent |
...
| `timing_validity` | 是 | string | ISO duration，如 `PT15M`（高波动）/ `PT4H`（主流币）；过期视为 hard 失效 |
| `expected_rr` | 条件必填 | object or null | `{ ratio: number, net_ratio?: number, fee_pct?: number, slippage_pct?: number }`；`net_ratio` 推荐填；entry 存在时必填 |
...
`management_rules` 无序，多条可并存。`status` 和 `triggered_at` 是监控性字段，OBSERVE 直接更新，不写 plan_history。每次 OBSERVE 检查 rule 时写一条 `rule_evaluation` 记录，保留完整评估历史。`targets[]` 描述静态出场目标，`management_rules` 描述动态调整逻辑，两者职责不交叉。
...
`validity_window` 已固定，子结构：`{ expires_at?: string, max_candles?: number, timeframe?: string, note?: string }`。`expires_at` 为绝对过期时间（RFC 3339）；`max_candles` 配合 `timeframe` 表示"N 根 K 线后失效"，由 OBSERVE 在每次 check 时比对当前 K 线数量；两者填一即可；均不填视为无时效限制。过期后 OBSERVE 触发 hard 失效，生成 `status=abandon` 的新 plan。

当前 `position_scope` 子结构尚未固定，先保持开放 object，不提前展开。
...
| `outcome` | 是 | `win / loss / breakeven / abandoned`；与 `plan.state.outcome` 对应 |
...
| `signal_accuracy` | 否 | object[]；每条 signal 的事后准确性评估：`{ indicator, timeframe, was_accurate: boolean, note? }` |
...
`signal_accuracy` 让 REVIEW 阶段积累每个指标在特定 regime 下的实际命中率，回测侧可按 `indicator + timeframe + regime` 聚合。
...
| `regime_filter` | 是 | string；对应 `market_context.regime` |
...
经 BACKTEST `verdict=confirmed` 的假设，或经多次 REVIEW 积累的高置信判断，进入 STRATEGY-POOL。PLAN 阶段的 `basis.refs.strategies` 可引用 STRATEGY-POOL 中的条目，作为决策依据之一。

STRATEGY-POOL 中每条 strategy 结构：

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `strategy_key` | 是 | uuid |
| `name` | 是 | 人类可读名称，如"BTC 4h 趋势回踩做多" |
| `regime` | 是 | 适用的 market regime（对应 `market_context.regime` 枚举） |
| `symbols` | 否 | string[]；null 表示通用；填写时表示只适用特定标的 |
| `side` | 是 | `long / short / both` |
| `setup_description` | 是 | string；策略核心入场条件描述 |
| `key_signals` | 是 | string[]；必要信号列表（对应 signals 中的 indicator + signal） |
| `invalidation_hints` | 是 | string[]；常见失效场景 |
| `backtest_summary` | 否 | object；引用最近一次 BACKTEST 的核心指标：`{ win_rate, avg_rr, sample_count, backtest_key }` |
| `review_count` | 是 | 累计 REVIEW 引用次数 |
| `last_updated` | 是 | RFC 3339 |
| `status` | 是 | `active / deprecated / experimental` |

`experimental`：样本不足或市场结构尚未确认，可以参考但不应作为主要依据；`deprecated`：历史有效但当前 regime 下已失效，保留供研究。
