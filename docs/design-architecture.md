...
- 因此主链路后续需要补一个 `全市场候选生成 -> 粗筛 -> shortlist` 的能力；否则 agent 很容易直接进入自由推理，而不是先做结构化收敛

当前判断：

- crypto 需要 `扫描能力`，但不需要直接照搬股票社区 skill 的 `pre-filter + trend template` 实现
- 可以借鉴的是“先缩小 universe，再把少量候选送进单标的细筛”的思想
...
- 先补 `交易可做性硬过滤`
  ...
  - 重大事件窗口
- 再补 `扫描内轻量排序`
  - 只用 scan 阶段现成可得的公共快照特征
  - 不在 scan 内重复执行 `single-symbol` 的结构判断
- 最后只把少量 shortlist 候选交给主 agent 收敛，再派发到与“用户直接给具体标的”相同的 `single-symbol` 细筛路线，而不是让 LLM 直接面对全市场原始噪音
