...
| **WHAT** | scope / side / entry / entry_tranches / stop / targets / invalidation / atr_ref / evidence_trust / timing_validity / max_holding_duration / expected_rr / risk_budget / liquidation_ref / key_risks / management_rules（含 trail_spec） | 中 | 人 + agent |
...
plan 在生命周期内跨 `draft → wait-condition → ready-execute → wait-until-fill → partial-fill → in-position → closed` 多个阶段，不同阶段可用信息严重不对称：草拟阶段连单都没挂，强求精确 `cluster_net_exposure` 是浪费；扣扳机前的 checklist 时刻才是永续保护字段真正该生效的地方。统一把所有字段一股脑要求，会把 agent 逼成填表游戏、瞎凑数字。分层激活让思考顺着交易员真实节奏走。

| status | 该阶段首次必填（叠加前阶段不变项） | 可留空 / 延后 |
| --- | --- | --- |
| `wait-condition` | WHY 全量（thesis / edge_type / regime / conviction / counter_thesis / probability_basis；win_probability 按 basis；合约**一般**必填 `derivatives_context`，但 `leg_role=hedge` 且本腿不是靠微结构做判断时允许 `null + R-018 ack`）、WHAT 轮廓（scope / side / entry / stop / targets / invalidation / timing_validity / evidence_trust / risk_budget / key_risks）、POSITION_ROLE、EXPOSURE 轻量（leg_role / existing_legs / correlation_cluster，必要时 `cluster_override_reason`）、NOW | EXECUTION_LANE 细节、`atr_ref`（仅 anchor=atr/composite 时才需）、`max_holding_duration`、`liquidation_ref`、`management_rules`、`expected_rr` 完整（先填 ratio，net_ratio 延后到 ready-execute）、精确 `net_after_fill` / `cluster_net_exposure` |
| `ready-execute` | **追加**：完整 EXECUTION_LANE（含 `precheck_ref`；永续加 `margin_mode` / `position_mode` / `protection_flags`）、`liquidation_ref`、`max_holding_duration` 软估、`expected_rr` 完整（含 net_ratio / expected_value / funding 如需）、精确 `net_after_fill` 与 `cluster_net_exposure` | `open_orders`、`execution_fills`、`entry_summary`、`management_rules`（可延到 in-position）、`outcome` |
...
| 字段 | 必填 | 类型 | 说明 |
| --- | --- | --- | --- |
| `order_type` | 是 | enum | `limit / market / stop_market / stop_limit / algo / oto / otoco`。这是**计划级默认 / 主执行家族**：单点入场时它就是实际方法；`entry_tranches[].order_type` 非 null 时，各 tranche 可覆盖自己的执行方法，`EXECUTION_LANE.order_type` 退回为默认值（通常等于首档或最常用的方法），避免多档 mixed-method 计划被迫假装所有档都同一种下单方式 |
...
| **R-010** | 活跃 plan 必须有最长持仓时限 | warn-ack | ≥ `partial-fill` | `scope.mode ∈ {"partial-fill","in-position"}` | `max_holding_duration` 非 null。ack 场景：`time_horizon === "position"` 的长期持仓，必须同时写 `next_check` 节奏（如"每周 WR 回顾"）代替硬时限 |
...
| **R-018** | 合约 plan 必须填衍生品微结构快照 | warn-ack | ≥ `wait-condition` | `market_type ∈ {usdm, coinm}` | 默认要求 `derivatives_context` 非 null，且 `funding_rate / oi_change_24h_pct / long_short_ratio / taker_buy_sell_ratio` 四项均非 null。ack 场景：(a) 标的流动性过低导致 long_short_ratio / taker 数据不可靠，需说明数据源问题；(b) `leg_role === "hedge"` 纯对冲腿，不基于微结构做决策，此时允许 `derivatives_context = null`，但必须显式 ack，禁止伪造占位数字 |
