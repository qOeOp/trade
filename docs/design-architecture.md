...
| `account_guardrails` | 是 | object | `{ max_day_loss_pct?: number, max_open_risk_pct_after_fill?: number, max_correlated_exposure_usdt?: number }`；账户级闸门，不回答“这笔亏多少”，而回答“这笔做完后今天还能不能继续打” |
...
| `portfolio_context` | 是 | object | `{ daily_realized_pnl_pct?: number, open_risk_pct?: number, correlated_exposure_usdt?: number, free_cash_usdt?: number }`；给 PLAN 一个账户视角，避免单笔 RR 正常但组合已过载 |
...
| **R-008** | 永续 expected_rr 必须计入 funding | `market_type ∈ {usdm, coinm}` | `expected_rr.funding_pct_per_day` 非 null，且 `net_ratio` 已扣除 funding |
| **R-009** | 永续执行语义必须显式 | `market_type ∈ {usdm, coinm}` | `margin_mode`、`position_mode` 非 null；保护单存在时 `protection_flags` 非 null |
| **R-010** | 活跃 plan 必须有最长持仓时限 | `scope.mode ∈ {pending-entry, in-position}` | `max_holding_duration` 非 null |
| **R-011** | 永续方向仓必须检查 liquidation buffer | `market_type ∈ {usdm, coinm}` 且 `side ∈ {long, short}` | `liquidation_ref.buffer_pct` 非 null，且 stop 与 liq_price 之间仍有正缓冲 |
