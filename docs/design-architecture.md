...
- 事件流为真相、自然语言为主
- 所有规则写在 rules.md（每条有 R- ID）；爆仓相关代码兜底，其余 LLM 判
- decision_card 渲染 = 校验
...
plan_event
  event_key   PK
  chain_id           -- 事件归属（当前语义更接近 strategy flow id；沿用旧字段名）
...
本阶段每个启用策略默认对应一条长期持续 append 的 plan 流。表结构里沿用 `chain_id` 字段名，但语义上更接近 `strategy_flow_id`。同一条流里，plan 会随 cron 持续更新，可经历多轮 observe、下单、持仓管理和多次阶段性 review。数据库里同时存在多条策略流，不假设系统只有一条最新主流。

- **创建 flow**：某策略首次进入系统时，trade-flow 生成 UUID，写进 first observe 的 `plan_event.chain_id`
- **延续 flow**：同一策略后续 cron 都沿用同一 `chain_id` append 新事件
- **分叉 flow**：只有当策略身份实质变化，或用户明确要把旧策略和新策略彻底分开管理时，才新开 flow
...
side: long | short
stop_price: number
risk_budget_usdt: number          # 全档成交假设下最大允许亏损（驱动 R-RISK-OPEN-CAP / R-RISK-DAY-FLOOR）
strategy_ref: S-xxx
...
# 软字段（自然语言；preflight 走 LLM 读 rules.md 判）
thesis: text
entry_intent: text
exit_intent: text
invalidation: text
...
reconcile_diffs: [...]            # 对账差异（详见 §对账）
preflight_result:
  verdict: armable | blocked | abstain
  violations: [{rule_id, reason}]    # 任一非空 → blocked
  warnings:   [{rule_id, reason}]    # 不阻拦但记录
decision_summary: text            # 本轮 cron 做了什么
```

每条 observe 是**最小完整快照**，不是 patch。若只刷局部槽位，上游先合并上一版完整 observe 再 append。

同一条策略流可以在空仓观察、等待条件、已挂单、持仓管理、平仓后再观察之间反复切换；不要求每出现一个新 setup 就新起一条流。
...
source: trade_flow | reconcile      # trade-flow 主动 vs 对账推断
```

`current_orders` / `current_position` reduce 时只读 `sub_kind / client_order_id / side / position_side / qty / filled_qty / avg_fill_price`；其余字段是审计 / 复盘用。Binance API 字段全集见 [tech-spec.md](tech-spec.md)。
...
- **Markdown**：rules / strategy policy —— 人编辑 + LLM 直读
...
位置：[.agents/skills/plan-preflight/rules.md](../.agents/skills/plan-preflight/rules.md)

每条 rule 有 `R-<DOMAIN>-<POINT>` 命名（对应 strategy 的 `S-`、plan 的 `P-` 模式）。新增规则 = rules.md 加一个 H2 段落；不分级、不打标签、不要 ack。强弱写在 rule 内容里，LLM 读上下文判。

### MVP 初始 rule 集

| Rule ID | 标题 | enforcement |
|---|---|---|
| `R-RISK-OPEN-CAP` | 成交后总 open risk 不超预算 | 代码兜底 |
| `R-RISK-DAY-FLOOR` | 今日累计亏损不穿底 | 代码兜底 |
| `R-OBS-FRESH` | observe 距 now ≤ 30s | 代码兜底 |
| `R-PLAN-INTENT-COMPLETE` | thesis / entry_intent / exit_intent / invalidation 必填非空 | 代码兜底 |
| `R-PLAN-VALID-WINDOW` | valid_until_at 已过期视为废弃 | 代码兜底 |
| `R-PLAN-INVALIDATION-TRIGGERED` | invalidation 已触发的 setup 不得继续执行 | LLM 判 |
| `R-STOP-LADDER-MONOTONIC` | stop_ladder 单调 | 代码兜底 |
| `R-TP-LADDER-RATIO-CAP` | takeprofit_ladder.qty_ratio 之和 ≤ 1.0 | 代码兜底 |
| `R-FUNDING-NET-RR` | 持仓 4H+ 必须把 funding 折进 expected_rr_net | LLM 判 |
| `R-FUNDING-BREAKEVEN` | 持仓 ≥ 24H 在 stop_ladder 加一档把累计 funding 折进 break_even | LLM 判 |
| `R-CORRELATED-EXPOSURE-CAP` | 同簇敞口受 max_correlated_exposure_usdt 约束 | LLM 判 |
| `R-RECON-CHAIN-NOT-STUCK` | 对账差异 ≠ 0 即拒；连续 ≥ 3 轮通知人工 | 代码兜底 |

### 爆仓护栏（R-RISK-OPEN-CAP / R-RISK-DAY-FLOOR）
...
1. 代码兜底的爆仓护栏（`R-RISK-*`）
2. 代码兜底的数据卫生（`R-OBS-FRESH` / `R-PLAN-*` / `R-STOP-*` / `R-TP-*`）
3. LLM 读完整 rules.md + plan + observe，判其余 rule

任何一段产出 violations 非空 → preflight verdict = blocked，本轮拒新动作。

### 复盘聚合

review 阶段按 `violations[].rule_id` group by，自然得到"哪条 rule 拦的胜率最高 / 哪条是 false positive / 哪条该升格为更强的语言"。
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `max_open_risk_pct` | 是 | R-RISK-OPEN-CAP 公式分母 |
| `max_day_loss_pct` | 是 | R-RISK-DAY-FLOOR 公式分母 |
| `max_correlated_exposure_usdt` | 否 | 同簇净方向敞口上限；R-CORRELATED-EXPOSURE-CAP 引用 |
| `max_correlated_gross_exposure_usdt` | 否 | 同簇总敞口上限；rules.md 引用 |
...
Strategy 是 `observe.body.strategy_ref` 指向的对象。MVP 默认一条 `strategy_ref` 对应一条长期 flow：同一策略在不同 cron 里更新 plan，不新开 flow；只有策略身份实质变化时，才分叉为新 flow。MVP 2 条种子（`S-GENERIC-TREND` / `S-GENERIC-MEANREVERT`），完整 policy 落 [.agents/skills/trade-flow/strategies/](../.agents/skills/trade-flow/strategies/)。schema 见 [tech-spec.md](tech-spec.md)。
...
| 视图 | 实现 |
| --- | --- |
| `strategy_flows` | `SELECT chain_id, MIN(created_at) AS bootstrapped_at, MAX(created_at) AS last_event_at FROM plan_event GROUP BY chain_id` |
| `active_flows` | 当前启用策略对应的 flow；由 strategy 配置 / bootstrap 结果决定，不再通过 `review` 反推 closed |
| `flow_meta(flow_id)` | latest `observe.body` 的 `symbol / strategy_ref`；`bootstrapped_at` 来自 `strategy_flows` |
...
下次 cron 跑直接读各条 flow 的最新 observe，没有"标记 stale"机制。`review` 只记录阶段性样本，不关闭整条 flow。
...
sequenceDiagram
    ...
    TF->>TF: 确认当前启用策略；缺 flow 的先 bootstrap
    TF->>DB: reduce 当前 active_flows
    TF->>TF: 对账（差异 → reconcile_diffs，≠0 拒本轮新动作）
    TF->>BN: 拉市场数据（内嵌进本轮 observe.body.microstructure）

    loop 每条 active flow
        TF->>TF: agent LLM 读 latest_observe + observe + rules.md
        TF->>TF: 决定 target_action + 结构化 request
        TF->>TF: preflight（rules：代码兜底 + LLM 判）
        alt fail
            TF->>DB: append observe（含 blocked + violations）
            TF->>N: 爆仓护栏 / 严重违规通知
        else pass
...
- `valid_until_at < now` → Plan 行标红，触发 `R-PLAN-VALID-WINDOW` 拒
- snapshot age > 20s 黄、> 30s 红（红色触发 `R-OBS-FRESH` 拒）
- Checks 行 violations 非空 → 卡片拒绝渲染为"可执行"，本轮跳过 EXECUTE
...
3. 对比差异 → 写本轮 `observe.body.reconcile_diffs`

差异 ≠ 0 时的处置（拒新动作 + 连续 ≥ 3 轮 escalation 通知）由 `R-RECON-CHAIN-NOT-STUCK` 承载。

---

## REVIEW（阶段性复盘）

输入是同一条策略流中某次阶段性闭合样本的完整 `plan_event` 切片。`review` 用来总结一段已完成的持仓 / plan，不关闭整个 flow。
...
同一条 flow 可以累计多条 `review`。REVIEW 是 MVP 终点。BACKTEST / ITERATE / STRATEGY-POOL 链路推迟到累积 30+ review 样本后再展开。
