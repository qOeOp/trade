...
| research replay / R&D / benchmark / forward holdout | `scripts/lib/strategy-*`, `scripts/lib/rnd-*`, `scripts/lib/forward-holdout.ts`, `scripts/forward-holdout.ts` | `trade-flow-typecheck` + `bun test ./scripts/lib/strategy-*.test.ts ./scripts/lib/rnd-*.test.ts ./scripts/lib/forward-holdout.test.ts ./scripts/lib/research-output-schemas.test.ts` |
