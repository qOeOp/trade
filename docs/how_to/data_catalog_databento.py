...
#
# Set up a Vibe Parquet data catalog with market data from Databento. The
# catalog provides efficient storage and querying for backtests and research.
#
# [View source on GitHub](https://github.com/qOeOp/trade/blob/main/docs/how_to/data_catalog_databento.py).
...
# %% [markdown]
# Use a `DatabentoDataLoader` to decode and load the data into Vibe objects.

# %%
loader = DatabentoDataLoader()

# %% [markdown]
# Passing an `instrument_id` is optional but speeds up loading by skipping symbology mapping. If provided, use the Vibe `symbol.venue` format (e.g., "ES.GLBX").
...
# %% [markdown]
# Now we'll expand on this workflow by preparing a month of AAPL trades on the Nasdaq exchange using the Databento `trade` schema, which will translate to Vibe `TradeTick` objects.
