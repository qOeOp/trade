...
- 当前：`research.rd-program-state action=plan_next` 可只读消费 `next_hypothesis_queue`，生成下一轮 `--strategy-rnd-loop` / `--strategy-rnd-campaign` payload 草案；`--rd-supervisor-run` 已把 plan、执行、写回、再规划串成自主 loop。
