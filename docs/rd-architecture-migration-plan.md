...
| Replay 权威准入 | 已完成第二十个受限子集 | Trial Reservation、Attempt Lease、Resume Authorization、Engine Checkpoint 与 Diagnostic Commit 形成运行闭包；Control Plane 单写 claim/renew/expire/finalize/authorize。只有带 committed diagnostic 的 `cancelled/expired` source Attempt 可授权同 authority 的 later active target Attempt；Runner 拒绝裸 commit locator，并校验 authorization self-hash、source producer、target worker 与 lease generation floor。authority mutation/target mismatch、续租代际、路径隔离、tamper rejection、clean/resume parity 已锁定。硬崩溃 checkpoint 自动发现及对象存储 CAS 仍待实现 |
