...
| Replay 权威准入 | 已完成第二十六个受限子集 | Trial Reservation v4 以 `[issued_at, expires_at)` 约束新 Attempt claim，并绑定两份规则 schedule 与 supplemental-facts hash；合法 claim 后只由 Attempt Lease/generation fencing。Checkpoint Receipt v2、Resume Authorization、Engine Checkpoint v3 与 Diagnostic Commit v2 形成恢复闭包。Runner 只通过 Attempt-scoped Artifact Store port 访问证据，local-v1 adapter 维持 `fsync + link-CAS` parity；Artifact v20 将完整 supplemental revision stream 纳入 required member。未认证 remote backend 在 engine 前 typed reject；listing 不是 authority。远端 adapter、故障注入与 provider 认证仍待实现 |
| Replay 数据准入 | 已完成第十九个受限子集 | Dataset Manifest v6 以有序、连续、半开 venue-risk/instrument-spec schedules 冻结规则 provenance，并声明不可变 supplemental revision stream。Request v12、Reservation v4 与 Fingerprint 绑定 canonical hashes；Adapter 按 `signal_time` 选择 `availability_at` 已到的最后 revision，未来修订仅保留 lineage，Result v18 固化 selected ids/hash。payload/sequence/revision/received-at/hash 漂移在首个事件前失败。自动历史采集、会改变 accounting 的 spec epoch 与动态多决策 join 仍待实现 |
| Replay accounting | 已完成第十九个受限子集 | Result v18 / Journal v4 / Margin v7 保持 isolated collateral 与 exact-risk full liquidation；margin observation、funding 后检查与 liquidation fee 按事件时间解析风险 epoch并记录 snapshot provenance，Attempt envelope 不进入经济 fingerprint。动态 collateral、partial liquidation、deficit/insurance/ADL、borrow、cross/shared portfolio 未实现 |
