...
| Replay 权威准入 | 已完成第二十一个受限子集 | Trial Reservation、Attempt Lease、Checkpoint Receipt、Resume Authorization、Engine Checkpoint 与 Diagnostic Commit 形成运行闭包。Runner 在边界写 immutable versioned payload/commit 并向控制回调交付 descriptor；Control Plane 只在当前 generation 内追加单调 receipt，过期后只授权 source 最新 receipt 到 later active target。stale generation/receipt、idempotent retry、expiry takeover、路径/tamper、clean/resume parity 已锁定。目录扫描不是 authority；对象存储 CAS 与目录 `fsync` 仍待实现 |
