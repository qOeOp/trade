...
| Replay 权威准入 | 已完成第二十九个受限子集 | Trial Reservation v5 以 `[issued_at, expires_at)` 约束新 Attempt claim，并绑定两份规则 schedule、supplemental-facts 与 Requirement Set hash；合法 claim 后只由 Attempt Lease/generation fencing。Checkpoint Receipt v2、Resume Authorization、Engine Checkpoint v6 与 Diagnostic Commit v2 形成恢复闭包；checkpoint 绑定 Decision Harness Bundle、Decision Input Snapshot、Harness Receipt 与 Loader Policy。Runner 只通过 Attempt-scoped Artifact Store port 访问证据，local-v1 adapter 维持 `fsync + link-CAS` parity；Artifact v23 将完整 supplemental revision stream、Bundle、Snapshot 与 nullable Receipt 纳入 required member。未认证 remote backend 在 engine 前 typed reject；listing 不是 authority。远端 adapter、故障注入与 provider 认证仍待实现 |
| Replay 数据准入 | 已完成第二十二个受限子集 | Experiment Contract v3 的 `replay_execution_input` 冻结 Requirement Set schema/hash，Control Plane 签发 Reservation v5 时从权威 contract 复核，Developer 无法降级为空。Dataset Manifest v7 声明连续半开规则 schedules、不可变 supplemental revision stream 与同一 hash；Request v14、Reservation 与 Fingerprint 绑定 canonical hashes。Adapter 按 `signal_time` 选择最后可见 revision 并生成自哈希 Decision Input Snapshot；future revision 不改变过去 snapshot。required lane 的 `harness_hash` 必须等于完整 Source Bundle v1 hash，Runner 只能经 Registry Capability v1 与固定 Loader Policy 解析 reviewed static entrypoint；Receipt v2 必须复现授权 Order，Engine 二次校验，缺失、未知 bundle、source/loader/Order 漂移均在市场执行前拒绝。Result v21/Fingerprint/Artifact v23/checkpoint 全链绑定，幂等重读不解析或执行。source-to-build attestation、进程隔离、动态多决策 join 与通用 feature DAG 重算仍待实现 |
| Replay accounting | 已完成第十九个受限子集 | Result v21 / Journal v4 / Margin v7 保持 isolated collateral 与 exact-risk full liquidation；margin observation、funding 后检查与 liquidation fee 按事件时间解析风险 epoch并记录 snapshot provenance，Attempt envelope 不进入经济 fingerprint；Decision Snapshot/Receipt 作为经济输入 provenance 进入 fingerprint。动态 collateral、partial liquidation、deficit/insurance/ADL、borrow、cross/shared portfolio 未实现 |
