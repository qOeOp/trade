...
| Replay 权威准入 | 已完成第二十四个受限子集 | Trial Reservation v2 以 `[issued_at, expires_at)` 约束新 Attempt claim，合法 claim 后只由 Attempt Lease/generation fencing；Checkpoint Receipt v2、Resume Authorization、Engine Checkpoint 与 Diagnostic Commit v2 形成恢复闭包。Runner 只通过 Attempt-scoped Artifact Store port 访问证据，local-v1 adapter 维持 `fsync + link-CAS` parity；Capability v1 冻结远端 `If-None-Match: *`、完整 SHA-256、strong read-after-write、Manifest-last 与 incomplete upload 清理要求。Control Plane 只登记当前 generation、受支持 storage policy 和单调 offset。未认证 remote backend 在 engine 前 typed reject；listing 不是 authority。远端 adapter、故障注入与 provider 认证仍待实现 |
