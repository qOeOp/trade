...
| Replay 权威准入 | 已完成第二十五个受限子集 | Trial Reservation v3 以 `[issued_at, expires_at)` 约束新 Attempt claim，并绑定两份规则 schedule hash；合法 claim 后只由 Attempt Lease/generation fencing。Checkpoint Receipt v2、Resume Authorization、Engine Checkpoint v2 与 Diagnostic Commit v2 形成恢复闭包。Runner 只通过 Attempt-scoped Artifact Store port 访问证据，local-v1 adapter 维持 `fsync + link-CAS` parity；Capability v1 冻结远端 `If-None-Match: *`、完整 SHA-256、strong read-after-write、Manifest-last 与 incomplete upload 清理要求。Control Plane 只登记当前 generation、受支持 storage policy 和单调 offset。未认证 remote backend 在 engine 前 typed reject；listing 不是 authority。远端 adapter、故障注入与 provider 认证仍待实现 |
| Replay 数据准入 | 已完成第十八个受限子集 | Dataset Manifest v5 以有序、连续、半开 venue-risk 与 instrument-spec epoch schedules 冻结规则 provenance；Request v11、Reservation v3 与 Fingerprint 绑定 canonical schedule hash。边界时先切换规则再处理同 timestamp source event，gap/overlap、availability 或 hash 漂移在首个事件前失败。自动历史规则采集、会改变 accounting 的 spec epoch 与 supplemental PIT revisions 仍待实现 |
| Replay accounting | 已完成第十八个受限子集 | Result v17 / Journal v4 / Margin v7 保持 isolated collateral 与 exact-risk full liquidation；margin observation、funding 后检查与 liquidation fee 按事件时间解析风险 epoch并记录 snapshot provenance，Attempt envelope 不进入经济 fingerprint。动态 collateral、partial liquidation、deficit/insurance/ADL、borrow、cross/shared portfolio 未实现 |
