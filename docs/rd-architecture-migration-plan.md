...
| Replay 权威准入 | 已完成第二十七个受限子集 | Trial Reservation v5 以 `[issued_at, expires_at)` 约束新 Attempt claim，并绑定两份规则 schedule、supplemental-facts 与 Requirement Set hash；合法 claim 后只由 Attempt Lease/generation fencing。Checkpoint Receipt v2、Resume Authorization、Engine Checkpoint v4 与 Diagnostic Commit v2 形成恢复闭包。Runner 只通过 Attempt-scoped Artifact Store port 访问证据，local-v1 adapter 维持 `fsync + link-CAS` parity；Artifact v21 将完整 supplemental revision stream 纳入 required member。未认证 remote backend 在 engine 前 typed reject；listing 不是 authority。远端 adapter、故障注入与 provider 认证仍待实现 |
| Replay 数据准入 | 已完成第二十个受限子集 | Experiment Contract v3 的 `replay_execution_input` 冻结 Requirement Set schema/hash，Control Plane 签发 Reservation v5 时从权威 contract 复核，Developer 无法降级为空。Dataset Manifest v7 声明连续半开规则 schedules、不可变 supplemental revision stream 与同一 hash；Request v13、Reservation 与 Fingerprint 绑定 canonical hashes。Adapter 按 `signal_time` 选择最后可见 revision，并闭世界拒绝缺失、陈旧、窗口外、未声明和多重匹配输入；未来修订仅保留 lineage，Result v19 固化 selected ids/hash 与逐项满足回执。动态多决策 join 与 feature 重算仍待实现 |
| Replay accounting | 已完成第十九个受限子集 | Result v19 / Journal v4 / Margin v7 保持 isolated collateral 与 exact-risk full liquidation；margin observation、funding 后检查与 liquidation fee 按事件时间解析风险 epoch并记录 snapshot provenance，Attempt envelope 不进入经济 fingerprint。动态 collateral、partial liquidation、deficit/insurance/ADL、borrow、cross/shared portfolio 未实现 |
