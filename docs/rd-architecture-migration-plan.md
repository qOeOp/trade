...
| Replay 权威准入 | 已完成第二十二个受限子集 | Trial Reservation、Attempt Lease、Checkpoint Receipt v2、Resume Authorization、Engine Checkpoint 与 Diagnostic Commit v2 形成运行闭包。Runner 以 `rd-replay-local-fsync-link-cas-v1` 不可变发布 Artifact member/Manifest 与 checkpoint payload/commit；Control Plane 只登记当前 generation、受支持 storage policy 和单调 offset，过期后只授权 source 最新 receipt 到 later active target。stale generation/receipt、idempotent retry、CAS collision、expiry takeover、路径/tamper、clean/resume parity 已锁定。目录扫描不是 authority；当前仅认证本地同文件系统，远端对象存储 conditional put/CAS 仍待实现 |
