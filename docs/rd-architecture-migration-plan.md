...
| Replay 权威准入 | 已完成第十九个受限子集 | Trial Reservation Snapshot v1、Replay Attempt Lease v1、Engine Checkpoint v1 与 Diagnostic Checkpoint Commit v1 形成运行闭包；Control Plane 单写 claim/renew/expire/finalize。Runner 在完整 source-event 边界验证续租 lease，并在外部 callback 前按 payload→commit 顺序原子落盘；另一进程只能凭可信 commit ref/hash 恢复。路径隔离、tamper rejection、clean/resume parity 与成功 Attempt 定向清理已锁定。硬崩溃自动发现/接管及对象存储 CAS 仍待实现 |
