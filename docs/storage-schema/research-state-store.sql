-- owner: research-strategy-development/research-control-plane/state-store
...
-- rd_universe_data_surface, rd_pipeline_registry_item, rd_universe_coverage,
-- rd_result_stage, rd_result_type, rd_lifecycle_transition_rule,
-- rd_trial_group, rd_trial_group_candidate, rd_proposal, rd_proposal_revision,
-- rd_experiment_contract, rd_trial, rd_experiment_result, rd_review_decision,
-- rd_review_decision_result, rd_experiment_blocker, rd_lifecycle_event,
-- rd_knowledge_node, rd_knowledge_edge, rd_knowledge_edge_evidence.
-- The runtime migration also installs identity and append-only triggers.
...
CREATE INDEX IF NOT EXISTS idx_rd_program_trial_program_time ON rd_program_trial(program_id, created_at DESC);

-- Research Control Plane schema. Keep aligned with research-control-plane-schema.ts.
