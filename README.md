...
- 先定义 Codex 如何参与一笔交易判断
- 先定义建议如何进入跟踪、更新和复盘
- skill 只保留最小能力方向，不预先固定数量、命名、边界和调用顺序

当前先停留在产品说明层，不把 schema、模块、records 或 workflow 预写成固定结构。

产品能力先按这个递进关系理解：

- 先做稳基础协作：在 Codex 里直接进入交易讨论，稳定产出可跟踪的建议
...
后续如果要把交易建议、市场快照和策略演化沉淀下来，这一节只固定三条主链之间的最小关系，用来支撑建议、复盘和迭代；不展开链内节点，也不讨论实现形式。

`决策链、市场快照链、策略链最少应该如何连接，才足以支撑建议、复盘和迭代。`

### 1. 市场快照进入决策链

市场快照链只负责保存“当时看到了什么”，决策链只负责保存“这些上下文后来如何进入判断”。
...
策略链只负责保存“这次判断主要受哪套方法影响”，决策链只负责保存“这套方法后来落成了什么判断或计划”。
...
前 1-3 条定义三条主链如何连接，4-5 条只补充两条链各自的演化约束。最小闭环本身只保留四个方向：
