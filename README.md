...
产品不只处理单笔交易建议，还需要同时承担市场背景、策略权重和正式判断三层判断。

### 市场状态

市场状态不是某一笔交易的私有备注，而是决策链之上的共享背景，用来统一回答“现在属于什么环境”以及“因此更该怎么打”。
...
这个视角是决策链之上的共享背景，而不是某一笔交易私有的备注。否则多个局部判断即使各自有理，整体上也可能出现风格漂移、策略错配和预算失衡。

### 策略优势与权重

一个更完整的交易系统，不只要能提出判断，还要能持续回答：哪些打法在什么环境下真的有优势，以及这些优势现在是否还值得继续信任。
...
这层不等于事后做一张业绩报表，而是要把复盘结果继续往前推，变成下一轮策略选择、仓位分配和风险取舍的一部分。否则市场状态、策略选择和组合预算虽然都存在，但它们之间仍然少了一层把“历史表现”转成“未来权重”的桥。
