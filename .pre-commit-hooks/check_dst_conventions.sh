...
#      reaches that bypass the vibe_network::net seam (the seam swaps to
...
check_rule1_hit() {
  ...
  report "rule1" "$norm_file" "$line_num" "$content" \
    "Route through vibe_core::time::duration_since_unix_epoch or a DST seam"
}
...
check_rule6_hit() {
  ...
  report "rule6" "$file" "$line_num" "$content" \
    "Route through vibe_network::net::{TcpStream, TcpListener} so the turmoil cfg-swap covers it"
}
...
# `vibe_common::live::dst`. Network and persistence own separate runtime
...
while IFS=: read -r file line_num content; do
  ...
  report "rule7" "$file" "$line_num" "$content" \
    "Route time, task, runtime, and signal calls through vibe_common::live::dst or cfg-gate the site"
done < <(rg -n --no-heading \
...
for c in "${RULE7_CRATES[@]}"; do
  while IFS= read -r file; do
    while IFS=: read -r line_num content; do
      ...
      report "rule7" "$file" "$line_num" "$content" \
        "Import time, task, runtime, and signal through vibe_common::live::dst or cfg-gate the site"
