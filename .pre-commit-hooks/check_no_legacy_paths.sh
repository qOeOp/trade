...
if matches=$(
  ...
  echo "Error: found legacy vibe_pyo3 references in Python-facing files"
  echo
  echo "$matches"
  echo
  echo "Use the public Python surface or _libvibe internals instead."
  exit 1
fi
...
if matches=$(
  ...
  echo "Error: found legacy vibe_pyo3 module paths in Rust sources"
  echo
  echo "$matches"
  echo
...
if [[ "$actual_ffi_manifests" != "$expected_ffi_manifests" ]]; then
  echo "Error: ffi features must exist only in vibe-core and vibe-model"
  echo
...
actual_ffi_dependency_manifests=$(
  rg -l 'vibe-(core|model)/ffi' crates --glob Cargo.toml | sort
)
if [[ "$actual_ffi_dependency_manifests" != "$expected_ffi_dependency_manifests" ]]; then
  echo "Error: only vibe-model may enable another crate's ffi feature"
  echo
...
if [[ "$actual_ffi_dirs" != "$expected_ffi_dirs" ]]; then
  echo "Error: ffi modules must exist only in vibe-core and vibe-model"
  echo
...
if [[ "$actual_staticlib_manifests" != "$expected_staticlib_manifests" ]]; then
  echo "Error: static library targets must exist only in vibe-core and vibe-model"
  echo
...
if [[ "$actual_cbindgen_configs" != "$expected_cbindgen_configs" ]]; then
  echo "Error: cbindgen configuration must exist only in vibe-core and vibe-model"
  echo
