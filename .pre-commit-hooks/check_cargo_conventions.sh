...
# 11. Adapter crates must obtain libfuzzer-sys through vibe-live
#
# Dependency groups are typically organized as:
# - Internal vibe-* dependencies
...
# Check 5: internal package metadata
# Every product crate is private and carries only metadata needed by local tooling.
package_violations=$(rg --files -g "Cargo.toml" --glob "!target/*" crates/ 2> /dev/null | while read -r file; do
  ...
  in_package && /^[a-zA-Z]/ {
    ...
    found[norm_field] = 1
    if (norm_field == "publish" && $0 !~ /=[[:space:]]*false([[:space:]]*#.*)?$/) {
      printf "  %s:%d [package] publish must be false\n", FILENAME, NR
    }
  }
...
if [[ -f "Cargo.toml" ]]; then
  ...
  in_ws_deps && /^[a-zA-Z][a-zA-Z0-9_-]*[[:space:]]*[.=]/ {
    ...
    if (dep == "vibe-backtest" || dep == "vibe-cli" || dep == "vibe-pyo3") next
...
# Check 11: Adapter crates must obtain libfuzzer-sys through vibe-live
...
if [[ -n "$adapter_libfuzzer_violations" ]]; then
  ...
  echo -e "${YELLOW}Enable vibe-live/fuzz instead${NC}"
  echo
...
if [[ $VIOLATIONS -gt 0 ]]; then
  ...
  echo "  - [package] must define name, version/edition/rust-version workspace inheritance,"
  echo "    publish = false, description, and repository.workspace"
  ...
  echo "  - Obtain adapter libfuzzer-sys support through vibe-live/fuzz"
  exit 1
fi
