#!/usr/bin/env bash
# Enforces Vibe conventions:
# 1. Vibe domain types should not be fully qualified in code
...
#
# Use '// vibe-import-ok' comment to allow specific exceptions

set -euo pipefail

# Exit cleanly if ripgrep is not installed
if ! command -v rg &> /dev/null; then
  echo "WARNING: ripgrep not found, skipping Vibe convention checks"
  exit 0
fi
...
ALLOW_MARKER="vibe-import-ok"

echo "Checking for fully qualified Vibe types..."

# Use rg with context (-B 5) to get surrounding lines in one pass
# Format: file:line:content (with -- separators for context)
# Exclude vibe_macros: proc-macros generate code for other crates; fully qualified paths required
output=$(rg -n -B 5 \
  --no-heading \
  --glob '!crates/persistence/macros/**' \
  'vibe_[a-z_]+(?:::[a-z_]+)+::[A-Z][A-Za-z0-9_]+' \
  crates examples \
  --type rust \
  2> /dev/null || true)
...
if [[ -z "$output" ]]; then
  echo "Vibe import conventions are valid"
else
  ...
  while IFS= read -r line; do
    ...
    if [[ "$line" =~ ^([^:]+):([0-9]+):(.*)$ ]]; then
      ...
      if [[ "$line_content" =~ (vibe_[a-z_]+(::[a-z_]+)+::[A-Z][A-Za-z0-9_]+) ]]; then
        matched="${BASH_REMATCH[1]}"
        echo -e "${RED}Error:${NC} Fully qualified Vibe type in $file:$line_num"
        echo "  Found: $matched"
  ...
  if [ $VIOLATIONS -gt 0 ]; then
    echo -e "${RED}Found $VIOLATIONS Vibe convention violation(s)${NC}"
    echo
    echo -e "${YELLOW}To fix:${NC} Import the type at the top of the file and use it directly"
    echo "Example: use vibe_model::identifiers::InstrumentId;"
    echo "         Then use: InstrumentId::new(...)"
    echo
    echo "Add '// vibe-import-ok' comment to allow specific exceptions"
  else
    echo "Vibe import conventions are valid"
  fi
...
macro_output=$(rg -n --no-heading \
  'vibe_(common|trading)::(vibe_actor|vibe_strategy)!' \
  crates examples \
  --type rust \
  2> /dev/null || true)
...
if [[ -n "$macro_output" ]]; then
  ...
  while IFS= read -r line; do
    if [[ "$line" =~ ^([^:]+):([0-9]+):(.*)$ ]]; then
      ...
      echo "  ${trimmed:0:100}"
      echo "  Import the macro and call it unqualified: vibe_actor!(...) or vibe_strategy!(...)"
      echo
  ...
  if [ $MACRO_VIOLATIONS -gt 0 ]; then
    ...
    echo
    echo -e "${YELLOW}To fix:${NC} Add 'use vibe_common::vibe_actor;' or"
    echo "'use vibe_trading::vibe_strategy;' and call the macro unqualified"
  fi
else
...
EXPECTED_PYO3_MODULES="analysis vibe_analysis::python::analysis
architect_ax vibe_architect_ax::python::architect_ax
backtest vibe_backtest::python::backtest
betfair vibe_betfair::python::betfair
binance vibe_binance::python::binance
bitmex vibe_bitmex::python::bitmex
blockchain vibe_blockchain::python::blockchain
bybit vibe_bybit::python::bybit
coinbase vibe_coinbase::python::coinbase
common vibe_common::python::common
core vibe_core::python::core
cryptography vibe_cryptography::python::cryptography
data vibe_data::python::data
databento vibe_databento::python::databento
deribit vibe_deribit::python::deribit
derive vibe_derive::python::derive
dydx vibe_dydx::python::dydx
execution vibe_execution::python::execution
hyperliquid vibe_hyperliquid::python::hyperliquid
indicators vibe_indicators::python::indicators
infrastructure vibe_infrastructure::python::infrastructure
interactive_brokers vibe_interactive_brokers::python::interactive_brokers
kraken vibe_kraken::python::kraken
lighter vibe_lighter::python::lighter
live vibe_live::python::live
model vibe_model::python::model
network vibe_network::python::network
okx vibe_okx::python::okx
persistence vibe_persistence::python::persistence
polymarket vibe_polymarket::python::polymarket
portfolio vibe_portfolio::python::portfolio
risk vibe_risk::python::risk
sandbox vibe_sandbox::python::sandbox
serialization vibe_serialization::python::serialization
tardis vibe_tardis::python::tardis
testkit vibe_testkit::python::testkit
trading vibe_trading::python::trading"
...
echo
echo "All Vibe conventions are valid"
exit 0
