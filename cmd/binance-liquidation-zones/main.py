...
PYTHON_REQUIREMENTS_PATH = ".agents/skills/binance-liquidation-zones/requirements.txt"


@dataclass
class Config:
    symbol: str
    aggtrades_file: str | None = None
...
def resolve_symbol(config_symbol: str, aggtrades_payload: dict[str, Any] | None, snapshot_payload: dict[str, Any] | None) -> str:
    candidates = []
    if config_symbol:
...
def resolve_aggtrades(
    config: Config,
    symbol: str,
    aggtrades_payload: dict[str, Any] | None,
    requests_module,
...
def trade_timestamp(trade: dict[str, Any]) -> int | None:
    value = trade.get("T")
    if value is None:
...
def normalize_dominant_side(value: Any) -> str | None:
...
def to_iso8601_from_millis(value: Any) -> str | None:
    if value is None:
        return None
    try:
