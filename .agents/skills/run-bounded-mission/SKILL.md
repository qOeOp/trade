...
Use project budgets when present. Otherwise allow three candidates per uncertain decision, one writable context, three implementation revisions per admitted slice, two non-progress cycles before replanning, and six admitted slices total for the mission.
