...
Use project budgets when present. Otherwise allow three candidates per uncertain decision, one writable context, three revisions per slice, two non-progress cycles before replanning, and six admitted slices before recontracting.

Track per-slice revision and non-progress limits separately from cumulative mission cost, escalation, and admitted-slice budgets. Every slice that enters execution or evaluation spends one admitted-slice unit, whether accepted or rejected. Each consumer-closed admitted slice gets at most three revisions; two non-progress cycles force replan or termination by the ordered predicates. A new slice needs a different consumer checkpoint, or a replan that materially changes contract or design to close a new counterexample. Renaming, retrying, changing context, or relabeling a same-mission read-only step never resets either class of budget. Slice exhaustion requires replan; only cumulative cost, escalation, or admitted-slice exhaustion can terminate the mission as `budget_exhausted`. An independent read-only task with a different outcome is a new mission with its own counters.
...
- `budget_exhausted`: a potentially valid next step remains, but a cumulative mission cost, escalation, or admitted-slice budget is spent;
