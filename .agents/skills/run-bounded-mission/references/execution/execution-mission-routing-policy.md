...
| independent candidate audit      | reviewer-selected dedicated route vs one fresh generic alternative when needed | exact committed candidate, neutral control, complete risk question, read-only observation, and Main reproduction | selected set; model/effort; terminal validity; fallback; correction                           |
