...
2. **Build evidence.** For writer-changed content, require the writer to return a patch and raw checks, never a commit. For an adopted candidate, inspect its complete exact diff against the selected base. In both cases run the real consumer journey, focused owner checks, and every project-required full gate from the main context.
