...
只读 skill。服务订单路由与参数收敛，不发真实订单。只做 Binance USDM 永续。
...
- 必填：`symbol` `side` `type`
- 二选一：`quantity` / `close-position true`
...
- 只要是 `STOP*` / `TAKE_PROFIT*` / `TRAILING_STOP_MARKET`，当前预演口径都会走 `futuresCreateAlgoOrder`
...
1. 明确订单意图和目标参数
2. 若是条件单，显式带上 `position-side`、`reduce-only`、`close-position`
