...
- 支持动作：
  ...
  - `--strategy-calibration-suite --json <payload>`：运行 buy-and-hold / cash baseline、固定趋势基准、固定横截面强弱基准，并可消费 dataset `indicator_report_path` 中的 exact funding events，输出 data_panel、beta、成本、funding、换手、暴露、时间稳定性、负对照与数据广度诊断；只回答 R&D 管线该先修哪里
...
- `--strategy-calibration-suite` 固定规则且禁止参数搜索；`data_panel` 用来暴露 schema、closed candle、source 与对齐质量问题；buy-and-hold 只做 beta 诊断，`execution_attribution / failure_analysis` 用来暴露系统问题和下一步修复动作；exact funding 覆盖不足只触发 `CAL-FUNDING-COVERAGE`，不得伪装成历史 funding 结果；suite 结果不进入 strategy evidence 或 promotion gate
