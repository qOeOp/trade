...
test("execution command spec schema matches executable routing contract", () => {
  ...
  assert.deepEqual(asArray(schema.required), ["target_action", "skill", "cwd", "command"])
  assert.deepEqual(asArray(asRecord(asRecord(schema.properties).target_action).enum), [...EXECUTABLE_TARGET_ACTIONS])
  assert.deepEqual(asArray(asRecord(asRecord(schema.properties).skill).enum), EXECUTABLE_SKILLS)
  ...
  assert.equal(spec.skill, "binance-order-cancel")
  assert.equal(spec.cwd, "/repo/.agents/skills/binance-order-cancel")
